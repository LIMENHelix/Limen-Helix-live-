...
## 0a. THEORETICAL NORTH STAR — the kernel is a REGULATION-DYNAMICS OBSERVER, not a distress detector

**The kernel measures a regulation architecture that runs in ALL complex adaptive systems; substrate differences are PRECISION-PARAMETER and OBSERVATIONAL-RESOLUTION differences, not architectural ones.** Grounding: Kelso metastability (the brain dwells/escapes/re-enters attractors non-sequentially as its constitutive mode — Book I already cites Tognoli & Kelso 2014) + Friston/Seth predictive-coding precision-weighting (same hierarchy, different confidence settings). The 11 phases are states the architecture cycles through CONSTANTLY; most transitions are too small to observe in stable systems. **Distress doesn't create the algebra — it makes it visible.**

**The observability property (mechanical, not metaphorical):** the kernel works on systems whose regulation transitions exceed their observation noise floor at the available sampling rate. Companies and neurodivergent regulation share this — both have large-amplitude transitions and lack the strong top-down damping that makes neurotypical regulation observationally smooth. This is WHY the daughter + bankruptcy cohort are the methodologically correct entry points (high SNR), and why healthy companies (Costco, Walmart) are hard (damped below the quarterly noise floor — need finer sampling, the financial equivalent of EEG vs behavior).

**⚠️ HONEST BOUNDARY (do not let the framing outrun the data):** VALIDATED claims = the LARGE-TRANSITION regime only (P3 ~0.80, P7 77%, triad 0.90, Going 12/12 — all high-SNR/distress). The universal-regulation-observer claim is theoretically grounded (Kelso/Friston) but EMPIRICALLY UNOBSERVED in finance — we lack the resolution to see the subclinical/healthy cycling. NO CLINICAL LENS IMPORTED: trauma/neurodivergence/illness are the same metastable dynamics at different precision; importing "companies have trauma/are sick" smuggles in pathology. Anchor on regulation dynamics; treat distress as the high-observability CASE.

**✅ VALIDATED ON OUR DATA — the transition-SNR envelope (`snr_envelope.py`):** transition amplitude = worst REVENUE YoY contraction (the unpolluted operating transition; cash/OCF YoY are buyback/WC noise). Perfect separation: distress 6/6 RELIABLE (revenue contracting 0.1-0.7) vs healthy+FPs 7/7 ABSTAIN (revenue 0.0, growing). **The healthy false positives (COST/PEP/FIS/SBUX) are NOT errors — they are OUT OF ENVELOPE** (composite fired on cash/buyback noise; no operating transition). The kernel now KNOWS when it can see — the honest version of a confidence score, and the principled version of the market-equity veto. Scope line: the TRAJECTORY kernel abstains on revenue-stable systems (pure-leverage/liquidity = the SOLVENCY kernel's job). **OPEN (lower confidence):** masking MECHANISM may differ across substrates even though the signature is identical — neurodivergent masking = effortful suppression; corporate masking = structural accounting lag. Same gap, different cause, different intervention. Architecture transfers; mechanism unproven.
