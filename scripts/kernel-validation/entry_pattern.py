...
# P1 collapse and P4 scaffold are the "transit"/intermediate markers
...
def main():
    ...
    print("%-6s %-16s %-10s %s" % ("TICK", "walk (oldest->event)", "entry", "phase before -> first distress"))
    jumps = seqs = 0
    for (t, cik, ev) in COHORT:
        ...
        if di is None:
            print("%-6s %-16s %-10s no distress entry in window" % (t, code, "-")); continue
        before = walk[di - 1] if di > 0 else "(start)"
        if di == 0:
            entry = "AT-START"
        elif before in HEALTH:
            entry = "JUMP"; jumps += 1
        elif before == "P1_collapse":
            entry = "SEQUENTIAL"; seqs += 1
        else:
            entry = "JUMP"; jumps += 1   # from scaffold/other non-cascade
        print("%-6s %-16s %-10s %s -> %s" % (t, code, entry, before, walk[di]))
        time.sleep(0.1)
    print("\n=== ENTRY PATTERN ===")
