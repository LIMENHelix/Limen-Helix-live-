...
(async function () {
  ...
  // ── T16 ─────────────────────────────────────────────────────────────────
  // THE LEGACY CHECKOUT IS RETIRED. /api/relay-checkout minted a real Stripe payment
  // link with no order record and no shipping address — money in, nothing to fulfil.
  // The pin is now the retirement itself: every method answers 410, the Stripe rail is
  // never touched, and no order or acceptance is written.
  console.log('T16: the legacy checkout is retired and moves no money');

  // Prove the retirement does not depend on configuration: even with the payment rail
  // reporting itself ready, the answer is still Gone.
  ...
  var ordersBefore16 = Object.keys((await db.get('relay:store:orders')) || {}).length;

  var c1 = await invoke(checkout, {
    method: 'POST', headers: {},
    body: { items: [{ id: 'dj1', qty: 1 }], policyAccepted: true }
  });
  assert('POST is answered 410 Gone', c1.status === 410, String(c1.status) + ' ' + JSON.stringify(c1.body).slice(0, 140));
  assert('and says nothing was charged', /nothing has been charged/i.test(JSON.stringify(c1.body)), JSON.stringify(c1.body).slice(0, 160));
  assert('and points at a checkout that exists', /storefront/i.test(JSON.stringify(c1.body)));

  var c2 = await invoke(checkout, { method: 'GET', headers: {} });
  assert('every other method gets the same retirement', c2.status === 410, String(c2.status));

  assert('the Stripe rail was never called', RAIL_CALLS === 0, 'calls: ' + RAIL_CALLS);
  assert('and no order was written',
    Object.keys((await db.get('relay:store:orders')) || {}).length === ordersBefore16);
