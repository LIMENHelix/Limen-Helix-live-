...
assert.equal(worker.schedulerGroup(candidates[6]), 'unowned');
...
assert.equal(at3[0].id, 'finance-b');
assert.equal(at4[0].id, 'science-b');
assert.equal(at5[0].id, 'medicine-b');
...
console.log('autofire owner fairness: Finance, Science, and Medicine rotate deterministically without dropping candidates');
