...
assert.equal(worker.schedulerGroup(candidates[8]), 'unowned');
...
assert.equal(at4[0].id, 'finance-b');
assert.equal(at5[0].id, 'science-b');
assert.equal(at6[0].id, 'medicine-b');
assert.equal(at7[0].id, 'education-b');
...
console.log('autofire owner fairness: Finance, Science, Medicine, and Education rotate deterministically without dropping candidates');
