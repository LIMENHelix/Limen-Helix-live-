...
const MISSING = ['OIL_SHOCK', 'PIPELINE_DISRUPTION', 'NUCLEAR_INCIDENT', 'SYSTEMIC_ENERGY_STRESS'];
...
(async function () {
  ...
  ['GRID_COLLAPSE', 'RENEWABLE_INTERMITTENCY'].concat(MISSING).forEach(function (id) {
  ...
  console.log('\n--- ACCEPTANCE ---');
  const checks = [
    ['live repo has EXACTLY the 2 real bundles (no fakes created)', files.length === 2 && files.indexOf('GRID_FREQUENCY_INSTABILITY.json') >= 0 && files.indexOf('INTERMITTENCY_SPIKE.json') >= 0],
