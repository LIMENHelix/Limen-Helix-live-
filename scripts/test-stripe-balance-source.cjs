...
(async function () {
  ...
  calls.length = 0;
  var science = await Source.read('science', { env: {}, fetch: fetch });
  ...
  var malformed = await Source.read('finance', { env: { STRIPE_SECRET_KEY: 'sk_test_secret' },
    fetch: async function () { return { ok: true, status: 200, json: async function () { return {}; } }; } });
  assert.equal(malformed.ok, false); assert.equal(malformed.reason, 'stripe-balance-response-invalid');

  var aborted = false;
  var stalled = await Source.read('finance', { env: { STRIPE_SECRET_KEY: 'sk_test_secret' }, timeoutMs: 5,
    fetch: function (_url, options) { return new Promise(function (_resolve, reject) {
      options.signal.addEventListener('abort', function () { aborted = true; reject(new Error('aborted')); });
    }); } });
  assert.equal(aborted, true); assert.equal(stalled.ok, false);
  assert.equal(stalled.reason, 'stripe-balance-transport-failed');
