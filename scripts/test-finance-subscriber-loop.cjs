...
var Decision = require('../lib/finance-subscriber-decision.js');
var Executor = require('../lib/finance-subscriber-executor.js');
...
function cognition(now, review) { return { ts: now, c: { domain: 'finance', immune: { immuneState: 'clear' }, awareness: { humanReviewRequired: !!review },
  ...
  serverPacket: { schemaVersion: 'civilization-domain-packet/1.0', domainId: 'finance', packetId: 'finance-packet-' + review, generatedAt: new Date(now).toISOString(),
...
function sub(email) { return { email: email, domain: 'finance', active: true, subscriptionId: 'sub-secret', customerId: 'cus-secret' }; }
function digest(key) { return { subject: 'Finance briefing', body: 'Source-grounded change.\n\nCheck any figure yourself: https://limenhelix.com/finance', key: key }; }
...
(async function () {
  ...
  assert.equal(held.status, 'NO_ACTION'); assert(held.blockers.includes('finance-human-review-veto'));
  ...
  assert.equal(noBudget.status, 'HELD'); assert.equal(noBudget.reason, 'finance-subscriber-email-unit-cost-not-configured'); assert.equal(sends, 0);
  ...
  var result = await Executor.execute(Object.assign({ store: store, specs: [{ candidate: c1, decision: released }, { candidate: c2, decision: d2 }], maxSends: 1, now: now,
    motorAuthorization: motor('finance-motor-1'), transport: { send: async function (_email, _subject, _body, options) {
  ...
  assert.equal(sends, 1); assert.equal(sawCommand, true); assert.equal(idem, 'finance-digest/' + released.actionId);
  var replay = await Executor.execute(Object.assign({ store: store, specs: [{ candidate: c1, decision: released }], maxSends: 1, now: now,
    motorAuthorization: motor('finance-motor-1'), transport: { send: async function () { sends++; } } }, budget()));
  ...
  var laterMotor = await Executor.execute(Object.assign({ store: store, specs: [{ candidate: c1, decision: released }], maxSends: 1, now: now,
    motorAuthorization: motor('finance-motor-2'), transport: { send: async function () { sends++; } } }, budget()));
  ...
  var budgetLimited = await Executor.execute({ store: budgetStore,
    ...
    motorAuthorization: motor('finance-budget-motor'), transport: { send: async function () {
  ...
  for (var n = 2; n <= 5; n++) {
    var itemN = { actionId: 'finance-action-' + n, decisionReceiptId: 'decision-' + n, contentHash: 'content-' + n };
  ...
  var recovery = await Recovery.recover({ store: store, command: result, actionId: released.actionId, observation: observation,
    motorAuthorization: motor('finance-motor-3'), now: now + 1 });
  ...
  var suppressed = await Executor.execute(Object.assign({ store: store, specs: [{ candidate: c3, decision: d3 }], maxSends: 1, now: now,
    motorAuthorization: motor('finance-motor-4'), transport: { send: async function () { sends++; } } }, budget()));
  assert.equal(suppressed.status, 'HELD'); assert.equal(suppressed.reason, 'finance-subscriber-all-candidates-suppressed'); assert.equal(sends, 1);
  ...
  console.log('finance subscriber sovereign B10/B14/delivery/recovery loop: PASS');
