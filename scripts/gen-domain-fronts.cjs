...
// Soft-domain desks (religion, education) and Civic desks (intelligence,
// population, law, governance) are hand-owned product pages. Regenerating
// them would wipe the free tool → paid Watch path. Culture is already
// absent from CONFIG, so it is not written here.
const PRODUCTIZED_FRONTS = new Set([
  'religion', 'education',
  'intelligence', 'population', 'law', 'governance'
]);
