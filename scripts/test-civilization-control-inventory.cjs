...
assert.equal(snap.summary.runtimeMotorValves, require('../lib/civilization-valve-registry.js').LINES.length + 1,
  'every local lane valve plus one global emergency valve is visible');
assert.equal(snap.summary.cadencePumps, VercelCrons.length, 'every declared Vercel cadence is visible');
...
console.log('civilization control inventory: ' + snap.summary.runtimeMotorValves + ' runtime valves, all env controls, ' +
  snap.summary.cadencePumps + ' cadences, 385 triggers, 111 diagnoses, B0-B17 and P0-P10 visible');
