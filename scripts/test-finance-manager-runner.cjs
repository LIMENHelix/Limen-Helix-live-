...
const proposal = { schemaVersion: Prompt.RESPONSE_SCHEMA, id: 'runner-proposal', company: { slug: 'example_co', ticker: 'EX' }, thesis: 'Bounded paper review.', invalidation: 'Invalidate on correction.', horizonDays: 30, scenarios: [{ name: 'base' }, { name: 'downside' }], projectedMarginRanking: { metric: 'risk-adjusted-expected-total-return-pct', methodology: 'expectedReturnPct - abs(min(downsideReturnPct, 0)) * (1 - confidence)', entries: [{ company: { slug: 'example_co', ticker: 'EX' }, expectedReturnPct: 12, downsideReturnPct: -10, confidence: 0.8, riskAdjustedMarginPct: 10 }] }, evidenceRefs: [{ role: 'semantic', sourceIdentity: { kind: 'publisher-item', value: semanticId } }, { role: 'market', sourceIdentity: { kind: 'market-quote', value: marketId } }, { role: 'network', sourceIdentity: { kind: 'network', value: networkId } }], independenceAssessment: { status: 'UNASSESSED', reason: 'Not established.' }, paperOnly: true, provenance: { producer: 'runner-test', generatedAt: '2026-08-24T16:02:00Z' } };
