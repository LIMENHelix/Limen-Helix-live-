...
| 14 | **Portals never built** | **170 runnable → 169** | measured 2026-09-04 after identity-aware queue reconciliation | **REGENERATION RESUMED, BOUNDED:** the old autonomous count of 175 included 5 already-satisfied curated aliases because it checked only slug/alias while the writer also joins on CIK and normalized legal name. The queue and writer now share the identity join. One canary portal, `edison_international` (EIX, CIK `0000827052`), passed fail-closed admission and canonical schema validation: 20 relationships, 9 categories, 100% topology/evidence tags, 0 placeholders, and no financial/kernel claim. Wiring repaired 5 curated aliases plus the EIX eligible pointer; curated wiring is now 100%, with no broken rewirable pointer. **169 eligible portals remain.** Evidence: `assets/data/audit/portal-regeneration-batch-1.json` |
...
| **Portal regen** | The manual bounded drain is now fail-closed and one canary portal has passed; autonomous scheduling remains deliberately unwired. Anthropic credits are exhausted, so the live path uses the already-configured metered xAI fallback. **169 eligible portals remain**, and generated relationship source claims are not independently verified. |
