...
| 20 | **Phase-belief abstentions are observable but not diagnosable by channel** | **3 of 20 domains** | **FIXED 2026-09-04**, production snapshot `1788577538341` | Exact merge `e6cfb58d` deployed and the first fresh worker tick preserved all three authoritative abstentions and exposed diagnostic-only channel precision. Industry total `0.066614` = company `0.030` + market `0.037`; Education `0.071879` = `0.032` + `0.040`; Law `0.079628` = `0.034` + `0.046`. In all three, `seriesScore` was uninformative and no `_legacyFeedStress`/`stressSource` was added. The cause is now measured without relaxing the protected floor or changing a domain value. Evidence: `DOMAIN_NEURO_ABSTENTION_PROOF_2026-09-04.md`; PR #294 production proof |
| 21 | **Validated scorer leaks a FRED credential in runtime logs** | **50 sampled events / 102 ms** | measured 2026-09-04, deployment `dpl_5CErEb4QvwuioTnYY2ko5Ts6LjiQ` | `/api/limen/score` ignored the encrypted `FRED_API_KEY`, called the byte-locked kernel's legacy hard-coded fetcher, and printed exception text containing the complete query URL. A wrapper-level adapter now uses the environment credential, preserves the quarterly-delta input, and emits only bounded secret-free status. The validated kernel and lock remain byte-identical. Production movement and external FRED-key rotation are **UNMEASURED until exact-SHA deployment/account action**. Evidence: `FRED_SCORER_LOG_REDACTION_PROOF_2026-09-04.md` |
