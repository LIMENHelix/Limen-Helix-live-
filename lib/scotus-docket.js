...
var PARSER_VERSION = 'scotus-docket/1.0.0';
...
function extractLinks(html) {
  var links = [], seen = {};
  String(html || '').replace(/<a\b[^>]*href\s*=\s*(["'])(.*?)\1[^>]*>([\s\S]*?)<\/a>/gi,
    function (_, q, href, label) {
      href = decodeEntities(href).trim();
      label = textFromHtml(label);
      if (!href || !label || /^javascript:/i.test(href) || /^mailto:/i.test(href)) return _;
      var absolute = href;
      try { absolute = new URL(href, OFFICIAL_ORIGIN).toString(); } catch (e) { return _; }
      if (absolute.indexOf(OFFICIAL_ORIGIN + '/') !== 0) return _;
      var key = absolute + '|' + label;
      if (!seen[key]) { seen[key] = true; links.push({ label: label, url: absolute }); }
      return _;
    });
  return links;
}
...
function parse(html, requestedDocket) {
  ...
  if (!docketed || !EVENT_DATE.test(docketed)) {
    EVENT_DATE.lastIndex = 0;
