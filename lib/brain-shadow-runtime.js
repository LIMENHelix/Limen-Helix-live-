...
async function runDomain(product, opts) {
  ...
  var report = {
    ...
    /**
     * BYTES ACTUALLY WRITTEN TO REDIS, measured by the store on the string it sent, not
     * estimated here from the object. Null until a state write succeeds, so a failed cycle
     * reports null rather than a number describing a payload that never landed.
     *
     * This exists because persisted state grows with ticks and nothing was measuring it:
     * offline replay put the largest domain at 3.7 MB after 470 ticks with no plateau, and
     * every cycle reads the whole thing and writes it back. Upstash bills bandwidth, so
     * this number, times two, times the installed count, times 24, is the daily cost. A
     * projection nobody can check against production is not a measurement, which is why it
     * is reported per cycle through the token-gated read.
     */
    stateBytes: null
  };
  ...
  try {
    ...
    report.stateBytes = await STORE.writeState(d.snapshot, {
      runtime: RUNTIME_VERSION,
      domain: d.snapshot,
      lastRowT: cursor,
      savedAt: startedAt,
