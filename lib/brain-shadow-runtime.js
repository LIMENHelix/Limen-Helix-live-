 ...
 *   3. THE OUTWARD CONSUMER IS DECLARED AND BOUNDED. `/api/brain-shadow` remains the
 *      token-gated operator read. The only action consumer is
 *      `handlers/limen-worker-autofire.js`, which reads the latest cycle for the owner
 *      domain and may release an artifact-generation candidate. It cannot submit a trade,
 *      publish a study, create a candidate from stress, or write through this store.
 *      Finance owns investment; Science and Medicine own research; all other binders keep
 *      `efferent:null`.
...
async function runDomain(product, opts) {
  ...
  var report = {
    ...
    resourceState: null,
    resourceRegulation: null,
    runtimeMeasurements: { eventLoopLagMs: null, recursionDepth: 0, crossDomainPropagationVolume: 0 },
    /**
  ...
  try {
    ...
    /* A real scheduling probe, measured by the async outer runtime. The synchronous
       kernel cannot measure event-loop lag from inside its own call without relabeling
       execution time as lag. One setImmediate turn measures how long runnable work had
       to wait before Node scheduled it. Tests may supply a pinned value. */
    var eventLoopLagMs = Number(opts.eventLoopLagMs);
    if (!Number.isFinite(eventLoopLagMs) && fresh.length) {
      var lagStarted = process.hrtime.bigint();
      await new Promise(function (resolve) { setImmediate(resolve); });
      eventLoopLagMs = Number(process.hrtime.bigint() - lagStarted) / 1e6;
    }
    if (!Number.isFinite(eventLoopLagMs)) eventLoopLagMs = null;
    report.runtimeMeasurements.eventLoopLagMs = eventLoopLagMs;
    ...
    fresh.forEach(function (row) {
      ...
      report.resourceState = tickReport.selfModel && tickReport.selfModel.resourceState || null;
      report.resourceRegulation = tickReport.regulation || null;
      report.ticks++;
      cursor = row.t;
    });
