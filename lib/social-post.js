...
function creds() {
  return {
    ...
    // Environment managers and clipboard-based setup can leave an otherwise
    // valid app password with a trailing newline. Bluesky app passwords do
    // not contain meaningful surrounding whitespace, so normalize the value
    // before authentication without ever logging or returning it.
    password: String(process.env.BLUESKY_APP_PASSWORD || '').trim()
  };
}
...
async function postToBluesky(text, o) {
  ...
  if (!text) return { ok: false, providerCalled: false, reason: 'Empty post.' };

  var len = graphemeLength(text);
  if (len > MAX_GRAPHEMES) {
    return { ok: false, providerCalled: false, reason: 'Post is ' + len + ' graphemes; Bluesky allows ' + MAX_GRAPHEMES + '.', length: len };
  }

  // 1. Operator pause first: it outranks any remaining quota.
  var blocked = await postingBlocked();
  if (blocked) return { ok: false, providerCalled: false, blocked: true, reason: 'Posting blocked: ' + blocked };
  ...
  if (o.dryRun) {
    return { ok: true, providerCalled: false, dryRun: true, length: len, facets: facets.length, record: record };
  }

  // 2. Rate limit.
  var slot = await claimRateSlot('bluesky');
  if (!slot.ok) return { ok: false, providerCalled: false, reason: slot.reason, rateLimited: !!slot.rateLimited };
  ...
  if (!s.ok) {
    ...
    return { ok: false, providerCalled: true, providerStage: 'authentication',
      providerStatus: Number.isFinite(Number(s.status)) ? Number(s.status) : null,
      definitiveFailure: Number(s.status) >= 400 && Number(s.status) < 500,
      externalEffectMayExist: false, reason: s.reason };
  }
  ...
  try {
    ...
    if (r.status !== 200 || !j || !j.uri) {
      await releaseRateSlot('bluesky');
      var definitive = r.status >= 400 && r.status < 500 && [408, 409, 425, 429].indexOf(r.status) < 0;
      return { ok: false, providerCalled: true, definitiveFailure: definitive, ambiguous: !definitive,
        reason: (j && j.message) || ('Bluesky createRecord returned ' + r.status) };
    }
    ...
    return { ok: true, providerCalled: true, uri: j.uri, cid: j.cid, url: url, used: slot.used, cap: slot.cap };
  } catch (e) {
    clearTimeout(tid);
    await releaseRateSlot('bluesky');
    return { ok: false, providerCalled: true, definitiveFailure: false, ambiguous: true,
      reason: 'Bluesky post failed: ' + (e.message || 'timeout') };
  }
}
...
module.exports = {
  ...
  creds: creds, normalizeHandle: normalizeHandle, buildFacets: buildFacets, graphemeLength: graphemeLength,
