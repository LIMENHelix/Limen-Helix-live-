...
async function postToBluesky(text, o) {
  ...
  try {
    ...
    if (r.status !== 200 || !j || !j.uri) {
      ...
      return { ok: false, providerCalled: true, reason: (j && j.message) || ('Bluesky createRecord returned ' + r.status) };
    }
    ...
    clearTimeout(tid);
    await releaseRateSlot('bluesky');
    return { ok: false, providerCalled: true, reason: 'Bluesky post failed: ' + (e.message || 'timeout') };
  }
}
