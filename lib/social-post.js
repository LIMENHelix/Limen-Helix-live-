...
async function postToBluesky(text, o) {
  ...
  // 1. Kill switch first: an operator pause outranks any remaining quota.
  try {
    if (await killSwitch.spendDisabled()) {
      return { ok: false, blocked: true, reason: 'Posting blocked: AI/spend is disabled or paused by the operator.' };
    }
  } catch (e) {
    return { ok: false, blocked: true, reason: 'Could not read the kill switch; refusing to post.' };
  }
...
module.exports = {
  postToBluesky: postToBluesky, deleteBlueskyPost: deleteBlueskyPost,
