...
async function authorize(store, productDomain, lane, now, deps) {
  ...
  if (!policy.enabled) return held('finance-subscriber-email-switch-closed');
  if (!policy.observerEnabled) return held('finance-subscriber-outcome-observer-switch-closed');
  try {
    store.assertDurable();
