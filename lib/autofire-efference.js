...
var RECORD_TTL_SECONDS = 90 * 86400;
var LOG_CAP = 1000;
...
function pendingKey(lane, cik, sourceIdentity) {
  return 'autofire_efference_pending:' + lane + ':' +
    hash({ cik: String(cik), sourceIdentity: sourceIdentity }).slice(0, 24);
}
...
function validateCommand(spec) {
  ...
  if (!spec.cik) return 'missing-cik';
...
async function retireUnresolved(db, prior, at, reason) {
  ...
  await db.set(recordKey(prior.id), unresolved, RECORD_TTL_SECONDS);
...
async function command(db, spec) {
  ...
  var pending = pendingKey(spec.lane, spec.cik, spec.sourceIdentity);
  var priorPending;
  ...
  try {
    await db.set(recordKey(id), copy, RECORD_TTL_SECONDS);
  } catch (err) {
    return { ok: false, error: 'efference-copy-not-persisted', detail: err.message };
  }
  try {
    await db.set(pending, { id: copy.id, emittedAt: copy.emittedAt }, RECORD_TTL_SECONDS);
    ...
    try { await db.set(recordKey(id), aborted, RECORD_TTL_SECONDS); } catch (_) {}
    return {
      ok: false,
...
async function resolve(db, copyOrId, result, resolvedAt) {
  ...
  try {
    await db.set(recordKey(id), resolved, RECORD_TTL_SECONDS);
  } catch (err) {
    return {
      ok: false,
