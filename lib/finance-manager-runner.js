...
async function run(input, options) {
  ...
  if (!response || response.ok !== true || typeof response.text !== 'string' || !response.text.trim()) {
    return afterProvider({ ok: false, status: 'ABSTAINED', reason: response && response.disabled ? 'finance_manager_ai_disabled' : 'finance_manager_provider_no_response' }, response);
  }
