...
async function run(input, options) {
  ...
  if (universe) {
    ...
    // Derived investment-margin arithmetic and ordering belong to code, not
    // the provider. Carry only the deterministic normalization downstream.
    adapted.proposal.projectedMarginRanking = clone(ranked.ranking);
  ...
  return afterProvider({
    ...
    proposal: clone(adapted.proposal),
    projectedMarginRanking: clone(adapted.proposal.projectedMarginRanking || null),
    selectedCompany: clone(adapted.proposal.company),
    candidate: candidate
  }, response);
}
