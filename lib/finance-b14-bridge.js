...
function intentFor(selection, tradeIntent) {
  ...
  var released = selection.criticDecision && selection.criticDecision.released;
  var candidateKindId = released && (released.candidateId || released.candidate && released.candidate.id) || null;
  if (!candidateKindId) fail('FINANCE_B14_MISSING_CRITIC_CANDIDATE_ID', 'the released B10 candidate identity is required');
  var composed = Object.assign({}, tradeIntent, {
    sourceArtifactId: String(sourceArtifactId),
    selectionId: identity.selectionId,
    ownerDomain: 'finance',
    // The reusable critic candidate identifies the action kind.  The released
    // selection identifies this exact packet/company/evidence episode and must
    // therefore be the durable causal action identity.
    actionId: identity.selectionId,
    decisionContext: {
      selectionId: identity.selectionId,
      sourceIdentity: identity.sourceIdentity,
      releaseAt: selection.at,
      cycleEvidence: selection.evidence || null,
      criticCandidateId: candidateKindId,
