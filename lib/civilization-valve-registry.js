...
var LINES = [
  ...
  line({ id: 'communication:social', productDomain: 'communication', ownerDomain: 'communication', lane: 'social', source: 'Exact subject-domain stress artifact + subject-domain release + Communication channel release', actionRoute: 'social-cron', schedule: '*/15 * * * *', destination: 'Bluesky PDS', hardGates: ['SOCIAL_POSTING_ENABLED'], observerRoute: 'communication-social-outcome-observer', observerSchedule: '6,21,36,51 * * * *', recoveryRoute: 'communication-social-recovery' }),
  line({ id: 'communication:social-commissioning', productDomain: 'communication', ownerDomain: 'communication', lane: 'social-commissioning', source: 'Fixed non-commercial marker only; no domain content selection', decision: 'permanent one-shot commissioning claim', motor: 'bounded create and rollback command', receipt: 'Bluesky create identity plus independent public presence and absence receipts', learning: 'short-lived capability lease renewed only from completed durable commissioning evidence', actionRoute: 'communication-social-capability', schedule: '*/2 * * * *', destination: 'Bluesky PDS temporary marker', hardGates: ['COMMUNICATION_SOCIAL_COMMISSIONING_ENABLED', 'SOCIAL_POSTING_ENABLED'], observerRoute: 'communication-social-capability', recoveryRoute: 'communication-social-capability' }),
  line({ id: 'communication:youtube', productDomain: 'communication', ownerDomain: 'communication', lane: 'video-publication', source: 'Exact subject-domain stress artifact + subject release + Communication selection + verified local render hash', actionRoute: 'communication-video-upload-work', schedule: 'local media worker pull', destination: 'YouTube Data API v3 · private until OAuth client audit permits public publication', hardGates: ['COMMUNICATION_VIDEO_UPLOAD_ENABLED', 'MEDIA_WORKER_TOKEN'], observerRoute: null, observerStatus: 'NOT_COMMISSIONED_PRIVATE_PROVIDER_READBACK_ONLY', recoveryRoute: 'communication-video-upload-work', recoveryStatus: 'IMPLEMENTED_DURABLE_RECEIPT_REPLAY_NO_PROVIDER_RETRY', learning: 'private provider receipt plus lost-acknowledgement recovery; independent public outcome remains explicitly uncommissioned' }),
