...
function buildState(item) {
  ...
  if (item.id === 'finance:broker-order') return Object.assign({}, common, {
    sequence: 'JOB_7_CURRENT',
    present: 'Finance paper-investment pilot: feed-confirmed selection and sandbox motor are active development; a mature independently graded broker outcome is not complete.',
  ...
  if (item.id === 'science:research-papers') return Object.assign({}, common, {
    sequence: 'JOB_7_CURRENT',
    present: 'Science developmental research pilot: one bounded internal artifact/provider receipt exists; independent evaluation and learned recovery remain incomplete.',
  ...
  if (item.id === 'medicine:research-papers') return Object.assign({}, common, {
    sequence: 'JOB_7_CURRENT',
    present: 'Medicine developmental research pilot: the bounded lane and permanent one-attempt contract exist; production artifact/evaluation proof remains incomplete.',
    future: 'Create the first genuine bounded artifact, collect independent evaluations, and prove recovery before any publication or sale lane.'
  });
