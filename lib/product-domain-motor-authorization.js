...
var capabilityContract = require('./product-domain-motor-capability.js');
...
async function authorize(store, productDomain, lane, now) {
  try {
    ...
    var capability = await capabilityContract.verifyPair(store, receipt, at);
    if (!capability.ok) return held(capability.reason, receipt);
    return {
      ok: true,
      authorized: true,
      status: 'AUTHORIZED',
