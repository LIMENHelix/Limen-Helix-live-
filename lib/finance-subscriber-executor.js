...
/** Finance-local capped batch executor with durable per-message identity. */
...
var Learning = require('./finance-subscriber-learning.js');
var AdapterGuard = require('./civilization-adapter-guard.js');
var SCHEMA = 'finance-subscriber-command/1.0';
var LOG_KEY = 'finance_subscriber_command_log', PENDING_LOG_KEY = 'finance_subscriber_pending_log';
var PREFIX = 'finance_subscriber_command:', MOTOR_PREFIX = 'finance_subscriber_motor_claim:', ACTION_PREFIX = 'finance_subscriber_action:';
var BUDGET_PREFIX = 'finance_subscriber_budget_slot:';
var SUPPRESSION_KEY = 'finance_subscriber_suppression_catalog';
...
async function claimBudgetSlot(store, actionId, slots, now, costUsd) {
  ...
  return { ok: false, reason: 'finance-subscriber-daily-budget-exhausted' };
}
async function setCommand(store, value) { await store.set(commandKey(value.commandId), value); var r = await store.get(commandKey(value.commandId));
  if (!r || r.commandId !== value.commandId || r.status !== value.status) throw new Error('finance subscriber command readback invalid'); return r; }
function held(reason, extra) { return Object.assign({ ok: true, status: 'HELD', accepted: 0, reason: reason, productDomain: 'finance', ownerDomain: 'finance', lane: 'subscriber-email', liveMoney: false }, extra || {}); }
async function execute(input) {
  ...
  if (!cap) return held('finance-subscriber-send-cap-zero');
  ...
  if (input.emailCostUsd == null || !Number.isFinite(emailCostUsd) || emailCostUsd < 0) return held('finance-subscriber-email-unit-cost-not-configured');
  if (!dailySendCap) return held('finance-subscriber-daily-send-cap-zero');
  if (emailCostUsd > 0 && (!Number.isFinite(dailyBudgetUsd) || dailyBudgetUsd < emailCostUsd)) return held('finance-subscriber-daily-dollar-budget-not-configured-or-too-small');
  var dailySlots = Math.min(dailySendCap, emailCostUsd === 0 ? dailySendCap : Math.floor((dailyBudgetUsd + 1e-12) / emailCostUsd));
  if (!dailySlots) return held('finance-subscriber-daily-budget-zero');
  specs = specs.filter(function (s) { return s && Decision.validateReceipt(s.decision, s.candidate, now); }).slice(0, cap);
  if (!specs.length) return held('finance-subscriber-no-released-exact-decisions');
  try {
    ...
    if (!specs.length) return held('finance-subscriber-all-candidates-suppressed');
    var authorize = input.motorAuthorization || MotorAuthorization;
    var motor = await authorize.authorize(store, 'finance', 'subscriber-email', now);
    if (!motor || !motor.authorized) return held(motor && motor.reason || 'finance-subscriber-motor-held', { motorReceiptId: motor && motor.receiptId || null, motorBlockers: motor && motor.blockers || [] });
    var commandId = 'fsc_' + hash({ motor: motor.receiptId, actions: specs.map(function (s) { return s.decision.actionId; }) }).slice(0, 24);
    ...
    var command = { schemaVersion: SCHEMA, commandId: commandId, status: 'COMMANDING', productDomain: 'finance', ownerDomain: 'finance', lane: 'subscriber-email',
    ...
    command = await store.get(commandKey(commandId)); if (!command || command.status !== 'COMMANDING') throw new Error('finance subscriber pre-dispatch command readback invalid');
    var motorClaim = { schemaVersion: SCHEMA, commandId: commandId, productMotorReceiptId: motor.receiptId, claimedAt: now };
    if (!(await store.setIfAbsent(motorClaimKey(motor.receiptId), motorClaim))) { command.status = 'REFUSED'; command.reason = 'finance-subscriber-motor-receipt-already-consumed'; return setCommand(store, command); }
    var restoredMotorClaim = await store.get(motorClaimKey(motor.receiptId));
    if (!restoredMotorClaim || restoredMotorClaim.commandId !== commandId) throw new Error('finance subscriber motor claim readback invalid');
    await store.lpush(PENDING_LOG_KEY, command); await store.ltrim(PENDING_LOG_KEY, 0, 999);
    var transport = input.transport; if (!transport || typeof transport.send !== 'function') throw new Error('finance subscriber transport missing');
    for (var i = 0; i < specs.length; i++) {
      var spec = specs[i], item = command.items[i], idempotencyKey = 'finance-digest/' + item.actionId;
      ...
      if (!restoredClaim || restoredClaim.commandId !== commandId || restoredClaim.status !== 'DISPATCHING') throw new Error('finance subscriber action claim readback invalid');
      ...
      var result; try {
        item.adapterGuard = await (input.adapterGuard || AdapterGuard).checkpoint(store, 'finance:subscriber-email', 'resend-subscriber-email');
        result = await transport.send(spec.candidate.email, spec.candidate.subject, spec.candidate.body, { idempotencyKey: idempotencyKey });
      }
      ...
      if (!restoredAction || restoredAction.status !== item.status || restoredAction.commandId !== commandId) throw new Error('finance subscriber action readback invalid');
  ...
  } catch (error) { return { ok: false, status: 'REFUSED', accepted: 0, reason: 'finance-subscriber-strict-boundary-unavailable', detail: String(error && error.message || error), liveMoney: false }; }
}
