...
function candidate(state, action, mail) { var email = text(state && state.email), leadId = text(state && state.leadId), domain = text(state && state.domain);
  ...
  if (!email || !leadId || domain !== 'intelligence' || state.consent !== true || !subject || !body || !kind || !channel || !/email/.test(channel)) return null;
  return { schemaVersion: 'intelligence-autopilot-candidate/1.0', productDomain: 'intelligence', ownerDomain: 'intelligence', lane: 'autopilot',
    ...
    consentVerified: true, actionKind: kind, channel: channel, transition: text(action.transition), subject: subject, subjectHash: hash(subject), body: body, contentHash: hash(body), liveMoney: false };
}
function validCandidate(v) { return !!(v && v.schemaVersion === 'intelligence-autopilot-candidate/1.0' && v.productDomain === 'intelligence' && v.ownerDomain === 'intelligence' &&
  v.lane === 'autopilot' && text(v.leadId) && v.leadHash === hash(v.leadId) && text(v.email) && v.emailHash === hash(v.email) && v.subjectDomain === 'intelligence' && v.consentVerified === true &&
