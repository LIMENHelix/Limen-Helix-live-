...
var Decision = require('./economy-investment-decision.js');
var Motor = require('./product-domain-motor-authorization.js');
var Developmental = require('./economy-investment-developmental-authority.js');
var Learning = require('./economy-investment-learning.js');
var B14 = require('./tradier-b14.js');
var SCHEMA = 'economy-investment-command/1.0';
var LOG_KEY = 'economy_investment_command_log';
var PREFIX = 'economy_investment_command:';
var ACTION_PREFIX = 'economy_investment_action:';
var MOTOR_PREFIX = 'economy_investment_motor_claim:';
var BUDGET_PREFIX = 'economy_investment_budget_slot:';
...
function held(reason, extra) { return Object.assign({ ok: true, status: 'HELD', accepted: 0, reason: reason, productDomain: 'economy', ownerDomain: 'economy', lane: 'investments', paperOnly: true, liveMoney: false }, extra || {}); }
...
async function save(store, command) {
  ...
  if (!restored || restored.status !== command.status || restored.actionId !== command.actionId) throw new Error('economy investment command readback invalid');
  return restored;
}
async function budgetSlot(store, actionId, now, cap, reservedNotional) {
  ...
  return { ok: false, reason: 'economy-investment-daily-order-cap-exhausted' };
}
async function execute(options) {
  ...
  if (!Decision.validateReceipt(decision, candidate, now)) return held('economy-investment-exact-b10-decision-required');
  if (!on(env.ECONOMY_INVESTMENT_PAPER_ORDER_ENABLED)) return held('economy-investment-paper-order-switch-off');
  if (!on(env.ECONOMY_INVESTMENT_RECOVERY_ENABLED)) return held('economy-investment-recovery-switch-must-be-on-before-entry');
  ...
  if (!Number.isFinite(hardMax) || hardMax <= 0) return held('economy-investment-max-notional-not-configured');
  if (!Number.isFinite(dailyBudget) || dailyBudget <= 0 || !cap) return held('economy-investment-daily-budget-or-cap-not-configured');
  var perSlot = dailyBudget / cap;
  if (candidate.maxNotionalUsd > hardMax || candidate.maxNotionalUsd > perSlot) return held('economy-investment-candidate-exceeds-conservative-budget-slot');
  if (!broker || typeof broker.quote !== 'function' || typeof broker.accountSnapshot !== 'function') return held('economy-investment-broker-adapter-missing');
  try {
    store.assertDurable();
    var authorization = await (options.motorAuthorization || Motor).authorize(store, 'economy', 'investments', now);
    if (!authorization || !authorization.authorized) {
      authorization = await (options.developmentalAuthorization || Developmental).authorize(store, candidate.requestId, env, now);
      if (!authorization || !authorization.authorized) return held(authorization && authorization.reason || 'economy-investment-motor-held', { authorization: authorization || null });
    }
    ...
    if (!limitPrice || !Number.isFinite(benchmarkBaseline) || benchmarkBaseline <= 0) return held('economy-investment-market-quote-unmeasured');
    var quantity = Math.floor(candidate.maxNotionalUsd / limitPrice);
    if (quantity < 1) return held('economy-investment-notional-cannot-buy-one-whole-paper-share');
    ...
    var commandId = 'eic_' + Decision.hash({ action: decision.actionId, motor: authorization.receiptId, quantity: quantity, limitPrice: limitPrice }).slice(0, 24);
    var command = { schemaVersion: SCHEMA, commandId: commandId, actionId: decision.actionId, decisionReceiptId: decision.decisionReceiptId,
      status: 'COMMANDING', productDomain: 'economy', ownerDomain: 'economy', lane: 'investments', productMotorReceiptId: authorization.receiptId,
    ...
    if (!command || command.status !== 'COMMANDING') throw new Error('economy investment pre-dispatch command readback invalid');
    if (!await store.setIfAbsent(motorKey(authorization.receiptId), { schemaVersion: SCHEMA, commandId: commandId, actionId: decision.actionId, claimedAt: now })) {
      command.status = 'REFUSED'; command.reason = 'economy-investment-motor-receipt-already-consumed'; return save(store, command);
    }
    var motorClaim = await store.get(motorKey(authorization.receiptId));
    if (!motorClaim || motorClaim.commandId !== commandId) throw new Error('economy investment motor claim readback invalid');
    ...
    var preview = await b14.createPreview(store, broker, { symbol: candidate.symbol, side: candidate.side, quantity: quantity,
      ...
      thesisId: candidate.thesisId, selectionId: decision.decisionReceiptId, actionId: decision.actionId, ownerDomain: 'economy', horizonDays: [30, 60, 90],
      decisionContext: { economyPacketId: decision.economyPacketId, brainOpportunityId: decision.brainOpportunityId, evidenceHash: candidate.evidenceHash } }, now);
    command.previewId = preview.previewId; command.status = 'PREVIEWED'; command = await save(store, command);
    var learnedCause = await Learning.recordCommand(store, candidate, decision, command, now);
    if (!learnedCause || !learnedCause.ok || !learnedCause.command) throw new Error('economy investment causal command memory unavailable');
    command.learningEpisodeId = learnedCause.command.episodeId; command.learningCauseDurable = true; command = await save(store, command);
    var brokerCommand = await b14.submitApproved(store, broker, {
      previewId: preview.previewId, confirmation: preview.confirmationSummary,
      approval: { mode: 'domain-autonomous', actor: 'economy-brain', ownerDomain: 'economy',
        authorizationReceiptId: authorization.receiptId, authorizationMode: authorization.authorizationMode || 'mature-production-capability' }
    }, now);
    ...
    if (!actionReadback || actionReadback.status !== command.status) throw new Error('economy investment action receipt readback invalid');
    ...
    return { ok: false, status: 'UNRESOLVED', accepted: 0, reason: 'economy-investment-strict-boundary-unavailable', detail: String(error && error.message || error), paperOnly: true, liveMoney: false };
  }
}
