...
async function observeExternalFeed(store, handle, commands, now, deps) {
  ...
  for (var i = 0; i < posts.length; i++) {
    ...
    var receipt = {
      ...
      firstObservedAt: prior && Number(prior.firstObservedAt) || at, observedAt: at,
  ...
  return { ok: true, inspected: posts.length, observedExternal: receipts.length,
    newlyObservedExternal: receipts.filter(function (row) { return row.firstObservedAt === at; }).length,
    receipts: receipts };
}
