...
var LEARNING_PENDING_LOG_KEY = 'communication_social_learning_pending_log';
var KEY_PREFIX = 'communication_social_observation:';
var EXTERNAL_LOG_KEY = 'communication_social_external_observation_log';
var EXTERNAL_KEY_PREFIX = 'communication_social_external_observation:';
var LOG_CAP = 1000;
var APPVIEW = 'https://public.api.bsky.app';
var TIMEOUT_MS = 15000;
var RECONCILE_WINDOW_MS = 5 * 24 * 60 * 60 * 1000;
var COMMAND_MATCH_LAG_MS = 5 * 60 * 1000;
var MAX_NEW_EXTERNAL_PER_RUN = 5;
...
function externalObservationKey(uri) {
  var id = text(uri);
  if (!id) throw new Error('communication social observer: external post uri required');
  return EXTERNAL_KEY_PREFIX + hash(id).slice(0, 32);
}
...
async function reconcilePending(store, pending, handle, now, deps) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (current && current.status === 'POSTED' && current.receipt && current.receipt.readbackVerified === true &&
        text(current.receipt.uri) && text(current.receipt.cid)) commands.push(current);
  ...
  if (!unresolved.length) return { inspected: rows.length, unresolved: 0, reconciled: 0, receipts: [], commands: commands };
  ...
  return { inspected: rows.length, unresolved: unresolved.length, reconciled: receipts.length,
    receipts: receipts, commands: commands };
}
...
async function observeExternalFeed(store, handle, commands, now, deps) {
  deps = deps || {};
  var at = Number.isFinite(Number(now)) ? Number(now) : Date.now();
  store.assertDurable();
  var known = Object.create(null);
  (commands || []).forEach(function (command) {
    var receipt = command && command.receipt || {};
    if (text(receipt.uri) && text(receipt.cid)) known[receipt.uri + '\u0000' + receipt.cid] = true;
  });
  var posts = await authorFeed(handle, deps.fetch);
  var receipts = [];
  var externalCandidates = 0;
  var maxNew = Number.isFinite(Number(deps.maxNew))
    ? Math.max(0, Math.floor(Number(deps.maxNew))) : MAX_NEW_EXTERNAL_PER_RUN;
  for (var i = 0; i < posts.length; i++) {
    var post = posts[i];
    var identity = postIdentity(post);
    var record = post && post.record || {};
    var createdAt = Date.parse(record.createdAt);
    if (!identity || known[identity.uri + '\u0000' + identity.cid] || !text(record.text) || !Number.isFinite(createdAt)) continue;
    externalCandidates++;
    var metrics = {
      replies: normalizeCount(post.replyCount), reposts: normalizeCount(post.repostCount),
      likes: normalizeCount(post.likeCount), quotes: normalizeCount(post.quoteCount)
    };
    metrics.total = metrics.replies + metrics.reposts + metrics.likes + metrics.quotes;
    var key = externalObservationKey(identity.uri);
    var receipt = {
      schemaVersion: 'communication-social-external-observation/1.0',
      observationId: 'cseo_' + hash({ uri: identity.uri, cid: identity.cid }).slice(0, 24),
      productDomain: 'communication', ownerDomain: 'communication', lane: 'social',
      status: 'OBSERVED_EXTERNAL', attribution: 'UNJOINED',
      postReceipt: identity, contentHash: contentHash(record.text),
      createdAt: new Date(createdAt).toISOString(), firstObservedAt: at, observedAt: at,
      sourceIdentity: { kind: 'bluesky-public-author-feed', value: identity.uri,
        provider: 'bluesky-public-appview', endpointHost: 'public.api.bsky.app' },
      metrics: metrics,
      eligibleForExecutionProof: false, eligibleForLearning: false,
      reason: 'no-matching-limen-command-receipt', liveMoney: false
    };
    // External posts are context only and never earn motor or learning credit.
    // Preserve the first observation instead of rewriting up to fifty records
    // on every run ahead of owned-command outcomes.
    var created = await store.setIfAbsent(key, receipt);
    if (!created) continue;
    var restored = await store.get(key);
    if (!restored || restored.observationId !== receipt.observationId ||
        restored.status !== 'OBSERVED_EXTERNAL' || restored.eligibleForLearning !== false) {
      throw new Error('communication social observer: external observation readback invalid');
    }
    await store.lpush(EXTERNAL_LOG_KEY, restored);
    await store.ltrim(EXTERNAL_LOG_KEY, 0, LOG_CAP - 1);
    receipts.push(restored);
    if (receipts.length >= maxNew) break;
  }
  return { ok: true, inspected: posts.length, externalCandidates: externalCandidates,
    observedExternal: receipts.length, newlyObservedExternal: receipts.length,
    boundedNewPerRun: maxNew, receipts: receipts };
}
...
async function observeOne(store, post, now, deps) {
  ...
  var receipt = {
    ...
    observedAt: at,
    commandId: text(post && post.commandId),
    paperOnly: false,
    liveMoney: false
  };
  if (receipt.commandId) await store.lpush(LEARNING_PENDING_LOG_KEY, receipt);
...
module.exports = {
  SCHEMA: SCHEMA,
  LOG_KEY: LOG_KEY,
  LEARNING_PENDING_LOG_KEY: LEARNING_PENDING_LOG_KEY,
  KEY_PREFIX: KEY_PREFIX,
  EXTERNAL_LOG_KEY: EXTERNAL_LOG_KEY,
  EXTERNAL_KEY_PREFIX: EXTERNAL_KEY_PREFIX,
  MAX_NEW_EXTERNAL_PER_RUN: MAX_NEW_EXTERNAL_PER_RUN,
  APPVIEW: APPVIEW,
  postIdentity: postIdentity,
  observationKey: observationKey,
  externalObservationKey: externalObservationKey,
  appviewRead: appviewRead,
  authorFeed: authorFeed,
  contentHash: contentHash,
  reconcilePending: reconcilePending,
  observeExternalFeed: observeExternalFeed,
