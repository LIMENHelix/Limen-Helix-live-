...
function calibration(forecasts, resolveResult) {
  ...
  return {
    ...
    directionHitRate: hitRate, byDirection: byDirection,
...
function buildProposal(domain, calib, drift, opts) {
  ...
  if (drift && drift.available) {
    if (drift.stability === 'runaway') {
      ...
      recs.push({ kind: 'promotion-candidate', reason: 'stable weights + hit-rate ' + calib.directionHitRate + ' >= 0.6', proposedChange: 'candidate for arming — HUMAN REVIEW REQUIRED, not auto-armed', applied: false });
    }
  }
