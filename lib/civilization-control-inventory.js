...
var Crons = require('../vercel.json').crons || [];
...
function inferDomain(name) {
  ...
  var domains = Registry.LINES.map(function (x) { return x.productDomain; });
...
function triggerInventory() {
  return Registry.LINES.map(function (line) { return line.productDomain; }).map(function (domain) {
    var table = TRIGGER_TABLES[domain];
    return {
      domain: domain,
      status: table ? 'MAPPED' : 'MISSING_DOMAIN_ALIAS_TABLE',
      conditionCodes: table ? keys(table.conditionCodes).sort() : [],
      diagnoses: table ? keys(table.diagnoses).sort() : [],
      pathway: domain + ' feed evidence → ' + domain + ' condition classifier → ' + domain + ' diagnosis gate → ' + domain + ' brain activation/extinction',
      authority: 'AUTOMATIC_COMPUTED_NOT_MANUAL',
      source: table ? 'assets/data/' + domain + '-condition-trigger-aliases.json' : null
    };
  });
}
function snapshot(env) {
  ...
  var conditionCount = triggers.reduce(function (n, x) { return n + x.conditionCodes.length; }, 0);
  var diagnosisCount = triggers.reduce(function (n, x) { return n + x.diagnoses.length; }, 0);
  return {
    ...
    summary: {
      ...
      missingDomainTriggerMaps: triggers.filter(function (x) { return x.status !== 'MAPPED'; }).map(function (x) { return x.domain; }),
      brainBlocksPerDomain: BRAIN_BLOCKS.length,
      brainBlockInstances: BRAIN_BLOCKS.length * Registry.LINES.length,
      phasesPerDomain: PhaseSpec.PHASES.length,
      phaseInstances: PhaseSpec.PHASES.length * Registry.LINES.length
    },
