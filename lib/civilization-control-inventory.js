...
function cronKind(path) {
  ...
  if (/cycle|autofire|social-cron|subscriber-digest|hero-image|autopilot|g0-domain-loop|g0-desk-fulfillment/.test(path)) return 'MOTOR_OR_DECISION';
  if (/ingest|snapshot|stress-refresh|feed-record|domain-text-read/.test(path)) return 'SENSING';
  if (/brain|score|resolve|sleep|g0-domain-comprehension/.test(path)) return 'COGNITION_OR_MAINTENANCE';
  return 'OTHER';
}
