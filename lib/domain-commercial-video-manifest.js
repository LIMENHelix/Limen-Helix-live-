...
function build(contract, state, artifact, nowValue) {
  ...
  var beats = [
    ...
    },
    {
      ...
      narration: wordClip(source.publisher, 4, 42) + ' feed title: ' + wordClip(source.title, 15, 115) + '.',
...
async function persist(store, contract, result) {
  ...
  // The immutable record and latest pointer may survive while a list append
  // fails. Inspect the log independently on every retry; creation status alone
  // cannot prove the provenance index exists.
  var indexed = await store.lrange(contract.videoManifestLog, 0, 199);
  if (!indexed.some(function (row) { return row && row.manifestId === restored.manifestId; })) {
