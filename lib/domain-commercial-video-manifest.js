...
var LOG_CLAIM_LEASE_MS = 20 * 60 * 1000;
...
function sourceRow(source) {
  return { sourceIdentity: source.sourceIdentity, title: source.title,
    publisher: source.publisher, sourceUrl: source.sourceUrl,
    authority: source.authority, fullTextVerified: false };
}
function expectedBeats(contract, artifact, source) {
  var domain = contract.productDomain, domainLabel = label(domain), stress = stressLabel(artifact.sourceStress);
  return [
...
function speechFits(beat) {
  var seconds = Number(beat && beat.durationSeconds), narration = beat && text(beat.narration);
  return !!(Number.isFinite(seconds) && seconds > 0 && narration &&
    wordCount(narration) <= Math.ceil(seconds * 3) && Array.from(narration).length <= Math.ceil(seconds * 20));
}
function sameSource(a, b) {
  return !!(a && b && hash(a.sourceIdentity) === hash(b.sourceIdentity) && a.title === b.title &&
    a.publisher === b.publisher && a.sourceUrl === b.sourceUrl && a.authority === b.authority &&
    a.fullTextVerified === false && b.fullTextVerified === false);
}
function comparePlans(a, b) {
  var ap = Number(a && a.sourcePlannedAt), bp = Number(b && b.sourcePlannedAt);
  if (Number.isFinite(ap) && Number.isFinite(bp) && ap !== bp) return ap - bp;
  var ai = String(a && a.sourceIntentId || ''), bi = String(b && b.sourceIntentId || '');
  if (ai !== bi) return ai < bi ? -1 : 1;
  var at = Number(a && a.preparedAt || 0), bt = Number(b && b.preparedAt || 0);
  if (at !== bt) return at - bt;
  var am = String(a && a.manifestId || ''), bm = String(b && b.manifestId || '');
...
function build(contract, state, artifact, nowValue) {
  ...
  var beats = expectedBeats(contract, artifact, source);
  ...
  var sourceLedger = [sourceRow(source)];
  ...
  return {
    ...
    manifest: {
      ...
      sourcePlannedAt: Number(artifact.sourcePlannedAt),
...
function validManifest(contract, manifest, state, artifact, nowValue) {
  try {
    var now = Number.isFinite(Number(nowValue)) ? Number(nowValue) : Date.now();
    var beats = list(manifest && manifest.beats), source = manifest && list(manifest.sourceLedger)[0];
    var artifactSource = artifact && list(artifact.sourceLedger)[0];
    if (beats.length !== 4 || beats.some(function (beat) { return !beat || typeof beat !== 'object' || !speechFits(beat); })) return false;
    ...
    Number(manifest.sourcePlannedAt) === Number(artifact.sourcePlannedAt) &&
    ...
    source && artifactSource && sameSource(source, sourceRow(artifactSource)) &&
    text(source.publisher) && source.publisher !== 'Publisher not supplied' &&
    hash(beats) === hash(expectedBeats(contract, artifact, sourceRow(artifactSource))) &&
    beats.reduce(function (sum, beat) { return sum + Number(beat.durationSeconds || 0); }, 0) ===
...
async function ensureLogged(store, contract, manifest) {
  ...
  try {
    ...
    if (!await store.indexListMemberIfValue(markerKey, claim, contract.videoManifestLog, row, 199, done)) {
      marker = await store.get(markerKey);
      if (!marker || marker.status !== 'INDEXED' || marker.manifestId !== manifest.manifestId) {
        throw new Error('domain commercial video manifest log lease lost before atomic append');
      }
    }
    var indexed = await store.lrange(contract.videoManifestLog, 0, 199);
    if (!indexed.some(function (entry) { return entry && entry.manifestId === manifest.manifestId; })) {
...
async function persist(store, contract, result) {
  ...
  for (var attempt = 0; attempt < 3; attempt++) {
    var sources = await Promise.all([store.get(contract.stateKey), store.get(contract.artifactStateKey),
      store.get(contract.videoManifestStateKey)]);
    var sourceState = sources[0], sourceArtifact = sources[1], current = sources[2];
    if (!validManifest(contract, restored, sourceState, sourceArtifact, Date.now())) {
      return current && validManifest(contract, current, sourceState, sourceArtifact, Date.now()) ? current
        : { status: 'ABSTAINED', reason: 'source-plan-advanced-before-video-promotion', manifest: null };
    }
    if (current && comparePlans(current, restored) > 0) return current;
    var written = await store.setIfSourcesAndCurrent(contract.stateKey, sourceState,
      contract.artifactStateKey, sourceArtifact, contract.videoManifestStateKey, current, restored);
...
module.exports = { SCHEMA: SCHEMA, build: build, validManifest: validManifest,
  ...
  expectedBeats: expectedBeats, speechFits: speechFits, sameSource: sameSource,
