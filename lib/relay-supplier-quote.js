...
async function revalidate(input) {
  ...
  const held = await cj.stock(vid, country);
  ...
  const effectiveCost = _round(cost - parseFloat(o.sourceShipping) + q.price);
  if (effectiveCost > cost * (1 + COST_DRIFT_TOLERANCE)) {
    return {
      ok: false,
      code: 'cost-drift',
      reason: 'shipping to this address costs $' + q.price.toFixed(2) + ' against $' +
