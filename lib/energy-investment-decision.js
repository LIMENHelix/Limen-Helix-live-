...
async function decide(store, value, now, deps) {
  ...
  try {
    ...
    if (!blockers.length) {
      if ((cognition.immune || {}).immuneState !== 'clear') blockers.push('energy-immune-veto');
    ...
    return persist(store, {
      schemaVersion: SCHEMA, decisionReceiptId: 'end_' + hash({ action: actionId, packet: packet.packetId || null, status: status, blockers: blockers }).slice(0, 24),
