...
var MAP = {
  ...
  industry: {
    source: 'NHTSA Recalls API via /api/industry-tools',
    phases: {
      p0: { signal: 'open safety recalls for your year, make and model', field: 'recallsByVehicle', verified: true, evidence: 'NHTSA results include parkIt and parkOutSide' },
      p1: { signal: 'a park-it or park-outside recall first appears for your vehicle', field: 'parkIt, parkOutSide, ReportReceivedDate', verified: true, evidence: 'structured flags on each recall row' },
      p2: { signal: 'the recurring recall brief as NHTSA republishes', field: 'recallsByVehicle refresh', verified: true, evidence: 'handler caches six hours and re-reads NHTSA' }
    }
  },

  communication: {
    source: 'Federal Register FCC documents via /api/communication-live',
    phases: {
      p0: { signal: 'recent FCC actions and communication coverage', field: 'fccDocs, themes', verified: true, evidence: 'Federal Register agency=FCC, last 30 days' },
      p1: { signal: 'a new FCC rule, notice or licence action first appears in your lane', field: 'type, publication_date', verified: true, evidence: 'document type and date on every row' },
      p2: { signal: 'the recurring FCC docket for proceedings, licences or bands you follow', field: 'fccDocs rollup', verified: true, evidence: 'daily Register source' }
    }
  },

  infrastructure: {
    source: 'FAA NAS status via /api/infrastructure-live',
    phases: {
      p0: { signal: 'ground stops, delays and closures right now', field: 'Ground_Stop, Ground_Delay, Airport_Closure', verified: true, evidence: 'nasstatus.faa.gov airport-status-information' },
      p1: { signal: 'FAA first posts a ground stop, delay or closure at an airport you watch', field: 'ARPT first appearance', verified: true, evidence: 'airport code on every disruption row' },
      p2: { signal: 'the recurring airport and corridor brief', field: 'FAA refresh ~10 min', verified: true, evidence: 'handler TTL 10 minutes' }
    }
  },

  energy: {
    source: 'Energy Bill X-Ray + /api/energy-entry utility aggregates',
    phases: {
      p0: { signal: 'the current bill x-ray for your utility and ZIP', field: 'bill, kwh, utility', verified: true, evidence: 'public /energy tool plus anonymized aggregate' },
      p2: { signal: 'the recurring utility rate and bill-pressure read', field: 'energy:agg:v1 utilities', verified: true, evidence: 'aggregate updated on each opted-in reading' }
    }
  },

  culture: {
    source: 'Royalty X-Ray on /culture + /api/culture-markets',
    phases: {
      p0: { signal: 'what your monthly streams pay across platforms', field: 'streams, mix, deal', verified: true, evidence: 'public /culture calculator' },
      p2: { signal: 'the monthly royalty and audience re-cut for the lanes you name', field: 'history + culture-markets', verified: true, evidence: 'browser history plus market quotes' }
    }
  }
