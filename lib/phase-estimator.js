 ...
 *  - `stuck` is the current LOW-SIDE transition-failure component, orthogonal to phase; it is
 *    not the complete two-sided distress state. High-side runaway/premature transition is not yet
...
function estimate(bundle, opts) {
  ...
  // informative[k] marks whether a channel supplied a REAL phase-signature (a likelihood vector) vs a
  // structural/context channel (unison, granularity — likelihood:null by design). This distinction is
  // load-bearing: a uniform likelihood adds the SAME constant to every phase's logpost, which is a
  // mathematical no-op after softmax regardless of the channel's weight. It CANNOT move belief shape.
  var keys = [], z = {}, L = {}, rcRaw = {}, usedHint = {}, informative = {}, anyUntransformed = false, anySelfConsistency = false;
  for (var i = 0; i < readings.length; i++) {
    ...
    var hasLikelihood = Array.isArray(rd.likelihood) && rd.likelihood.length === NUM_PHASES;
    informative[k] = hasLikelihood;
    L[k] = hasLikelihood ? normalize(rd.likelihood) : uniform();

    // raw precision: independent precisionHint preferred (true inverse-variance), else self-consistency.
    // UNINFORMATIVE (null-likelihood) channels get NO rcRaw here — they are excluded from the precision/
    // fusion machinery entirely (see below). BUG FIXED 2026-07-20, found in live production: a uniform
    // likelihood trivially "agrees" with an already-uncertain (near-uniform) fused belief, so its
    // self-consistency residual collapses toward 0 and its precision RUNS AWAY toward infinity — which
    // then dilutes every other (real) channel's weight via the shared totalPrecision denominator, even
    // though the uninformative channel itself cannot move belief shape at all. Confirmed live: unison/
    // granularity precision-inflation silenced a real feed_supply=0.941 signal during an actual crisis,
    // reporting confidence 0.999 over an essentially uniform (uninformative) belief.
    if (hasLikelihood) {
  ...
  // ── Decorrelation (eff_c) + staleness (rec_c) => final precision rc — INFORMATIVE CHANNELS ONLY ────
  var corr = updateCorrelation(keys, z, prev);   // z-value correlation tracked for ALL channels (unrelated to the fix; used for eff_c below)
  var rc = {}, totalPrecision = 0;
  for (i = 0; i < keys.length; i++) {
    var key = keys[i];
    if (!informative[key]) continue;   // excluded from precision/fusion: a uniform likelihood is a no-op on belief shape and must not dilute real channels
    var absSum = 0; for (var j2 = 0; j2 < keys.length; j2++) { if (informative[keys[j2]]) absSum += Math.abs(corr.C[key][keys[j2]]); }
    var effc = absSum > 0 ? (1 / absSum) : 1;                       // effective-independence discount, over informative channels only
  ...
  // ── Fusion (§2.2): logpost[p] = log(beliefPred[p]) + Σ_c (rc/Σrk)·log(Lc[p]) — INFORMATIVE ONLY ────
  var logpost = new Array(NUM_PHASES);
  for (var p = 0; p < NUM_PHASES; p++) logpost[p] = Math.log(beliefPred[p] + EPS);
  for (i = 0; i < keys.length; i++) {
    var kk = keys[i];
    if (!informative[kk]) continue;    // a uniform L[kk] would add the same constant to every phase — skip, don't just no-op through the math
  ...
  // Uninformative channels are skipped: they never read resVar back (excluded from precision above), and
  // tracking KL(uniform, belief) for them is exactly the degenerate quantity that caused the bug.
  var resVar = {};
  for (i = 0; i < keys.length; i++) {
    var kn = keys[i];
    if (!informative[kn]) continue;
  ...
  var informativeCount = 0; for (i = 0; i < keys.length; i++) if (informative[keys[i]]) informativeCount++;
  var confidence = informativeCount > 0 ? clamp01(totalPrecision / (totalPrecision + informativeCount)) : 0;
  ...
  return {
    ...
    channels: keys.map(function (k) { return { key: k, precision: informative[k] ? r3(rc[k]) : 0, informative: informative[k] === true, fromHint: usedHint[k] === true }; }),
    degraded: (anyUntransformed || anySelfConsistency || informativeCount < keys.length) ? {
      ...
      uninformativeChannels: keys.length - informativeCount,   // structural/context channels (null likelihood) excluded from precision+fusion, reported for transparency
      note: 'Untransformed channels are on a different scale until history fills. Self-consistency precision is a prior (groupthink-prone), decorrelated by eff_c; independent precisionHint preferred. Uninformative (null-likelihood, structural) channels are excluded from precision/fusion entirely — a uniform likelihood cannot move belief shape and must not be allowed to dilute real channels via the precision denominator (2026-07-20 fix). Taxonomy + transition A are STATED priors.'
    } : null,
    corrState: {
