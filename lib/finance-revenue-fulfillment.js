...
/** Routes Stripe-triggered Finance mail through Finance's own B10/B14 effector. */
...
var SCHEMA = 'finance-revenue-fulfillment/1.0';
var PREFIX = 'finance_revenue_fulfillment:', PENDING_KEY = 'finance_revenue_fulfillment_pending';
function hash(value) { return crypto.createHash('sha256').update(JSON.stringify(value)).digest('hex'); }
function taskId(eventId, kind) { return 'frf_' + hash({ eventId: eventId, kind: kind }).slice(0, 24); }
...
async function fulfill(input) {
  ...
  if (!eventId || ['welcome', 'renewal'].indexOf(kind) < 0 || !message || !message.subject || !message.body ||
      String(subscriber && subscriber.domain || '').toLowerCase() !== 'finance') {
    return { ok: false, status: 'REFUSED', reason: 'exact-finance-stripe-fulfillment-candidate-required', providerCalls: 0, liveMoney: false };
  }
  var candidate = Decision.candidate(subscriber, { subject: message.subject, body: message.body, key: 'stripe:' + eventId + ':' + kind });
  if (!candidate) return { ok: false, status: 'REFUSED', reason: 'active-paid-finance-subscriber-required', providerCalls: 0, liveMoney: false };
  ...
  var result = await Executor.execute({ store: store, specs: [{ candidate: candidate, decision: decision }], now: now, maxSends: 1,
    emailCostUsd: input.emailCostUsd != null ? input.emailCostUsd : numericEnv('FINANCE_SUBSCRIBER_EMAIL_USD', null),
    dailyBudgetUsd: input.dailyBudgetUsd != null ? input.dailyBudgetUsd : numericEnv('FINANCE_SUBSCRIBER_DAILY_BUDGET_USD', null),
    dailySendCap: input.dailySendCap != null ? input.dailySendCap : numericEnv('FINANCE_SUBSCRIBER_DAILY_SEND_CAP', 5),
...
async function persistTask(store, task) {
  ...
  if (!restored || restored.taskId !== task.taskId || restored.status !== task.status) throw new Error('finance revenue fulfillment readback invalid');
  return restored;
}
async function enqueueAndAttempt(input) {
  ...
  if (!eventId || ['welcome', 'renewal'].indexOf(kind) < 0 || !input.subscriber ||
      String(input.subscriber.domain || '').toLowerCase() !== 'finance' || !input.message) {
    return { ok: false, status: 'REFUSED', reason: 'exact-finance-stripe-fulfillment-task-required', providerCalls: 0 };
  }
  ...
  if (!task || task.schemaVersion !== SCHEMA || task.eventId !== eventId || task.kind !== kind) throw new Error('finance revenue task creation readback invalid');
...
async function attemptTask(input) {
  ...
  if (!task || task.schemaVersion !== SCHEMA) return { ok: false, status: 'REFUSED', reason: 'finance-revenue-task-not-found', providerCalls: 0 };
