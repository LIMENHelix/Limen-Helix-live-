 ...
 *   lib/stripe-rail       verifySignature               prove a webhook event came from Stripe
...
// The ledger RETAINS 5000 (lib/finance-ledger.js MAX_LEDGER). Scanning less than that
// meant a webhook's entry could sit just outside the window and this would conclude
// nothing was booked — and then book it again. The scan has to cover everything the
// ledger keeps, or it is not a deduplication check.
const LEDGER_SCAN = 5000;
...
async function incomeAlreadyBooked(ids) {
  ...
  const inLedger = events.some(function (e) {
  ...
  // The QUEUE counts too. A ledger write that failed soft is sitting in
  // relay:finance:unreported waiting to drain, and it is the same charge: booking again
  // because it has not landed yet means the drain then adds a second copy. Pending is
  // booked, just not yet visible in the ledger.
  try {
    const pending = (await db.get(PENDING_KEY)) || [];
    return pending.some(function (p) {
      const m = (p && p.entry && p.entry.meta) || {};
      return wanted.indexOf(String(m.orderId)) !== -1 || wanted.indexOf(String(m.id)) !== -1;
    });
  } catch (e) {
    // Same rule as the ledger: cannot read, cannot be sure, do not write.
    return null;
  }
}
 ...
 * The one INBOUND-money proof Relay needs: a Stripe webhook event must be verified
 * before it is allowed to mark an order paid. Delegated to stripe-rail's verifier —
 * HMAC over the raw body against the STRIPE_WEBHOOK_SECRET family — because the firewall
 * above allows this file, and only this file, to touch that module. Never reimplement
 * the HMAC at a Relay call site: a verifier that drifts from the rail's is a verifier
 * that accepts what the rail would refuse.
 */
function verifyWebhookSignature(rawBody, sigHeader) {
  return _stripe().verifySignature(rawBody, sigHeader);
}

/**
 * Whether any Stripe webhook endpoint secret is configured. Mirrors stripe-rail's
 * SECRET_VARS list; a webhook with no configured secret cannot be authenticated and
 * must be refused (503), which a caller cannot distinguish from a bad signature (403)
 * without this.
 */
function webhookConfigured() {
  return ['STRIPE_WEBHOOK_SECRET', 'STRIPE_WEBHOOK_SECRET_SUBS', 'STRIPE_WEBHOOK_SECRET_2']
    .some(function (n) { const v = process.env[n]; return !!(v && String(v).trim()); });
}
