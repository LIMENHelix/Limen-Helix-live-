...
let _INHIBITORY_LOAD_ERROR = null;   // why the last load returned nothing; null when it loaded cleanly
function loadInhibitoryEdges() {
  ...
  try {
    ...
    /**
     * NEVER LET THIS FAIL SILENTLY AGAIN.
     *
     * This catch returned [] with no record, which made "the connectome file is not on disk"
     * indistinguishable from "the connectome contains no inhibitory edges." It hid a real
     * outage for weeks: .github/workflows/immune-system.yml did a SPARSE checkout that never
     * listed assets/data/brain-connectome.json, so every snapshot that job committed carried
     * inhibitoryEdgesLoaded:0 and the propagation ran purely additive. The file was tracked in
     * git the whole time and deploys as a static asset, so nothing else pointed at it.
     *
     * Still fail-soft, deliberately: a missing connectome must degrade the reading, never throw.
     * A throw here reaches module scope of a handler, and a bad require in ANY handler takes
     * down every /api/* route on the Hono catch-all. The reason is recorded and reported in
     * stats.inhibitoryLoadError instead, so absence is visible without being fatal.
     */
    _INHIBITORY_LOAD_ERROR = String((_err && _err.message) || _err);
    _INHIBITORY_EDGES_CACHE = [];
  }
  return _INHIBITORY_EDGES_CACHE;
}

// The reason loadInhibitoryEdges() last returned nothing, or null when it loaded cleanly.
// Read by serializeResult so a zero edge count always ships its cause.
function inhibitoryLoadError() { return _INHIBITORY_LOAD_ERROR; }
...
function runPropagation(portalsBySlug, commandBoardEntries) {
  ...
  return {
    ...
    stats: {
      ...
      // Ships the CAUSE alongside the count. A 0 with a null cause means the connectome parsed
      // and genuinely carries no inhibitory edges; a 0 with a message means it could not be read.
      inhibitoryLoadError: inhibitoryLoadError(),
