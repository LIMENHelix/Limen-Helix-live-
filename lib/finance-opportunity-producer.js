...
var SCHEMA = 'finance-opportunity-proposal/1.1';
...
function walkForbidden(value, path, found) {
  ...
  Object.keys(value).forEach(function (key) {
    ...
    // `side` is an order-envelope field everywhere except the manager's
    // explicitly non-executable LONG/SHORT projected-margin ledger. Keep the
    // recursive motor-field brake while allowing that one schema-owned path.
    var rankingSide = key === 'side' && /^projectedMarginRanking\.entries\.\d+\.side$/.test(at);
    if (FORBIDDEN.indexOf(key) >= 0 && !rankingSide) found.push(at);
...
function ledgerEvidence(ledger) {
  ...
  list(l.semanticEvidence).forEach(function (row) {
    var key = identityKey(row && row.sourceIdentity);
    if (key) out[key] = 'semantic';
  });
  var md = l.marketData || {};
  list(md.quotes).forEach(function (row) {
    var key = identityKey(row && row.sourceIdentity);
    if (key) out[key] = 'market';
  });
  list(l.networkEvidence).forEach(function (row) {
    var key = identityKey(row && row.sourceIdentity);
    if (key) out[key] = 'network';
  });
  return out;
}
...
function build(input) {
  ...
  var blockers = [];
  if (!ledger || ledger.schemaVersion !== 'finance-input-ledger/1.0' ||
  ...
  if (!proposal || HORIZONS.indexOf(proposal.horizonDays) < 0) blockers.push('proposal_horizon_must_be_supported_multiscale_days');
  ...
  refs.forEach(function (ref, index) {
    ...
    var actualRole = evidence[identityKey(ref.sourceIdentity)];
  ...
  return {
    ...
    projectedMarginRanking: clone(proposal && proposal.projectedMarginRanking || null),
