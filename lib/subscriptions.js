...
/** Strict counterpart for outward fulfillment. No process-memory subscriber can authorize email. */
async function activeListStrict(store) {
  store = store || strictStore; store.assertDurable();
  var m = await store.get(KEY);
  if (!m) m = {};
  if (typeof m !== 'object' || Array.isArray(m)) throw new Error('strict subscriber store malformed');
  return Object.keys(m).map(function (k) { return m[k]; }).filter(function (s) { return s && s.active; });
}

async function getStrict(email, store) {
  store = store || strictStore; store.assertDurable();
  var m = await store.get(KEY);
...
async function activateStrict(o, store) {
  store = store || strictStore; store.assertDurable();
  var email = norm(o && o.email);
  if (!validEmail(email)) return { ok: false, reason: 'invalid email' };
  var m = await store.get(KEY); if (!m) m = {};
  if (typeof m !== 'object' || Array.isArray(m)) throw new Error('strict subscriber store malformed');
  var prev = m[email] || null, at = new Date().toISOString();
  m[email] = {
    email: email, domain: o.domain || (prev && prev.domain) || null, rung: o.rung || (prev && prev.rung) || null,
    offer: o.offer || (prev && prev.offer) || null, watch: o.watch != null ? o.watch : (prev ? prev.watch : null),
    priceCents: o.priceCents != null ? o.priceCents : (prev ? prev.priceCents : null),
    subscriptionId: o.subscriptionId || (prev && prev.subscriptionId) || null,
    customerId: o.customerId || (prev && prev.customerId) || null,
    active: true, since: (prev && prev.since) || at, activatedAt: at, endedAt: null,
    lastSentAt: (prev && prev.lastSentAt) || null, lastSentKey: (prev && prev.lastSentKey) || null
  };
  await store.set(KEY, m); var restored = await store.get(KEY), subscriber = restored && restored[email];
  if (!subscriber || !subscriber.active || subscriber.subscriptionId !== m[email].subscriptionId) throw new Error('strict subscriber activation readback invalid');
  return { ok: true, subscriber: subscriber, returning: !!prev, readbackVerified: true };
}
...
async function deactivateStrict(match, reason, store) {
  store = store || strictStore; store.assertDurable();
  var m = await store.get(KEY); if (!m) m = {};
  if (typeof m !== 'object' || Array.isArray(m)) throw new Error('strict subscriber store malformed');
  var hits = [], at = new Date().toISOString();
  Object.keys(m).forEach(function (k) {
    var s = m[k]; if (!s) return;
    var matched = (match.email && norm(match.email) === s.email) ||
      (match.subscriptionId && match.subscriptionId === s.subscriptionId) ||
      (match.customerId && match.customerId === s.customerId);
    if (!matched || !s.active) return;
    s.active = false; s.endedAt = at; s.endedReason = reason || 'stripe'; hits.push(s.email);
  });
  if (!hits.length) return { ok: true, changed: 0, readbackVerified: true };
  await store.set(KEY, m); var restored = await store.get(KEY);
  var valid = restored && hits.every(function (email) { return restored[email] && restored[email].active === false && restored[email].endedReason === (reason || 'stripe'); });
  if (!valid) throw new Error('strict subscriber deactivation readback invalid');
  return { ok: true, changed: hits.length, emails: hits, readbackVerified: true };
}
...
async function markSentStrict(email, sendKey, store) {
  store = store || strictStore; store.assertDurable();
  var m = await store.get(KEY);
  if (!m || typeof m !== 'object' || Array.isArray(m)) return false;
  var normalized = norm(email), s = m[normalized]; if (!s) return false;
  s.lastSentAt = new Date().toISOString(); s.lastSentKey = sendKey || null;
  await store.set(KEY, m); var restored = await store.get(KEY);
  return !!(restored && restored[normalized] && restored[normalized].lastSentKey === (sendKey || null));
}
...
module.exports = {
  all: all, activeList: activeList, activeListStrict: activeListStrict, get: get, getStrict: getStrict,
  activate: activate, activateStrict: activateStrict, deactivate: deactivate, deactivateStrict: deactivateStrict,
  markSent: markSent, markSentStrict: markSentStrict, stats: stats,
