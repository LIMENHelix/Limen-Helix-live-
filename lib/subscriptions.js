...
async function activeListStrict() {
  strictStore.assertDurable();
  var m = await strictStore.get(KEY);
  if (!m) m = {};
...
async function markSentStrict(email, sendKey) {
  strictStore.assertDurable();
  var m = await strictStore.get(KEY);
  ...
  await strictStore.set(KEY, m); var restored = await strictStore.get(KEY);
...
module.exports = {
  all: all, activeList: activeList, activeListStrict: activeListStrict, get: get, activate: activate,
  deactivate: deactivate, markSent: markSent, markSentStrict: markSentStrict, stats: stats,
