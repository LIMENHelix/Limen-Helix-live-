...
var Observer = require('./infrastructure-real-estate-observer.js');

var SCHEMA = 'infrastructure-real-estate-recovery/1.0';
var LOG_KEY = 'infrastructure_real_estate_recovery_log';
var PREFIX = 'infrastructure_real_estate_recovery:';
...
async function recover(input) {
  ...
  var policy = trigger.type === 'infrastructure-real-estate-policy' && trigger.id;
  ...
  if (!auth || !auth.authorized) return { ok: true, status: 'HELD', reason: auth && auth.reason || 'infrastructure-real-estate-recovery-motor-held', liveMoney: false };
  var id = 'irr_' + hash({ action: command.actionId, observation: observation && observation.observationId || null, trigger: trigger.id, motor: auth.receiptId }).slice(0, 24);
  ...
  var recovery = { schemaVersion: SCHEMA, recoveryId: id, commandId: command.commandId, actionId: command.actionId,
    ...
    productDomain: 'infrastructure', ownerDomain: 'infrastructure', lane: 'real-estate', productMotorReceiptId: auth.receiptId,
  ...
  if (!(await store.setIfAbsent(Executor.motorKey(auth.receiptId), { schemaVersion: SCHEMA, recoveryId: id, productMotorReceiptId: auth.receiptId, claimedAt: now }))) { recovery.status = 'REFUSED'; recovery.reason = 'infrastructure-real-estate-motor-receipt-already-consumed'; await store.set(key(id), recovery); return recovery; }
  var motorClaim = await store.get(Executor.motorKey(auth.receiptId)); if (!motorClaim || motorClaim.recoveryId !== id) throw new Error('infrastructure real-estate recovery motor claim readback invalid');
  ...
  if (!restoredCatalog || !restoredCatalog[suppressionIdentity] || restoredCatalog[suppressionIdentity].recoveryId !== id) throw new Error('infrastructure real-estate suppression readback invalid');
  ...
  await store.set(key(id), recovery); var restored = await store.get(key(id)); if (!restored || restored.status !== recovery.status) throw new Error('infrastructure real-estate recovery readback invalid');
