...
// A provider-reviewed approve/reject/abstain receipt is a decision episode, not
// a cache. Keep it after the source packet ages out so later outcomes can still
// be attributed to the evidence and rationale that existed at decision time.
...
async function productionInput(options) {
  ...
  return {
    input: {
      ...
      companyLearningPatterns: results[4] && results[4].companyPatterns || [],
...
async function execute(store, bundle, request, options) {
  ...
  var created = await store.setIfAbsent(key, commanded);
  if (!created) {
