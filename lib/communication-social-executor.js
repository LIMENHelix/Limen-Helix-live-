...
async function execute(input) {
  ...
  try {
    ...
    if (text(spec.sourceArtifactId)) {
      var priorArtifactClaim = await store.get(artifactClaimKey(subjectDomain, spec.sourceArtifactId));
      if (priorArtifactClaim) {
        return { ok: false, status: 'REFUSED', reason: 'domain-commercial-artifact-already-distributed-or-claimed',
          published: false, commandId: priorArtifactClaim.commandId || null,
