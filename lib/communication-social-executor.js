...
async function execute(input) {
  ...
  try {
    ...
    try {
      ...
      var dispatchAt = Number((input.nowFn || Date.now)());
      if (!Decision.validateReceipt(spec.decisionReceipt, receiptCandidate, dispatchAt)) {
        await markPreProviderFailure(store, key, command, 'communication-social-authority-expired-before-dispatch');
        await releaseClaims(store, acquiredClaims);
        return { ok: false, status: 'REFUSED', reason: 'communication-social-authority-expired-before-dispatch',
          published: false, commandId: id, liveMoney: false };
      }
    ...
    if (resolved && resolved.status === 'FAILED' && resolved.definitiveFailure === true) {
      // The provider conclusively refused the write. Preserve that in-memory
      // classification across a failed journal write, make a second durable
      // transition attempt, and release no-publication claims either way.
      try {
        await store.set(key, resolved);
        var definitiveCommand = await store.get(key);
        if (definitiveCommand && definitiveCommand.status === 'FAILED' &&
            definitiveCommand.definitiveFailure === true) await append(store, definitiveCommand);
      } catch (_) {}
      await releaseClaims(store, acquiredClaims);
    } else if (providerAttempted === false) {
