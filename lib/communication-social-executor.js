...
async function execute(input) {
  ...
  try {
    ...
    if (text(spec.sourceArtifactId)) {
      var priorClaims = await Promise.all([
        store.get(artifactClaimKey(subjectDomain, spec.sourceArtifactId)),
        store.get(contentClaimKey(subjectDomain, body))
      ]);
      if (priorClaims[0] || priorClaims[1]) {
        return { ok: false, status: 'REFUSED',
          reason: priorClaims[1] ? 'domain-commercial-public-content-already-distributed-or-claimed' :
            'domain-commercial-artifact-already-distributed-or-claimed',
          published: false, commandId: (priorClaims[1] || priorClaims[0]).commandId || null,
    ...
    if (!posted || posted.ok !== true || !text(posted.uri) || !text(posted.cid) || !text(posted.url)) {
      resolved = Object.assign({}, command, {
        status: 'FAILED', providerCalled: posted && posted.providerCalled === false ? false : true, resolvedAt: Date.now(),
    ...
    if (resolved.status === 'FAILED' && providerAttempted === false) {
      await releaseClaims(store, acquiredClaims);
    }
    return publicResult(restored, false);
  } catch (error) {
    if (providerAttempted === false) await releaseClaims(store, acquiredClaims);
