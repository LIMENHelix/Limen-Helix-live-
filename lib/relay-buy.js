...
async function buyFromCJ(job) {
  ...
  // CJ may bill slightly differently from the quote. Refuse silently overspending the
  // amount the autonomy gate actually authorised.
  if (r.amount != null && isFinite(job.maxCost) && r.amount > job.maxCost * 1.1) {
    return {
      ok: false,
      overspent: true,
      error: 'CJ charged $' + r.amount.toFixed(2) + ' against an authorised $' +
             Number(job.maxCost).toFixed(2) + '; order ' + r.sourceOrderId + ' needs review'
    };
  }
  return { ok: true, provider: 'cj', sourceOrderId: r.sourceOrderId, amount: r.amount != null ? r.amount : job.maxCost };
}
