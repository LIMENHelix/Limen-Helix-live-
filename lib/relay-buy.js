...
// Read here, not only in the engine: reconcile-before-replace has to live in the buy
// itself, or a retry from ANY caller can double-place a line whose first attempt's
// answer was lost.
...
async function buyFromCJ(job) {
  ...
  // ── RECONCILE-BEFORE-REPLACE ─────────────────────────────────────────────────────
  // A line whose previous attempt's answer was lost (timeout after accept, crash between
  // accept and settle) must NOT be re-placed blind: CJ may already hold the order, and
  // orderNumber idempotency is an assumption, not a guarantee we verified. When this
  // line has a reservation that already went to CJ, ask CJ first.
  //
  //   found:true      recover the provider identity and settle — no second create.
  //   found:false     CJ definitively has nothing under this number; place normally.
  //   'ambiguous'     outcome unknown: hold the reservation, place nothing, and mark the
  //                   row so the engine's reconciliation pass keeps asking.
  const canReadback = typeof cj.getOrderDetail === 'function';
  const expected = { vid: vid, quantity: job.quantity || 1 };
  const readback = async function () {
    if (!canReadback) return { found: 'ambiguous', orderNumber: orderNumber, error: 'readback unavailable in this build' };
    try {
      return await cj.getOrderDetail(orderNumber, expected);
    } catch (e) {
      return { found: 'ambiguous', orderNumber: orderNumber, error: e.message };
    }
  };
  const recover = async function (decisionId, look) {
    const amount = look.amountUsd != null ? look.amountUsd : job.maxCost;
    if (decisionId) {
      await autonomy.settle(decisionId, { amount: amount, sourceOrderId: look.cjOrderId });
    }
    const overspent = amount != null && isFinite(job.maxCost) && amount > job.maxCost * 1.1;
    return {
      ok: true,
      provider: 'cj',
      recovered: true,
      ...
      reviewReason: overspent
        ? 'CJ charged $' + amount.toFixed(2) + ' against an authorised $' +
          Number(job.maxCost).toFixed(2) + '; order ' + look.cjOrderId + ' needs review'
        : null
    };
  };
  ...
  if (job.decisionId) {
    try {
      prior = await autonomy.getDecision(job.decisionId);
    } catch (e) {
      // Cannot tell whether this line already went to CJ. Placing anyway is how a paid
      // order gets bought twice, so this fails closed and stays retryable.
      return {
        ok: false,
        transient: true,
        error: 'spend ledger unreadable before ordering; not ordering: ' + e.message
      };
    }
  }
  const attemptedBefore = !!(prior && (prior.cjAttemptedAt || prior.reconciliationPending || prior.sourceOrderId));
  if (attemptedBefore) {
    const look = await readback();
    if (look.found === true) return recover(prior.id, look);
    if (look.found !== false) {
      await autonomy.markReconciliationPending(prior.id, { orderNumber: orderNumber, error: look.error || 'readback ambiguous' });
      return {
        ok: false,
        transient: true,
        reconciliationPending: true,
        error: 'CJ outcome for ' + orderNumber + ' is unknown (' + (look.error || 'ambiguous') +
               '); the reservation stays held and reconciliation will retry'
      };
    }
    // Definitive absence: the earlier attempt never landed. Fall through and place.
  }
  ...
  // STAMP THE ATTEMPT BEFORE THE MONEY MOVES. If this process dies between
  // createOrderV2 and settle(), the row still names the orderNumber a later pass can ask
  // CJ about. Without the stamp, that crash is indistinguishable from "never ordered".
  if (job.decisionId && !attemptedBefore) {
    const noted = await autonomy.noteCjAttempt(job.decisionId, orderNumber);
    if (!noted.ok) {
      return {
        ok: false,
        transient: true,
        error: 'could not record the attempt before ordering; not ordering: ' + (noted.error || 'unknown')
      };
    }
  }
  ...
  if (!r.ok) {
    // AMBIGUOUS CREATE: the supplier may have accepted. A timeout after accept, an
    // accepted order with no id, or a transport break all leave the wallet possibly
    // charged. Only a definitive readback absence earns a release; anything else holds
    // the reservation for reconciliation. NEVER turn 'provider outcome unknown' into
    // 'money not spent'.
    const ambiguousCreate = /no order id|timed?\s*out|timeout|abort|ECONNRESET|socket|429|rate|502|503|504/i.test(r.error || '');
    if (ambiguousCreate && job.decisionId) {
      const look = await readback();
      if (look.found === true) return recover(job.decisionId, look);
      if (look.found === false) return r;   // CJ has nothing: the create genuinely failed
      await autonomy.markReconciliationPending(job.decisionId, { orderNumber: orderNumber, error: r.error });
      return {
        ok: false,
        transient: true,
        reconciliationPending: true,
        error: (r.error || 'create outcome unknown') + '; whether CJ accepted is unconfirmed, ' +
               'so the reservation stays held and reconciliation will retry'
      };
    }
    return r;
  }
...
async function execute(job) {
  ...
  if (provider === 'cj') {
    ...
    const transient = /timeout|abort|ECONNRESET|socket|429|rate|temporarily|502|503|504|authentication failed/i
      .test(r.error || '');
    if (transient || r.reconciliationPending === true) {
      return {
        ok: false,
        transient: true,
