...
async function buyFromCJ(job) {
  ...
  if (job.sourceShipping != null && isFinite(job.maxCost)) {
    const requote = await cj.freight(vid, job.quantity || 1, addrCountry, (job.shippingAddress || {}).postalCode);
    if (!requote) {
      return { ok: false, error: 'CJ will not quote shipping to ' + addrCountry + '; not buying blind' };
    }
