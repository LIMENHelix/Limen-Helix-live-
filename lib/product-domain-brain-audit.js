...
var REQUIRED_PARTS = [
  ...
  'cortexRetrieval', 'refreshController', 'resourceMetabolism', 'motorReadiness'
];
...
function resourceAuthority(source) {
  ...
  if (!body) return { present: false, ownerDomain: null, policyId: null, sandboxLane: null, externalAction: null };
  return {
    present: true,
    ownerDomain: firstMatch(body, /ownerDomain\s*:\s*['"]([^'"]+)['"]/),
    policyId: firstMatch(body, /policyId\s*:\s*['"]([^'"]+)['"]/),
    sandboxLane: firstMatch(body, /sandboxLane\s*:\s*['"]([^'"]+)['"]/),
    externalAction: firstMatch(body, /externalAction\s*:\s*(true|false)/) === 'true'
  };
}

function motorAuthority(source) {
  var body = firstMatch(source, /this\.motorAuthority\s*=\s*\{([^;]+)\};/);
  if (!body) return { present: false, ownerDomain: null, contractId: null, lane: null, external: null };
  return {
    present: true,
    ownerDomain: firstMatch(body, /ownerDomain\s*:\s*['"]([^'"]+)['"]/),
    contractId: firstMatch(body, /contractId\s*:\s*['"]([^'"]+)['"]/),
    lane: firstMatch(body, /lane\s*:\s*['"]([^'"]+)['"]/),
    decisionContract: firstMatch(body, /decisionContract\s*:\s*['"]([^'"]+)['"]/),
    budgetId: firstMatch(body, /budgetId\s*:\s*['"]([^'"]+)['"]/),
    receiptClass: firstMatch(body, /receiptClass\s*:\s*['"]([^'"]+)['"]/),
    outcomeClass: firstMatch(body, /outcomeClass\s*:\s*['"]([^'"]+)['"]/),
    rollbackClass: firstMatch(body, /rollbackClass\s*:\s*['"]([^'"]+)['"]/),
    executorVerified: firstMatch(body, /executorVerified\s*:\s*(true|false)/) === 'true',
    outcomeObserverVerified: firstMatch(body, /outcomeObserverVerified\s*:\s*(true|false)/) === 'true',
    external: firstMatch(body, /external\s*:\s*(true|false)/) === 'true'
  };
}
...
function inspect(descriptor) {
  ...
  var ownMotor = motorAuthority(source);
  ...
  var parts = {
    ...
    resourceMetabolism: ownResource.present && ownResource.ownerDomain === descriptor.runtime && !!ownResource.policyId &&
      LaneContract.list().indexOf(ownResource.sandboxLane) >= 0,
    motorReadiness: ownMotor.present && ownMotor.ownerDomain === descriptor.runtime && !!ownMotor.contractId &&
      ownMotor.lane === ownResource.sandboxLane && !!ownMotor.decisionContract && !!ownMotor.budgetId &&
      !!ownMotor.receiptClass && !!ownMotor.outcomeClass && !!ownMotor.rollbackClass
  };
