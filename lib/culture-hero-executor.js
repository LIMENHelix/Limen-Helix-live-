...
async function execute(input) {
  ...
  try {
    ...
    var command = {
      schemaVersion: SCHEMA, commandId: commandId, status: 'DISPATCHING', productDomain: 'culture', ownerDomain: 'culture', lane: 'hero-image',
    ...
    var result;
    try { result = await provider.generate(candidate); }
    catch (error) { result = { ok: false, ambiguous: true, error: String(error && error.message || error), spentUsd: null }; }
