...
/** Finance-local outcome learning ledger consumed only by the Finance brain. */
var ExternalSchema = 'product-domain-external-learning/1.0';
var SCHEMA = 'finance-subscriber-learning/1.0';
var STATE_KEY = 'finance_subscriber_learning_state';
var CAUSE_PREFIX = 'finance_subscriber_learning_cause:';
var CAP = 200;
function causeKey(actionId) { return CAUSE_PREFIX + actionId; }
function fresh() { return { schemaVersion: SCHEMA, domain: 'finance', lane: 'subscriber-email', resolvedCount: 0, signals: [], processedObservationIds: [], lastOutcomeAt: null }; }
async function load(store) {
  var state = await store.get(STATE_KEY); if (!state) return fresh();
  if (state.schemaVersion !== SCHEMA || state.domain !== 'finance' || state.lane !== 'subscriber-email' || !Array.isArray(state.signals) || !Array.isArray(state.processedObservationIds)) throw new Error('finance subscriber learning state malformed');
  return state;
}
async function save(store, state) { await store.set(STATE_KEY, state); var r = await store.get(STATE_KEY);
  if (!r || r.schemaVersion !== SCHEMA || r.resolvedCount !== state.resolvedCount) throw new Error('finance subscriber learning state readback invalid'); return r; }
async function recordCommand(store, command, item) {
  var cause = { schemaVersion: SCHEMA, domain: 'finance', lane: 'subscriber-email', actionId: item.actionId,
  ...
  if (!restored || restored.actionId !== item.actionId || restored.commandId !== command.commandId) throw new Error('finance subscriber learning cause readback invalid');
...
async function recordObservation(store, observation) {
  if (!observation || !observation.observationId || !observation.actionId || !resolved(observation.lastEvent)) return { ok: false, reason: 'resolved-finance-observation-required' };
  var cause = await store.get(causeKey(observation.actionId)); if (!cause) return { ok: false, reason: 'finance-action-cause-missing' };
  ...
  var signal = { schemaVersion: ExternalSchema, signalId: 'els_' + observation.observationId, eventId: observation.observationId,
    actionId: observation.actionId, ownerDomain: 'finance', lane: 'subscriber-email', eventType: 'OUTCOME_SUBSCRIBER_' + observation.lastEvent.toUpperCase(),
...
async function readForBrain(store) {
  ...
  return { schemaVersion: ExternalSchema, domain: 'finance', status: signal ? 'ELIGIBLE' : 'ABSTAINED',
