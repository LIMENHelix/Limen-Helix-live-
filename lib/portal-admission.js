...
function prepareGeneratedPortal(rawPortal, target, currentPortal) {
  ...
  const sanitization = { nestedCiksCleared: 0, singularRelationshipsNormalized: 0, preservedKernelFields: !!source };
  ...
  // The repository schema models the auditor as a singular relationship. LLMs
  // sometimes wrap a single auditor in an array even when the prompt asks for
  // an object; normalize that losslessly before the schema/admission checks.
  const auditor = portal.functionalNetwork && portal.functionalNetwork.auditor;
  if (Array.isArray(auditor)) {
    if (auditor.length === 0) delete portal.functionalNetwork.auditor;
    else if (auditor.length === 1 && auditor[0] && typeof auditor[0] === 'object') {
      portal.functionalNetwork.auditor = auditor[0];
      sanitization.singularRelationshipsNormalized++;
    }
  }
...
function validatePortalAdmission(portal) {
  ...
  if (portal.functionalNetwork && Array.isArray(portal.functionalNetwork.auditor)) errors.push({ code: 'NETWORK_CATEGORY_SHAPE', field: 'auditor' });
  ...
  for (const { category, index, entry } of rows) {
    ...
    const noteWords = String(entry.relationshipNote || '').trim().split(/\s+/).filter(Boolean).length;
    if (entry.relationshipNote && noteWords < 18) errors.push({ code: 'NETWORK_NOTE_TOO_SHORT', path, wordCount: noteWords });
