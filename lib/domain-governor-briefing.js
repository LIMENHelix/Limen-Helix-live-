...
var CommercialSocialLearning = require('./domain-commercial-social-learning.js');
var GovernorPosture = require('./domain-governor-posture.js');

var SCHEMA = 'domain-governor-briefing/1.0';
var MAX_COGNITION_AGE_MS = 45 * 60 * 1000;
var MAX_COMMERCIAL_WORK_AGE_MS = 48 * 60 * 60 * 1000;
...
function domainLines(domain, env) {
  return ValveRegistry.LINES.filter(function (line) {
    ...
    return {
      ...
      observerStatus: line.observerStatus || (line.observerRoute ? 'ROUTE_DECLARED' : 'UNDECLARED'),
      recoveryStatus: line.recoveryStatus || (line.recoveryRoute ? 'ROUTE_DECLARED' : 'UNDECLARED'),
...
async function commercialReflex(domain, store, nowValue) {
  ...
  try {
    ...
    var pair = await Promise.all([
      ...
      CommercialSocialLearning.readForBrain(store, domain)
    ]);
    var state = pair[0];
    var artifact = pair[1];
    var socialOutcome = pair[2];
    ...
    var evaluatedAt = Number(state.evaluatedAt);
    var plannedAt = Number(state.intent && state.intent.plannedAt);
    if (!Number.isFinite(evaluatedAt) || now < evaluatedAt || now - evaluatedAt > MAX_COMMERCIAL_WORK_AGE_MS ||
        (state.intent && (!Number.isFinite(plannedAt) || now < plannedAt || now - plannedAt > MAX_COMMERCIAL_WORK_AGE_MS))) {
      return { status: 'UNOBSERVED', reason: 'domain-commercial-reflex-stale', state: null, latestArtifact: null };
    }
    ...
    if (artifact && artifact.schemaVersion === 'domain-commercial-artifact/1.0' &&
        ...
        artifact.intentId === state.lastPlannedIntentId &&
        Number.isFinite(Number(artifact.freshnessExpiresAt)) && Number(artifact.freshnessExpiresAt) > now) {
    ...
    return {
      ...
      publicSocialOutcome: socialOutcome,
...
async function build(domain, options) {
  ...
  var reflex = await commercialReflex(id, options.store, now);
  var governorPosture = GovernorPosture.derive(
    observed.cognition,
    observed.serverObservation && observed.serverObservation.stress
  );
  var blockers = [];
  ...
  var packet = {
    ...
    governorPosture: governorPosture,
    ...
    truthPolicy: {
      ...
      governorPostureCannotGrantAuthority: true
    }
  };
...
module.exports = {
  ...
  MAX_COMMERCIAL_WORK_AGE_MS: MAX_COMMERCIAL_WORK_AGE_MS,
  build: build,
  canonical: canonical,
  ...
  ,deriveGovernorPosture: GovernorPosture.derive
};
