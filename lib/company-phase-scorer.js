...
var CONFIG = {
  ...
  // Scoring infrastructure — points to THIS deployment's kernel.
  // The /api/limen/score handler accepts {cik} body or path param.
  // VERCEL_URL is set at runtime; falls back to canonical public URL.
  SCORE_API_BASE: (process.env.VERCEL_URL
    ? 'https://' + process.env.VERCEL_URL
    : 'https://limenhelix.com') + '/api/limen/score',
  SCORE_TIMEOUT_MS: 12000,            // bumped: kernel can be 1-2s + Pass 3B portal-load
  ...
  for (var i = 0; i < _CB_PATHS.length; i++) {
    try {
      ...
      for (var j = 0; j < companies.length; j++) {
        ...
        seen[cik] = true;
        out.push({ cik: cik, ticker: c.t || null, domain: c.d || 'unknown' });
      }
      return out;
...
async function buildDomainJoin() {
  ...
  var domainSet = {};
  for (var i = 0; i < COMPANY_REGISTRY.length; i++) {
    domainSet[COMPANY_REGISTRY[i].domain] = true;
  }
  ...
  for (var ci = 0; ci < COMPANY_REGISTRY.length; ci++) {
    ...
    var domain = co.domain;
    join[domain].mapped_company_count++;

    var cached = await db.get('company_phase_' + co.cik);
    if (!cached) continue;
    if (Date.now() - cached.timestamp > CONFIG.COMPANY_PHASE_TTL * 1000) continue;
