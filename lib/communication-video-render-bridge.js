...
var UPLOAD_PENDING_LOG = 'communication_video_upload_pending_log';
var UPLOAD_QUEUE_PREFIX = 'communication_video_upload_queue_index:';
...
function uploadPendingItem(command, receipt) {
  return { schemaVersion: RECEIPT_SCHEMA, commandId: command.commandId,
    subjectDomain: command.subjectDomain, manifestId: command.manifestId,
    renderReceiptId: receipt.receiptId, fileSha256: receipt.fileSha256,
    renderedAt: receipt.renderedAt, expiresAt: command.expiresAt };
}
...
async function queueUpload(store, command, receipt) {
  var markerKey = UPLOAD_QUEUE_PREFIX + command.commandId;
  var row = uploadPendingItem(command, receipt);
  var claim = { schemaVersion: RECEIPT_SCHEMA, status: 'INDEXING', commandId: command.commandId,
    renderReceiptId: receipt.receiptId };
  var done = Object.assign({}, claim, { status: 'INDEXED', indexedAt: Date.now() });
  await store.setIfAbsent(markerKey, claim);
  var marker = await store.get(markerKey);
  if (marker && marker.status === 'INDEXING' && marker.renderReceiptId === receipt.receiptId) {
    await store.indexListMemberIfValue(markerKey, marker, UPLOAD_PENDING_LOG, row, 9999, done);
    marker = await store.get(markerKey);
  }
  if (!marker || marker.status !== 'INDEXED' || marker.renderReceiptId !== receipt.receiptId) {
    throw new Error('communication video upload queue readback invalid');
  }
}
async function transitionRendered(store, command, receipt, now) {
  var renderCompleteOrLater = ['RENDERED_LOCAL', 'DISPATCHING_PRIVATE_UPLOAD',
    'PROVIDER_CALL_AUTHORIZED', 'UPLOADED_PRIVATE'];
  if (renderCompleteOrLater.indexOf(command.status) >= 0 &&
      command.renderReceiptId === receipt.receiptId) {
...
async function complete(store, workerId, body, nowValue) {
  ...
  if (existing) {
    ...
    var restoredCommand = await transitionRendered(store, command, existing, now);
    await indexReceipt(store, existing);
    if (restoredCommand.status === 'RENDERED_LOCAL') await queueUpload(store, restoredCommand, existing);
  ...
  await queueUpload(store, command, restoredReceipt);
...
module.exports = { RECEIPT_SCHEMA: RECEIPT_SCHEMA, RECEIPT_PREFIX: RECEIPT_PREFIX,
  ...
  UPLOAD_PENDING_LOG: UPLOAD_PENDING_LOG, UPLOAD_QUEUE_PREFIX: UPLOAD_QUEUE_PREFIX,
  LEASE_MS: LEASE_MS, receiptKey: receiptKey, receiptIndexKey: receiptIndexKey,
  uploadPendingItem: uploadPendingItem, queueUpload: queueUpload,
