...
var DomainDistribution = require('./domain-commercial-distribution-decision.js');
...
function validEntry(value, runtimeDomain, packetDomain, now) {
  var c = value && value.c;
  var ts = Number(value && value.ts);
  var packet = c && c.serverPacket;
  var generatedAt = Date.parse(packet && packet.generatedAt);
  return !!(c && c.domain === runtimeDomain && Number.isFinite(ts) && now >= ts && now - ts <= MAX_COGNITION_AGE_MS &&
    packet && packet.schemaVersion === 'civilization-domain-packet/1.0' && packet.domainId === packetDomain &&
...
function validLegacySource(candidate, now) {
...
async function validDomainSource(store, candidate, now) {
  var source = candidate && candidate.sourceIdentity;
  var domain = text(candidate && candidate.subjectDomain);
  var contract = Contracts.get(domain);
  if (!contract || !source || source.kind !== 'domain-commercial-artifact' || source.subjectDomain !== domain ||
      source.artifactId !== candidate.sourceArtifactId || source.intentId !== candidate.sourceIntentId ||
      source.packetId !== candidate.sourcePacketId || source.value !== contract.artifactPrefix + source.artifactId ||
      source.responseHash == null || source.fullTextVerified !== false ||
      !DomainDistribution.validate(candidate.domainDecisionReceipt, candidate, now)) return false;
  var pair = await Promise.all([store.get(contract.artifactStateKey),
    store.get(DomainDistribution.key(domain, candidate.sourceArtifactId,
      candidate.domainDecisionReceipt && candidate.domainDecisionReceipt.decisionReceiptId))]);
  var artifact = pair[0];
  var release = pair[1];
  return !!(artifact && artifact.schemaVersion === 'domain-commercial-artifact/1.0' &&
    artifact.status === 'ARTIFACT_PREPARED' && artifact.productDomain === domain &&
    artifact.ownerDomain === contract.ownerDomain && artifact.artifactId === candidate.sourceArtifactId &&
    artifact.intentId === candidate.sourceIntentId && artifact.sourcePacketId === candidate.sourcePacketId &&
    artifact.contentHash === source.responseHash && Number(artifact.freshnessExpiresAt) > now &&
    release && release.decisionReceiptId === candidate.domainDecisionReceipt.decisionReceiptId);
}
async function validSource(store, candidate, now) {
  if (candidate && candidate.sourceIdentity && candidate.sourceIdentity.kind === 'domain-commercial-artifact') {
    return validDomainSource(store, candidate, now);
  }
  return validLegacySource(candidate, now);
}
...
async function decide(store, candidate, now, deps) {
  ...
  try {
    store.assertDurable();
    if (!await validSource(store, normalized, at)) {
      return held('communication-b10-candidate-refused', ['candidate-live-source-identity-invalid-or-stale']);
    }
    // A prepared commercial artifact arrives with a durable, exact-artifact
    // release from its owning domain. That receipt is the basal-ganglia action
    // selection. Communication owns transport safety, not a second unrelated
    // content-selection vote based on Communication's own stress. Its local
    // brake/emittedCount still governs legacy content that Communication itself
    // originates.
    var subjectSelectedDistribution = normalized.sourceIdentity.kind === 'domain-commercial-artifact' &&
      DomainDistribution.validate(normalized.domainDecisionReceipt, normalized, at);
    var communication = await entry('communication', deps);
    var subjectContract = Contracts.get(subjectDomain);
    var subjectRuntimeDomain = subjectContract ? subjectContract.ownerDomain : subjectDomain;
    // Cognition records are stored under the canonical product key while the
    // payload keeps the runtime owner alias (science/research, medicine/health,
    // trade/supplyChain). Bind both identities explicitly.
    var subject = await entry(subjectDomain, deps);
    if (!validEntry(communication, 'communication', 'communication', at)) blockers.push('communication-brain-state-missing-or-stale');
    if (!validEntry(subject, subjectRuntimeDomain, subjectDomain, at)) blockers.push('subject-brain-state-missing-or-stale');
    if (!blockers.length) {
      ...
      if (!subjectSelectedDistribution && text(auto.holdReason)) {
        blockers.push('communication-b10-brake-held:' + auto.holdReason);
      }
      if (!subjectSelectedDistribution && !(Number(auto.emittedCount) > 0)) {
        blockers.push('communication-b10-no-action-selected');
      }
    ...
    if (blockers.length) {
      var heldId = 'csd_' + hash({ status: 'NO_ACTION', contentHash: contentHash,
        ...
        sourceResponseHash: normalized.sourceIdentity.responseHash, sourceArtifactId: normalized.sourceArtifactId || null,
        domainDecisionReceiptId: normalized.domainDecisionReceipt && normalized.domainDecisionReceipt.decisionReceiptId || null,
      ...
      return persist(store, {
        ...
        sourceArtifactId: normalized.sourceArtifactId || null,
        sourceIntentId: normalized.sourceIntentId || null,
        sourcePacketId: normalized.sourcePacketId || null,
        candidateHash: normalized.candidateHash || null,
        selectedProgram: normalized.selectedProgram || null,
        domainDecisionReceiptId: normalized.domainDecisionReceipt && normalized.domainDecisionReceipt.decisionReceiptId || null,
        communicationPacketId: commPacket && commPacket.packetId || null,
        subjectPacketId: subjectPacket && subjectPacket.packetId || null,
    ...
    var id = 'csd_' + hash({ contentHash: contentHash, communicationPacketId: commPacket.packetId,
      subjectPacketId: subjectPacket.packetId, sourceResponseHash: normalized.sourceIdentity.responseHash,
      sourceArtifactId: normalized.sourceArtifactId || null,
      domainDecisionReceiptId: normalized.domainDecisionReceipt && normalized.domainDecisionReceipt.decisionReceiptId || null }).slice(0, 24);
    var receipt = {
      ...
      selectionAuthority: subjectSelectedDistribution
        ? 'subject-domain-distribution-receipt'
        : 'communication-brain-action-selection',
      selectionReasons: subjectSelectedDistribution
        ? ['subject-domain-selected-action', 'communication-channel-safety-cleared',
          'subject-brain-salience-observed', 'live-source-grounded']
      ...
      decidedAt: at,
      // Communication may narrow the originating domain's authority but may
      // never extend it. The executor also revalidates the nested receipt.
      expiresAt: normalized.domainDecisionReceipt
        ? Math.min(at + MAX_DECISION_AGE_MS, Number(normalized.domainDecisionReceipt.expiresAt))
        : at + MAX_DECISION_AGE_MS,
...
function validateReceipt(receipt, candidate, now) {
  ...
  return !!(receipt && receipt.schemaVersion === SCHEMA && receipt.status === 'RELEASED' && receipt.released === true &&
    ...
    (receipt.sourceArtifactId || null) === (candidate.sourceArtifactId || null) &&
    (receipt.sourceIntentId || null) === (candidate.sourceIntentId || null) &&
    (receipt.sourcePacketId || null) === (candidate.sourcePacketId || null) &&
    (receipt.candidateHash || null) === (candidate.candidateHash || null) &&
    (receipt.domainDecisionReceiptId || null) === (candidate.domainDecisionReceipt && candidate.domainDecisionReceipt.decisionReceiptId || null) &&
    (!candidate.sourceArtifactId || (DomainDistribution.validate(candidate.domainDecisionReceipt, candidate, at) &&
      Number(receipt.expiresAt) <= Number(candidate.domainDecisionReceipt.expiresAt))) &&
...
module.exports = {
  ...
  validLegacySource: validLegacySource, validDomainSource: validDomainSource, validSource: validSource,
