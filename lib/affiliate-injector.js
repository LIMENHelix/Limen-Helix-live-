...
function inject(content, stream, opts) {
  ...
  // Prefer the env var (override), fall back to the config tag. The Amazon
  // Associates store ID is NOT a secret (it appears in every affiliate link),
  // so baking it into affiliate-config.json is safe and avoids an env round-trip.
  const azTag = az && ((az.tagEnv && process.env[az.tagEnv]) || az.tag);
