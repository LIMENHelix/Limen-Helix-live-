...
/** Finance-local B10 decision for one exact paid-subscriber fulfillment candidate. */
var crypto = require('node:crypto');
var Redis = require('./redis-kv.js');
var SCHEMA = 'finance-subscriber-decision/1.0';
var LOG_KEY = 'finance_subscriber_decision_log';
var PREFIX = 'finance_subscriber_decision:';
...
function candidate(subscriber, digest) {
  ...
  return {
    schemaVersion: 'finance-subscriber-candidate/1.0', productDomain: 'finance', ownerDomain: 'finance', lane: 'subscriber-email',
...
function validateCandidate(v) {
  return !!(v && v.schemaVersion === 'finance-subscriber-candidate/1.0' && v.productDomain === 'finance' &&
    v.ownerDomain === 'finance' && v.lane === 'subscriber-email' && text(v.email) && v.email === v.email.toLowerCase() &&
...
async function cognition(deps) { return deps && deps.cognition ? deps.cognition : (deps && deps.redisGet || Redis.redisGet)('limen:brain:cognition:finance'); }
function validCognition(entry, now) {
  ...
  return !!(c && c.domain === 'finance' && Number.isFinite(ts) && now >= ts && now - ts <= MAX_COGNITION_AGE_MS &&
    p && p.schemaVersion === 'civilization-domain-packet/1.0' && p.domainId === 'finance' && p.sourceIdentity &&
...
async function persist(store, receipt) {
  ...
  if (!restored || restored.schemaVersion !== SCHEMA || restored.status !== receipt.status || restored.actionId !== receipt.actionId) throw new Error('finance subscriber decision readback invalid');
...
function held(reason, blockers, extra) { return Object.assign({ ok: true, status: 'NO_ACTION', released: false, reason: reason,
  blockers: blockers || [], productDomain: 'finance', ownerDomain: 'finance', lane: 'subscriber-email',
...
async function decide(store, value, now, deps) {
  ...
  if (!validateCandidate(value)) return held('finance-b10-candidate-refused', ['paid-subscriber-candidate-invalid']);
  try {
    store.assertDurable(); var entry = await cognition(deps || {}), blockers = [];
    if (!validCognition(entry, at)) blockers.push('finance-brain-state-missing-or-stale');
    ...
    if (!blockers.length) {
      ...
      if (!truth.feedHealth || !(Number(truth.feedHealth.live) > 0)) blockers.push('finance-live-feeds-unavailable');
    ...
    var actionId = 'fsa_' + hash({ emailHash: value.emailHash, digestKey: value.digestKey, contentHash: value.contentHash }).slice(0, 24);
    var status = blockers.length ? 'NO_ACTION' : 'RELEASED';
    var receipt = { schemaVersion: SCHEMA, decisionReceiptId: 'fsd_' + hash({ actionId: actionId, packetId: p.packetId || null, status: status, blockers: blockers }).slice(0, 24),
      actionId: actionId, status: status, released: status === 'RELEASED', reason: blockers.length ? 'finance-b10-held' : null, blockers: blockers,
      productDomain: 'finance', ownerDomain: 'finance', lane: 'subscriber-email', decisionContract: 'paid-subscriber-fulfillment-decision/1',
      ...
      contentHash: value.contentHash, financePacketId: p.packetId || null,
      selectionReasons: blockers.length ? [] : ['active-paid-subscription', 'changed-source-grounded-digest', 'finance-brain-safe-to-fulfill'],
  ...
  } catch (error) { return held('finance-b10-unavailable', ['decision-persistence-or-input-unavailable'], { detail: String(error && error.message || error) }); }
}
function validateReceipt(receipt, value, now) {
  ...
  return !!(validateCandidate(value) && receipt && receipt.schemaVersion === SCHEMA && receipt.status === 'RELEASED' && receipt.released === true &&
    receipt.actionId === 'fsa_' + hash({ emailHash: value.emailHash, digestKey: value.digestKey, contentHash: value.contentHash }).slice(0, 24) &&
