...
function priceFromPagemap(pm) {
  if (!pm) return null;
  const candidates = [];
  const push = function (v, cur) { if (v != null) candidates.push({ v: v, cur: cur }); };

  (pm.offer || []).forEach(function (o) { push(o.price, o.pricecurrency || o.priceCurrency); });
  (pm.product || []).forEach(function (o) { push(o.price, o.pricecurrency || o.priceCurrency); });
  (pm.aggregateoffer || []).forEach(function (o) { push(o.lowprice || o.price, o.pricecurrency); });
  (pm.metatags || []).forEach(function (m) {
    push(m['og:price:amount'] || m['product:price:amount'] || m['twitter:data1'],
         m['og:price:currency'] || m['product:price:currency']);
  });

  for (const c of candidates) {
    // Refuse anything not clearly USD. A GBP figure treated as dollars silently
    // mis-prices the listing and eats the margin.
    if (c.cur && String(c.cur).toUpperCase() !== 'USD') continue;
    const raw = String(c.v).replace(/[^0-9.]/g, '');
    if (!raw) continue;
