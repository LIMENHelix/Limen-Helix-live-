...
/**
 * detectSchema(wb) — returns { ok:true, schema, headers } or a refusal.
 * Requires BOTH the structural merges AND the normalised header cells at their positions.
 */
function detectSchema(wb) {
  ...
  /* Period labels must be two distinct four-digit years plus a percent-change column, in each
     of the three groups. This is what stops a reordered or re-based table parsing as normal. */
  var yr = /^(19|20)\d\d$/;
  ...
  return {
    ...
    priorPeriod: +String(y.B).replace(/[^\d]/g, ''),
    currentPeriod: +String(y.C).replace(/[^\d]/g, ''),
...
function parse(buf, opts) {
  ...
  return parseWorkbookObject(wb, opts);
}
...
function parseWorkbookObject(wb, opts) {
  ...
  return {
    ok: true,
    table: 'B',
    tableTitle: (rowByNumber(wb, 1) || {}).A || null,
