...
var PARSER_VERSION = 'uscourts-caseload/1.2.0';
...
function parse(buf, opts) {
  ...
  /* Push source identity down onto every individual observation, so a figure lifted out of
     the payload stays attributable. The caller adds retrievedAt / sourceUpdatedAt. */
  stampObservations(out, {
    sourceRef: ((opts && opts.filename) || 'unnamed') + '@' + out.sourceSha256.slice(0, 12),
...
function parseWorkbookObject(wb, opts) {
  ...
  if (endpoint && title) {
    ...
    /* The month/day alone is not enough. "Ending June 30, 2024 and 2025" satisfies a June
       endpoint check while describing entirely different periods from the ones the columns
       and the filename claim. The title names both periods, so both are checked. */
    var titleYears = (titleFlat.match(/\b(?:19|20)\d\d\b/g) || []).map(Number);
    if (titleYears.indexOf(det.priorPeriod) < 0 || titleYears.indexOf(det.currentPeriod) < 0) {
      return refusal('TITLE_PERIOD_MISMATCH',
        'the workbook title does not name the periods its columns report',
        {
          headerPriorPeriod: det.priorPeriod, headerCurrentPeriod: det.currentPeriod,
          yearsFoundInTitle: titleYears, title: titleFlat.slice(0, 200)
        });
    }
  }
...
/**
 * stampObservations(parsed, fields) — attach provenance to EVERY observation.
 *
 * Response-level provenance is not observation-level provenance. Once a single
 * circuit-variable figure is copied out of the payload — into a note, a screenshot, a
 * downstream store — a hash sitting at the top of the response does not travel with it,
 * and the number becomes unattributable. Each observation therefore carries its own source
 * identity, source-updated time, retrieval time, and the versions of the code that produced
 * it. The duplication is the point.
 *
 * Called twice: once by parse() with what the bytes themselves establish, and once by the
 * caller with the network-time facts the parser cannot know.
 */
function stampObservations(parsed, fields) {
  if (!parsed || !parsed.ok || !Array.isArray(parsed.circuits)) return parsed;
  parsed.circuits.forEach(function (c) {
    Object.keys(c.variables).forEach(function (k) {
      var v = c.variables[k];
      if (!v.provenance) v.provenance = {};
      for (var f in fields) {
        if (Object.prototype.hasOwnProperty.call(fields, f) && fields[f] !== undefined) v.provenance[f] = fields[f];
      }
    });
  });
  return parsed;
...
module.exports = {
  ...
  stampObservations: stampObservations,
