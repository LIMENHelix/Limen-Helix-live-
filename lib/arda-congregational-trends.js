...
var PARSER_VERSION = 'arda-congregational-trends/1.0.1';
...
function parseBodies(html, population) {
  ...
  var body = table.match(/<tbody\b[^>]*>[\s\S]*?<\/tbody>/i);
  if (!body) return refusal('BODY_SECTION_MISSING', 'The reviewed body-table tbody is absent.');

  var rows = [], tr;
  var tre = /<tr\b[^>]*>([\s\S]*?)<\/tr>/gi;
  while ((tr = tre.exec(body[0]))) {
