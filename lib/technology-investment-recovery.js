...
var Executor = require('./technology-investment-executor.js');
...
var SCHEMA = 'technology-investment-recovery/1.0';
var LOG_KEY = 'technology_investment_recovery_log';
var PREFIX = 'technology_investment_recovery:';
...
async function save(store, receipt) {
  ...
  if (!restored || restored.status !== receipt.status || restored.commandId !== receipt.commandId) throw new Error('technology investment recovery readback invalid');
  return restored;
}
async function recover(options) {
  ...
  if (!on(env.TECHNOLOGY_INVESTMENT_RECOVERY_ENABLED)) return { ok: true, status: 'HELD', reason: 'technology-investment-recovery-switch-off', liveMoney: false };
  if (!command || command.schemaVersion !== Executor.SCHEMA || command.status !== 'COMMAND_RECEIPTED' || !command.brokerCommandId) {
    return { ok: false, status: 'REFUSED', reason: 'technology-investment-command-receipt-required', liveMoney: false };
  }
  if (!((trigger.type === 'technology-investment-kill' || trigger.type === 'independent-risk-breach') && String(trigger.id || '').trim())) {
  ...
  try {
    ...
    if (!authorization || !authorization.authorized) return { ok: true, status: 'HELD', reason: authorization && authorization.reason || 'technology-recovery-motor-held', liveMoney: false };
    var recoveryId = 'tir_' + require('./technology-investment-decision.js').hash({ command: command.commandId, trigger: trigger.id, motor: authorization.receiptId }).slice(0, 24);
    ...
    var receipt = { schemaVersion: SCHEMA, recoveryId: recoveryId, commandId: command.commandId, actionId: command.actionId,
      ...
      productDomain: 'technology', ownerDomain: 'technology', lane: 'investments', productMotorReceiptId: authorization.receiptId,
    ...
    if (executed <= 0 && !['filled', 'canceled', 'rejected', 'expired', 'error'].includes(String(order.status || '').toLowerCase())) {
      if (!brokerCommand.rollback || !brokerCommand.rollback.confirmationSummary) throw new Error('technology investment cancel receipt unavailable');
      brokerCommand = await b14.cancelApproved(store, broker, {
        ...
        approval: { mode: 'recovery', actor: 'technology-brain-recovery', ownerDomain: 'technology',
      ...
      if (!closePrice) throw new Error('technology investment close quote unmeasured');
      var preview = await b14.createPreview(store, broker, { symbol: command.symbol, side: closeSide, quantity: executed,
        ...
        ownerDomain: 'technology', horizonDays: [30, 60, 90], decisionContext: { recoveryOf: command.commandId, trigger: receipt.trigger } }, now);
    ...
    return { ok: false, status: 'RECOVERY_UNRESOLVED', reason: 'technology-investment-recovery-unavailable', detail: String(error && error.message || error), paperOnly: true, liveMoney: false };
  }
}
