...
function observer(lane, deps) {
  ...
  return async function handler(req, res) {
    ...
    if (!auth.enforce(req, res)) return;
    if (!SubscriberPolicy.resolve(env, lane.config.envNames).observerEnabled) {
    ...
    try {
      ...
      var observations = [], inspected = 0, providerReadAttempts = 0, seen = {};
      var learned = 0, duplicates = 0, abstentions = [], recoveries = [], recoveryHolds = [];
      var pendingMigration = await lane.observer.reconcileLegacyPending(store);
      var pendingAtStart = await lane.observer.depth(store);
      /* Only real provider reads consume the 40-read budget. Deferred positive
         lifecycle refs are still rotated so due refs behind them are reached. */
      var maxIterations = Math.min(pendingAtStart, 1000);
      var maxProviderReads = Math.max(1, Math.min(40, Number(deps.maxProviderReads) || 40));
      for (var i = 0; i < maxIterations && providerReadAttempts < maxProviderReads; i++) {
        ...
        try {
          ...
          if (observation && observation.providerReadAttempted) providerReadAttempts++;
          ...
          if (lane.observer.isFinal(observation) && downstreamComplete) {
            await lane.observer.acknowledge(store, pending.pendingRef);
          }
        } finally {
          await lane.observer.releaseLease(store, pending.pendingRef, pending.leaseToken);
        }
      }
      return json(res, 200, {
        ok: true,
        ...
        deferredPendingOutcomes: Math.max(0, pendingAtStart - inspected),
        pendingMigration: pendingMigration,
        providerReadAttempts: providerReadAttempts,
        maxProviderReads: maxProviderReads,
...
function recovery(lane, deps) {
  ...
  return async function handler(req, res) {
    ...
    if (!gate.hasDomain(pass, lane.config.ownerDomain) &&
        !gate.hasDomain(pass, lane.config.productDomain)) return gate.deny(res);
    try {
      var body = bodyOf(req);
