...
function observer(lane, deps) {
  ...
  return async function handler(req, res) {
    ...
    try {
      store.assertDurable();
      var pendingOutcomes = await lane.observer.observePending(store, {
        fetch: deps.fetch || global.fetch,
        apiKey: deps.apiKey
      });
      var observations = [];
      var learned = 0, duplicates = 0, abstentions = [], recoveries = [], recoveryHolds = [];
      for (var i = 0; i < pendingOutcomes.length; i++) {
        var pending = pendingOutcomes[i] || {}, observation = pending.observation;
        ...
        if (observation.status === 'TERMINAL_OBSERVED' && downstreamComplete) {
      ...
      return json(res, 200, {
        ...
        productDomain: lane.config.productDomain,
        ownerDomain: lane.config.ownerDomain,
        inspectedPendingOutcomes: pendingOutcomes.length,
