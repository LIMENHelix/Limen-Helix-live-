...
var CANONICAL_REASONS = Object.freeze({
  'No domain has a fresh source-linked commercial artifact ready for public distribution.':
...
function token(value, fallback) {
  ...
  if (Object.prototype.hasOwnProperty.call(CANONICAL_REASONS, value)) {
    return CANONICAL_REASONS[value];
  }
...
function emit(cycle, result, logger) {
  ...
  try { output('[autonomy-cycle] ' + JSON.stringify(summary)); }
  catch (_) { /* observability must never become a motor dependency */ }
  return summary;
}
