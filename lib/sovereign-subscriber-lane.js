...
function create(input) {
      ...
      for (var i = 0; i < specs.length; i++) {
        ...
        if (item.status === 'HELD_INHIBITED') {
          ...
          if (item.status === 'ACCEPTED') {
            await store.lpush(keys.observationPending, { commandId: commandId, actionId: item.actionId,
              providerEmailId: item.providerEmailId, enqueuedAt: Date.now() });
          }
        }
  ...
  async function observePending(store, deps) {
    var refs = await store.lrange(keys.observationPending, -100, -1), rows = [], seen = {};
    for (var i = 0; i < (refs || []).length; i++) {
      var ref = refs[i];
      if (!ref || !ref.commandId || !ref.actionId || seen[ref.actionId]) {
      ...
      rows.push(result);
      await store.lrem(keys.observationPending, 1, ref);
      if (!result || result.status !== 'TERMINAL_OBSERVED') {
        await store.lpush(keys.observationPending, Object.assign({}, ref, { lastObservedAt: Date.now() }));
      }
    }
    return rows;
  }
  ...
  async function retryRecent(input) {
    input = input || {}; var store = input.store || Store, refs = await store.lrange(keys.fulfillmentPending, -100, -1), results = [], seen = {};
    for (var i = 0; i < (refs || []).length; i++) {
      var ref = refs[i], id = ref && ref.taskId;
      if (!id || seen[id]) { if (ref) await store.lrem(keys.fulfillmentPending, 1, ref); continue; }
      seen[id] = true;
      ...
      results.push(result);
      await store.lrem(keys.fulfillmentPending, 1, ref);
      if (result.status === 'HELD') await store.lpush(keys.fulfillmentPending,
        Object.assign({}, ref, { lastAttemptAt: Date.now() }));
    }
    return results;
  }
