...
function create(input) {
  ...
  function validCapability(value, now) {
    return !!(value && value.schemaVersion === schemas.capability && value.productDomain === productDomain &&
      value.ownerDomain === ownerDomain && value.lane === LANE && text(value.capabilityId) &&
      value.environment === 'production' && value.provider === 'resend' &&
      value.externalEffectVerified === true && value.independentReadbackVerified === true &&
      value.recoveryVerified === true && value.executorVerified === true &&
      value.independentOutcomeObserverVerified === true && value.rollbackVerified === true &&
      Number.isFinite(Number(value.verifiedAt)) && now >= Number(value.verifiedAt) &&
      now - Number(value.verifiedAt) <= MAX_CAPABILITY_AGE_MS);
  }
  ...
  async function authorize(store, requestedProduct, requestedLane, now, deps) {
    ...
    try {
      store.assertDurable();
      var capability = await store.get(keys.capability);
      if (!validCapability(capability, at)) return heldAuthorization(productDomain + '-subscriber-production-capability-missing-or-stale');
      var entry = await readCognition(deps || {}), blockers = [];
      if (!validCognition(entry, at)) blockers.push(productDomain + '-brain-state-missing-or-stale');
      ...
      var receiptId = short + 'smr_' + hash({ packetId: packet.packetId, capabilityId: capability.capabilityId, lane: LANE, at: at }).slice(0, 24);
      var receipt = {
  ...
  async function execute(input) {
    ...
    try {
      ...
      var suppression = await store.get(keys.suppression); if (!suppression || typeof suppression !== 'object') suppression = {};
      specs = specs.filter(function (row) { return !(suppression[row.candidate.emailHash] && suppression[row.candidate.emailHash].suppressed); });
      ...
      if (existing) return Object.assign({ ok: true, replayed: true, accepted: existing.accepted || 0 }, existing);
      var command = {
        schemaVersion: schemas.command, commandId: commandId, status: 'COMMANDING', productDomain: productDomain,
        ownerDomain: ownerDomain, lane: LANE, productMotorReceiptId: motor.receiptId, capabilityId: motor.capabilityId,
      ...
      for (var i = 0; i < specs.length; i++) {
        ...
        try {
          ...
          result = { ok: false, providerCalled: error && error.code === AdapterGuard.INHIBITED ? false : true,
            ...
            ambiguous: error && error.code !== AdapterGuard.INHIBITED, error: String(error && error.message || error) };
        }
  ...
  async function recover(input) {
    ...
    var suppression = await store.get(keys.suppression); if (!suppression || typeof suppression !== 'object') suppression = {};
    suppression[item.emailHash] = { suppressed: true, recoveryId: recoveryId, actionId: item.actionId,
      reason: observation && observation.lastEvent || trigger.type, at: now };
    await store.set(keys.suppression, suppression); var restoredSuppression = await store.get(keys.suppression);
    if (!restoredSuppression || !restoredSuppression[item.emailHash] || restoredSuppression[item.emailHash].recoveryId !== recoveryId) throw new Error(productDomain + ' subscriber suppression readback invalid');
  ...
  function minimizeTerminalTask(task) {
    if (!task || ['COMPLETED', 'AMBIGUOUS', 'FAILED'].indexOf(task.status) < 0) return task;
  ...
  async function enqueueAndAttempt(input) {
    ...
    if (created) { await store.lpush(keys.fulfillmentPending, { taskId: id, productDomain: productDomain, enqueuedAt: task.createdAt }); await store.ltrim(keys.fulfillmentPending, 0, 999); }
    return attemptTask(Object.assign({}, input, { task: task, store: store }));
  }
  async function retryRecent(input) {
    input = input || {}; var store = input.store || Store, refs = await store.lrange(keys.fulfillmentPending, 0, 99), results = [], seen = {};
    for (var i = 0; i < (refs || []).length; i++) {
      var id = refs[i] && refs[i].taskId; if (!id || seen[id]) continue; seen[id] = true;
      var task = await store.get(keys.fulfillment(id)); if (!task || task.productDomain !== productDomain || ['PENDING', 'HELD'].indexOf(task.status) < 0) continue;
  ...
  return Object.freeze({
    ...
    authorization: Object.freeze({ authorize: authorize, validCapability: validCapability }),
    executor: Object.freeze({ SCHEMA: schemas.command, HARD_MAX_SENDS: HARD_MAX_SENDS,
      ...
      LOG_KEY: keys.commandLog, SUPPRESSION_KEY: keys.suppression }),
    observer: Object.freeze({ SCHEMA: schemas.observation, LOG_KEY: keys.observationLog,
      TERMINAL: TERMINAL, key: keys.observation, observe: observe, observeRecent: observeRecent }),
