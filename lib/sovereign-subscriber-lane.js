...
function create(input) {
  ...
  async function execute(input) {
    ...
    try {
      ...
      for (var i = 0; i < specs.length; i++) {
        ...
        var resumablePrior = prior && ['PRE_SEND', 'HELD_INHIBITED', 'HELD_PRE_SEND'].indexOf(prior.status) >= 0;
        if (prior && !resumablePrior) {
          item.status = prior.status === 'DISPATCHING' ? 'AMBIGUOUS' : (prior.status || 'PREVIOUSLY_CLAIMED');
          item.providerEmailId = prior.providerEmailId || null;
          item.providerCalled = prior.providerCalled !== false;
          if (prior.status === 'DISPATCHING') {
            item.failure = 'prior-dispatch-outcome-unknown'; item.resolvedAt = Date.now();
            await store.set(keys.action(item.actionId), Object.assign({}, prior, { status: 'AMBIGUOUS',
              providerCalled: true, failure: item.failure, resolvedAt: item.resolvedAt }));
          }
        ...
        var claim = { schemaVersion: schemas.command, productDomain: productDomain, actionId: item.actionId,
          ...
          idempotencyKey: idempotencyKey, claimedAt: Date.now() };
        var claimed = resumablePrior ? (await store.set(keys.action(item.actionId), claim)) :
        ...
        if (!result) {
          try {
            await store.set(keys.action(item.actionId), Object.assign({}, claim, { status: 'DISPATCHING',
  ...
  async function observe(store, command, item, deps) {
    ...
    if (previous && previous.productDomain === productDomain && previous.actionId === item.actionId &&
      ...
      return { ok: true, status: 'OBSERVATION_DEFERRED', reason: 'positive-lifecycle-follow-up-not-due',
        ...
        followUpUntil: previous.followUpUntil, sendProviderCalled: false, liveMoney: false };
    }
    ...
    var receipt = {
      ...
      sendEndpointCalled: false, observedAt: observedNow, liveMoney: false
    };
  ...
  async function observeNext(store, deps, seen) {
    ...
    if (seen[ref.actionId]) {
      await store.lrem(keys.observationPending, 1, ref);
      return { skipped: true, duplicateRef: true };
    }
    ...
    return { observation: await observe(store, command, item, deps), pendingRef: ref };
  }
  ...
  async function recordObservation(store, observation) {
    ...
    try {
      ...
      if (!(await store.setIfLockOwned(keys.learningLock, lockToken, keys.learning, state))) {
        throw new Error(productDomain + ' subscriber learning lock expired before commit');
      }
