...
function create(input) {
    ...
    try {
      ...
      for (var i = 0; i < specs.length; i++) {
        ...
        if (prior && !resumablePrior) {
          if (prior.status === 'PRE_SEND' || prior.status === 'DISPATCHING') {
  ...
  return Object.freeze({
    ...
    observer: Object.freeze({ SCHEMA: schemas.observation, LOG_KEY: keys.observationLog,
      ...
      releaseLease: releaseObservationLease, isResolved: resolved, isFinal: finalObservation, depth: observationDepth }),
