...
function compactCognition(rec, now) {
  ...
  var regulationControl = model.regulationControl && typeof model.regulationControl === 'object'
    ? model.regulationControl
    : {};
  var awareness = c.awareness && typeof c.awareness === 'object' ? c.awareness : {};
  return {
    present: true,
    ...
    regulationControl: {
      gain: round(regulationControl.gain),
      inhibition: round(regulationControl.inhibition),
      outputScale: round(regulationControl.outputScale),
      starving: bool(regulationControl.starving),
      flooding: bool(regulationControl.flooding),
      looping: bool(regulationControl.looping),
      stale: bool(regulationControl.stale),
      overconfident: bool(regulationControl.overconfident),
      surprised: bool(regulationControl.surprised)
    },
    predictionError: round(model.predictionError),
    predictedStress: round(model.predictedStress),
    immune: text(immune.immuneState, 48),
    immuneSeverity: round(immune.severity),
    awareness: {
      selfState: text(awareness.selfState, 64),
      selfNarrative: text(awareness.selfNarrative, 600),
      humanReviewRequired: bool(awareness.humanReviewRequired),
      knownCount: finite(awareness.knownCount),
      uncertaintyCount: finite(awareness.uncertaintyCount),
      role: 'downstream summary of this domain brain state; not evidence or authority'
    },
    interoception: {
      ...
      integrated: round(intero.integrated),
      uncertainty: round(intero.uncertainty),
