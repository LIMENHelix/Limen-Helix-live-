...
// r is keyed by NORMALIZED headers (lowercase alphanumeric) because CA's .xlsx header cells carry
// embedded newlines ("Notice\r\nDate", "Layoff/\r\nClosure") — see nk() in warn-fetch.js.
function normCA(r) {
  return shape({ state: 'CA', company: r.company, address: r.address, county: (r.countyparish || '').replace(/ (county|parish)$/i, ''),
    affected: r.noofemployees, noticeDate: r.noticedate, effectiveDate: r.effectivedate,
    closureType: r.layoffclosure, industry: r.relatedindustry });
}
function shape(o) {
  var d = {
