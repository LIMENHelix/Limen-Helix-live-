'use strict';
var crypto = require('node:crypto'), Decision = require('./intelligence-autopilot-decision.js'), Motor = require('./product-domain-motor-authorization.js'), Learning = require('./intelligence-autopilot-learning.js');
...
async function execute(input) { input = input || {}; var store = input.store, v = input.candidate, decision = input.decision, now = Number(input.now) || Date.now();
  ...
  try { store.assertDurable(); var suppression = await store.get(SUPPRESSION_KEY); if (suppression && suppression[v.emailHash] && suppression[v.emailHash].suppressed) return held('intelligence-autopilot-recipient-suppressed');
    var auth = await (input.motorAuthorization || Motor).authorize(store, 'intelligence', 'autopilot', now); if (!auth || !auth.authorized) return held(auth && auth.reason || 'intelligence-autopilot-motor-held', { motorReceiptId: auth && auth.receiptId || null, motorBlockers: auth && auth.blockers || [] });
    ...
    var commandId = 'iac_' + hash({ actionId: decision.actionId, motor: auth.receiptId }).slice(0, 24), command = { schemaVersion: SCHEMA, commandId: commandId, actionId: decision.actionId,
      ...
      predictedOutcome: decision.predictedOutcome, emailCostUsd: cost, dailyBudgetUsd: cost === 0 ? 0 : budget, dailyEmailCap: cap, providerCalls: 0, commandedAt: now, liveMoney: false };
    ...
    var result; try { result = await input.transport.send(v.email, v.subject, v.body, { idempotencyKey: idem }); } catch (e) { result = { ok: false, providerCalled: true, ambiguous: true, error: String(e && e.message || e) }; }
