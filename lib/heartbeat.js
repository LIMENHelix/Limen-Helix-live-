 ...
 * and finance-paper-cycle (calls the manager and Tradier sandbox motor).
...
var OUTWARD = ['social-cron', 'communication-social-capability', 'subscriber-digest', 'autopilot', 'automail', 'finance-paper-cycle'];
...
function wrap(job, handler) {
  return async function (req, res) {
    ...
    try {
      ...
      if (!(res && res._limenAuthRejected === true)) {
...
/** Notify an optional read-only observer without letting telemetry become a motor dependency. */
async function observeVeto(options, gate, req) {
  if (!options || typeof options.onVeto !== 'function') return false;
  try {
    await options.onVeto(gate, req);
    return true;
  } catch (_) {
    return false;
  }
