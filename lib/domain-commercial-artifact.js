...
var CONTINUITY_REASONS = [
  'no-meaningful-afferent-or-stress-change',
  'commercial-cadence-inhibited',
  'commercial-daily-artifact-cap-reached'
];
...
// Promotion has to use the same total ordering as the durable intent queue.
// Timestamps alone are not a total order: two independent cron invocations can
// plan different intents in the same millisecond. The immutable intent ID is
// therefore the deterministic tie-breaker, while preparedAt only orders retries
// or freshness generations of the same plan.
function compareSourcePlans(a, b) {
  var at = Number(a && a.sourcePlannedAt);
  var bt = Number(b && b.sourcePlannedAt);
  if (Number.isFinite(at) && Number.isFinite(bt) && at !== bt) return at - bt;
  var ai = String(a && a.intentId || '');
  var bi = String(b && b.intentId || '');
  if (ai !== bi) return ai < bi ? -1 : 1;
  return Number(a && a.preparedAt || 0) - Number(b && b.preparedAt || 0);
}
...
function build(contract, state, nowValue) {
  ...
  // A partially persisted attempt may leave the immutable artifact record but
  // fail before promoting latest-state or acknowledging the intent. A retry in
  // the same freshness generation is idempotent; after that artifact expires,
  // a new deterministic generation gets a new identity instead of restoring
  // stale inventory and discarding the still-valid work order.
  var freshnessWindowMs = freshnessMs(intent.cadence);
  var freshnessGeneration = Math.floor(now / freshnessWindowMs);
  var artifactId = 'dca_' + hash({ domain: contract.productDomain, intent: intent.intentId,
    content: contentHash, freshnessGeneration: freshnessGeneration }).slice(0, 24);
  return {
    ...
    artifact: {
      ...
      sourcePlannedAt: Number(intent.plannedAt),
      evidenceFingerprint: state.evidenceFingerprint,
      sourceStress: state.lastStress,
      sourcePriority: state.priority,
      ...
      freshnessGeneration: freshnessGeneration,
      preparedAt: now,
      freshnessExpiresAt: now + freshnessWindowMs,
      truthBoundary: {
        fullTextRead: false,
...
/**
 * Renew the distribution envelope of an unchanged prepared artifact.
 *
 * Cadence and daily caps regulate creation of new customer content. They must
 * not silently make the last accepted artifact undistributable while that
 * exact plan is still the current non-safety plan. Renewal copies the already
 * reviewed bytes and provenance; it cannot change text, sources, program,
 * stress, offer, or content hash and therefore is not a new commercial plan.
 */
function renew(contract, state, artifact, nowValue) {
  var now = Number.isFinite(Number(nowValue)) ? Number(nowValue) : Date.now();
  if (!contract || !state || !artifact) return { status: 'ABSTAINED', reason: 'commercial-artifact-renewal-input-missing', artifact: null };
  if (state.schemaVersion !== 'domain-commercial-reflex/1.0' || state.readbackVerified !== true ||
      state.status !== 'ABSTAINED' || CONTINUITY_REASONS.indexOf(state.reason) < 0) {
    return { status: 'ABSTAINED', reason: 'commercial-artifact-renewal-not-continuity-eligible', artifact: null };
  }
  if (artifact.schemaVersion !== SCHEMA || artifact.status !== 'ARTIFACT_PREPARED' ||
      artifact.externalEffectAuthorized !== false || state.productDomain !== contract.productDomain ||
      state.ownerDomain !== contract.ownerDomain || artifact.productDomain !== contract.productDomain ||
      artifact.ownerDomain !== contract.ownerDomain) {
    return { status: 'ABSTAINED', reason: 'commercial-artifact-renewal-identity-invalid', artifact: null };
  }
  if (state.lastPlannedIntentId !== artifact.intentId || !state.evidenceFingerprint ||
      state.evidenceFingerprint !== artifact.evidenceFingerprint) {
    return { status: 'ABSTAINED', reason: 'commercial-artifact-renewal-plan-mismatch', artifact: null };
  }
  var plannedAt = Number(artifact.sourcePlannedAt), preparedAt = Number(artifact.preparedAt);
  var expiresAt = Number(artifact.freshnessExpiresAt), windowMs = expiresAt - preparedAt;
  var allowedWindows = [freshnessMs('IMMEDIATE'), freshnessMs('TWELVE_HOURS'), freshnessMs('DAILY')];
  if (!Number.isFinite(plannedAt) || now < plannedAt || now - plannedAt > MAX_INTENT_AGE_MS ||
      !Number.isFinite(preparedAt) || !Number.isFinite(expiresAt) || now < expiresAt ||
      allowedWindows.indexOf(windowMs) < 0 || !text(artifact.contentHash) || !text(artifact.intentId)) {
    return { status: 'ABSTAINED', reason: 'commercial-artifact-renewal-evidence-stale-or-invalid', artifact: null };
  }
  var generation = Math.floor(now / windowMs);
  var renewed = JSON.parse(JSON.stringify(artifact));
  renewed.artifactId = 'dca_' + hash({ domain: contract.productDomain, intent: artifact.intentId,
    content: artifact.contentHash, freshnessGeneration: generation }).slice(0, 24);
  renewed.freshnessGeneration = generation;
  renewed.preparedAt = now;
  renewed.freshnessExpiresAt = now + windowMs;
  renewed.renewedFromArtifactId = artifact.artifactId;
  renewed.renewalReason = state.reason;
  renewed.externalEffectAuthorized = false;
  renewed.providerCalled = false;
  renewed.spendUsd = 0;
  return { status: 'ARTIFACT_PREPARED', reason: null, artifact: renewed, renewed: true };
}
...
async function persist(store, contract, result) {
  ...
  for (var attempt = 0; attempt < 3; attempt++) {
    ...
    if (current && compareSourcePlans(current, restored) > 0) return current;
    var written = current
...
module.exports = {
  ...
  CONTINUITY_REASONS: CONTINUITY_REASONS.slice(),
  compareSourcePlans: compareSourcePlans,
  hash: hash
};
