...
async function persist(store, contract, result) {
  ...
  for (var attempt = 0; attempt < 3; attempt++) {
    ...
    if (current && Number.isFinite(Number(current.sourcePlannedAt)) &&
        Number.isFinite(Number(restored.sourcePlannedAt)) &&
        Number(current.sourcePlannedAt) > Number(restored.sourcePlannedAt)) return current;
