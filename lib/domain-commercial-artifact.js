...
function build(contract, state, nowValue) {
  ...
  var subject = domainLabel + ' signal brief — ' + new Date(now).toISOString().slice(0, 10);
  var body = sections.map(function (section) { return section.heading + '\n' + section.body; }).join('\n\n');
  var contentHash = hash({ subject: subject, body: body, sources: sources, intentId: intent.intentId });
