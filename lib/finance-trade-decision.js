...
var SYSTEM = [
  ...
  'Thing 0 only states whether validated Thing 1 is applicable. Thing 1 may be considered only when explicitly applicable. Thing 2 only reconciles Thing 1 with the long-arc snapshot for alignment, divergence, or possible masking: copy the supplied reconciliation status and set thing2DecisionWeight to exactly 0.',
...
function parseProposal(raw, candidate, evidence) {
  ...
  var expectedThing2Observed = !!(evidence && evidence.helixReport && evidence.helixReport.thing2 && evidence.helixReport.thing2.observed);
  if (!value || value.thing2Observed !== expectedThing2Observed) blockers.push('trade_decision_thing2_observation_acknowledgement_required');
  if (!value || value.thing2DecisionWeight !== 0) blockers.push('trade_decision_thing2_decision_weight_must_be_zero');
  if (!value || value.thing2Role !== 'alignment_and_masking_reconciliation_only') blockers.push('trade_decision_thing2_role_invalid');
  var expectedReconciliation = evidence && evidence.helixReport && evidence.helixReport.thing2 && evidence.helixReport.thing2.maskingAssessment || 'unassessed';
  if (!value || value.thing2ReconciliationStatus !== expectedReconciliation) blockers.push('trade_decision_thing2_reconciliation_acknowledgement_required');
...
async function execute(store, broker, request, options) {
  ...
  var requestBody = {
    ...
    responseSchema: {
      ...
      thing2Observed: evidence.helixReport.thing2.observed,
      thing2DecisionWeight: 0,
      thing2Role: 'alignment_and_masking_reconciliation_only',
      thing2ReconciliationStatus: evidence.helixReport.thing2.maskingAssessment,
