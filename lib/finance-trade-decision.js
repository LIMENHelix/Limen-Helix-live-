 ...
 *
 * The provider may choose BUY, SELL, or ABSTAIN and explain that choice. It
...
var PROPOSAL_SCHEMA = 'finance-trade-decision-proposal/1.0';
...
var SYSTEM = [
  ...
  'The schema is finance-trade-decision-proposal/1.0.',
  'Choose BUY, SELL, or ABSTAIN for the supplied company only.',
  'Use only supplied evidence and copy every evidenceRefs entry exactly.',
  'Do not choose quantity, price, order type, broker, leverage, options, shorts, or live execution.',
...
function parseProposal(raw, candidate) {
  ...
  if (['BUY', 'SELL', 'ABSTAIN'].indexOf(value && value.action) < 0) blockers.push('trade_decision_action_invalid');
...
function buildIntent(candidate, proposal, quote, account, risk) {
  ...
  return {
    ...
    tradeIntent: {
      symbol: symbol,
      side: proposal.action.toLowerCase(),
      quantity: quantity,
...
function b10(candidate, proposal, tradeIntent, now) {
  var actionKind = proposal.action === 'BUY' ? 'open_sandbox_long' : 'close_sandbox_long';
  var actor = PROP.makeCandidate({
    ...
    expectedBenefits: ['a measured paper position episode', '30/60/90-day outcome evidence'],
    ...
    rollbackPlan: 'do not submit, cancel an open sandbox order, or close only an established long position',
    expectedEvaluationMs: 30 * 24 * 60 * 60 * 1000,
    movesVariable: 'finance.position.' + candidate.company.ticker + '.quantity',
    expectedMagnitude: proposal.action === 'BUY' ? tradeIntent.quantity : -tradeIntent.quantity
  });
...
async function execute(store, broker, request, options) {
  ...
  var symbol = candidate.company.ticker;
  var observed = await Promise.all([broker.quote(symbol), broker.accountSnapshot()]);
  var quote = observed[0], account = observed[1], risk = before.policy;
  ...
  var commandedAt = options.now || new Date().toISOString();
  var commanded = {
  ...
  var created = await store.setIfAbsent(key(before.packetId), commanded, RETENTION_SECONDS);
  if (!created) {
  ...
  if (!resourceBefore.allowsProviderCall) {
    ...
    await store.set(key(before.packetId), inhibited, RETENTION_SECONDS);
  ...
  var parsed = response && response.ok === true ? parseProposal(response.text, candidate) : {
    ok: false,
  ...
  var intentResult = parsed.ok ? buildIntent(candidate, parsed.proposal, quote, account, effectiveRisk) : {
  ...
  await store.set(key(before.packetId), receipt, RETENTION_SECONDS);
  await store.lpush(LOG_KEY, {
