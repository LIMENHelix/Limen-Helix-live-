...
var MAX_PROVIDER_RESPONSE_CHARS = 24 * 1024;
var FACTOR_NAMES = [
...
function extractSingleJsonObject(raw) {
  ...
  if (source.length > MAX_PROVIDER_RESPONSE_CHARS) {
    return { ok: false, reason: 'trade_decision_json_too_large', diagnostic: { objectCount: 0, responseChars: source.length } };
  }
  var objects = [], start = -1, depth = 0, inString = false, escaped = false;
  for (var i = 0; i < source.length; i++) {
    ...
    if (char === '{') {
      ...
      if (depth === 0) {
        return { ok: false, reason: 'trade_decision_json_invalid', diagnostic: { objectCount: objects.length, responseChars: source.length } };
      }
      depth--;
      if (depth === 0 && start >= 0) {
  ...
  if (!objects.length) {
    return { ok: false, reason: 'trade_decision_json_required', diagnostic: { objectCount: 0, responseChars: source.length } };
  }
  if (objects.length !== 1) {
    return { ok: false, reason: 'trade_decision_json_ambiguous', diagnostic: { objectCount: objects.length, responseChars: source.length } };
  }
  return { ok: true, json: objects[0], diagnostic: { objectCount: 1, responseChars: source.length } };
}

function parseProposal(raw, candidate, evidence) {
  var extracted = extractSingleJsonObject(raw);
  if (!extracted.ok) return extracted;
  var value;
  try { value = JSON.parse(extracted.json); }
  catch (_) { return { ok: false, reason: 'trade_decision_json_invalid', diagnostic: extracted.diagnostic }; }
  var blockers = [];
  ...
  return blockers.length
    ? { ok: false, reason: blockers[0], blockers: blockers, diagnostic: extracted.diagnostic }
    : { ok: true, proposal: value, diagnostic: extracted.diagnostic };
}
...
function postDecisionReconciliation(evidence) {
  ...
  var news = (evidence && Array.isArray(evidence.issuerFeeds) ? evidence.issuerFeeds : []).slice(0, 12).map(function (row) {
    return {
      title: row && row.title || null,
      publisher: row && row.publisher || null,
      feedName: row && row.feedName || null,
      sourceUpdatedAt: row && row.sourceUpdatedAt || null,
      recordedAt: row && row.recordedAt || null,
      sourceIdentity: clone(row && row.sourceIdentity || null)
    };
  });
  var status = thing2.maskingAssessment || 'unassessed';
  var possible = status === 'possible_masking_or_snapshot_divergence';
  return {
    sequence: 'current_exact_issuer_news_then_thing1_result_then_thing2_snapshot',
    displaySequence: 'current_exact_issuer_news_first_then_possible_masking',
    currentNewsFirst: {
      status: news.length ? 'CURRENT_EXACT_ISSUER_NEWS_PRESENT' : 'UNAVAILABLE',
      observations: news,
    ...
    status: status,
    possibleMasking: possible,
    maskingConfirmation: !possible ? 'NO_POSSIBLE_MASKING_FLAG_TO_CONFIRM'
      : (news.length ? 'UNCONFIRMED_NEWS_PRESENT_FOR_COMPARISON' : 'UNCONFIRMED_CURRENT_NEWS_MISSING'),
    ...
    note: 'Thing 2 may identify possible masking. Current exact-issuer news is displayed first for comparison; the masking flag remains unconfirmed until that evidence is explicitly reconciled.'
  };
}
...
function b10(candidate, proposal, tradeIntent, now, selectionContext) {
  ...
  return { critic: critic, gate: gate };
}
...
async function execute(store, broker, request, options) {
  ...
  var parsed = response && response.ok === true ? parseProposal(response.text, candidate, evidence) : {
    ok: false,
    reason: response && response.disabled ? 'finance_trade_decision_ai_disabled'
      : response && response.errorType === 'structured_output_truncated' ? 'trade_decision_output_truncated'
        : response && response.errorType === 'structured_output_refused' ? 'trade_decision_output_refused'
          : 'finance_trade_decision_provider_failed',
    diagnostic: {
      objectCount: 0,
      responseChars: 0
    }
  ...
  var resourceAfter = FinanceMetabolism.evaluate({
    ...
    account: account,
    motorPolicy: risk,
    providerCallsUsed: response && Number.isFinite(Number(response.providerAttempts))
      ? Number(response.providerAttempts) : calls,
    env: env
  });
  ...
  if (intentResult.status === 'INTENT_READY') {
    var learning = options.learning || Learning;
    var learningContext = await learning.selectionContext(store, 'finance');
    criticResult = b10(candidate, parsed.proposal, intentResult.tradeIntent, Date.parse(commandedAt), learningContext);
    await learning.persistSelectionGate(store, 'finance', criticResult.gate);
  }
  var critic = criticResult && criticResult.critic;
  ...
  var receipt = {
    ...
    provider: {
      ...
      route: response && response.providerRoute || null,
      attempts: response && Number.isFinite(Number(response.providerAttempts)) ? Number(response.providerAttempts) : null,
      fallback: clone(response && response.fallback || null),
      structuredOutput: response && response.structuredOutput === true,
    ...
    proposalParse: clone(parsed.diagnostic || null),
...
module.exports = {
  ...
  MAX_PROVIDER_RESPONSE_CHARS: MAX_PROVIDER_RESPONSE_CHARS,
  FACTOR_NAMES: FACTOR_NAMES,
  SYSTEM: SYSTEM,
  key: key,
  policy: policy,
  extractSingleJsonObject: extractSingleJsonObject,
