...
async function adversarialReview(input) {
  ...
  const system = 'You are a HOSTILE, highly experienced ' + funder + ' reviewer who has already decided to DECLINE this proposal and must justify it. Tear it apart. For EACH rubric lesson, find the specific weakness in THIS document (quote the offending text), rate severity, and give the exact fix. Be uncharitable; assume the worst-case skeptical reviewer. Separately raise any INTEGRITY problem: results stated as validated/proven that are not clearly reproducible. '
    + 'Output STRICT JSON only: {"gate":"block|major-revise|minor-revise|pass","teardown":[{"lesson":"<id>","severity":"critical|high|medium|low","finding":"","fix":""}],"integrityFlags":["..."],"topThreeFixes":["","",""]}';
  const prompt = 'FUNDER: ' + funder + ' | LANE: ' + lane + '\n\nRUBRIC (apply every item):\n'
    ...
    + '\n\nDOCUMENT:\n' + String(input.text).slice(0, 16000);
  const r = await orch.call('AUTHOR_PATTERN', { system: system, prompt: prompt, maxTokens: 4000 });
  if (!r.ok) return { ok: false, error: 'adversarial review failed: ' + r.error, budget: r.budget || null };
  let review = _extractJson(r.text);
  if (!review) review = { gate: 'unknown', raw: r.text.slice(0, 1500) };
  return { ok: true, review: review, provider: r.provider, rubricApplied: lessons.length };
}
...
async function scoreLanes(card) {
  ...
  const system = 'You are the LIMEN per-lane routing scorer. Score a brain<->business pattern card for fit against the FOUR capital lanes, exactly as a rigorous, harsh human reviewer would. Be specific and skeptical; do not inflate.\n\n'
    + 'PER-LANE CRITERIA (score each 0-10):\n'
    + '- patent: USPTO 35 USC 101 eligibility + 103 non-obviousness + 112 enablement. Concrete, novel, enablable algorithm/method, NOT a decorative neural analogy. Established product categories (risk scoring, churn prediction, observability) score low.\n'
    + '- grant: mission match to a REAL, current funding line (NSF/NIH/DOE/DoD/USDA/AHRQ/BARDA/etc.). Name the plausible solicitation. Decorative or mismatched program scores low.\n'
    ...
    + '- existenceAudit: are all named entities, citations, PA/solicitation numbers, recall numbers, and segment names real/verifiable? Fail if any looks fabricated or unverifiable.\n'
    ...
    + 'TYPOLOGY: type "A" = pathological internal behavior (strongest lanes Research + Investment); type "B" = structural external constraint (strongest lanes Grant + Patent).\n\n'
    + 'DECISION RULE: keep if >=1 lane scores >=6; else reject. Route ONLY to lanes scoring >=6.\n\n'
    + 'Output STRICT JSON only: {"lanes":{"patent":{"score":0,"rationale":""},"grant":{"score":0,"rationale":""},"research":{"score":0,"rationale":""},"investment":{"score":0,"rationale":""}},"gates":{"existenceAudit":{"pass":true,"reason":""},"steelman":{"pass":true,"reason":""},"dataFeasibility":{"pass":true,"reason":""}},"type":"A","verdict":"keep","route":["grant","research"],"topFixes":["",""]}';
