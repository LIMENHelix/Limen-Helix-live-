...
async function reconcilePayments(opts) {
  ...
  for (const o of waiting) {
    ...
    const underpaid = st.amount != null && fresh.total != null && st.amount < fresh.total - 0.005;
    const doublePaid = Array.isArray(st.duplicatePayments) && st.duplicatePayments.length > 1;
    if (underpaid || doublePaid) {
      update.status = 'payment-review';
      update.reviewReason = underpaid
  ...
  let orphans = [];
  try {
    orphans = (await store.ordersByStatus('paid', ORDER_SCAN))
