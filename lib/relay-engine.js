...
async function reconcileCjAttempts(opts) {
  ...
  for (const row of held) {
    ...
    try {
      ...
      if (listing && listing.sourceId) expected = { vid: listing.sourceId };
    } catch (e) { expected = {}; }
    ...
    if (look.found === true) {
      ...
      await autonomy.release(row.id, 'reconciliation: CJ has no order ' + orderNumber + '; the attempt never landed');
      results.push({ decisionId: row.id, orderNumber: orderNumber, released: true });
    } else {
...
async function runCycle(opts) {
  ...
  let cjReconciled = [];
  try {
    cjReconciled = (await reconcileCjAttempts()).results;
  } catch (e) {
    console.error('[relay-engine] CJ attempt reconciliation failed:', e.message);
  }
