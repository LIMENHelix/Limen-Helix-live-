...
async function fulfillLine(order, line, shippingAddress, opts) {
  ...
  // Why an approved reservation could not be used, surfaced so an operator's click that
  // silently fell back to a fresh authorize() is not reported as a plain queue refusal.
  ...
  if (opts && opts.decisionId) {
    ...
    // Another line's approval. Leave this line exactly as it is — still queued, still
    // holding its own reservation — instead of re-authorising it into a duplicate.
    else if (consumed.mismatch) {
      return {
        listingId: line.listingId,
        ok: false,
        state: 'awaiting-approval',
        decisionId: null,
        reason: 'not the approved line; left queued',
        skipped: true
      };
    }
    else consumeRefusal = consumed.reason || null;
  }
  ...
  if (!decision.allowed) {
    ...
    return {
      ...
      reason: consumeRefusal
        ? 'approval not used: ' + consumeRefusal + (decision.reason ? ' (then: ' + decision.reason + ')' : '')
        : decision.reason,
      approvalRefused: consumeRefusal,
      mode: decision.mode,
      taskId: taskId
    };
  }
