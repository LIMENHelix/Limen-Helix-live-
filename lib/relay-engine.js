...
async function reconcilePayments(opts) {
  ...
  let waiting;
  try {
    waiting = await store.ordersByStatus('awaiting-payment', limit);
  } catch (e) {
  ...
  for (const o of waiting) {
    ...
    // Once, and never at the cost of the order. See rules 3 and 4 above.
    let income = null;
    if (!fresh.incomeReportedAt) {
      const lines = Array.isArray(fresh.lines) ? fresh.lines : [];
      const sourceCostTotal = _round(lines.reduce(function (s, l) {
        return s + ((parseFloat(l.sourceCost) || 0) * (parseInt(l.qty, 10) || 1));
      }, 0));
      income = await finance.reportIncome({
        amount: collected,
        orderId: o.id,
        buyerId: fresh.buyerId,
        lineCount: lines.length,
        sourceCostTotal: sourceCostTotal || null,
        margin: sourceCostTotal ? _round(collected - sourceCostTotal) : null,
        source: 'stripe'
      });
      if (income && income.ok) {
        await store.updateOrder(o.id, { incomeReportedAt: new Date().toISOString() });
      }
    }
    ...
    checked.push({
      ...
      incomeRecorded: income ? !!income.recorded : null
    });
  }
  ...
  for (const o of orphans) {
    ...
    const income = await finance.reportIncome({
      ...
      lineCount: lines.length,
      sourceCostTotal: sourceCostTotal || null,
      margin: sourceCostTotal ? _round(o.total - sourceCostTotal) : null,
      source: 'stripe'
    });
    if (income && income.ok) {
      await store.updateOrder(o.id, { incomeReportedAt: new Date().toISOString() });
...
async function runCycle(opts) {
  ...
  // Ask Stripe which of our unpaid orders have actually been paid, THEN sweep. Order
  // matters: an order paid since the last cycle has to become 'paid' before the sweep
  // below looks for 'paid', or it waits a whole extra cycle for no reason.
  let reconciled = [];
  try {
    reconciled = (await reconcilePayments({ limit: 25 })).checked || [];
  } catch (e) {
