...
async function reconcilePayments(opts) {
  ...
  try {
    ...
    waiting = all.sort(function (a, b) {
      const ca = a.lastPaymentCheckAt ? new Date(a.lastPaymentCheckAt).getTime() : 0;
      const cb = b.lastPaymentCheckAt ? new Date(b.lastPaymentCheckAt).getTime() : 0;
  ...
  for (const o of waiting) {
    ...
    // Stamped on every ANSWERED check, so the queue rotates. Not stamped when we could
    // not reach Stripe: an order we failed to ask about has not had its turn.
    if (st.ok) {
      try { await store.updateOrder(o.id, { lastPaymentCheckAt: new Date().toISOString() }); } catch (e) {}
    }
    if (!st.ok) {
    ...
    // EVERY charge Stripe took against this link, not just the first. When a link was
    // paid twice the money is all ours to account for; booking one session leaves the
    // rest missing from net income and lendable surplus while Stripe holds it.
    const collected = Array.isArray(st.duplicatePayments) && st.duplicatePayments.length > 1
      ? _round(st.duplicatePayments.reduce(function (s, d) { return s + (parseFloat(d.amount) || 0); }, 0))
      : (st.amount != null ? st.amount : fresh.total);
    const update = {
      status: 'paid',
    ...
    // into silence, because nothing revisits a settled order's link. Best effort: a
    // settled sale is not unwound because Stripe would not close a link.
    try { await finance.closePaymentLink(o.paymentLinkId); } catch (e) {}
  ...
  try {
    ...
    orphans = paidish
      .filter(function (o) { return !o.incomeReportedAt && o.total > 0; })
      .slice(0, limit);
