...
// Readback against the supplier's own order book, for the reconciliation pass that
// settles or releases reservations whose purchase attempt ended ambiguously.
...
async function discoverAndList(opts) {
  ...
  for (const item of found.items) {
    ...
    const price = require('./relay-pricing-settings').priceFor(item.price, margin, gateCfg);
...
async function fulfillLine(order, line, shippingAddress, opts) {
  ...
  if (!decision) {
    // A line whose previous CJ attempt ended with the answer unheard HOLDS its
    // reservation (relay-buy marks it reconciliation-pending). Re-authorising here would
    // take a second reservation over money whose fate is unknown, and the fresh row
    // would carry no attempt marker — so the retry would place the order AGAIN. Spend
    // the held row instead; relay-buy reconciles it against CJ before placing anything.
    let held = null;
    try { held = await autonomy.findHeldLineReservation(order.id, listing.id); } catch (e) { held = null; }
    if (held) {
      decision = { allowed: true, decisionId: held.id, mode: held.mode || null, reusedHeldReservation: true };
    }
  }
  ...
  if (result.ok) {
    const settled = await autonomy.settle(decision.decisionId, {
      amount: result.amount,
      sourceOrderId: result.sourceOrderId
    });
    if (!settled || !settled.ok) {
      // The provider purchase is irreversible, but reusable budget headroom would be
      // worse. Keep the line out of the purchased state and let supplier readback repair
      // it after the ledger is reachable; never release this reservation here.
      return {
        listingId: line.listingId,
        ok: false,
        state: 'failed',
        ...
        sourceOrderId: result.sourceOrderId,
        error: 'supplier purchase confirmed but spend settlement failed: ' +
          ((settled && (settled.error || settled.reason)) || 'unknown ledger failure')
      };
    }
    return {
      listingId: line.listingId,
  ...
  // Could not spend — OR CANNOT TELL. relay-buy flags reconciliationPending when the
  // supplier's answer was lost: the wallet may already be charged, so the reservation
  // stays HELD (not released, not settled) until the reconciliation pass reads CJ's own
  // order book. The line stays 'failed' so the sweep keeps revisiting it; the retry
  // reuses the held reservation rather than taking a second one.
  ...
  // Give the ceiling headroom back so a blocked line does not
...
// How many stuck CJ reservations one cycle may reconcile, and how old an unmarked
// attempt must be before the pass asks CJ about it (longer than any purchase can still
// be in flight; relay-autonomy's RESERVATION_INFLIGHT_MS is the same bound).
const CJ_RECONCILE_BATCH = 5;

/**
 * Reconcile reservations whose CJ attempt ended ambiguously. Bounded, idempotent, and
 * safe to run every cycle:
 *   exact match        settle the reservation and repair the order's fulfilment line.
 *   definitive absence the create genuinely never landed; release the headroom.
 *   ambiguous          leave it held. 'Provider outcome unknown' never becomes 'money
 *                      not spent'.
 */
async function reconcileCjAttempts(opts) {
  opts = opts || {};
  const results = [];
  let held;
  try {
    held = await autonomy.listHeldCjAttempts({
      olderThanMs: opts.olderThanMs,
      limit: opts.limit || CJ_RECONCILE_BATCH
    });
  } catch (e) {
    return { ok: false, error: e.message, results: results };
  }

  for (const row of held) {
    const orderNumber = row.cjOrderNumber ||
      (row.orderId && row.listingId ? row.orderId + '-' + row.listingId : row.orderId);
    if (!orderNumber) continue;

    // The expected line, so a bound order carrying a different variant cannot be
    // accepted as this purchase.
    let expected = {};
    try {
      const listing = row.listingId ? await store.getListing(row.listingId) : null;
      const order = row.orderId ? await store.getOrder(row.orderId) : null;
      const line = order && Array.isArray(order.lines)
        ? order.lines.find(function (item) { return item && item.listingId === row.listingId; })
        : null;
      if (listing && listing.sourceId) {
        expected = {
          vid: listing.sourceId,
          quantity: Math.max(1, parseInt(row.quantity != null ? row.quantity : (line && line.qty), 10) || 1)
        };
      }
    } catch (e) { expected = {}; }
    let look;
    try {
      look = typeof cj.getOrderDetail === 'function'
        ? await cj.getOrderDetail(orderNumber, expected)
        : { found: 'ambiguous', error: 'readback unavailable in this build' };
    } catch (e) {
      look = { found: 'ambiguous', error: e.message };
    }

    if (look.found === true && !expected.vid) {
      results.push({
        decisionId: row.id,
        orderNumber: orderNumber,
        held: true,
        reason: 'CJ order exists but local listing identity is unavailable; refusing unbound reconciliation'
      });
    } else if (look.found === true) {
      const settled = await autonomy.settle(row.id, {
        amount: look.amountUsd != null ? look.amountUsd : row.amount,
        sourceOrderId: look.cjOrderId
      });
      if (!settled || !settled.ok) {
        results.push({
          decisionId: row.id,
          ...
          held: true,
          reason: (settled && (settled.error || settled.reason)) || 'ledger settlement failed'
        });
        continue;
      }
      let repairError = null;
      try { await _repairReconciledLine(row, look); } catch (e) {
        repairError = e.message;
        console.error('[relay-engine] reconciliation repair failed:', e.message);
      }
      results.push({
        decisionId: row.id,
        orderNumber: orderNumber,
        settled: true,
        cjOrderId: look.cjOrderId || null,
        repairFailed: !!repairError,
        repairError: repairError
      });
    } else if (look.found === false) {
      try {
        await _markAbsentLineRetryable(row);
      } catch (e) {
        // Keep the reservation held until the order state can be made retryable. Releasing
        // first creates a one-way crash window: the row disappears from reconciliation
        // while the partial order still says reconciliationPending forever.
        results.push({
          decisionId: row.id,
          orderNumber: orderNumber,
          held: true,
          orderRepairFailed: true,
          reason: 'definitive absence observed but retry state could not be persisted: ' + e.message
        });
        continue;
      }
      const released = await autonomy.release(row.id, 'reconciliation: CJ has no order ' + orderNumber + '; the attempt never landed');
      if (!released || !released.ok) {
        // A concurrent exact-match pass may have settled between the absence read and our
        // release. The local line was already made retryable above, so reconcile it back
        // to purchased from the fresh terminal row before any paid sweep can run. If the
        // ledger itself cannot be read, restore the unknown-outcome hold: stranded is
        // visible and recoverable; a duplicate supplier order is not.
        let current = null;
        try { current = await autonomy.getDecision(row.id); } catch (e) { current = null; }
        if (current && current.state === 'settled' && current.sourceOrderId) {
          await _repairReconciledLine(current, {
            cjOrderId: current.sourceOrderId,
            amountUsd: current.amount
          });
        } else if (!current || current.state !== 'released') {
          await _markLineOutcomeUnknown(row,
            'ledger release did not commit after CJ absence; outcome remains inhibited');
        }
      }
      results.push({
        decisionId: row.id,
        orderNumber: orderNumber,
        released: !!(released && released.ok),
        releaseFailed: !(released && released.ok),
        reason: released && !released.ok ? (released.error || released.reason || 'ledger release failed') : null
      });
    } else {
      results.push({ decisionId: row.id, orderNumber: orderNumber, held: true, reason: look.error || 'ambiguous' });
    }
  }
  return { ok: true, results: results };
}

/** Bring the order's fulfilment record in line with a purchase reconciliation confirmed. */
async function _repairReconciledLine(row, look) {
  if (!row.orderId) return;
  const order = await store.getOrder(row.orderId);
  if (!order || !order.fulfillment || !Array.isArray(order.fulfillment.lines)) return;
  let changed = false;
  const lines = order.fulfillment.lines.map(function (l) {
    if (!l || l.listingId !== row.listingId || l.state === 'purchased') return l;
    changed = true;
    return Object.assign({}, l, {
      ...
      decisionId: row.id,
      provider: 'cj',
      sourceOrderId: look.cjOrderId || null,
      actualCost: look.amountUsd != null ? look.amountUsd : row.amount,
      reconciled: true
    });
  });
  if (!changed) return;
  const purchasedCount = lines.filter(function (l) { return l && l.state === 'purchased'; }).length;
  const update = {
    fulfillment: Object.assign({}, order.fulfillment, {
      lines: lines,
      state: purchasedCount === lines.length ? 'purchased'
           : purchasedCount > 0 ? 'partial'
           : order.fulfillment.state
    })
  };
  if (purchasedCount === lines.length && order.status === 'paid') update.status = 'shipped';
  await store.updateOrder(order.id, update);
}

/** A provider-confirmed absence makes this line safe to attempt again. */
async function _markAbsentLineRetryable(row) {
  if (!row.orderId) return;
  const order = await store.getOrder(row.orderId);
  if (!order || !order.fulfillment || !Array.isArray(order.fulfillment.lines)) return;
  let changed = false;
  const lines = order.fulfillment.lines.map(function (line) {
    if (!line || line.listingId !== row.listingId || line.state === 'purchased') return line;
    changed = true;
    return Object.assign({}, line, {
      state: 'failed',
      transient: true,
      reconciliationPending: false,
      error: 'CJ confirmed the prior attempt did not create an order; retry is permitted'
    });
  });
  if (!changed) return;
  const purchasedCount = lines.filter(function (line) { return line && line.state === 'purchased'; }).length;
  await store.updateOrder(order.id, {
    fulfillment: Object.assign({}, order.fulfillment, {
      lines: lines,
      state: purchasedCount > 0 ? 'partial' : 'failed',
      reason: 'provider confirmed an absent line; retry is permitted'
    })
  });
}

/** Restore the anti-duplicate hold when a terminal ledger transition is uncertain. */
async function _markLineOutcomeUnknown(row, reason) {
  if (!row.orderId) return;
  const order = await store.getOrder(row.orderId);
  if (!order || !order.fulfillment || !Array.isArray(order.fulfillment.lines)) return;
  let changed = false;
...
async function runCycle(opts) {
  ...
  // Supplier readback is also on the non-spending side of the gate. Turning autonomy off
  // must stop NEW purchases, not stop learning whether CJ already accepted one whose
  // response was lost. This pass may settle/release our ledger and repair our order, but
  // it never creates a supplier order.
  let cjReconciliation = { ok: true, results: [] };
  try {
    cjReconciliation = await reconcileCjAttempts();
  } catch (e) {
    cjReconciliation = { ok: false, error: e.message, results: [] };
  }
  ...
  if (cfg.mode === 'off' && !opts.force) {
    return {
      ...
      cjAttemptsReconciled: cjReconciliation.results,
      cjReconciliationOk: cjReconciliation.ok === true,
      cjReconciliationError: cjReconciliation.ok === true ? null : (cjReconciliation.error || 'unknown')
    };
  }
  ...
  try {
    ...
    const pendingOrders = orders.filter(function (o) {
      if (o.status !== 'paid' || !o.fulfillment) return o.status === 'paid';
      if (o.fulfillment.state === 'failed') return true;
      // A cart can be partial after one line succeeded and another ambiguous CJ attempt
      // was later proven absent. Retry only definite failed lines; manual and approval
      // holds remain outside this automatic sweep.
      return o.fulfillment.state === 'partial' && Array.isArray(o.fulfillment.lines) &&
        o.fulfillment.lines.some(function (line) {
          return line && line.state === 'failed' && line.reconciliationPending !== true;
        });
    }).slice(0, 5);
  ...
  // Reservations whose CJ attempt ended ambiguously stay held on purpose (relay-buy
  // refuses to release money whose fate is unknown). This is the pass that resolves
  // them from the supplier's own records — bounded, and idempotent across cycles.
  const cjReconciled = cjReconciliation.results;
  ...
  const report = {
    ...
    ordersSwept: swept,
    cjAttemptsReconciled: cjReconciled,
    cjReconciliationOk: cjReconciliation.ok === true,
    cjReconciliationError: cjReconciliation.ok === true ? null : (cjReconciliation.error || 'unknown')
  };
