...
async function reconcilePayments(opts) {
  ...
  for (const o of waiting) {
    ...
    // Stamped on EVERY attempt, answered or not. Stamping only on success meant an order
    // whose link Stripe consistently refuses to answer for — a deleted link, a permanent
    // 404 — was never stamped, sorted to the front of every cycle and blocked the rotation
    // for everyone behind it. A failure still gets its turn; it just stops getting every
    // turn. The failure itself is reported in `checked`, so it is visible rather than
    // buried by the rotation.
  ...
  try {
    const unclosed = (await store.ordersByStatus('paid', ORDER_SCAN))
      .concat(await store.ordersByStatus('payment-review', ORDER_SCAN))
      .filter(function (o) { return o.paymentLinkId && !o.paymentLinkClosedAt; })
      .slice(0, limit);
    for (const o of unclosed) {
      const closed = await finance.closePaymentLink(o.paymentLinkId);
      if (closed.ok) await store.updateOrder(o.id, { paymentLinkClosedAt: new Date().toISOString(), paymentLinkCloseError: null });
    }
