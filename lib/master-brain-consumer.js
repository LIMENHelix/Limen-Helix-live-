...
function buildInbox(corpusPortalsIterable, options) {
  options = options || {};
  const all = [];
  ...
  // Keep the cross-lane top 25 as a presentation surface. The autonomous
  // feeder uses the complete active-lane pool below; otherwise every ready
  // item below rank 25 would remain stranded forever.
  const readyForAutofire = all.filter(x => x.status === 'READY_TO_FIRE').sort((a, b) => b.fireScore - a.fireScore);
  const topPriority = readyForAutofire.slice(0, 25);
