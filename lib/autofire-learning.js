...
var INVESTMENT_HORIZONS = [1, 3, 7, 14, 30, 60, 90];
...
function fresh(domain, lane) {
  return {
    ...
    commands: [],
    investmentCohorts: { '1': [], '3': [], '7': [], '14': [], '30': [], '60': [], '90': [] },
    investmentCohortSeen: { '1': [], '3': [], '7': [], '14': [], '30': [], '60': [], '90': [] },
    companyPatterns: {},
...
async function load(store, domain) {
  ...
  if (!value.companyPatterns || typeof value.companyPatterns !== 'object') value.companyPatterns = {};
...
async function save(store, state) {
  // This is the owning domain's consolidated learning state. Individual
  // observations can be bounded inside it, but the learned model itself has
  // no wall-clock expiry.
  await store.set(stateKey(state.domain), state);
  return state;
}
  ...
  try {
    ...
    ep.procedureContext = {
      ...
      expectedResult: spec.selection.lane === 'investment'
        ? 'a persisted investment artifact followed by graded 1/3/7/14-day regulation and 30/60/90-day consolidation outcomes'
    ...
    var command = {
      ...
      subjectId: spec.selection.candidate.subjectId || null,
      ticker: spec.selection.candidate.ticker || null,
      decisionTrace: {
        status: spec.selection.status,
        reasons: Array.isArray(spec.selection.reasons) ? spec.selection.reasons.slice(0, 16) : [],
        sourceArtifactRef: spec.selection.candidate.sourceArtifactRef || null,
        sourcePatternSig: spec.selection.candidate.sourcePatternSig || null
      },
      emittedAt: at
    };
...
async function recordProductInvestmentCommand(store, spec) {
  spec = spec || {};
  var domain = String(spec.domain || '').toLowerCase();
  var candidate = spec.candidate || {};
  var decision = spec.decision || {};
  var command = spec.command || {};
  if (['economy', 'energy', 'technology'].indexOf(domain) < 0) {
    return { ok: false, error: 'unsupported_product_investment_domain' };
  }
  if (candidate.ownerDomain !== domain || decision.ownerDomain !== domain || command.ownerDomain !== domain ||
      decision.status !== 'RELEASED' || decision.actionId !== command.actionId ||
      decision.decisionReceiptId !== command.decisionReceiptId) {
    return { ok: false, error: 'product_investment_causal_identity_mismatch' };
  }
  return recordCommand(store, {
    selection: {
      id: decision.decisionReceiptId,
      status: 'RELEASED',
      ownerDomain: domain,
      lane: 'investment',
      command: 'submit_' + domain + '_investment_order',
      reasons: Array.isArray(decision.blockers) ? decision.blockers.slice(0, 16) : [],
      candidate: {
        cik: candidate.cik || null,
        ticker: candidate.symbol || null,
        sourceArtifactRef: candidate.requestId || null,
        sourcePatternSig: candidate.evidenceHash || null
      }
    },
    efferenceCopy: {
      id: domain + '_investment_efference:' + decision.actionId,
      actionId: decision.actionId,
      traceId: decision.decisionReceiptId,
      emittedAt: Number(spec.emittedAt) || Number(command.commandedAt) || Date.now()
    }
  });
}
...
function gradeInvestment(event) {
  ...
  var missing = [];
  if (INVESTMENT_HORIZONS.indexOf(d.horizonDays) < 0) missing.push('horizonDays_must_be_multiscale_investment_horizon');
...
function updateCompanyPattern(state, command, event, assessment) {
  if (!assessment || !assessment.graded || event.lane !== 'investment' || !command) return null;
  var companyId = command.cik ? 'cik:' + String(command.cik)
    : command.ticker ? 'ticker:' + String(command.ticker).toUpperCase() : null;
  if (!companyId) return null;
  var terms = assessment.terms || {};
  var pattern = state.companyPatterns[companyId] || {
    schemaVersion: 'investment-company-pattern/1.0',
    ownerDomain: state.domain,
    companyId: companyId,
    cik: command.cik || null,
    ticker: command.ticker || null,
    resolvedCount: 0,
    outcomes: { SUCCESS: 0, FAILURE: 0, NEUTRAL: 0 },
    horizons: { '1': 0, '3': 0, '7': 0, '14': 0, '30': 0, '60': 0, '90': 0 },
    totals: { netPnl: 0, returnPct: 0, benchmarkReturnPct: 0, excessReturnPct: 0, maxDrawdownPct: 0 },
    failureModes: { riskBreach: 0, absoluteLoss: 0, benchmarkUnderperformance: 0 },
    episodes: [],
    recommendation: { status: 'INSUFFICIENT_RESOLVED_HISTORY', advisoryOnly: true, minimumResolved: 3 }
  };
  pattern.resolvedCount++;
  pattern.outcomes[assessment.outcome] = Number(pattern.outcomes[assessment.outcome] || 0) + 1;
  pattern.horizons[String(terms.horizonDays)] = Number(pattern.horizons[String(terms.horizonDays)] || 0) + 1;
  ['netPnl', 'returnPct', 'benchmarkReturnPct', 'excessReturnPct'].forEach(function (name) {
    pattern.totals[name] = Number(pattern.totals[name] || 0) + Number(terms[name] || 0);
  });
  pattern.totals.maxDrawdownPct = Number(pattern.totals.maxDrawdownPct || 0) + Math.abs(Number(terms.maxDrawdownPct || 0));
  if (terms.riskBreach === true) pattern.failureModes.riskBreach++;
  if (Number(terms.netPnl) < 0) pattern.failureModes.absoluteLoss++;
  if (Number(terms.excessReturnPct) < 0) pattern.failureModes.benchmarkUnderperformance++;
  pattern.episodes.push({
    eventId: event.eventId,
    actionId: event.actionId,
    observedAt: event.ts,
    horizonDays: terms.horizonDays,
    outcome: assessment.outcome,
    reward: assessment.reward,
    netPnl: terms.netPnl,
    returnPct: terms.returnPct,
    benchmarkReturnPct: terms.benchmarkReturnPct,
    excessReturnPct: terms.excessReturnPct,
    maxDrawdownPct: terms.maxDrawdownPct,
    riskBreach: terms.riskBreach,
    decisionReasons: command.decisionTrace && command.decisionTrace.reasons || []
  });
  pattern.episodes = trim(pattern.episodes, 90);
  var n = pattern.resolvedCount;
  var mean = function (name) { return Number(pattern.totals[name] || 0) / n; };
  pattern.means = {
    netPnl: mean('netPnl'),
    returnPct: mean('returnPct'),
    benchmarkReturnPct: mean('benchmarkReturnPct'),
    excessReturnPct: mean('excessReturnPct'),
    maxDrawdownPct: mean('maxDrawdownPct')
  };
  var status = 'INSUFFICIENT_RESOLVED_HISTORY';
  if (n >= 3 && pattern.failureModes.riskBreach > 0) status = 'TIGHTEN_RISK_LIMIT_OR_ABSTAIN';
  else if (n >= 3 && pattern.failureModes.benchmarkUnderperformance > pattern.outcomes.SUCCESS) status = 'REQUIRE_STRONGER_BENCHMARK_EDGE_OR_ABSTAIN';
  else if (n >= 3 && pattern.outcomes.FAILURE > pattern.outcomes.SUCCESS) status = 'REQUIRE_STRONGER_COMPANY_EVIDENCE_OR_ABSTAIN';
  else if (n >= 3 && pattern.outcomes.SUCCESS > pattern.outcomes.FAILURE && pattern.means.excessReturnPct > 0) status = 'PRESERVE_CURRENT_ENTRY_POLICY';
  else if (n >= 3) status = 'HOLD_POLICY_PENDING_MORE_EVIDENCE';
  pattern.recommendation = {
    status: status,
    advisoryOnly: true,
    minimumResolved: 3,
    reason: 'derived only from this company\'s independently resolved investment outcomes; never authorizes a trade'
  };
  pattern.lastResolvedAt = event.ts;
  state.companyPatterns[companyId] = pattern;
  return pattern;
}
...
async function processOutcome(store, event) {
  ...
  /* One command may legitimately have several later observations: publication,
     then an evaluated research result, or multiscale investment results.
  ...
  outcomeEpisode.procedureContext = {
    ...
    requiredEvidence: [
      'owning domain current L3 evidence complete',
      'declared outward consumer',
      event.lane === 'investment' ? 'graded 1/3/7/14/30/60/90-day investment terms' : 'independently assessed research evidence with all four mappings'
    ],
    contraindications: [],
  ...
  var companyPattern = updateCompanyPattern(state, command, event, assessment);
  state.memory = MEM.serialize(mem);
...
module.exports = {
  ...
  updateCompanyPattern: updateCompanyPattern,
  recordCommand: recordCommand,
  recordProductInvestmentCommand: recordProductInvestmentCommand,
