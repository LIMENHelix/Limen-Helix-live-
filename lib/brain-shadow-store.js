...
/**
 * ATOMIC create-if-absent, and the ONLY way a chunk is written. Returns { created, bytes }.
 *
 * An unconditional `writeArchiveChunk` used to sit here. Nothing called it any more once
 * creation became atomic, but an exported plain SET over an archive key is a live bypass of
 * the write-once guarantee, not dead weight: the next caller to reach for the obvious-looking
 * name would silently reintroduce the race this design exists to prevent. It is gone, and
 * S-archive-surface in brain-v2/test/shadow-runtime.js fails if it comes back.
 *
 * Write-once means two workers racing one sequence must produce one winner and one explicit
 * loser, never two writes where the last one silently wins.
 */
