...
/** Religion-authorized compensating recovery for irreversible email delivery. */
...
var Observer = require('./religion-subscriber-outcome-observer.js');
var SCHEMA = 'religion-subscriber-recovery/1.0';
var LOG_KEY = 'religion_subscriber_recovery_log', PREFIX = 'religion_subscriber_recovery:';
...
async function recover(input) {
  ...
  var now = Number(input.now) || Date.now(), authorization = await (input.motorAuthorization || MotorAuthorization).authorize(store, 'religion', 'subscriber-email', now);
  if (!authorization || !authorization.authorized) return { ok: true, status: 'HELD', reason: authorization && authorization.reason || 'religion-subscriber-recovery-motor-held', liveMoney: false };
  var recoveryId = 'rsr_' + hash({ actionId: item.actionId, observation: observation && observation.observationId || null,
  ...
  var recovery = { schemaVersion: SCHEMA, recoveryId: recoveryId, status: 'SUPPRESSING', commandId: command.commandId,
    ...
    productDomain: 'religion', ownerDomain: 'religion', lane: 'subscriber-email', productMotorReceiptId: authorization.receiptId,
  ...
  if (!(await store.setIfAbsent(Executor.motorClaimKey(authorization.receiptId), claim))) { recovery.status = 'REFUSED'; recovery.reason = 'religion-subscriber-motor-receipt-already-consumed'; await store.set(key(recoveryId), recovery); return recovery; }
  var restoredClaim = await store.get(Executor.motorClaimKey(authorization.receiptId));
  if (!restoredClaim || restoredClaim.recoveryId !== recoveryId) throw new Error('religion subscriber recovery motor claim readback invalid');
  ...
  if (!restoredCatalog || !restoredCatalog[item.emailHash] || restoredCatalog[item.emailHash].recoveryId !== recoveryId) throw new Error('religion subscriber suppression readback invalid');
  ...
  if (!restored || restored.status !== recovery.status || restored.recoveryId !== recoveryId) throw new Error('religion subscriber recovery readback invalid');
