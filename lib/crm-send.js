...
function emailConfig() {
  ...
  else if (/resend\.dev/i.test(from)) reason = 'from is a resend.dev sandbox — verify your own domain';
  else if (UNVERIFIABLE.test(from)) reason = 'from is a free-mail domain (e.g. gmail) — Resend cannot send from it; use an address on a domain you verified in Resend';
  var sandbox = !from || /resend\.dev/i.test(from) || UNVERIFIABLE.test(from);
  return { hasKey: !!apiKey, from: from, replyTo: replyTo, addr: addr, sandbox: sandbox, ready: !!apiKey && !sandbox, reason: reason, apiKey: apiKey };
}
...
async function sendToLead(toEmail, subject, bodyText) {
  if (!validEmail(toEmail)) return { ok: false, error: 'no valid email' };
  var cfg = emailConfig();
  if (!cfg.hasKey) return { ok: false, error: 'RESEND_API_KEY not set', notReady: true };
  if (cfg.sandbox) return { ok: false, error: 'RESEND_FROM_EMAIL unset/sandbox — verify a domain', notReady: true };
  if (await isSuppressed(toEmail)) return { ok: false, error: 'suppressed', suppressed: true };
  var sr = await sendEmailResend(cfg, toEmail, subject, bodyText);
  return sr.ok ? { ok: true, id: sr.id, warning: cfg.addr ? null : 'no CRM_SENDER_ADDRESS (CAN-SPAM)' } : { ok: false, error: sr.error, status: sr.status };
}
