...
function ohOwnerFromAttrs(a, cfg, deal, matchMode) {
  ...
  // STATE (catches out-of-state owners even when the scraped property address has no house number)
  var absentee = !!((propHN && mailHN && propHN !== mailHN) || (state && dst && state.toUpperCase() !== dst));
...
var TX_COUNTIES = {
  'Travis':        { url: 'https://services1.arcgis.com/HGcSYZ5bvjRswoCb/arcgis/rest/services/TCAD_Parcels_Dec_2025/FeatureServer/0',
                     pf: 'geo_id', own: ['py_owner_name'], st: ['py_address'], val: 'market_value',
