...
function scoreDeal(deal, sig) {
  ...
  // OWNER-OCCUPANT lane = a living homeowner occupying their home in foreclosure (reachable AT the
  // property; playbook = fast sale / sale-leaseback before the sale date). Gate hard: a non-absentee
  // owner that is actually an ESTATE (probate) / TRUST / CORPORATION is NOT an owner-occupant — those
  // carry their own stronger lane, so don't double-count or mislabel them here.
  if (sig.ownerOccupied && !sig.probate && !sig.trust && !sig.corporate) {
    var occ = 16 + Math.min(24, Math.max(0, Math.round(effEquity / 12000)));
    score += occ; reasons.push('OWNER-OCCUPANT');
  } else if (sig.ownerOccupied) {
...
/* ---- servicer / tax-lockbox mailing detection ----
   A mailing address shared by many DIFFERENT owners is a loan-servicer / tax-escrow lockbox
   (CoreLogic Coppell, LERETA Irving, etc), not the seller's door — a letter there never reaches
   a decision-maker. We down-weight these so the mail-first work list surfaces reachable humans
   (owner-occupant + individual absentee), not escrow processors. Frequency is the main signal
   (self-updating); a small curated seed catches the confirmed national lockboxes on sight. */
var SERVICER_MAIL = [
  /P\.?\s?O\.?\s?BOX\s*9202\b[\s\S]*COPPELL/i,        // CoreLogic tax service (Coppell TX)
  /\b3001\s+HACKBERRY\b[\s\S]*IRVING/i,               // LERETA tax service (Irving TX)
  /\b901\s+CORPORATE\s+CENTER\s+DR[\s\S]*POMONA/i,    // LERETA HQ (Pomona CA 91768)
  /\b1\s+CORELOGIC\s+(DR|DRIVE|WAY)/i,                // CoreLogic (Westlake TX)
  /\b2727\s+L\.?\s?B\.?\s?J\.?\b/i,                   // Dallas registered-agent tower (CSC/CT Corp, 75234)
  /\b2727\s+ALLEN\s+P(KWY|ARKWAY)/i,                  // Nationstar/Mr Cooper (Houston)
  /\bLERETA\b/i, /\bCORELOGIC\b/i, /\bTAX\s+SERVIC/i, /\bLOAN\s+SERVIC/i, /\bESCROW\s+DEPT/i,
  /\bC[\/\s]?O\s+(CT\s+CORP|CORPORATION\s+SERVICE|COGENCY|REGISTERED\s+AGENT)/i
];
function mailKey(o) {
  if (!o || !o.mailAddr) return '';
  return (String(o.mailAddr) + '|' + String(o.mailCity || '') + '|' + String(o.mailState || ''))
    .toUpperCase().replace(/\s+/g, ' ').trim();
}
function isCuratedServicer(o) {
  if (!o || !o.mailAddr) return false;
  var full = [o.mailAddr, o.mailCity, o.mailState, o.mailZip].filter(Boolean).join(' ');
  return SERVICER_MAIL.some(function (re) { return re.test(full); });
}
// mail keys that behave like a lockbox across a batch: same address shared by >= minOwners
// DISTINCT owner names (a landlord mailing many of their OWN parcels to one address stays
// reachable, so we key on distinct owners, not deal count).
function servicerMailSet(deals, minOwners) {
  minOwners = minOwners || 3;
  var byKey = {};
  (deals || []).forEach(function (d) {
    var o = d.owner; if (!o || !o.mailAddr) return;
    var k = mailKey(o); if (!k) return;
    (byKey[k] = byKey[k] || {})[(o.name || '').toUpperCase().trim()] = 1;
  });
  var set = {};
  Object.keys(byKey).forEach(function (k) { if (Object.keys(byKey[k]).length >= minOwners) set[k] = 1; });
  return set;
}
function isServicerMail(o, servicerSet) {
  if (!o || !o.mailAddr) return false;
  return isCuratedServicer(o) || !!(servicerSet && servicerSet[mailKey(o)]);
}
// down-weight a servicer-mailed deal: unreachable by mail, so it can't be work-first. Cap at WARM
// (still a real distressed property — pursuable at auction / after skip-trace) but off the mail list.
function servicerTier(deal, servicerSet) {
  if (!deal || !isServicerMail(deal.owner, servicerSet)) return null;
  if ((deal.tier || 9) < 3) return { tier: 3, tierLabel: 'WARM', add: 'SERVICER-MAIL' };
  return { tier: deal.tier, tierLabel: deal.tierLabel, add: 'SERVICER-MAIL' };
}
...
/* ===== FL per-county property-appraiser sources (verified live 2026-07-06) =====
   Most FL counties enrich fine via the statewide cadastral (PARCEL_ID). These few key their
   auction/tax parcel on the county's OWN account number, which doesn't match the statewide STRAP,
   so we hit the county PA layer directly (it keys on the exact id the auction feed scrapes). Value
   comes from the scrape's appraised value, so a county with no clean value field is still fine.
   keys: t:'int' fields query unquoted (and only when the id is short enough to be an account). */
var FL_COUNTIES = {
  'St. Lucie':   { url: 'https://map.paslc.gov/arcgis/rest/services/PROD/SLCPA_PublicParcels/MapServer/0',
                   keys: [{ f: 'AccountNumber', t: 'int' }, { f: 'PARCELNO', t: 'str' }], own: ['Owners'], st: ['MailingAddress1'], csz: 'MailingCityStateZip', val: 'JustMarketValue' },
  'Brevard':     { url: 'https://gis.brevardfl.gov/gissrv/rest/services/Base_Map/Parcel_New_WKID2881/MapServer/5',
                   keys: [{ f: 'TaxAcct', t: 'int' }], own: ['OWNER_NAME1'], struct: { street: 'OWNER_STREET_NAME', city: 'OWNER_CITY', state: 'OWNER_STATE', zip: 'OWNER_ZIP5' }, val: null },
  'Pinellas':    { url: 'https://egis.pinellas.gov/gis/rest/services/PublicWebGIS/Parcels/MapServer/1',
                   keys: [{ f: 'STRAP', t: 'str' }], own: ['OWNER1'], struct: { street: 'OWNADD_1', city: 'OWNCITY', state: 'OWNSTATE', zip: 'OWNZIP' }, val: null },
  'Marion':      { url: 'https://gis.ocalafl.org/arcgis/rest/services/Public/Parcels/FeatureServer/0',
                   keys: [{ f: 'alt_key', t: 'int' }], own: ['name'], st: ['add_1'], csz: 'add_2', val: 'tot_val' },
  'Alachua':     { url: 'https://services.arcgis.com/cNo3jpluyt69V8Ek/arcgis/rest/services/PublicParcel/FeatureServer/0',
                   keys: [{ f: 'Name', t: 'str' }, { f: 'Prop_ID', t: 'int' }], own: ['Owner_Mail_Name'], struct: { street: 'Owner_Mail_Addr1', city: 'Owner_Mail_City', state: 'Owner_Mail_State', zip: 'Owner_Mail_Zip' }, val: null },
  'Santa Rosa':  { url: 'https://services.arcgis.com/Eg4L1xEv2R3abuQd/arcgis/rest/services/ParcelsOpenData/FeatureServer/0',
                   keys: [{ f: 'ParcelDisp', t: 'str' }, { f: 'PAR_NUM', t: 'str' }], own: ['OwnerName'], struct: { street: 'Addr2', city: 'City', state: 'State', zip: 'Zip5' }, val: null },
  'Indian River': { url: 'https://gisportal.ircgov.com/server3/rest/services/IRCPA/Parcels_MS/MapServer/0',
                   keys: [{ f: 'NEW_PROP_ID', t: 'int' }, { f: 'PP_PIN', t: 'str' }], own: ['OWNER_NAME'], struct: { street: 'OWN_ADDR1', city: 'OWN_CITY', state: 'OWN_STATE', zip: 'OWN_ZIP' }, val: 'CAMA_VALUE' }
};
async function flCountyLookup(parcel, deal, cfg) {
  var raw = String(parcel || '').trim(), digits = raw.replace(/[^0-9]/g, ''), clauses = [];
  cfg.keys.forEach(function (k) {
    if (k.t === 'int') { if (digits && digits.length <= 9) clauses.push(k.f + '=' + digits.replace(/^0+/, '')); }
    else { clauses.push(k.f + "='" + raw.replace(/'/g, "''") + "'"); if (digits && digits !== raw) clauses.push(k.f + "='" + digits + "'"); }
  });
  if (!clauses.length) return { status: 'no-parcel' };
  var url = cfg.url + '/query?where=' + encodeURIComponent(clauses.join(' OR ')) + '&outFields=*&returnGeometry=false&f=json';
  try {
    var j = await ohFetchJson(url);
    var a = j && j.features && j.features[0] && j.features[0].attributes;
    if (!a) return { status: 'no-match' };
    return ohOwnerFromAttrs(a, cfg, deal, 'fl-' + String(deal.county || '').toLowerCase().replace(/[^a-z]/g, ''));
  } catch (e) { return { status: 'error', error: String(e && e.message || e) }; }
}

async function ownerForFL(parcel, deal) {
  // county PA source first for the mapped counties (keys on the exact scraped account id)
  var fcfg = deal && deal.county && FL_COUNTIES[deal.county];
  if (fcfg) { var fr = await flCountyLookup(parcel, deal, fcfg); if (fr && fr.owner) return fr; }
  var pid = String(parcel || '').replace(/[^A-Za-z0-9]/g, '').toUpperCase();
  if (!pid) return { status: 'no-parcel' };
  var out = '&outFields=' + encodeURIComponent(CAD_FIELDS) + '&returnGeometry=false&f=json';
  try {
    // 1) exact PARCEL_ID
    var j = await (await fetch(FL_BASE + '?where=' + encodeURIComponent("PARCEL_ID='" + pid + "'") + out, { headers: { accept: 'application/json' } })).json();
    var a = j && j.features && j.features[0] && j.features[0].attributes;
    if (a) return ownerFromAttrs(a, 'parcel');
    // 2) prefix LIKE — several counties' cadastral PARCEL_ID appends a check char the RealAuction id
    //    lacks (Duval "0131281170" -> "0131281170R"). Only for ids long enough to be unambiguous;
    //    if the prefix hits multiple parcels, disambiguate by house # so we never grab a neighbor.
    if (pid.length >= 8) {
      var lj = await (await fetch(FL_BASE + '?where=' + encodeURIComponent("PARCEL_ID LIKE '" + pid + "%'") + out.replace('&returnGeometry', '&resultRecordCount=25&returnGeometry'), { headers: { accept: 'application/json' } })).json();
      var fs = (lj && lj.features) || [];
      if (fs.length === 1) return ownerFromAttrs(fs[0].attributes, 'parcel-like');
      if (fs.length > 1) {
        var want = houseNo(deal && (deal.street || deal.address));
        if (want) for (var i = 0; i < fs.length; i++) { if (houseNo(fs[i].attributes.PHY_ADDR1) === want) return ownerFromAttrs(fs[i].attributes, 'parcel-like-hn'); }
      }
    }
...
var OH_COUNTIES = {
  ...
  Hamilton:   { url: 'https://services.arcgis.com/JyZag7oO4NteHGiq/arcgis/rest/services/Open_Data/FeatureServer/51',
                pf: 'PARCELID', own: ['OWNNM1'], st: ['MLADR1'], csz: 'MLADR2', val: 'MKT_TOTAL_VAL', fmt: hamiltonPin },
  ...
  // ---- batch 2: 10 more counties (verified live 2026-07-06) ----
  Lorain:      { url: 'https://maps.lcauditor.com/hosting/rest/services/ParcelFabric/Ownership_Parcels_ISSG/MapServer/1',
                 pf: 'Parcel_Number', own: ['Owner_Contact_Name'],
                 struct: { street: 'Owner_Contact_Address', city: 'Owner_Contact_City', state: 'Owner_Contact_State', zip: 'Owner_Contact_ZipCode' }, val: 'Appraised_Total_100' },
  'Lake OH':   { url: 'https://gis.lakecountyohio.gov/arcgis/rest/services/Auditor/GIS_BOR_Parcel_Publish/MapServer/1',
                 pf: 'PIN', own: ['A_OWNER_NAME'], st: ['G_O_FLADDRESS'], csz: 'G_O_FLCITY', val: 'A_VAL_TOTAL' },
  Medina:      { url: 'https://services5.arcgis.com/m37BbrYBtVXq1nb8/arcgis/rest/services/Medina_Parcel_Layer_version_2/FeatureServer/0',
                 pf: 'IdentificationNumber', own: ['Owner'], st: ['MailingAdd'], val: 'Total_Market_Value' },
  Champaign:   { url: 'https://services5.arcgis.com/HBIN2hfRscrws7eM/arcgis/rest/services/parcel_joined/FeatureServer/0',
                 pf: 'Parcel', own: ['PPOwner'], st: ['PPOwnerAddress'], val: 'PPTotalValue' },
  Greene:      { url: 'https://gis.greenecountyohio.gov/webgis2/rest/services/OpenData/OpenData/MapServer/1',
                 pf: 'Parcel_Number', own: ['Owner_Name'], st: ['Tax_Mailing_Address'], csz: 'Tax_Mailing_City_St_Zip', val: 'Appraised_Total', fmt: greenePin },
  'Delaware OH': { url: 'https://maps.delco-gis.org/arcgiswebadaptor/rest/services/DelawareCountyData/MapServer/0',
                 pf: 'PARCEL_NO', own: ['OWNER1'], st: ['MAILADDR1'], csz: 'MAILADDR2', val: 'MARKET_TOT' },
  Licking:     { url: 'https://gis.lickingcounty.gov/server/rest/services/Auditor/Parcels/MapServer/0',
                 pf: 'Parcel', own: ['OwnerName'], struct: { street: 'OwnerAddress', city: 'OwnerCity', state: 'OwnerState', zip: 'OwnerZip' }, val: 'MarketTotalValue' },
  'Miami OH':  { url: 'https://services3.arcgis.com/wCWf4EGMg4PzHwzA/arcgis/rest/services/parcel_joined/FeatureServer/0',
                 pf: 'PARCEL', own: ['PPOwner'], struct: { street: 'TaxPAddr', city: 'TaxPCity', state: 'TaxPState', zip: 'TaxPZip' }, val: 'PPTotalValue' },
  'Clark OH':  { url: 'https://services3.arcgis.com/VYdZ1CPI3COqRVej/arcgis/rest/services/parcel_joined/FeatureServer/0',
                 pf: 'PIN', own: ['Own1'], st: ['TaxPayerStreet'], csz: 'TaxPayerCityStateZip', val: 'PPTotalValue' },
  'Marion OH': { url: 'https://mcogis.co.marion.oh.us/arcgis/rest/services/Reval2025/MapServer/0',
                 pf: 'PINTEXT', own: ['OWNER'], st: ['ADDRESS1'], csz: 'ADDRESS2', val: 'CURRENT' }
};
...
// Hamilton parcels come off RealAuction as book-plate-parcel (e.g. "520-110-121") but the
// auditor layer keys PARCELID as each segment zero-padded to 4 and concatenated ("052001100121").
function hamiltonPin(parcel) {
  var seg = String(parcel || '').split(/[^0-9]+/).filter(Boolean);
  if (seg.length < 2 || seg.length > 4) return [];
  return [seg.map(function (s) { return s.slice(0, 4).padStart(4, '0'); }).join('')];
}
// Greene auditor keys Parcel_Number dashed as [3-4-4-1-4-2]; the scrape gives it un-dashed
function greenePin(parcel) {
  var s = String(parcel || '').replace(/[^0-9A-Za-z]/g, '');
  if (s.length !== 18) return [];
  return [s.slice(0, 3) + '-' + s.slice(3, 7) + '-' + s.slice(7, 11) + '-' + s.slice(11, 12) + '-' + s.slice(12, 16) + '-' + s.slice(16, 18)];
}
function ohOwnerFromAttrs(a, cfg, deal, matchMode) {
  ...
  var dst = String(deal && deal.state || '').toUpperCase();
  var propZip = cfg.situsZip ? String(a[cfg.situsZip] || '').replace(/[^0-9]/g, '').slice(0, 5) : '';
  var mailZip5 = String(zip || '').slice(0, 5);
  // absentee if the mail street differs from the property street, OR the owner mails to a different
  // STATE, OR to a different ZIP than the property's own situs zip (catches same-state, different-city
  // owners even when the scraped property address has no house number, e.g. Travis "FERGUSON")
  var absentee = !!(
    (propHN && mailHN && propHN !== mailHN) ||
    (state && dst && state.toUpperCase() !== dst) ||
    (propZip && mailZip5 && propZip !== mailZip5)
  );
...
// fetch JSON with one retry — county auditor servers (Butler et al) intermittently time out
// under the cron's concurrency, and a bare failure would drop the deal to the name-only fallback
async function ohFetchJson(url) {
  for (var attempt = 0; attempt < 2; attempt++) {
    try { return await (await fetch(url, { headers: { accept: 'application/json' }, signal: AbortSignal.timeout(18000) })).json(); }
    catch (e) { if (attempt === 1) throw e; await wait(1200); }
  }
}
async function ohCountyLookup(parcel, deal, cfg) {
  var cands = ohParcelCandidates(parcel);
  if (cfg.fmt) cands = cands.concat(cfg.fmt(parcel)); // county-specific parcel-id transform (e.g. Hamilton pad-4)
  cands = cands.filter(function (c, i, arr) { return c && arr.indexOf(c) === i; });
...
async function ownerNameOH(parcel, county) {
  var cands = ohParcelCandidates(parcel);
  county = String(county || '').replace(/\s+(OH|TX|LA|NY)$/i, '').trim(); // ODNR COUNTY has no state suffix
...
/* ===== TEXAS owner lookup (per-county appraisal district ArcGIS; no usable statewide feed) =====
   Like OH, TX has no free statewide cadastral with mailing — each County Appraisal District (CAD)
   publishes owner + mailing on its own ArcGIS layer. Verified live 2026-07-05. Travis (the live
   tax-deed county) keys `geo_id` = first 10 digits of the scraped 14-digit id. Montgomery keys an
   internal integer PIN that the scraped account won't match, so it falls back to a situs-address
   match. Big-metro CADs (Harris/Bexar/Collin/Dallas/Denton) are mapped in owner-enrichment-coverage
   memory, ready to add here once the scraper covers those counties. Tax Code 25.025 blanks a small
   % of owners (judges/LEO/DV) — expected, not a bug. Reuses the OH owner-extraction helpers. */
var TX_COUNTIES = {
  'Travis':        { url: 'https://services1.arcgis.com/HGcSYZ5bvjRswoCb/arcgis/rest/services/TCAD_Parcels_Dec_2025/FeatureServer/0',
                     pf: 'geo_id', own: ['py_owner_name'], st: ['py_address'], val: 'market_value', situsZip: 'situs_zip',
                     fmt: function (p) { var d = String(p || '').replace(/[^0-9A-Za-z]/g, ''); return d.length > 10 ? [d.slice(0, 10)] : []; } },
  'Montgomery TX': { url: 'https://services1.arcgis.com/PRoAPGnMSUqvTrzq/arcgis/rest/services/Tax_Parcel_view/FeatureServer/0',
                     pf: 'PIN', own: ['ownerName'], st: ['ownerAddress'], val: null, situs: 'situs' }
};
async function txCountyLookup(parcel, deal, cfg) {
  var cands = ohParcelCandidates(parcel);
  if (cfg.fmt) cands = cands.concat(cfg.fmt(parcel));
  cands = cands.filter(function (c, i, arr) { return c && arr.indexOf(c) === i; });
  var mode = 'tx-' + (deal.county || '').toLowerCase().replace(/\s+/g, '-');
  try {
    if (cands.length) {
      var inList = cands.map(function (c) { return "'" + c.replace(/'/g, "''") + "'"; }).join(',');
      var url = cfg.url + '/query?where=' + encodeURIComponent(cfg.pf + ' IN (' + inList + ')') + '&outFields=*&returnGeometry=false&f=json';
      var j = await ohFetchJson(url);
      var a = j && j.features && j.features[0] && j.features[0].attributes;
      if (a) return ohOwnerFromAttrs(a, cfg, deal, mode);
    }
    // situs-address fallback: CAD keys an internal id the scraped account can't match (Montgomery)
    var street = deal && (deal.street || deal.address), hn = houseNo(street);
    if (cfg.situs && hn) {
      var core = String(street).toUpperCase().replace(/[^A-Z0-9 ]/g, ' ').replace(/\s+/g, ' ').trim().replace(/'/g, "''");
      var u2 = cfg.url + '/query?where=' + encodeURIComponent("UPPER(" + cfg.situs + ") LIKE '%" + core + "%'") + '&outFields=*&returnGeometry=false&resultRecordCount=6&f=json';
      var feats = ((await ohFetchJson(u2)) || {}).features || [];
      for (var k = 0; k < feats.length; k++) { if (houseNo(feats[k].attributes[cfg.situs]) === hn) return ohOwnerFromAttrs(feats[k].attributes, cfg, deal, mode + '-situs'); }
    }
    return { status: 'no-match' };
  } catch (e) { return { status: 'error', error: String(e && e.message || e) }; }
}
async function ownerForTX(parcel, deal) {
  var cfg = deal && deal.county && TX_COUNTIES[deal.county];
  if (!cfg) return { status: 'no-enricher' };
  return await txCountyLookup(parcel, deal, cfg);
}

// per-state owner enricher registry (FL cadastral + spatial; OH per-county auditor + ODNR; TX CAD)
var ENRICHERS = { FL: ownerForFL, OH: ownerForOH, TX: ownerForTX };
...
module.exports = {
  ...
  signalsFor: signalsFor, scoreDeal: scoreDeal, ownerForFL: ownerForFL, ownerForOH: ownerForOH, ownerForTX: ownerForTX,
  ...
  servicerMailSet: servicerMailSet, isServicerMail: isServicerMail, servicerTier: servicerTier,
