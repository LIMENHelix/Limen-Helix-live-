...
function scoreDeal(deal, sig) {
  ...
  if (sig.ownerOccupied) {
    // owner-occupant in active foreclosure: motivated to sell before losing the home AND their
    // equity. Reachable AT the property itself — a different playbook than investor offload
    // (fast sale / sale-leaseback before the sale date), so tag it distinctly and score off
    // equity-at-risk. The value floor below still demotes no-equity owner-occupants.
