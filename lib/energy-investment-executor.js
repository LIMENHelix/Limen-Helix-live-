...
var Decision = require('./energy-investment-decision.js');
...
var Learning = require('./energy-investment-learning.js');
var B14 = require('./tradier-b14.js');
var SCHEMA = 'energy-investment-command/1.0';
var LOG_KEY = 'energy_investment_command_log';
var PREFIX = 'energy_investment_command:';
var ACTION_PREFIX = 'energy_investment_action:';
var MOTOR_PREFIX = 'energy_investment_motor_claim:';
var BUDGET_PREFIX = 'energy_investment_budget_slot:';
...
function held(reason, extra) { return Object.assign({ ok: true, status: 'HELD', accepted: 0, reason: reason, productDomain: 'energy', ownerDomain: 'energy', lane: 'investments', paperOnly: true, liveMoney: false }, extra || {}); }
...
async function save(store, command) {
  ...
  if (!restored || restored.status !== command.status || restored.actionId !== command.actionId) throw new Error('energy investment command readback invalid');
  return restored;
}
async function budgetSlot(store, actionId, now, cap, reservedNotional) {
  ...
  return { ok: false, reason: 'energy-investment-daily-order-cap-exhausted' };
}
async function execute(options) {
  ...
  if (!Decision.validateReceipt(decision, candidate, now)) return held('energy-investment-exact-b10-decision-required');
  if (!on(env.ENERGY_INVESTMENT_PAPER_ORDER_ENABLED)) return held('energy-investment-paper-order-switch-off');
  if (!on(env.ENERGY_INVESTMENT_RECOVERY_ENABLED)) return held('energy-investment-recovery-switch-must-be-on-before-entry');
  ...
  if (!Number.isFinite(hardMax) || hardMax <= 0) return held('energy-investment-max-notional-not-configured');
  if (!Number.isFinite(dailyBudget) || dailyBudget <= 0 || !cap) return held('energy-investment-daily-budget-or-cap-not-configured');
  var perSlot = dailyBudget / cap;
  if (candidate.maxNotionalUsd > hardMax || candidate.maxNotionalUsd > perSlot) return held('energy-investment-candidate-exceeds-conservative-budget-slot');
  if (!broker || typeof broker.quote !== 'function' || typeof broker.accountSnapshot !== 'function') return held('energy-investment-broker-adapter-missing');
  try {
    store.assertDurable();
    ...
    if (!authorization || !authorization.authorized) {
      ...
      if (!authorization || !authorization.authorized) return held(authorization && authorization.reason || 'energy-investment-motor-held', { authorization: authorization || null });
    }
    ...
    if (!limitPrice || !Number.isFinite(benchmarkBaseline) || benchmarkBaseline <= 0) return held('energy-investment-market-quote-unmeasured');
    var quantity = Math.floor(candidate.maxNotionalUsd / limitPrice);
    if (quantity < 1) return held('energy-investment-notional-cannot-buy-one-whole-paper-share');
    ...
    var commandId = 'enc_' + Decision.hash({ action: decision.actionId, motor: authorization.receiptId, quantity: quantity, limitPrice: limitPrice }).slice(0, 24);
    var command = { schemaVersion: SCHEMA, commandId: commandId, actionId: decision.actionId, decisionReceiptId: decision.decisionReceiptId,
      status: 'COMMANDING', productDomain: 'energy', ownerDomain: 'energy', lane: 'investments', productMotorReceiptId: authorization.receiptId,
    ...
    if (!command || command.status !== 'COMMANDING') throw new Error('energy investment pre-dispatch command readback invalid');
    if (!await store.setIfAbsent(motorKey(authorization.receiptId), { schemaVersion: SCHEMA, commandId: commandId, actionId: decision.actionId, claimedAt: now })) {
      command.status = 'REFUSED'; command.reason = 'energy-investment-motor-receipt-already-consumed'; return save(store, command);
    }
    var motorClaim = await store.get(motorKey(authorization.receiptId));
    if (!motorClaim || motorClaim.commandId !== commandId) throw new Error('energy investment motor claim readback invalid');
    ...
    var preview = await b14.createPreview(store, broker, { symbol: candidate.symbol, side: candidate.side, quantity: quantity,
      ...
      thesisId: candidate.thesisId, selectionId: decision.decisionReceiptId, actionId: decision.actionId, ownerDomain: 'energy', horizonDays: [30, 60, 90],
      decisionContext: { energyPacketId: decision.energyPacketId, brainOpportunityId: decision.brainOpportunityId, evidenceHash: candidate.evidenceHash } }, now);
    ...
    if (!learnedCause || !learnedCause.ok || !learnedCause.command) throw new Error('energy investment causal command memory unavailable');
    ...
    var brokerCommand = await b14.submitApproved(store, broker, {
      ...
      approval: { mode: 'domain-autonomous', actor: 'energy-brain', ownerDomain: 'energy',
    ...
    if (!actionReadback || actionReadback.status !== command.status) throw new Error('energy investment action receipt readback invalid');
    ...
    return { ok: false, status: 'UNRESOLVED', accepted: 0, reason: 'energy-investment-strict-boundary-unavailable', detail: String(error && error.message || error), paperOnly: true, liveMoney: false };
  }
}
