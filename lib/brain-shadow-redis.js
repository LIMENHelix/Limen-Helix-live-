...
/**
 * SET ... NX — ATOMIC CREATE-IF-ABSENT. Returns true if this call created the key, false if
 * it already existed. Never overwrites.
 *
 * WHY THIS EXISTS. The archive used GET then an unconditional SET then GET. That is a
 * check-then-act race: two overlapping workers both read an absent sequence, both write, the
 * second silently overwrites the first, and each can pass its own read-back because each
 * reads what it just wrote. The shadow runtime has no lock and its own docs say idempotency
 * is SEQUENTIAL ONLY, so the race is reachable the moment a retry overlaps a slow cycle.
 *
 * Redis answers OK when it set the key and nil when NX refused. NIL IS A NORMAL ANSWER HERE,
 * not a protocol error, which is why this cannot reuse set(): that helper throws on anything
 * other than OK, and would turn a legitimate "already exists" into a failed cycle.
 */
async function setNX(key, value) {
  var r = await command('SET', [k(key), value, 'NX']);
  var res = (r && typeof r === 'object' && 'result' in r) ? r.result : r;
  if (res === 'OK') return true;
  if (res === null || res === undefined) return false;
  throw new Error('shadow redis: SET NX ' + key + ' returned ' + JSON.stringify(r) +
    ', expected "OK" or nil');
}
