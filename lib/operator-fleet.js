...
// Server mirror of the browser decision + workspace logic. Vercel excludes assets/**
// from the function bundle (vercel.json), so we require the bundled lib/ copy, not the
// browser files. lib/fleet-decision.js is a verbatim, kept-in-sync copy.
var decision = require('./fleet-decision');    // .decide(state)
var workspace = require('./fleet-decision');   // .synthesize(domainsObj)
...
// Run the whole fleet in one pass. `states` maps runtimeKey/id to the bounded
// brain-v2 adapter shape from fleet-signals. Missing measured state is honest, not
// fabricated: that operator monitors and names the missing fused state. Pure and
// free: no network, no AI, no writes.
function runFleet(states, ranAt) {
  ...
  for (var i = 0; i < DOMAINS.length; i++) {
    ...
    var hasSignal = st.brainV2Ready === true;
  ...
  // Rank through the same brain-v2-native salience function the workspace uses:
  // absolute fused departure × measured confidence × bounded posture weight.
  function stateFor(op) {
    var descriptor = null;
    for (var j = 0; j < DOMAINS.length; j++) {
      if (DOMAINS[j].id === op.domain) { descriptor = DOMAINS[j]; break; }
    }
    return states[op.domain] ||
      (descriptor && states[descriptor.runtimeKey]) || {};
  }
  operators.forEach(function (op) {
    op.salience = Math.round(
      decision.salienceFor(stateFor(op), op.decision) * 1000
    ) / 1000;
  });
