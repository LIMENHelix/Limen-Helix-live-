...
const { generateForPortal, generateInvestment, generateResearch } = require('./engine-output-generator.js');
const { loadPortal } = require('./portal-loader');
...
async function refreshArtifact({ portalSlug, lane, index, strictness, actionId }) {
  const portal = (await loadPortal(portalSlug)).portal;
