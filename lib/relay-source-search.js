 ...
 */
const DIRECT_SUPPLIER_DOMAINS = ['cjdropshipping.com'];

/**
 * Matched on the HOSTNAME, not anywhere in the URL. A substring test threw away a
 * perfectly sourceable merchant whose product slug or tracking parameter happened to
 * contain the word. An unparseable URL is refused rather than admitted: it cannot be
 * ordered from either way, and guessing in the permissive direction is how an
 * unfulfillable item reaches a buyer.
 */
function isDirectSupplierUrl(url) {
  ...
  return DIRECT_SUPPLIER_DOMAINS.some(function (d) {
