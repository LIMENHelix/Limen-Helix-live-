...
function normalizeCrossref(item, response, retrievedAt) {
  ...
  if (!message || !canonical) {
    return {
      ...
      portalTargets: item.portalTargets || [],
      reuseClass: (item.portalTargets || []).length > 1 ? 'REUSED_PORTAL_TEMPLATE_REFERENCE' : 'SINGLE_PORTAL_REFERENCE',
      status: 'UNRESOLVED',
  ...
  return {
    ...
    limitations: [
      ...
      'repeated use across portal files is not independent corroboration',
