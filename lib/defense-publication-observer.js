...
var SCHEMA = 'defense-publication-observation/1.0';
var LOG_KEY = 'defense_publication_observation_log';
var ENGAGEMENT_LOG_KEY = 'defense_publication_engagement_log';
var PREFIX = 'defense_publication_observation:';
var ENGAGEMENT_PREFIX = 'defense_publication_engagement:';
...
async function recordSourceClick(store, article, sourceIndex, visitorToken, headers, now) {
  ...
  if (!article || article.schemaVersion !== 'defense-publication-article/1.0' || article.published !== true ||
  ...
  var eventId = 'dpe_' + hash({ article: article.articleId, source: source.sourceIdentity.value, visitor: visitorIdentityHash, day: day }).slice(0, 24);
  var engagementEligible = userActivatedBrowser(headers);
  var event = {
    schemaVersion: 'defense-publication-engagement/1.0',
    eventId: eventId,
  ...
  if (!restored || restored.eventId !== eventId || restored.actionId !== article.actionId) throw new Error('defense publication engagement readback invalid');
...
async function observePresence(store, command, fetcher, baseUrl) {
  ...
  var url = String(baseUrl || 'https://limenhelix.com').replace(/\/$/, '') + '/api/defense-publication-public?id=' + encodeURIComponent(command.articleId);
  var response;
  try { response = await fetcher(url, { method: 'GET', headers: { accept: 'application/json', 'user-agent': 'limen-defense-publication-observer/1.0' }, signal: AbortSignal.timeout(15000) }); }
  ...
  var observation = {
    ...
    observationId: 'dpo_' + hash({ command: command.commandId, status: response.status, content: article && article.contentHash }).slice(0, 24),
  ...
  if (!restored || restored.status !== observation.status) throw new Error('defense publication presence readback invalid');
...
async function observeEngagement(store, event, commandsByAction) {
  if (!event || event.schemaVersion !== 'defense-publication-engagement/1.0' || event.eventType !== 'SOURCE_LINK_CLICK') {
  ...
  var observation = {
    schemaVersion: SCHEMA,
    observationId: 'dpo_' + hash({ command: command.commandId, event: event.eventId }).slice(0, 24),
    commandId: command.commandId, actionId: command.actionId, articleId: command.articleId,
  ...
  var created = await store.setIfAbsent(key(observation.observationId), observation);
  var restored = await store.get(key(observation.observationId));
  if (!restored || restored.status !== observation.status) throw new Error('defense publication engagement observation readback invalid');
  if (created) { await store.lpush(LOG_KEY, restored); await store.ltrim(LOG_KEY, 0, 1999); }
  return Object.assign({ ok: true, duplicate: !created }, restored);
}

async function publicAbsent(fetcher, articleId, baseUrl) {
  var url = String(baseUrl || 'https://limenhelix.com').replace(/\/$/, '') + '/api/defense-publication-public?id=' + encodeURIComponent(articleId);
