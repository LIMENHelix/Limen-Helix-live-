...
function computeFunnel(agg, config) {
  ...
  Object.keys(enroll).forEach(function (ds) {
    ...
    estimatedRevenue += cents;
  });
  // Payment rails can book exact cash directly on an enrollment unit. Keep it
  // separate from configured/simulated deal-value estimates so a real Stripe
  // receipt is neither reported as zero nor estimated a second time.
  let actualRevenue = 0;
  const enrollmentUnits = agg['shows>enrollments'] || {};
  Object.keys(enrollmentUnits).forEach(function (name) {
    actualRevenue += Number(enrollmentUnits[name] && enrollmentUnits[name].revenueCents) || 0;
  });
  out.actualRevenueCents = actualRevenue;
  out.estimatedRevenueCents = estimatedRevenue;
  out.revenueCents = actualRevenue + estimatedRevenue;
  out.revenueByDeal = byDeal;
  out.profitCents = out.revenueCents - spend;
  out.roas = spend > 0 ? out.revenueCents / spend : null; // return on ad spend (x)
