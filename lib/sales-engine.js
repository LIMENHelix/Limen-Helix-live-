...
function computeFunnel(agg, config) {
  ...
  // headline ROI: revenue from enrollments vs total spend
  let spend = 0;
  config.transitions.forEach(function (def) {
    const t = agg[def.id] || {};
...
function templateCopy(unit, dealSize, trigger, layerMap) {
  // A readable SET/FBA scaffold so a rep sees the actual words, not just codes.
  return {
    fact: 'FACT: reaching a ' + dealSize + ' prospect via ' + unit + ' typically needs multiple touches.',
    need: 'NEED: the ' + trigger + ' trigger goes unaddressed without a structured follow-up.',
    tie: 'TIE-DOWN: that is exactly why we run ' + unit + ' on cadence — agreed that makes sense?',
    feature: 'FEATURE: cadenced ' + unit + ' outreach.',
    benefit: 'BENEFIT: fewer prospects slip; more reach the next stage.',
    advantage: 'ADVANTAGE: vs one-and-done, cadence lifts the ' + dealSize + '/' + trigger + ' segment.'
  };
}
