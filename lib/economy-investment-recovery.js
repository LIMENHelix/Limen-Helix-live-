...
var Executor = require('./economy-investment-executor.js');
...
var SCHEMA = 'economy-investment-recovery/1.0';
var LOG_KEY = 'economy_investment_recovery_log';
var PREFIX = 'economy_investment_recovery:';
...
async function save(store, receipt) {
  ...
  if (!restored || restored.status !== receipt.status || restored.commandId !== receipt.commandId) throw new Error('economy investment recovery readback invalid');
  return restored;
}
async function recover(options) {
  ...
  if (!on(env.ECONOMY_INVESTMENT_RECOVERY_ENABLED)) return { ok: true, status: 'HELD', reason: 'economy-investment-recovery-switch-off', liveMoney: false };
  if (!command || command.schemaVersion !== Executor.SCHEMA || command.status !== 'COMMAND_RECEIPTED' || !command.brokerCommandId) {
    return { ok: false, status: 'REFUSED', reason: 'economy-investment-command-receipt-required', liveMoney: false };
  }
  if (!((trigger.type === 'economy-investment-kill' || trigger.type === 'independent-risk-breach') && String(trigger.id || '').trim())) {
  ...
  try {
    ...
    if (!authorization || !authorization.authorized) return { ok: true, status: 'HELD', reason: authorization && authorization.reason || 'economy-recovery-motor-held', liveMoney: false };
    var recoveryId = 'eir_' + require('./economy-investment-decision.js').hash({ command: command.commandId, trigger: trigger.id, motor: authorization.receiptId }).slice(0, 24);
    ...
    var receipt = { schemaVersion: SCHEMA, recoveryId: recoveryId, commandId: command.commandId, actionId: command.actionId,
      ...
      productDomain: 'economy', ownerDomain: 'economy', lane: 'investments', productMotorReceiptId: authorization.receiptId,
    ...
    if (executed <= 0 && !['filled', 'canceled', 'rejected', 'expired', 'error'].includes(String(order.status || '').toLowerCase())) {
      if (!brokerCommand.rollback || !brokerCommand.rollback.confirmationSummary) throw new Error('economy investment cancel receipt unavailable');
      brokerCommand = await b14.cancelApproved(store, broker, {
        commandId: brokerCommand.commandId, confirmation: brokerCommand.rollback.confirmationSummary,
        approval: { mode: 'recovery', actor: 'economy-brain-recovery', ownerDomain: 'economy',
      ...
      if (!closePrice) throw new Error('economy investment close quote unmeasured');
      var preview = await b14.createPreview(store, broker, { symbol: command.symbol, side: closeSide, quantity: executed,
        ...
        ownerDomain: 'economy', horizonDays: [30, 60, 90], decisionContext: { recoveryOf: command.commandId, trigger: receipt.trigger } }, now);
      var closeCommand = await b14.submitApproved(store, broker, {
    ...
    return { ok: false, status: 'RECOVERY_UNRESOLVED', reason: 'economy-investment-recovery-unavailable', detail: String(error && error.message || error), paperOnly: true, liveMoney: false };
  }
}
