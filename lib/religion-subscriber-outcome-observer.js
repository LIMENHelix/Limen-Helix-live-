...
var SCHEMA = 'religion-subscriber-observation/1.0';
var LOG_KEY = 'religion_subscriber_observation_log', PREFIX = 'religion_subscriber_observation:';
...
async function observe(store, command, item, deps) {
  ...
  var receipt = { schemaVersion: SCHEMA, observationId: 'rso_' + crypto.createHash('sha256').update(item.providerEmailId + ':' + event + ':' + String(body.created_at || '')).digest('hex').slice(0, 24),
  ...
  if (!restored || restored.observationId !== receipt.observationId || restored.lastEvent !== event) throw new Error('religion subscriber observation readback invalid');
