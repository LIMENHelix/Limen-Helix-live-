...
async function snapshot(env, store) {
  ...
  var records = await Promise.all(ids.map(function (id) { return read(id, store); }));
  ...
  var lines = Registry.LINES.map(function (item, i) {
    ...
    return Object.assign({}, item, {
      runtime: runtime,
      hardGate: hard,
      build: Registry.buildState(item),
...
module.exports = { SCHEMA: SCHEMA, GLOBAL_ID: GLOBAL_ID, LOG_KEY: LOG_KEY, key: key, read: read, set: set, authorize: authorize, snapshot: snapshot };
