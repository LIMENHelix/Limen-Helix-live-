...
var RESPONSE_SCHEMA = 'finance-manager-proposal/1.1';
...
function withoutThing2(value) {
  if (Array.isArray(value)) return value.map(withoutThing2);
  if (!value || typeof value !== 'object') return value;
  var out = {};
  Object.keys(value).forEach(function (key) {
    if (String(key).toLowerCase() === 'thing2') return;
    out[key] = withoutThing2(value[key]);
  });
  return out;
}
...
function candidateContexts(candidates) {
  return list(candidates).slice(0, 12).map(function (candidate) {
    // Thing 2 is a post-result masking/alignment comparison.  Excluding the
    // snapshot from the selection prompt is stronger than asking a model to
    // see it and pretend it had zero influence.
    var projected = withoutThing2(candidate);
...
function buildRequest(input) {
  ...
  return {
    ...
    instructions: [
      ...
      'Set horizonDays to one integer: 1, 3, 7, 14, 30, 60, or 90. It must never be an array or a range.',
      'Rank every supplied company and direction (LONG or SHORT) by projected risk-adjusted investment margin. These are explicitly Finance-manager estimates, not observed facts.',
      'For each ranking entry calculate riskAdjustedMarginPct = expectedReturnPct - abs(min(downsideReturnPct, 0)) * (1 - confidence). Sort descending, select rank 1, and abstain if rank 1 is not positive.',
      'Treat expectedReturnPct as net of modeled costs. Subtract at least the supplied round-trip cost floor, and for SHORT also subtract the supplied borrow-cost floor; unknown extra costs reduce confidence.',
      'Do not call accounting gross margin, operating margin, model confidence, stress, or homology a projected investment margin.',
      ...
      'A title, stress value, Thing 1 result, or model confidence cannot directly authorize an investment.',
      'The Thing 2 snapshot is deliberately absent from this decision request. It is reconciled with the Thing 1 result only after the proposal is complete and cannot alter selection, direction, confidence, sizing, leverage, or execution.',
      ...
      'Company learning, when supplied, is compact resolved-outcome history for that exact CIK. Use it only to adjust or abstain; its recommendation is advisory and never authorizes a trade.',
    ...
    outputSchema: {
      ...
      thesis: 'string',
      invalidation: 'string',
      horizonDays: 'integer; exactly one of 1, 3, 7, 14, 30, 60, or 90',
      ...
      projectedMarginRanking: {
        ...
        entries: [{
          company: { slug: 'exact supplied slug', ticker: 'exact supplied ticker' },
          side: 'LONG | SHORT',
          expectedReturnPct: 'number; Finance-manager estimate over horizonDays',
          downsideReturnPct: 'number <= 0; Finance-manager downside estimate over horizonDays',
          confidence: 'number 0..1',
          riskAdjustedMarginPct: 'number calculated with the required methodology'
        }]
      },
    ...
    context: {
      projectedMarginPolicy: { roundTripCostFloorPct: 0.2, shortBorrowCostFloorPct: 1.0 },
      ...
      postDecisionMaskingReconciliationDeferred: true,
...
function parseResponse(raw) {
  ...
  if ([1, 3, 7, 14, 30, 60, 90].indexOf(value.horizonDays) < 0) blockers.push('manager_response_horizon_required');
  ...
  var ranking = value.projectedMarginRanking;
  if (ranking !== undefined) {
    if (!ranking || ranking.metric !== 'risk-adjusted-expected-total-return-pct' ||
        ranking.methodology !== 'expectedReturnPct - abs(min(downsideReturnPct, 0)) * (1 - confidence)' ||
        !Array.isArray(ranking.entries) || !ranking.entries.length) {
      blockers.push('manager_response_projected_margin_ranking_invalid');
    } else if (!ranking.entries.every(function (row) {
      return row && row.company && text(row.company.slug) && text(row.company.ticker) &&
        (row.side === undefined || ['LONG', 'SHORT'].indexOf(row.side) >= 0) &&
        finite(row.expectedReturnPct) && finite(row.downsideReturnPct) && row.downsideReturnPct <= 0 &&
        finite(row.confidence) && row.confidence >= 0 && row.confidence <= 1 && finite(row.riskAdjustedMarginPct);
    })) {
      blockers.push('manager_response_projected_margin_entries_invalid');
    }
  }
