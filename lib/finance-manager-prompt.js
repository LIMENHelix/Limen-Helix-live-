...
function buildRequest(input) {
  ...
  return {
    ...
    context: {
      ...
      companyCandidates: clone(list(ctx.companyCandidates)).slice(0, 12)
    }
  };
}
...
function parseResponse(raw) {
  ...
  if (!Array.isArray(value.evidenceRefs) || !value.evidenceRefs.length) blockers.push('manager_response_evidence_refs_required');
  else if (!value.evidenceRefs.every(function (ref) {
    return ref && ['semantic', 'market', 'network'].indexOf(ref.role) >= 0 && sourceIdentity(ref.sourceIdentity);
