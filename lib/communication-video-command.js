...
async function prepare(store, subjectReceipt, channelReceipt, nowValue) {
  ...
  try {
    ...
    var rows = await Promise.all([store.get(contract.stateKey), store.get(contract.artifactStateKey),
      ...
      store.get(Release.selectionKey(channelReceipt && channelReceipt.selectionReceiptId))]);
    var state = rows[0], artifact = rows[1], manifest = rows[2], selection = rows[3];
    if (!Release.currentPair(contract, state, artifact, manifest, now) ||
        ...
        !Release.validSelection(selection, subjectReceipt, manifest, now)) {
    ...
    var command = {
      ...
      selectionReceiptId: channelReceipt.selectionReceiptId,
      workOrder: manifest,
    ...
    if (!restored || restored.schemaVersion !== SCHEMA || restored.commandId !== commandId ||
        ...
        restored.selectionReceiptId !== channelReceipt.selectionReceiptId) {
