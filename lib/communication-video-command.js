...
async function appendOnce(store, command) {
  var marker = { schemaVersion: SCHEMA, commandId: command.commandId, status: 'INDEXED', indexedAt: Date.now() };
  var created = await store.setIfAbsent(queueKey(command.commandId), marker);
  if (created) {
    try {
      await store.lpush(PENDING_KEY, { schemaVersion: SCHEMA, commandId: command.commandId,
        subjectDomain: command.subjectDomain, manifestId: command.manifestId,
        commandedAt: command.commandedAt, expiresAt: command.expiresAt });
      await store.lpush(LOG_KEY, command);
      await store.ltrim(LOG_KEY, 0, 999);
    } catch (error) {
      await store.deleteIfValue(queueKey(command.commandId), marker).catch(function () {});
      throw error;
    }
  }
  var restored = await store.get(queueKey(command.commandId));
  if (!restored || restored.commandId !== command.commandId || restored.status !== 'INDEXED') {
    throw new Error('communication video queue index readback invalid');
  }
}
...
module.exports = { SCHEMA: SCHEMA, LOG_KEY: LOG_KEY, PENDING_KEY: PENDING_KEY,
  ...
  appendOnce: appendOnce, hash: hash };
