...
/**
 * Baskets are keyed by CANONICAL name (medicine / science / trade), but the worker and the
 * snapshot look domains up by RUNTIME key (health / research / supplyChain). Callers may pass
 * either; lib/domain-names.js reconciles them, and it is the only place that mapping lives.
 *
 * Worth remembering how this failed, because it is not loud: the worker looked baskets up by
 * runtime key, got an EMPTY array back, and correctly abstained from the market channel. Three
 * domains went dark and it reported as "thin basket or fetch failure" rather than as a name
 * mismatch. Honest abstention is right everywhere else in this system, which is exactly what
 * disguised it. If a domain looks unexpectedly empty, check the NAME first.
 */
function get(domain) {
  var k = DOMAIN_NAMES.toCanonical(domain);
