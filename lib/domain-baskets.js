...
/**
 * RUNTIME KEY -> CANONICAL BASKET NAME.
 *
 * The system carries two names for the same three domains: the snapshot and the worker say
 * health / research / supplyChain, while the registry, the console and these basket files say
 * medicine / science / trade. Resolved here so callers can pass either.
 *
 * This is the same gap that made /api/feed-resolve?domain=medicine report zero resolved
 * forecasts while health reported 238 from identical rows (fixed 2026-07-28 in
 * handlers/feed-resolve.js). It bit again the same day, in a new place: the worker looks up
 * baskets by runtime key, so health / research / supplyChain silently got an EMPTY basket and
 * abstained from the market channel entirely — three domains dark, and it read as "thin basket"
 * rather than as a name mismatch. Anywhere a domain looks unexpectedly empty, check the naming
 * system before believing the data is missing.
 */
var RUNTIME_ALIAS = { health: 'medicine', research: 'science', supplyChain: 'trade' };

function get(domain) {
  var k = String(domain || '');
  if (Object.prototype.hasOwnProperty.call(RUNTIME_ALIAS, k)) k = RUNTIME_ALIAS[k];
  return Object.prototype.hasOwnProperty.call(BASKETS, k) ? BASKETS[k].slice() : [];
}

module.exports = { BASKETS: BASKETS, get: get, domains: Object.keys(BASKETS), RUNTIME_ALIAS: RUNTIME_ALIAS };
