...
/* The packet is the only Finance opportunity input.  The legacy master inbox
 * is intentionally absent here: a replay candidate must have been produced
 * by the source-grounded Finance manager and released for sandbox paper mode. */
function financeCandidates(packet) {
  var truth = packet && packet.truth || {};
  return array(truth.opportunities).filter(function (item) {
    return item && item.status === 'READY_TO_FIRE' && item.lane === 'investment' &&
      item.paperOnly === true && item.liveExecution === false &&
      text(item.artifactRef) && text(item.portalSlug) && text(item.portalTicker) &&
...
function summarize(input) {
  ...
  var candidates = financeCandidates(packet);
  ...
  if (!candidates.length) blockers.push('finance_packet_has_no_released_investment_candidate');
