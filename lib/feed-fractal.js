...
var CHANNEL_TYPES = {
  ...
  supply:      { re: /supply (chain|shortage|disruption)|shortage|outage|bottleneck|logistics|shipment delay|inventory (glut|shortfall)|stockout|hormuz|\bstrait\b|blockade|tanker (attack|seiz|threat)|naval (blockade|threat)|shipping route|port (attack|closure)/i,
                 sig: likFrom({ 3: 0.6, 5: 0.4 }) },                     // supply disruption = Darkness + Endurance. Extended 2026-07-20 for
                                                                          // the real available geopolitical text (Hormuz/tanker/naval): a strait
                                                                          // closure IS a supply disruption, same signature, no new category needed.
  // Added 2026-07-20: energy's real live feed (limen-worker-ingest.js latest_ingest, real Google News
  // RSS headlines, already running) is geopolitical/military, not corporate — the original 8 categories
  // above were designed for company-side text (layoffs, leadership, pricing...) that has NO live source
  // yet (company newsFeed[] is empty; see feed-fractal audit). `conflict` classifies the text that IS
  // actually available and actually relevant right now (Hormuz/Iran war coverage). Deliberately excludes
  // a bare \bwar\b token to avoid false-positiving on "price war"/"trade war"/"bidding war", which
  // `competition` already classifies correctly.
  conflict:    { re: /\binvasion\b|military (buildup|escalation|strike|conflict)|armed conflict|declares? war|war ?zone|wartime|missile (strike|attack)|airstrike|naval (fleet|warship|blockade)|nuclear (threat|weapon|strike)|troops? (deployed|mobili[sz]ed)|mobili[sz]ation/i,
                 sig: likFrom({ 9: 0.5, 3: 0.4, 5: 0.1 }) },             // armed conflict/war = Threshold (existential rupture) + Darkness
