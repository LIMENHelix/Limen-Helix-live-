...
var NO_EFFECT_RETRY_COOLDOWN_MS = 60 * 60 * 1000;
...
function publicState(state) {
  ...
  return {
    ...
    attemptCount: Number(state.attemptCount) || null,
    ...
    providerStage: state.providerStage || null,
    providerStatus: Number.isFinite(Number(state.providerStatus)) ? Number(state.providerStatus) : null,
    externalEffectMayExist: state.externalEffectMayExist === true,
    liveMoney: false
  };
}
...
function failedReason(result) {
  if (result && result.blocked) return 'commissioning-posting-blocked';
  if (result && result.rateLimited) return 'commissioning-rate-limited';
  if (result && result.providerStage === 'authentication') return 'commissioning-provider-authentication-failed';
  if (result && result.providerCalled === false) return 'commissioning-provider-preflight-failed';
  if (result && result.definitiveFailure) return 'commissioning-provider-definitive-failure';
  return 'commissioning-post-failed';
}
function initialState(motor, at, attemptCount) {
  var attempt = Math.max(1, Number(attemptCount) || 1);
  return {
    schemaVersion: SCHEMA,
    commissioningId: 'cscap_' + hash({ motorReceiptId: motor.receiptId, text: TEXT, attempt: attempt }).slice(0, 24),
    ...
    attemptCount: attempt, providerCalled: false, externalEffectMayExist: false, liveMoney: false
  };
}
  ...
  if (state.status === 'CLAIMED') {
    ...
    if (!posted || !posted.ok) {
      var failed = Object.assign({}, claimed.state, {
        ...
        providerStage: posted && posted.providerStage || null,
        providerStatus: Number.isFinite(Number(posted && posted.providerStatus)) ? Number(posted.providerStatus) : null,
        externalEffectMayExist: !!(posted && posted.ambiguous),
...
async function commission(store, now, deps) {
  ...
  var initial = initialState(motor, at, 1);
  ...
  if (state.status === 'FAILED' && !state.postIdentity &&
      (state.externalEffectMayExist === false ||
       (state.reason === 'commissioning-post-failed' && state.providerCalled === false))) {
    var legacyFailure = state.externalEffectMayExist == null;
    var retryAt = Number(state.completedAt || state.claimedAt) + NO_EFFECT_RETRY_COOLDOWN_MS;
    if (!legacyFailure && at < retryAt) {
      return { ok: true, status: 'HELD', reason: 'commissioning-no-effect-retry-cooldown',
        retryAt: retryAt, commissioning: publicState(state), liveMoney: false };
    }
    var retry = initialState(motor, at, Number(state.attemptCount || 1) + 1);
    retry.previousFailureReason = state.reason;
    var reset = await transition(store, state, retry);
    state = reset.state;
  }
...
module.exports = {
  ...
  NO_EFFECT_RETRY_COOLDOWN_MS: NO_EFFECT_RETRY_COOLDOWN_MS,
  TEXT: TEXT, hash: hash, validMotor: validMotor, publicState: publicState,
  failedReason: failedReason, initialState: initialState,
