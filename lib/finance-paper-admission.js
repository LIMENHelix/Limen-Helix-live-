...
// Admission is a durable go/no-go episode. Source packets may expire; the
// compact reason for admitting or refusing one must not expire with them.
...
async function execute(store, request, options) {
  ...
  var created = await store.setIfAbsent(admissionKey(request.packetId), receipt);
  if (!created) {
