...
const API_KEY = process.env.CJ_DROPSHIPPING_API_KEY || '';
...
async function _cachedToken() {
  try {
    ...
    // A token records which key minted it. Without this, rotating CJ_DROPSHIPPING_API_KEY or moving
...
async function stock(vid, countryCode) {
  ...
  // Prefer the destination's own warehouse; otherwise ship from wherever it actually is.
  if (inCountry > 0) return { qty: inCountry, from: want };
  return best;
}
...
async function search(opts) {
  opts = opts || {};
  if (!configured()) {
    return { ok: false, items: [], reason: 'CJ_DROPSHIPPING_API_KEY not set' };
  }
  ...
  const t = await token();
  if (!t) return { ok: false, items: [], reason: 'CJ authentication failed; check CJ_DROPSHIPPING_API_KEY' };
...
async function placeOrder(opts) {
  opts = opts || {};
  if (!configured()) return { ok: false, error: 'CJ_DROPSHIPPING_API_KEY not set' };
