...
async function _call(path, opts) {
  ...
  const timer = setTimeout(function () { ctl.abort(); }, opts.timeoutMs || TIMEOUT_MS);
  try {
...
/**
 * The prepaid wallet balance CJ actually debits. → { ok:true, available, ... } or
 * { ok:false, error } when we cannot tell.
 *
 * WHY THIS EXISTS
 * placeOrder pays with payType 2, which draws from this wallet and nothing else. The
 * autonomy gate's requireFunds was reading a PayPal balance — an account no Relay purchase
 * has ever debited — so it refused every sale on a number that had nothing to do with the
 * money being spent. Nothing in the client could read the real one, which is why nobody
 * knew. This is that read.
 *
 * NEVER RETURNS A NUMBER IT IS NOT SURE OF. An unreadable balance is ok:false, not zero:
 * the caller must be able to tell "the wallet is empty" from "I could not ask", because
 * the first is a fact about the money and the second is a fact about the network. That
 * distinction is exactly what the PayPal reader got wrong — it answers `cachedBalance || 0`
 * on every error path, so an outage arrives as a confident $0.
 *
 * AVAILABLE UNDER-REPORTS ON PURPOSE. freezeAmount is money CJ is holding against
 * something else, so it is subtracted. A field present but unparseable makes the whole
 * answer unknown rather than being quietly treated as zero, because rounding an unknown
 * DOWN to zero refuses a sale we could make, while rounding it UP authorises one we
 * cannot pay for.
 */
async function balance(opts) {
  opts = opts || {};
  const t = await token();
  if (!t) return { ok: false, error: 'CJ not configured' };

  const r = await _call('/shopping/pay/getBalance', { token: t, timeoutMs: opts.timeoutMs || 4000 });
  if (!r.ok) return { ok: false, error: r.error || 'CJ refused the balance read' };

  const d = r.data;
  if (!d || typeof d !== 'object') return { ok: false, error: 'CJ returned no balance data' };

  // Explicit parsing. _num() nulls a negative, and for a wallet a negative is meaningful
  // data rather than noise, so it is not used here.
  ...
  if (amount === null) return { ok: false, error: 'CJ balance amount was not a number' };

  // Absent is fine (nothing frozen). Present-but-unparseable is NOT: we cannot tell how
  // much of the balance is actually spendable.
  const hasFreeze = d.freezeAmount !== undefined && d.freezeAmount !== null && d.freezeAmount !== '';
  const freeze = hasFreeze ? num(d.freezeAmount) : 0;
  if (freeze === null) return { ok: false, error: 'CJ freezeAmount was not a number' };

  return {
    ok: true,
    available: Math.round((amount - freeze) * 100) / 100,
    amount: Math.round(amount * 100) / 100,
    frozen: Math.round(freeze * 100) / 100
  };
}
...
/**
 * getOrderDetail(orderNumber, expected) — the READ half of the purchase contract.
 *
 * createOrderV2 is the only way this module has ever looked at CJ's order book, which is
 * how an ambiguous outcome (timeout after accept, ok-but-no-order-id) could become a
 * duplicate supplier order or a released reservation over money actually spent. This is
 * the readback: GET /shopping/order/getOrderDetail, whose orderId parameter accepts the
 * custom order number we place under.
 *
 * THREE ANSWERS, because "could not ask" is not "not there":
 *   found: true        CJ returned a record EXACTLY bound to this orderNumber — one of
 *                      orderNum/orderNumber/orderId matches — and, when CJ returned a
 *                      product list and the caller passed expected.vid/quantity, the line
 *                      agrees. Anything less is not a match.
 *   found: false       CJ answered and definitively has no such order (null data, or an
 *                      explicit not-found refusal).
 *   found: 'ambiguous' timeout, rate limit, transport error, malformed payload, or a
 *                      record that does not bind to the number we asked about. The caller
 *                      must treat this as "money status unknown": hold, do not re-place,
 *                      do not release.
 */
async function getOrderDetail(orderNumber, expected) {
  expected = expected || {};
  const asked = String(orderNumber || '');
  if (!asked) return { found: 'ambiguous', orderNumber: null, error: 'orderNumber required' };
  if (!configured()) return { found: 'ambiguous', orderNumber: asked, error: 'CJ_API_KEY not set' };

  const t = await token();
  if (!t) return { found: 'ambiguous', orderNumber: asked, error: 'CJ authentication failed' };

  const r = await _call('/shopping/order/getOrderDetail?orderId=' + encodeURIComponent(asked), { token: t });
  if (!r.ok) {
    // A business refusal that SAYS the order does not exist is a definitive absence;
    // anything else (429, 5xx, timeout, auth blip) is unknowable.
    if (/not exist|not found|no such|does not exist|no data/i.test(r.error || '')) {
      return { found: false, orderNumber: asked };
    }
    return { found: 'ambiguous', orderNumber: asked, error: r.error };
  }

  const d = r.data;
  if (d === null || d === undefined) return { found: false, orderNumber: asked };
  if (typeof d !== 'object') return { found: 'ambiguous', orderNumber: asked, error: 'malformed getOrderDetail payload' };

  // 'Some row exists' is not a match. The record has to bind to the number we asked for.
  const boundIds = [d.orderNum, d.orderNumber, d.orderId]
    .filter(function (v) { return v !== undefined && v !== null; })
    .map(String);
  if (boundIds.indexOf(asked) === -1) {
    return {
      found: 'ambiguous',
      orderNumber: asked,
      mismatch: true,
      error: 'CJ returned an order that does not bind to ' + asked
    };
  }

  const products = (Array.isArray(d.productList) ? d.productList :
                    Array.isArray(d.products) ? d.products :
                    Array.isArray(d.productListVos) ? d.productListVos : [])
    .map(function (p) {
      return {
        vid: p.vid != null ? String(p.vid) : (p.variantId != null ? String(p.variantId) : null),
        quantity: parseInt(p.quantity != null ? p.quantity : (p.productNum != null ? p.productNum : p.num), 10) || null
      };
    });
  ...
  const out = {
    found: true,
    orderNumber: asked,
    cjOrderId: d.orderId != null ? String(d.orderId) : (d.cjOrderId != null ? String(d.cjOrderId) : null),
    status: d.orderStatus != null ? d.orderStatus : (d.status != null ? d.status : null),
    amountUsd: _num(d.orderAmount != null ? d.orderAmount : d.amount),
    products: products,
    shipping: (d.shippingCustomerName || d.shippingAddress || d.shippingCountryCode) ? {
      name: d.shippingCustomerName || null,
      address: d.shippingAddress || null,
      city: d.shippingCity || null,
      province: d.shippingProvince || null,
      zip: d.shippingZip || null,
      country: d.shippingCountryCode || d.shippingCountry || null
    } : null,
    tracking: (d.trackNumber || d.trackingNumber || d.logisticName) ? {
      number: d.trackNumber || d.trackingNumber || null,
      carrier: d.logisticName || d.logisticNameCode || null
    } : null
  };

  // When CJ returned a product list AND the caller knows what line it placed, the line
  // must agree. A bound order number carrying a different variant is not a reconciliation
  // — it is a sign the number collided with something we did not place.
  if (products.length && expected.vid) {
    const wantQty = Math.max(1, parseInt(expected.quantity, 10) || 1);
    const lineOk = products.some(function (p) {
      return p.vid === String(expected.vid) && (p.quantity == null || p.quantity === wantQty);
    });
    if (!lineOk) {
      return Object.assign(out, {
        found: 'ambiguous',
        mismatch: true,
        error: 'the CJ order for ' + asked + ' carries a different product line than the Relay line'
      });
    }
  }
...
// Read-only enrichment for a listed variant. Existing sourcing and order paths do not
// call this function. Use the same authentication and throttle as other CJ requests.
async function productDetails(vid) {
  if (!/^[a-zA-Z0-9-]{1,100}$/.test(String(vid || ''))) return null;
  const t = await token();
  if (!t) return null;
  const vr = await _call('/product/variant/queryByVid?vid=' + encodeURIComponent(vid), { token: t, timeoutMs: 6000 });
  const v = vr.ok && vr.data;
  if (!v || String(v.vid) !== String(vid) || !v.pid) return null;
  const pr = await _call('/product/query?pid=' + encodeURIComponent(v.pid), { token: t, timeoutMs: 6000 });
  // A product failure can still return the verified variant. Never infer a different one.
  const p = pr.ok && pr.data && String(pr.data.pid) === String(v.pid) ? pr.data : {};
  return {
    variant: v.variantKey || '', title: p.productNameEn || v.variantNameEn || '',
    description: p.description || '', material: p.productMaterialEn || '',
