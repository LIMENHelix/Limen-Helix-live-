...
async function probe(opts) {
  ...
  for (const p of list) {
    ...
    const variant = await firstVariant(id);
    if (!variant || variant.price == null) {
      out.dropped.push({ p: name, why: variant ? 'variant has no price' : 'no variant returned' });
      continue;
    }
    ...
    const held = await stock(variant.vid, country);
    if (held.qty < 1) {
      out.dropped.push({ p: name, why: 'no stock in any warehouse', vid: variant.vid });
      continue;
    }
    out.stages.inStock++;
    ...
    const ship = await freight(variant.vid, 1, country, opts.zip, held.from);
    if (!ship) {
      out.dropped.push({ p: name, why: 'freight would not quote', vid: variant.vid, from: held.from });
      continue;
    }
