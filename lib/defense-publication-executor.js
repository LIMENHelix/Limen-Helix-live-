...
var Decision = require('./defense-publication-decision.js');
...
var SCHEMA = 'defense-publication-command/1.0';
var LOG_KEY = 'defense_publication_command_log';
var PENDING_KEY = 'defense_publication_pending_log';
var SUPPRESSION_KEY = 'defense_publication_suppression_catalog';
var COMMAND_PREFIX = 'defense_publication_command:';
var ACTION_PREFIX = 'defense_publication_action:';
var MOTOR_PREFIX = 'defense_publication_motor_claim:';
var BUDGET_PREFIX = 'defense_publication_budget_slot:';
...
function held(reason, extra) { return Object.assign({ ok: true, status: 'HELD', accepted: 0, reason: reason, providerCalls: 0, productDomain: 'defense', ownerDomain: 'defense', lane: 'publication', liveMoney: false }, extra || {}); }
...
async function save(store, command) {
  ...
  if (!restored || restored.status !== command.status || restored.actionId !== command.actionId) throw new Error('defense publication command readback invalid');
  return restored;
}
...
async function budgetSlot(store, actionId, slots, now, cost) {
  ...
  return { ok: false, reason: 'defense-publication-daily-budget-exhausted' };
}
...
async function execute(input) {
  ...
  if (!Decision.validateReceipt(decision, candidate, now)) return held('defense-publication-exact-b10-decision-required');
  ...
  if (input.operationCostUsd == null || !Number.isFinite(cost) || cost < 0) return held('defense-publication-operation-cost-not-configured');
  if (!cap) return held('defense-publication-daily-publication-cap-zero');
  if (cost > 0 && (!Number.isFinite(budget) || budget < cost)) return held('defense-publication-daily-dollar-budget-not-configured-or-too-small');
  var slots = Math.min(cap, cost === 0 ? cap : Math.floor((budget + 1e-12) / cost));
  try {
    store.assertDurable();
    var suppression = await store.get(SUPPRESSION_KEY);
    if (suppression && suppression[candidate.sourceFingerprint] && suppression[candidate.sourceFingerprint].suppressed) return held('defense-publication-source-fingerprint-suppressed');
    var authorization = await (input.motorAuthorization || Motor).authorize(store, 'defense', 'publication', now);
    if (!authorization || !authorization.authorized) return held(authorization && authorization.reason || 'defense-publication-motor-held', { motorReceiptId: authorization && authorization.receiptId || null });
    ...
    var commandId = 'dpcmd_' + Source.hash({ action: decision.actionId, motor: authorization.receiptId }).slice(0, 24);
    var command = {
      schemaVersion: SCHEMA, commandId: commandId, actionId: decision.actionId, decisionReceiptId: decision.decisionReceiptId,
      status: 'COMMANDING', productDomain: 'defense', ownerDomain: 'defense', lane: 'publication',
      productMotorReceiptId: authorization.receiptId, candidateId: candidate.candidateId, contentHash: candidate.contentHash,
      sourceFingerprint: candidate.sourceFingerprint, defensePacketId: candidate.defensePacketId,
    ...
    if (!command || command.status !== 'COMMANDING') throw new Error('defense publication pre-dispatch readback invalid');
    if (!await store.setIfAbsent(motorKey(authorization.receiptId), { schemaVersion: SCHEMA, commandId: commandId, productMotorReceiptId: authorization.receiptId, claimedAt: now })) {
      command.status = 'REFUSED'; command.reason = 'defense-publication-motor-receipt-already-consumed'; return save(store, command);
    }
    var motorClaim = await store.get(motorKey(authorization.receiptId));
    if (!motorClaim || motorClaim.commandId !== commandId) throw new Error('defense publication motor claim readback invalid');
    ...
    var result;
    try {
      command.adapterGuard = await (input.adapterGuard || AdapterGuard).checkpoint(store, 'defense:publication', 'public-article-publication');
      result = await (input.publisher || Publisher).publish(store, candidate, decision.actionId, now);
    }
    catch (error) { result = { ok: false, providerCalled: false,
      definitiveFailure: error && error.code === AdapterGuard.INHIBITED,
      ambiguous: error && error.code !== AdapterGuard.INHIBITED,
    ...
    if (!restoredAction || restoredAction.status !== command.status) throw new Error('defense publication action receipt readback invalid');
    ...
    return { ok: false, status: 'REFUSED', accepted: 0, reason: 'defense-publication-strict-boundary-unavailable', detail: String(error && error.message || error), providerCalls: 0, liveMoney: false };
  }
}
