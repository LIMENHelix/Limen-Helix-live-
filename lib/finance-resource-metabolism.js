...
// One logical decision route may make one primary transport attempt and one
// alternate-provider attempt after a non-generating availability failure.
var HARD_PROVIDER_CALLS_PER_PACKET = 2;
// Must match the sandbox motor ceiling. Tradier documents whole-number equity
// quantities, so the paper metabolism admits one share up to this bound rather
// than pretending fractional execution is available.
...
function evaluate(input) {
  ...
  if (motor.cashOnly !== true || motor.longOnly !== false || motor.marginAllowed !== false ||
      motor.optionsAllowed !== false || motor.shortingAllowed !== true ||
      motor.shortCollateralPolicy !== 'tradier-sandbox-preview-required') {
