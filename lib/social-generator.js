...
async function environment() {
  ...
  if (!j || !Array.isArray(j.rows) || !j.rows.length || !j.totalAcres) return null;
  var worst = j.rows[0];
  if (!worst.name || !worst.acres) return null;
  return {
    domain: 'environment',
    text: j.rows.length + ' wildfires over 100 acres are burning right now, '
      + j.totalAcres.toLocaleString() + ' acres between them.\n\n'
      + 'Largest: ' + worst.name + ' in ' + (worst.state || 'the US') + ', '
      + worst.acres.toLocaleString() + ' acres.\n\nAir quality at your ZIP:\n' + utm('environment')
  };
}
...
async function medicine() {
  ...
  return {
    domain: 'medicine',
    text: j.total.toLocaleString() + ' drugs are in active FDA shortage right now.\n\n'
