...
async function get(path) {
  ...
  if (!(r.status === 200 && r.body && r.body.ok !== false)) return null;
  var body = r.body;
  Object.defineProperty(body, '__limenSource', {
    enumerable: false,
    value: {
      kind: 'limen-live-tool-response',
      value: API + path,
      subjectDomain: null,
...
function sourced(post, body) {
  if (!post || !body || !body.__limenSource) return post;
  post.sourceIdentity = Object.assign({}, body.__limenSource, { subjectDomain: post.domain });
  return post;
}
...
async function economy() {
  ...
  return sourced({
    domain: 'economy',
    // "this year" reads as a calendar year; the handler's own note says fiscal year TO DATE.
    text: cents + ' cents of every dollar Washington collected so far this fiscal year went to '
      + 'interest on debt run up before you paid it.\n\nIt spent $' + per + ' for every $1 it took in.\n\n'
...
async function population() {
  ...
  // The series is MEDLISPRIUS, the median LISTING price. "costs" claims a transaction price
  // that this number is not; the handler's caveat says plainly it is what sellers ASK.
  var line = 'The typical US home is listed at ' + now + ' years of the median household income.';
  if (then) line += '\n\nIn ' + j.thenYear + ' it was ' + then + '.';
  line += '\n\nLenders used to call 3x affordable.\n\nCheck your state:\n' + utm('population');
  return sourced({ domain: 'population', text: line }, j);
}
...
async function technology() {
  ...
  return sourced({
    domain: 'technology',
    // KEV is a cumulative catalogue of CONFIRMED exploitation, not a live "right now" tally.
    text: j.total.toLocaleString() + ' software flaws have confirmed exploitation in the wild. '
...
async function agriculture() {
  ...
  // The two price moves are sourced. WHO captured the spread is not shown by them, and
  // fertiliser lags gas by months, so the obvious accusation has an innocent explanation the
  // data cannot rule out. The divergence is the hook on its own; stating it and stopping is
  // the version that survives a reply from someone who works in the industry.
  return sourced({
    domain: 'agriculture',
...
/**
 * Build every domain's post at once, for the operator review board.
 *
 * Runs the builders in PARALLEL. Sequentially this is eleven round trips to our own tool
 * endpoints and would run past the function timeout; in parallel it is one round trip wide.
 * A domain with no live data comes back ok:false with its reason rather than being dropped,
 * because "this one would post nothing today" is exactly what the reviewer needs to see.
 */
async function previewAll() {
  return Promise.all(BUILDERS.map(async function (b) {
    var post = null;
    try { post = await b.fn(); } catch (e) {
      return { domain: b.name, ok: false, reason: 'Source error: ' + (e.message || 'unknown') };
    }
    if (!post || !post.text) {
      return { domain: b.name, ok: false, reason: 'No live data for this domain right now, so it would post nothing.' };
    }
    var len = social.graphemeLength(post.text);
    if (len > social.MAX_GRAPHEMES) {
      return { domain: b.name, ok: false, length: len, text: post.text,
               reason: 'Too long: ' + len + ' of ' + social.MAX_GRAPHEMES + ' graphemes, so it would be skipped.' };
    }
    return { domain: b.name, ok: true, text: post.text, length: len, links: social.buildFacets(post.text).length };
  }));
}
...
module.exports = {
  ...
  BUILDERS: BUILDERS.map(function (b) { return b.name; })
};
