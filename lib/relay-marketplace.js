...
async function createListing(data) {
  ...
  const l = {
    ...
    saleMode: data.saleMode || 'fixed-price',
    assetRef: data.assetRef || null,
    assetRefHash: data.assetRefHash || null,
    contentHash: data.contentHash || null,
    auctionEndsAt: data.auctionEndsAt || null,
    bindingSaleAuthorized: data.bindingSaleAuthorized === true,
    orderAcceptanceAuthorized: data.orderAcceptanceAuthorized === true,
    paymentAuthorized: data.paymentAuthorized === true,
    // Sourcing provenance. Set by relay-engine when it publishes an auto-sourced item;
    // null on a human seller's own listing. Fulfilment cannot buy without sourceUrl.
