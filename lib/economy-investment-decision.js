...
var SCHEMA = 'economy-investment-decision/1.0';
var CANDIDATE_SCHEMA = 'economy-investment-candidate/1.0';
var LOG_KEY = 'economy_investment_decision_log';
var PREFIX = 'economy_investment_decision:';
...
function candidate(input) {
  ...
  return {
    schemaVersion: CANDIDATE_SCHEMA, productDomain: 'economy', ownerDomain: 'economy', lane: 'investments',
...
function validate(value) {
  return !!(value && value.schemaVersion === CANDIDATE_SCHEMA && value.productDomain === 'economy' && value.ownerDomain === 'economy' &&
...
function validBrain(envelope, now) {
  ...
  return !!(cognition && cognition.domain === 'economy' && Number.isFinite(ts) && now >= ts && now - ts <= MAX_BRAIN_AGE_MS &&
    packet && packet.schemaVersion === 'civilization-domain-packet/1.0' && packet.domainId === 'economy' &&
...
function feedMatches(titleSets, value, now) {
  ...
  value.feedEvidence.forEach(function (wanted) {
    ...
    rows.forEach(function (set) {
      ...
      if (found || !set || set.d !== 'economy' || !Number.isFinite(recorded) || now < recorded || now - recorded > MAX_FEED_AGE_MS || !Array.isArray(set.items)) return;
...
async function persist(store, receipt) {
  ...
  if (!restored || restored.actionId !== receipt.actionId || restored.status !== receipt.status) throw new Error('economy investment decision readback invalid');
...
async function decide(store, value, now, deps) {
  ...
  if (!validate(value)) return { ok: true, status: 'NO_ACTION', reason: 'economy-investment-candidate-invalid', blockers: ['exact-paper-investment-record-required'], liveMoney: false };
  try {
    store.assertDurable();
    var envelope = deps && deps.cognition, titleSets = deps && deps.titleSets;
    if (!validBrain(envelope, at)) blockers.push('economy-brain-state-missing-or-stale');
    var cognition = envelope && envelope.c || {}, packet = cognition.serverPacket || {}, organs = cognition.brainOrgans || {};
    if (!blockers.length) {
      if ((cognition.immune || {}).immuneState !== 'clear') blockers.push('economy-immune-veto');
      if ((cognition.awareness || {}).humanReviewRequired === true) blockers.push('economy-human-review-veto');
      var emission = organs.autonomousInternalEmission || {};
      if (emission.holdReason) blockers.push('economy-b10-brake-held:' + emission.holdReason);
      if (!(Number(emission.emittedCount) > 0)) blockers.push('economy-b10-no-action-selected');
      if (!(Number(packet.truth && packet.truth.feedHealth && packet.truth.feedHealth.live) > 0)) blockers.push('economy-live-feeds-unavailable');
      ...
      if (!opportunity) blockers.push('economy-exact-investable-opportunity-not-selected');
      var feeds = feedMatches(titleSets, value, at);
      if (!feeds.ok) blockers.push('economy-exact-current-feed-evidence-not-confirmed');
      var metabolism = organs.resourceMetabolism || {}, gates = metabolism.gates || {};
      if (metabolism.state !== 'AVAILABLE' || gates.mayRunInternalCycle !== true) blockers.push('economy-resource-metabolism-inhibited');
      var configuredMax = Number(deps && deps.maxNotionalUsd);
      if (!Number.isFinite(configuredMax) || configuredMax <= 0) blockers.push('economy-investment-max-notional-not-configured');
      else if (value.maxNotionalUsd > configuredMax) blockers.push('economy-investment-max-notional-exceeded');
    }
    var actionId = 'eia_' + hash({ request: value.requestId, symbol: value.symbol, side: value.side, thesis: value.thesisId, evidence: value.evidenceHash }).slice(0, 24);
    var status = blockers.length ? 'NO_ACTION' : 'RELEASED';
    return persist(store, {
      schemaVersion: SCHEMA, decisionReceiptId: 'eid_' + hash({ action: actionId, packet: packet.packetId || null, status: status, blockers: blockers }).slice(0, 24),
      actionId: actionId, status: status, released: status === 'RELEASED', reason: blockers.length ? 'economy-b10-held' : null, blockers: blockers,
      productDomain: 'economy', ownerDomain: 'economy', lane: 'investments', decisionContract: 'capital-decision/1',
      requestId: value.requestId, symbol: value.symbol, side: value.side, thesisId: value.thesisId, evidenceHash: value.evidenceHash,
      economyPacketId: packet.packetId || null, brainOpportunityId: value.brainOpportunityId,
    ...
    return { ok: true, status: 'NO_ACTION', reason: 'economy-b10-unavailable', blockers: ['decision-persistence-or-input-unavailable'], detail: String(error && error.message || error), liveMoney: false };
  }
}
function validateReceipt(receipt, value, now) {
  ...
  return !!(validate(value) && receipt && receipt.schemaVersion === SCHEMA && receipt.status === 'RELEASED' &&
    receipt.actionId === 'eia_' + hash({ request: value.requestId, symbol: value.symbol, side: value.side, thesis: value.thesisId, evidence: value.evidenceHash }).slice(0, 24) &&
    receipt.productDomain === 'economy' && receipt.ownerDomain === 'economy' && receipt.lane === 'investments' &&
