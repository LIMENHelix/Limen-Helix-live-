...
function fromBrainState(domainId, state, snapshotMeta, refreshId, now) {
  ...
  return buildPacket({
    ...
    truth: {
      ...
      opportunities: (Array.isArray(state.opportunities) ? state.opportunities : []).slice(0, MAX_ITEMS).map(normalizeOpportunity),
