...
var RESEARCH_OWNERS = ['research', 'health', 'science', 'medicine'];
...
function buildResearchPublication(input) {
  ...
  if (RESEARCH_OWNERS.indexOf(owner) < 0) fail('OUTCOME_RESEARCH_OWNER_REQUIRED', 'research ownerDomain must be research, health, science, or medicine');
...
function buildInvestmentPnl(input, options) {
  ...
  var derivedNet = terms.positionMarketValue - invested - terms.fees;
  var derivedReturn = invested > 0 ? (derivedNet / invested) * 100 : NaN;
  var derivedBenchmarkReturn = input.benchmarkBaselineValue > 0
