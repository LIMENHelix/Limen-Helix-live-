...
var RESEARCH_OWNERS = ['research', 'health', 'science', 'medicine', 'education', 'environment'];
var INVESTMENT_OWNERS = ['finance', 'economy', 'energy', 'technology'];
...
function base(input, lane, owner) {
  ...
  if (!RESEARCH_OWNERS.includes(owner) && !INVESTMENT_OWNERS.includes(owner)) fail('OUTCOME_OWNER_UNSUPPORTED', 'ownerDomain is not an active research/investment owner');
  var event = {
...
function buildInvestmentPnl(input, options) {
  ...
  if (INVESTMENT_OWNERS.indexOf(owner) < 0) fail('OUTCOME_INVESTMENT_OWNER_REQUIRED', 'investment ownerDomain must be finance, economy, energy, or technology');
  var event = base(input, 'investment', owner);
  var d = input.outcomeData || {};
  if (HORIZONS.indexOf(d.horizonDays) < 0) fail('OUTCOME_HORIZON_REQUIRED', 'horizonDays must be 1, 3, 7, 14, 30, 60, or 90');
...
module.exports = {
  ...
  INVESTMENT_OWNERS: INVESTMENT_OWNERS.slice(),
  INVESTMENT_HORIZONS: HORIZONS.slice(),
