...
function valid(contract, state, artifact, now) {
  ...
  // A reflex cadence/cap abstention regulates *new production*; it must not
  // erase fresh inventory prepared from the same last accepted plan.  Without
  // this continuity rule, the next reflex tick strands a valid artifact until
  // its freshness window expires simply because no second artifact may be
  // created yet.  Only the three non-safety abstentions below can preserve an
  // artifact.  Feed, cognition, immune/human-review, identity, and metabolism
  // holds still fail closed and the subject-domain release re-reads current
  // cognition immediately before any public effect.
  var continuityReasons = [
    'no-meaningful-afferent-or-stress-change',
    'commercial-cadence-inhibited',
    'commercial-daily-artifact-cap-reached'
  ];
  var unchangedPlan = state.status === 'ABSTAINED' && continuityReasons.indexOf(state.reason) >= 0 &&
    text(state.evidenceFingerprint) && state.evidenceFingerprint === artifactEvidenceFingerprint &&
    text(state.lastPlannedIntentId) && state.lastPlannedIntentId === artifact.intentId;
...
function render(contract, state, artifact, nowValue) {
  ...
  var rawStress = interoception.stress == null ? artifact.sourceStress : interoception.stress;
  var stress = rawStress == null ? null : Number(rawStress);
  var delta = Number(interoception.delta);
  var stressText = stress != null && Number.isFinite(stress) ? Math.round(stress * 100) + '%' : 'unavailable';
  ...
  var candidateHash = hash({ domain: contract.productDomain, contentHash: artifact.contentHash, text: body });
  return {
    ok: true,
