...
(function (root) {
  ...
  function inferArchetypes(opp) {
    ...
    if (/buy|convert|resale|flip|inventory|refurbish|arbitrage/.test(text)) set['product-flip'] = 1;
    if (/own|platform|acquire|position|fund/.test(text)) set['owned-operation'] = 1;
