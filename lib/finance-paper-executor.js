...
function selectedActionId(selection) {
  var released = selection && selection.criticDecision && selection.criticDecision.released;
  var candidateKindId = released && (released.candidateId || released.candidate && released.candidate.id) || null;
  return candidateKindId && selection.id || null;
}
...
async function execute(options) {
  ...
  try {
    ...
    var expectedActionId = selectedActionId(decision.selection);
    var actionId = previewResult.preview && previewResult.preview.intent && previewResult.preview.intent.actionId;
    if (!expectedActionId) {
      var selectionIdentityError = new Error('Finance B10 release has no causal action identity');
      selectionIdentityError.code = 'FINANCE_PAPER_B10_ACTION_ID_MISSING';
      throw selectionIdentityError;
    }
    if (!actionId || actionId !== expectedActionId) {
      var actionError = new Error('Finance B14 preview action identity does not match the B10 release');
      actionError.code = 'FINANCE_PAPER_PREVIEW_ACTION_ID_MISMATCH';
      throw actionError;
    }
