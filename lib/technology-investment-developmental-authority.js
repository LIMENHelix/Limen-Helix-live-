...
var SLOT_KEY = 'technology_investment_developmental_slot:1';

function enabled(env) {
  var value = env && env.LIMEN_TECHNOLOGY_INVESTMENT_DEVELOPMENTAL_ENABLED;
...
async function authorize(store, requestId, env, now) {
  if (!enabled(env)) return held('technology-investment-developmental-switch-off');
  store.assertDurable();
  ...
  var motor = await store.get(MotorReceipt.receiptKey('technology'));
  if (!motor || motor.schemaVersion !== MotorReceipt.SCHEMA || motor.productDomain !== 'technology' || motor.ownerDomain !== 'technology' ||
      motor.contractId !== 'technology-motor/1' || motor.lane !== 'investments') return held('technology-motor-receipt-identity-invalid', motor);
  var at = Number(now) || Date.now();
  var slot = { schemaVersion: 'technology-investment-developmental-authority/1.0', slot: 1, requestId: requestId,
  ...
  if (!made) {
    ...
    if (!slot || slot.requestId !== requestId) return held('technology-developmental-paper-order-cap-reached', slot);
  }
  return { ok: true, authorized: true, status: 'AUTHORIZED_DEVELOPMENTAL_PAPER', authorizationMode: 'developmental-paper-commissioning',
    receiptId: 'technology-developmental-slot-1', productDomain: 'technology', ownerDomain: 'technology', contractId: motor.contractId,
