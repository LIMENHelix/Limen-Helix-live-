...
var JOBS = [
  ...
  { job: 'communication-social-capability', source: 'vercel', schedule: '9,39 * * * *', path: '/api/communication-social-capability', kind: 'outward', role: 'verification', note: 'One-shot bounded Bluesky create -> independent AppView read -> delete -> independent absence proof; never selects or publishes domain content' },
  ...
  { job: 'domain-subscriber-fulfillment', source: 'vercel', schedule: '*/10 * * * *', path: '/api/domain-subscriber-fulfillment', kind: 'outward', role: 'efference', note: 'Shared transport clock retries seven exact-domain paid fulfillment queues per cycle; each domain retains its own B10/B14, budget, valve and receipt namespace' },
  ...
  { job: 'subscriber-digest',          source: 'vercel', schedule: '30 * * * *',                path: '/api/subscriber-digest',          kind: 'outward', role: 'motor',      note: 'Emails subscribers only when exact-domain content changes' },
