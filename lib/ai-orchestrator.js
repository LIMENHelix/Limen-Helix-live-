...
// Budget counter is stored in Redis via limen-db, NOT on disk. (Prior version wrote
// to a file under the deployment dir, which is READ-ONLY on Vercel — every write
// failed silently, so tokensUsed reset to 0 on every invocation and the per-tick cap
// never actually enforced. 2026-07-13 fix: Redis-backed so the cap holds across
// invocations. `_lastBudget` mirrors the last-seen value for the sync status() path.)
const db = require('./limen-db');
const BUDGET_KEY = 'ai:budget';   // limen-db prefixes with "limen:"

function _now() { return new Date().toISOString(); }
function _fresh() { return { tickStartedAt: _now(), tokensUsed: 0, callsMade: 0, byProvider: {}, lastCallAt: null }; }
let _lastBudget = _fresh();

async function _loadBudget() {
  try {
    const b = await db.get(BUDGET_KEY);
    if (b && b.tickStartedAt) {
      // rotate tick window — reset counters after an hour
      const ageMin = (Date.now() - new Date(b.tickStartedAt).getTime()) / 60000;
      if (ageMin > 60) { _lastBudget = _fresh(); return _lastBudget; }
      _lastBudget = b;
      return b;
    }
  } catch (e) { /* Redis unreachable — fall through to a fresh window */ }
  _lastBudget = _fresh();
  return _lastBudget;
}
async function _saveBudget(b) { _lastBudget = b; try { await db.set(BUDGET_KEY, b); } catch (e) {} }

async function _record(provider, tokensEstimate) {
  const b = await _loadBudget();
...
async function _callAnthropic(opts) {
  ...
  await _record('anthropic', tokensIn + tokensOut);
...
async function _callOpenAI(opts) {
  ...
  await _record('openai', tokensIn + tokensOut);
...
async function _callGrok(opts) {
  ...
  const tokensIn = (j.usage && j.usage.prompt_tokens) || 0;
  const tokensOut = (j.usage && j.usage.completion_tokens) || 0;
  await _record('grok', tokensIn + tokensOut);
  const text = (j.choices && j.choices[0] && j.choices[0].message && j.choices[0].message.content) || '';
...
async function call(intent, opts) {
  // Kill switch — env boundary (LIMEN_AI_ENABLED=1) AND the operator's instant runtime pause
  // (ai:spend:paused). Either one blocks all autonomous paid-AI billing.
  if (await require('./ai-kill-switch').spendDisabled()) return { ok: false, disabled: true, error: 'AI spend disabled (env kill switch or operator pause)' };
  const gate = await _budgetGate();
...
// Sync status — best-effort budget from the in-memory mirror (accurate within a warm
// lambda after any call). For the authoritative live counter, use statusAsync().
function status() {
  return { providers: _providersAvailable(), budget: _lastBudget, limits: { maxTokensPerTick: MAX_TOKENS_PER_TICK, maxCallsPerTick: MAX_CALLS_PER_TICK } };
}
// Async status — reads the live Redis budget counter (authoritative across invocations).
async function statusAsync() {
  return { providers: _providersAvailable(), budget: await _loadBudget(), limits: { maxTokensPerTick: MAX_TOKENS_PER_TICK, maxCallsPerTick: MAX_CALLS_PER_TICK } };
}

module.exports = { call, status, statusAsync, _callAnthropic, _callOpenAI, _callGrok };
