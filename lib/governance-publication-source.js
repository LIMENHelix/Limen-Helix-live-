...
var SCHEMA = 'governance-publication-candidate/1.0';
...
function collect(titleSets, now) {
  ...
  (Array.isArray(titleSets) ? titleSets : []).forEach(function (set) {
    ...
    if (!set || set.d !== 'governance' || !recordedMs || at < recordedMs || at - recordedMs > MAX_TITLE_SET_AGE_MS ||
    ...
    set.items.forEach(function (item, index) {
      ...
      seen[item.au] = true;
      var identity = 'governance:' + String(set.f) + ':' + String(set.hh) + ':' + String(item.i == null ? index : item.i) + ':' +
        hash({ url: String(item.au), title: String(item.ti) }).slice(0, 16);
      sources.push({
...
function build(titleSets, cognition, now) {
  ...
  if (!packet || packet.schemaVersion !== 'civilization-domain-packet/1.0' || packet.domainId !== 'governance' ||
  ...
  var title = 'Governance Source Watch — ' + day;
  var summary = 'An automated, source-linked watch of recently observed Governance feed headlines and LIMEN model state.';
  var disclaimer = 'This source watch records what public feeds published. It does not independently verify policy claims, infer official intent, predict elections or legislation, or recommend an investment, vote, or lobbying action.';
  var body = [
    summary,
    disclaimer,
  ...
  return {
    ...
    productDomain: 'governance',
    ownerDomain: 'governance',
    lane: 'publication',
    candidateId: 'gpc_' + hash({ day: day, source: sourceFingerprint, selected: selected.id }).slice(0, 24),
    title: title,
    summary: summary,
    body: body,
    ...
    governancePacketId: packet.packetId,
...
function validate(candidate) {
  return !!(candidate && candidate.schemaVersion === SCHEMA && candidate.productDomain === 'governance' &&
    candidate.ownerDomain === 'governance' && candidate.lane === 'publication' && text(candidate.candidateId) &&
    ...
    candidate.contentHash === hash({ title: candidate.title, summary: candidate.summary, body: candidate.body, sources: candidate.sources,
      packetId: candidate.governancePacketId, selectedId: candidate.brainSelection && candidate.brainSelection.id }));
}
