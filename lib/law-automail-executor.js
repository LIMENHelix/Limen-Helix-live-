...
var Learning = require('./law-automail-learning.js');
...
async function execute(input) {
  ...
  try { store.assertDurable(); var authorize = input.motorAuthorization || Motor, motor = await authorize.authorize(store, 'law', 'automail', now);
    ...
    command.budgetSlot = budgetClaim.slot; var learnedCause = await Learning.recordCommand(store, command);
    if (!learnedCause || learnedCause.ok !== true) throw new Error('law automail causal memory unavailable');
    ...
    var result; try {
      command.adapterGuard = await (input.adapterGuard || AdapterGuard).checkpoint(store, 'law:automail', 'lob-physical-mail-create');
      result = await input.provider.create(candidate, idempotencyKey);
