 ...
 * keep working.
 */
// payment-review is a safety hold, not an intermediate state on the ordinary payment
// path.  A stale exact-payment writer must never clear it back to paid and let the paid
// sweep buy an order whose charge is disputed.  There is no automatic transition out of
// review; a future explicit operator-resolution command must be a separate, audited API.
const STATUS_RANK = { pending: 0, 'awaiting-payment': 1, 'payment-failed': 1, paid: 2, shipped: 3, 'payment-review': 4 };
...
function _mergeFulfillment(existing, incoming) {
  if (!existing || typeof existing !== 'object') return incoming;
  if (!incoming || typeof incoming !== 'object') return existing;

  const oldLines = Array.isArray(existing.lines) ? existing.lines : [];
  const newLines = Array.isArray(incoming.lines) ? incoming.lines : [];
  if (!oldLines.length || !newLines.length) {
    return _rank(FULFILL_RANK, incoming.state) < _rank(FULFILL_RANK, existing.state)
      ? existing : Object.assign({}, existing, incoming);
  }

  // CAS serializes map writes, but the update object may have been computed from an old
  // order snapshot. Merge per-line progress against the fresh row seen inside the CAS so
  // two reconciliation workers buying different lines cannot erase one another.
  const byListing = new Map();
  const order = [];
  oldLines.forEach(function (line) {
    if (!line || !line.listingId) return;
    if (!byListing.has(line.listingId)) order.push(line.listingId);
    byListing.set(line.listingId, line);
  });
  newLines.forEach(function (line) {
    if (!line || !line.listingId) return;
    const prior = byListing.get(line.listingId);
    if (!prior) {
      order.push(line.listingId);
      byListing.set(line.listingId, line);
      return;
    }
    const oldRank = _rank(FULFILL_RANK, prior.state);
    const newRank = _rank(FULFILL_RANK, line.state);
    byListing.set(line.listingId, oldRank >= 0 && newRank >= 0 && newRank < oldRank
      ? prior : Object.assign({}, prior, line));
  });

  const lines = order.map(function (id) { return byListing.get(id); });
  const purchased = lines.filter(function (line) { return line && line.state === 'purchased'; });
  let state;
  if (lines.length && purchased.length === lines.length) state = 'purchased';
  else if (purchased.length) state = 'partial';
  else {
    state = _rank(FULFILL_RANK, incoming.state) >= _rank(FULFILL_RANK, existing.state)
      ? incoming.state : existing.state;
  }

  const merged = Object.assign({}, existing, incoming, { state: state, lines: lines });
  const margins = purchased.map(function (line) { return Number(line.margin); });
  if (margins.length && margins.every(Number.isFinite)) {
    merged.margin = _round(margins.reduce(function (sum, value) { return sum + value; }, 0));
  }
  return merged;
}
...
function _applyOrderUpdate(existing, updates) {
  ...
  if (updates && updates.fulfillment) {
    next.fulfillment = _mergeFulfillment(existing && existing.fulfillment, updates.fulfillment);
    // A pair of concurrent partial repairs can become complete only inside this fresh CAS
    // view. Promote paid to shipped here so the aggregate cannot remain paid/partial after
    // every line has in fact been recovered. A payment-review hold is never bypassed.
    if (next.fulfillment && next.fulfillment.state === 'purchased' && next.status === 'paid') {
      next.status = 'shipped';
    }
  }
  return next;
