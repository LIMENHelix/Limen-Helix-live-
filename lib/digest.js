...
async function latestCommercialArtifact(domain, store, nowValue) {
  ...
  try {
    store.assertDurable();
    var artifact = await store.get(contract.artifactStateKey);
    if (!artifact || artifact.schemaVersion !== 'domain-commercial-artifact/1.0' ||
        ...
        !artifact.subject || !artifact.body || !artifact.contentHash ||
...
async function buildFor(sub, options) {
  ...
  if (PERSONAL[domain] && watch) {
    ...
    // Domain-wide buyers receive the newest durable artifact made by this
    // domain's own stress-to-business reflex. Legacy social text remains a
    // fail-soft fallback while artifact inventory is first commissioned.
...
module.exports = { buildFor: buildFor, latestCommercialArtifact: latestCommercialArtifact,
  isPersonal: isPersonal, PERSONAL_DOMAINS: Object.keys(PERSONAL) };
