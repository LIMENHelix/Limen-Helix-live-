...
async function preflight(store, workerId, body, nowValue, env, deps) {
  ...
  var armed = Object.assign({}, command, {
    status: 'PROVIDER_CALL_AUTHORIZED', uploadAuthorizationId: authorizationId,
    providerPermit: { token: permitToken, authorizedAt: now, expiresAt: authorization.expiresAt },
    uploadAdapterGuard: guard, externalEffectAuthorized: true,
    providerCalled: false, uploaderCalled: false
  });
  if (!await store.replaceIfValue(Command.key(command.commandId), command, armed)) {
    return { ok: false, status: 'REFUSED', reason: 'upload-preflight-raced-or-ambiguous',
      providerCalled: null, externalEffectAuthorized: false, liveMoney: false };
  }
  await store.setIfAbsent(authKey(command.commandId), authorization);
  var restoredAuth = await store.get(authKey(command.commandId));
  var restoredCommand = await store.get(Command.key(command.commandId));
  if (!authorizationValid(restoredAuth, restoredCommand, now) ||
      !restoredCommand.providerPermit || restoredCommand.providerPermit.token !== permitToken) {
  ...
  return { ok: true, status: 'PROVIDER_CALL_AUTHORIZED', commandId: command.commandId,
    authorizationId: authorizationId, providerPermitToken: permitToken,
