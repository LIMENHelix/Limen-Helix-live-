...
var PARSER_VERSION = 'wjp-rule-of-law/1.0.0';
...
function parseRows(rows, workbookMeta) {
  ...
  var required = Object.keys(FIELDS).map(function (k) { return FIELDS[k]; });
  ...
  rows.slice(1).forEach(function (r) {
    ...
    if (!country || !region || !/^(?:2012-2013|20\d{2})$/.test(yearText)) {
      problems.push({ row: r.number, field: 'identity', country: country, region: region, year: yearText }); return;
    }
    var year = yearText === '2012-2013' ? 2012 : Number(yearText);
    ...
    observations.push({
      countryYear: normaliseHeader(v[positions[FIELDS.countryYear]]),
