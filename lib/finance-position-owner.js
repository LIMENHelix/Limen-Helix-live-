...
var Provider = require('./finance-position-owner-provider.js');
...
function postDecisionReconciliation(context) {
  ...
  var news = (context && Array.isArray(context.currentIssuerEvidence) ? context.currentIssuerEvidence : []).slice(0, 12).map(function (row) {
    return {
  ...
  return {
    ...
    note: 'Thing 2 may identify possible masking. Current exact-issuer news is displayed first for comparison and cannot directly determine HOLD, SELL, or COVER.'
  };
}
...
async function execute(options) {
  ...
  try {
    ...
    var candidateKindId = releasedAction && (releasedAction.candidateId || releasedAction.candidate && releasedAction.candidate.id);
    if (!candidateKindId || !selected.selection.id) {
      var actionIdentityError = new Error('Finance position B10 release has no causal selection identity');
      actionIdentityError.code = 'FINANCE_POSITION_ACTION_ID_MISSING';
      throw actionIdentityError;
    }
    var actionId = selected.selection.id;
    ...
    intentResult.intent.decisionContext.criticCandidateId = candidateKindId;
    ...
    var command = await b14.submitApproved(store, broker, {
      ...
      approval: {
        mode: 'domain-autonomous', actor: 'finance-brain', ownerDomain: 'finance',
        authorizationReceiptId: motor.receiptId,
