...
var SYSTEM = [
  ...
  'Thing 2 only reconciles Thing 1 with the long-arc snapshot for alignment, divergence, or possible masking. Copy the supplied reconciliation status. Its direct decision weight is exactly zero and it cannot support HOLD, SELL, COVER, confidence, sizing, or leverage.',
...
function parseProposal(raw, context) {
  ...
  if (!value || value.thing2Observed !== !!(context && context.helixReport && context.helixReport.thing2 && context.helixReport.thing2.observed)) blockers.push('position_owner_thing2_observation_acknowledgement_required');
  if (!value || value.thing2DecisionWeight !== 0) blockers.push('position_owner_thing2_decision_weight_must_be_zero');
  if (!value || value.thing2Role !== 'alignment_and_masking_reconciliation_only') blockers.push('position_owner_thing2_role_invalid');
  var expectedReconciliation = context && context.helixReport && context.helixReport.thing2 && context.helixReport.thing2.maskingAssessment || 'unassessed';
  if (!value || value.thing2ReconciliationStatus !== expectedReconciliation) blockers.push('position_owner_thing2_reconciliation_acknowledgement_required');
