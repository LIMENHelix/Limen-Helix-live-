...
var INDEX_KEY = 'governance_publication_public_index';
var ARTICLE_PREFIX = 'governance_publication_article:';
...
async function publish(store, candidate, actionId, now) {
  ...
  var articleId = 'gov_' + Source.hash({ action: actionId, content: candidate.contentHash }).slice(0, 24);
  var article = {
    schemaVersion: 'governance-publication-article/1.0',
    articleId: articleId,
    actionId: actionId,
    ...
    publicPath: '/governance-briefs?id=' + encodeURIComponent(articleId),
    providerCalled: false,
    liveMoney: false
  };
  var created = await store.setIfAbsent(articleKey(articleId), article);
  var restored = await store.get(articleKey(articleId));
  if (!restored || restored.articleId !== articleId || restored.contentHash !== candidate.contentHash || restored.published !== true) throw new Error('governance publication article readback invalid');
  if (created) {
...
async function unpublish(store, articleId, recoveryId, now) {
  ...
  var restored = await store.get(articleKey(articleId));
  if (!restored || restored.published !== false || restored.recoveryId !== recoveryId) throw new Error('governance publication unpublish readback invalid');
