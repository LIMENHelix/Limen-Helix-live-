...
async function produce(stream, brand) {
  ...
  let retrieved = null, retrieveProvider = null;
  try {
  ...
  } catch (e) { /* retrieval optional — proceed without it */ }
  ...
  const prompt = 'Stream: ' + (stream.name || stream.id) + '\nCategory: ' + (stream.category || '') + '\nConnector: ' + (stream.connector || '')
    ...
    + (retrieved ? ('\nCurrent context (use what is relevant, ignore the rest):\n' + retrieved) : '')
  ...
  const artifact = {
    ...
    provenance: { retrieve: retrieveProvider, author: draftRes.provider, verify: verifyProvider },
