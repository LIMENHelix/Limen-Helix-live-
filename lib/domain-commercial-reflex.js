...
function homologousState(cognition, truth, evidence, priorState, now) {
  ...
  var publicOutcome = cognition.brainOrgans && cognition.brainOrgans.commercialReflex &&
    cognition.brainOrgans.commercialReflex.publicSocialOutcome || null;
  var outcomeReady = !!(publicOutcome && publicOutcome.status === 'ELIGIBLE' &&
    publicOutcome.learningGate && publicOutcome.learningGate.ready === true && finite(publicOutcome.normalizedCredit));
  if (outcomeReady) predictionError = Math.max(predictionError, clamp01(1 - publicOutcome.normalizedCredit));
  ...
  return {
    ...
    publicSocialOutcome: publicOutcome ? clone(publicOutcome) : null,
    publicSocialPolicyEligible: outcomeReady,
    salience: salience,
...
function evaluate(contract, cognitionRecord, priorState, nowValue) {
  ...
  var intent = {
    ...
    adaptationContext: {
      publicSocialOutcome: neural.publicSocialOutcome,
      policyChangeEligible: neural.publicSocialPolicyEligible,
      rule: 'engagement-adjusts-format-and-distribution-only-never-factual-truth'
    },
    renderContract: {
  ...
  return {
    ...
    homology: {
      ...
      reafference: neural.publicSocialOutcome && neural.publicSocialOutcome.status === 'ELIGIBLE'
        ? { source: 'independent-public-appview', outcome: neural.publicSocialOutcome,
          eligibleForPolicyChange: neural.publicSocialPolicyEligible }
        : 'AWAITING_INDEPENDENT_EXTERNAL_OUTCOME'
    },
    intent: intent
  };
}
