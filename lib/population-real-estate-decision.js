...
var SCHEMA = 'population-real-estate-decision/1.0';
var CANDIDATE_SCHEMA = 'population-real-estate-candidate/1.0';
var LOG_KEY = 'population_real_estate_decision_log';
var PREFIX = 'population_real_estate_decision:';
...
function validCandidate(value) {
  return !!(value && value.schemaVersion === CANDIDATE_SCHEMA && value.productDomain === 'population' &&
    value.ownerDomain === 'population' && value.lane === 'real-estate' && text(value.inquiryId) &&
...
async function brainEntry(deps) {
  ...
  return (deps && deps.redisGet || Redis.redisGet)('limen:brain:cognition:population');
}
...
function validBrain(entry, now) {
  ...
  return !!(cognition && cognition.domain === 'population' && Number.isFinite(ts) && now >= ts &&
    now - ts <= MAX_BRAIN_AGE_MS && packet && packet.schemaVersion === 'civilization-domain-packet/1.0' &&
    packet.domainId === 'population' && packet.sourceIdentity &&
...
async function persist(store, receipt) {
  ...
  if (!restored || restored.actionId !== receipt.actionId || restored.status !== receipt.status) throw new Error('population real-estate decision readback invalid');
...
async function decide(store, value, now, deps) {
  ...
  if (!validCandidate(value)) return { ok: true, status: 'NO_ACTION', released: false, reason: 'population-real-estate-candidate-invalid', blockers: ['exact-non-binding-property-interest-record-required'], providerCalled: false, liveMoney: false };
  try {
    store.assertDurable();
    var entry = await brainEntry(deps);
    var blockers = [];
    if (!validBrain(entry, at)) blockers.push('population-brain-state-missing-or-stale');
    var cognition = entry && entry.c || {};
    var packet = cognition.serverPacket || {};
    var organs = cognition.brainOrgans || {};
    if (!blockers.length) {
      if ((cognition.immune || {}).immuneState !== 'clear') blockers.push('population-immune-veto');
      if ((cognition.awareness || {}).humanReviewRequired === true) blockers.push('population-human-review-veto');
      var emission = organs.autonomousInternalEmission || {};
      if (emission.holdReason) blockers.push('population-b10-brake-held:' + emission.holdReason);
      if (!(Number(emission.emittedCount) > 0)) blockers.push('population-b10-no-action-selected');
      if (!packet.truth || !packet.truth.feedHealth || !(Number(packet.truth.feedHealth.live) > 0)) blockers.push('population-live-feeds-unavailable');
      ...
      if (!opportunity) blockers.push('population-exact-brain-opportunity-not-selected');
      var maxIndicationUsd = Number(deps && deps.maxIndicationUsd);
      if (!Number.isFinite(maxIndicationUsd) || maxIndicationUsd <= 0) blockers.push('population-real-estate-indication-cap-not-configured');
      else if (value.indicationPriceUsd > maxIndicationUsd) blockers.push('population-real-estate-indication-exceeds-cap');
      var metabolism = organs.resourceMetabolism || {}, gates = metabolism.gates || {};
      if (metabolism.state !== 'AVAILABLE' || gates.mayRunInternalCycle !== true) blockers.push('population-resource-metabolism-inhibited');
    }
    var actionId = 'pra_' + hash({ inquiryId: value.inquiryId, counterparty: value.counterpartyEmailHash, property: value.propertyRefHash,
    ...
    return persist(store, {
      schemaVersion: SCHEMA, decisionReceiptId: 'prd_' + hash({ actionId: actionId, packet: packet.packetId || null, status: status, blockers: blockers }).slice(0, 24),
      actionId: actionId, status: status, released: status === 'RELEASED', reason: blockers.length ? 'population-b10-held' : null,
      blockers: blockers, productDomain: 'population', ownerDomain: 'population', lane: 'real-estate',
      ...
      populationPacketId: packet.packetId || null,
    ...
    return { ok: true, status: 'NO_ACTION', released: false, reason: 'population-b10-unavailable', blockers: ['decision-persistence-or-input-unavailable'], detail: String(error && error.message || error), providerCalled: false, liveMoney: false };
  }
}
