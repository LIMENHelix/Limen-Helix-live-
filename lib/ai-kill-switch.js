...
// Operator AGENT BOXES (domain-agent / master-agent) also have a narrower UI gate:
// each is admin-passcode-gated (anon = 403, no model call) + per-day Redis-capped. Setting
// LIMEN_AGENT_BOXES_DISABLED=1 hides that entire paid-language surface. A provider call still
// passes spendDisabled() at the provider choke point, so this narrower switch can only inhibit;
// it cannot reopen spend past the global env boundary or runtime pause.
...
async function spendPausedRuntime() {
  ...
  } catch (e) { return true; }    // a missing control path cannot silently authorize paid dispatch
}
...
async function setSpendPaused(paused) {
  ...
  var restored = await db.get('ai:spend:paused');
  if ((restored === true || restored === 1 || restored === '1') !== !!paused) {
    throw new Error('AI runtime pause readback verification failed');
  }
  return !!paused;
}
