...
var SCHEMA = 'governance-publication-learning/1.0';
var EXTERNAL_SCHEMA = 'product-domain-external-learning/1.0';
var STATE_KEY = 'governance_publication_learning_state';
var CAUSE_PREFIX = 'governance_publication_learning_cause:';

function causeKey(actionId) { return CAUSE_PREFIX + actionId; }
function fresh() { return { schemaVersion: SCHEMA, domain: 'governance', lane: 'publication', resolvedCount: 0, signals: [], processedObservationIds: [] }; }
async function load(store) {
  var value = await store.get(STATE_KEY);
  if (!value) return fresh();
  if (value.schemaVersion !== SCHEMA || !Array.isArray(value.signals) || !Array.isArray(value.processedObservationIds)) throw new Error('governance publication learning state malformed');
  return value;
}
async function save(store, value) {
  await store.set(STATE_KEY, value, 365 * 86400);
  var restored = await store.get(STATE_KEY);
  if (!restored || restored.resolvedCount !== value.resolvedCount) throw new Error('governance publication learning readback invalid');
  return restored;
}
async function recordCommand(store, command) {
  var cause = { schemaVersion: SCHEMA, domain: 'governance', lane: 'publication', actionId: command.actionId,
  ...
  if (!restored || restored.commandId !== command.commandId) throw new Error('governance publication cause readback invalid');
...
async function recordObservation(store, observation) {
  ...
  if (!await store.get(causeKey(observation.actionId))) return { ok: false, reason: 'governance-publication-action-cause-missing' };
  ...
  var signal = {
    ...
    ownerDomain: 'governance', lane: 'publication',
