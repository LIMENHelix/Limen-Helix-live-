...
var SLOT_KEY = 'energy_investment_developmental_slot:1';

function enabled(env) {
  var value = env && env.LIMEN_ENERGY_INVESTMENT_DEVELOPMENTAL_ENABLED;
...
async function authorize(store, requestId, env, now) {
  if (!enabled(env)) return held('energy-investment-developmental-switch-off');
  store.assertDurable();
  ...
  var motor = await store.get(MotorReceipt.receiptKey('energy'));
  if (!motor || motor.schemaVersion !== MotorReceipt.SCHEMA || motor.productDomain !== 'energy' || motor.ownerDomain !== 'energy' ||
      motor.contractId !== 'energy-motor/1' || motor.lane !== 'investments') return held('energy-motor-receipt-identity-invalid', motor);
  var at = Number(now) || Date.now();
  var slot = { schemaVersion: 'energy-investment-developmental-authority/1.0', slot: 1, requestId: requestId,
  ...
  if (!made) {
    ...
    if (!slot || slot.requestId !== requestId) return held('energy-developmental-paper-order-cap-reached', slot);
  }
  return { ok: true, authorized: true, status: 'AUTHORIZED_DEVELOPMENTAL_PAPER', authorizationMode: 'developmental-paper-commissioning',
    receiptId: 'energy-developmental-slot-1', productDomain: 'energy', ownerDomain: 'energy', contractId: motor.contractId,
