...
// NOTE: one '..' — this file lives in /lib (root level). It was moved from api/lib
// (the Vercel fn-count cleanup) but the path kept two '..', which pointed ABOVE the
// repo so bridge-patterns.json never loaded. Fixed 2026-06-12. (Same two-'..' bug
// remains in other lib/ modules — bridge-engine, engine-output-generator, etc. —
// to fix in a verified batch.)
...
// Never-trimmed terminal-status archives. The pending queue (REDIS_KEY) is
// LTRIM'd to stay bounded, which used to EVICT old APPROVED/REJECTED proposals
// as new pending ones pushed them past the cap. Operator decisions now ALSO live
// in these per-id hashes, which are never trimmed — so an approval/rejection is
// preserved permanently regardless of queue churn.
const APPROVED_KEY = 'limen:pattern-proposals:approved';
const REJECTED_KEY = 'limen:pattern-proposals:rejected';
...
// ── Terminal-status archives (approved / rejected) ───────────────────────────
// All ops are best-effort: a failure logs and degrades to the prior queue-only
// behavior — it must never break the proposal endpoint.
async function _archivePut(bucketKey, proposal) {
  if (!_redisEnabled()) return;
  const id = proposal && proposal.pattern && proposal.pattern.id;
  if (!id) return;
  try { await _redisCmd(['HSET', bucketKey, id, JSON.stringify(proposal)]); }
  catch (e) { console.warn('[pattern-author] archive HSET failed:', e.message); }
}
async function _archiveDel(bucketKey, patternId) {
  if (!_redisEnabled() || !patternId) return;
  try { await _redisCmd(['HDEL', bucketKey, patternId]); }
  catch (e) { console.warn('[pattern-author] archive HDEL failed:', e.message); }
}
async function _archiveGetAll(bucketKey) {
  if (!_redisEnabled()) return [];
  try {
    const raw = await _redisCmd(['HGETALL', bucketKey]);
    const out = [];
    if (Array.isArray(raw)) {
      // Upstash REST returns HGETALL as a flat [field, value, field, value, …]
      for (let i = 1; i < raw.length; i += 2) { try { out.push(JSON.parse(raw[i])); } catch (e) {} }
    } else if (raw && typeof raw === 'object') {
      for (const k of Object.keys(raw)) { try { out.push(JSON.parse(raw[k])); } catch (e) {} }
    }
    return out;
  } catch (e) { console.warn('[pattern-author] archive HGETALL failed:', e.message); return []; }
}
...
CRITICAL RULES:
- LANES: only "investment" and "research" exist. Patent, grant, and loan/SBA/franchise lanes are RETIRED — never emit them in derivedAngles or anywhere else.
...
async function _loadProposals() {
  ...
  if (_redisEnabled()) {
    try {
      const raw = await _redisCmd(['LRANGE', REDIS_KEY, '0', '500']);
      const queue = (raw || []).map(s => { try { return JSON.parse(s); } catch (e) { return null; } }).filter(Boolean);
      // Pull the never-trimmed terminal-status archives. These hold approved /
      // rejected proposals even after the queue's LTRIM has evicted them.
      const approved = await _archiveGetAll(APPROVED_KEY);
      const rejected = await _archiveGetAll(REJECTED_KEY);
      const archivedIds = new Set([...approved, ...rejected].map(p => p && p.pattern && p.pattern.id).filter(Boolean));
      // Self-heal: archive any APPROVED/REJECTED still ONLY in the queue, before
      // the next LTRIM can evict it. Terminal statuses are few, so this is cheap.
      for (const p of queue) {
        const id = p && p.pattern && p.pattern.id;
        if (!id || archivedIds.has(id)) continue;
        if (p.status === 'APPROVED') { await _archivePut(APPROVED_KEY, p); archivedIds.add(id); approved.push(p); }
        else if (p.status === 'REJECTED') { await _archivePut(REJECTED_KEY, p); archivedIds.add(id); rejected.push(p); }
      }
      // Compose: queue first (preserves newest-first ordering for pending/held),
      // skipping any id the archive owns; then the canonical archived terminals.
      const seen = new Set();
      const proposals = [];
      for (const p of queue) {
        const id = p && p.pattern && p.pattern.id;
      ...
      for (const p of [...approved, ...rejected]) {
        const id = p && p.pattern && p.pattern.id;
        if (!id || seen.has(id)) continue;
        seen.add(id);
        proposals.push(p);
...
async function _saveProposal(newProposal) {
  if (_redisEnabled()) {
    try {
      ...
      // Headroom for the pending/held queue. Terminal decisions no longer rely on
      // this cap — they're preserved in the never-trimmed approved/rejected hashes.
      await _redisCmd(['LTRIM', REDIS_KEY, '0', '999']);
  ...
  state.proposals = state.proposals.slice(0, 1000);
...
async function _updateProposalStatus(patternId, updates) {
  if (_redisEnabled()) {
    try {
      ...
      let found = all.find(p => p.pattern && p.pattern.id === patternId);
      // Fall back to the terminal-status archives — the proposal may have been
      // evicted from the queue but is still preserved there (e.g. restoring an
      // old rejection).
      if (!found) {
        const arch = [...await _archiveGetAll(APPROVED_KEY), ...await _archiveGetAll(REJECTED_KEY)];
        found = arch.find(p => p && p.pattern && p.pattern.id === patternId);
      }
      if (!found) return { ok: false, error: 'not found' };
      Object.assign(found, updates);
      await _redisCmd(['LPUSH', REDIS_KEY, JSON.stringify(found)]);
      return { ok: true, storage: 'redis', proposal: found };
  ...
  try {
    ...
    return { ok: true, storage: 'file', proposal: p };
...
// Surface the approved-pattern LIBRARY (bridge-patterns.json) as APPROVED proposal
// entries. Approved patterns graduate OUT of the queue into this file, so the review
// page (which reads the queue) otherwise can't show them. Read-only, best-effort.
function approvedLibraryAsProposals() {
  try {
    const lib = JSON.parse(fs.readFileSync(PATTERNS_PATH, 'utf8'));
    return (lib.patterns || []).map(p => ({
      id: p.id,
      pattern: p,
      status: 'APPROVED',
      sourcePortal: '(approved library)',
      approvedAt: lib.generatedAt || null,
      fromLibrary: true
    }));
  } catch (e) { return []; }
}
...
async function approveProposal(proposalId) {
  ...
  const upd = await _updateProposalStatus(proposalId, {
    status: 'APPROVED',
  ...
  // Preserve the approval in the never-trimmed approved archive (and clear any
  // prior rejection of the same id) so queue churn can never lose it.
  await _archivePut(APPROVED_KEY, (upd && upd.proposal) || Object.assign({}, p, { status: 'APPROVED', approvedAt: new Date().toISOString() }));
  await _archiveDel(REJECTED_KEY, proposalId);
...
async function rejectProposal(proposalId) {
  const r = await _updateProposalStatus(proposalId, { status: 'REJECTED', rejectedAt: new Date().toISOString() });
  // Preserve the rejection in the never-trimmed rejected archive.
  if (r && r.ok && r.proposal) await _archivePut(REJECTED_KEY, r.proposal);
  return r;
}
...
async function restoreProposal(proposalId) {
  const r = await _updateProposalStatus(proposalId, { status: 'PENDING_REVIEW', restoredAt: new Date().toISOString(), rejectedAt: null });
  // No longer rejected — drop it from the rejected archive so the archived
  // REJECTED copy doesn't shadow the restored PENDING_REVIEW one on next read.
  if (r && r.ok) await _archiveDel(REJECTED_KEY, proposalId);
  return r;
}

module.exports = { proposePattern, listProposals, approvedLibraryAsProposals, approveProposal, rejectProposal, restoreProposal, repairPattern };
