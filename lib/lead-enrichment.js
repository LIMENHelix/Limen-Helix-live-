...
var BACKENDS = [
  ...
  // ── PAID PROVIDER: People Data Labs (person enrichment) ────────────────────
  // Resolves a work email/phone from a CONTACT NAME + company via PDL
  // /v5/person/enrich. Needs a person name distinct from the company, so
  // company-only leads (finance-desk EDGAR names) return null and are NOT billed
  // (PAYG = pay per match). Runs ONLY when: LEAD_ENRICH_PAID_ENABLED=1 AND
  // PDL_API_KEY set (backendActive) AND a budget gate is supplied and allows the
  // spend (ctx.budget; fail-closed). Cost per match = PDL_COST_USD (default $0.10),
  // debited from the autonomy budget. A lookup only; contacts no one.
  {
    id: 'pdl',
    label: 'People Data Labs (person enrichment)',
    costModel: 'per-lead',
    requiresEnv: 'PDL_API_KEY',
    enabled: function () { return true; },
    run: async function (lead, ctx) {
      var key = process.env.PDL_API_KEY;
      if (!key) return null;
      var nm = String((lead && lead.name) || '').trim();
      var org = String((lead && lead.org) || '').trim();
      // person-enrich needs a contact name distinct from the company; skip
      // company-only leads so we never burn a call that cannot match a person.
      if (nm.length < 3 || nm.toLowerCase() === org.toLowerCase()) return null;
      // A paid call is fail-closed: it requires a budget gate that allows it.
      var b = ctx && ctx.budget;
      if (!b || typeof b.check !== 'function' || typeof b.record !== 'function') return null;
      var cost = Number(process.env.PDL_COST_USD || 0.10) || 0.10;
      var g = await b.check(cost);
      if (!g.allow) return null;
      var fetchImpl = (ctx && ctx.fetchImpl) || (typeof fetch === 'function' ? fetch : null);
      if (!fetchImpl) return null;
      var qs = 'name=' + encodeURIComponent(nm) + (org ? ('&company=' + encodeURIComponent(org)) : '') + '&min_likelihood=6';
      try {
        var r = await fetchImpl('https://api.peopledatalabs.com/v5/person/enrich?' + qs, {
          headers: { 'X-Api-Key': key, 'accept': 'application/json' },
          signal: (typeof AbortSignal !== 'undefined' && AbortSignal.timeout) ? AbortSignal.timeout(10000) : undefined
        });
        if (!r || !r.ok) return null; // 404 no-match (not billed) or transient error
        var data = (await r.json());
        var d = data && data.data ? data.data : null;
        if (!d) return null;
        var email = '';
        if (validEmail(d.work_email)) email = d.work_email;
        else if (Array.isArray(d.emails)) { for (var i = 0; i < d.emails.length; i++) { var ea = d.emails[i] && d.emails[i].address; if (validEmail(ea)) { email = ea; break; } } }
        if (!email && validEmail(d.recommended_personal_email)) email = d.recommended_personal_email;
        var phone = '';
        if (normPhone(d.mobile_phone).length === 10) phone = normPhone(d.mobile_phone);
        else if (Array.isArray(d.phone_numbers)) { for (var k2 = 0; k2 < d.phone_numbers.length; k2++) { if (normPhone(d.phone_numbers[k2]).length === 10) { phone = normPhone(d.phone_numbers[k2]); break; } } }
        if (!email && !phone) return null; // matched a person but no usable contact
        try { await b.record(cost, { streamId: 'lead-enrichment', note: 'PDL person-enrich' }); } catch (e) {}
        return { email: String(email).toLowerCase(), phone: phone, source: 'pdl', costCents: Math.round(cost * 100) };
      } catch (e) { return null; }
    }
  }
];
