...
// scrape candidates.
var DDG_SKIP_HOSTS = /(linkedin\.|facebook\.|twitter\.|(^|\.)x\.com|instagram\.|youtube\.|wikipedia\.|yelp\.|crunchbase\.|zoominfo\.|bloomberg\.|reuters\.|wsj\.com|marketwatch\.|stocktwits\.|yahoo\.com|glassdoor\.|indeed\.|bbb\.org|pitchbook\.|dnb\.com|apollo\.io|rocketreach\.|leadiq\.|owler\.|tracxn\.|globenewswire\.|prnewswire\.|businesswire\.)/i;
...
// base domain, skipping directories/socials.
function filterCandidateUrls(rawUrls, max) {
  var out = [], seen = {};
...
function ddgResultUrls(html, max) {
  var text = String(html == null ? '' : html);
  var raw = [], m;
  var re = /uddg=([^&"'<>]+)/g;
  ...
  return filterCandidateUrls(raw, max);
}
...
// page. Low recall, near-zero junk.
function pickContactForHost(html, host) {
  ...
  return { email: matched, phone: phones[0] || '' };
}
...
var BACKENDS = [
  ...
  },
  {
    ...
    enabled: function () { return !!process.env.DDGS_API_URL || !ddgTripped; },
    run: async function (lead, ctx) {
      ...
      var urls = null;
      var api = String((ctx && ctx.ddgsApiUrl) || process.env.DDGS_API_URL || '').replace(/\/+$/, '');
      if (api) {
        try {
          var ar = await fetchImpl(api + '/search/text?query=' + encodeURIComponent(q) + '&max_results=8', fetchOpts());
          if (ar && ar.ok) {
            var data = await ar.json();
            urls = filterCandidateUrls((data && data.results || []).map(function (x) { return x && x.href; }), 3);
          }
      ...
      if (urls == null && !ddgTripped) {
        try {
          ...
          urls = ddgResultUrls(serp, 3);
        } catch (e) { return null; }
      }
      for (var i = 0; i < (urls || []).length; i++) {
        try {
          var p = await fetchImpl(urls[i], fetchOpts());
          if (!p || !p.ok) continue;
          var c = pickContactForHost(await p.text(), new URL(urls[i]).hostname);
          if (c) return { email: c.email, phone: c.phone, source: 'ddg-search', costCents: 0 };
