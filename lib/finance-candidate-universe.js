...
function validateProjectedMarginRanking(universe, proposal) {
  ...
  var seen = Object.create(null), prior = Infinity;
  for (var i = 0; i < entries.length; i++) {
    ...
    var confidence = Number(row.confidence), score = Number(row.riskAdjustedMarginPct);
    var side = row.side === undefined ? 'LONG' : row.side;
    if (['LONG', 'SHORT'].indexOf(side) < 0 || ![expectedReturn, downside, confidence, score].every(Number.isFinite) || downside > 0 || confidence < 0 || confidence > 1) {
    ...
    if (Math.abs(calculated - score) > 0.01) {
      return { ok: false, status: 'ABSTAINED', reason: 'projected_margin_ranking_arithmetic_invalid' };
    }
    if (score > prior) return { ok: false, status: 'ABSTAINED', reason: 'projected_margin_ranking_not_descending' };
    prior = score;
  }
  var top = entries[0];
  ...
  normalized.entries.forEach(function (row) { if (row.side === undefined) row.side = 'LONG'; });
  var selected = clone(top);
  if (selected.side === undefined) selected.side = 'LONG';
