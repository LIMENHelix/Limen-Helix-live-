    ...
    if (isCJ) {
      ...
      const committed = _round((rows || []).reduce(function (t, r) {
        ...
        if (r.state === 'reserved') {
          ...
          if (r.day && r.day !== _today()) return t;
          return t + (r.amount || 0);
        }
        ...
        // The snapshot alone is not enough. settle() clears the cache, so the very next
        // read is NEWER than the settlement it was triggered by - and if CJ has not
        // applied the debit yet, that fresh figure is still a pre-purchase number while
        // the bound says the debit is accounted for. That fails OPEN, which is the one
        // direction that ends with a paid, unfulfillable order.
        //
