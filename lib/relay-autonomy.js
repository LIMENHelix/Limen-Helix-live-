 ...
 */
async function _fundsCover(cfg, amount, marketplace) {
  ...
  try {
    if (isCJ) {
      ...
      if (bal.available < need) {
        return {
          ok: false,
          reason: 'CJ wallet has $' + bal.available.toFixed(2) + ' available and this needs $' + need.toFixed(2)
        };
      }
      return { ok: true };
    }
...
function _velocityCheck(cfg, rows, amount, excludeId) {
  ...
  if (recent.length >= maxOrders) {
    return {
      ok: false,
      reason: maxOrders + ' purchases already in the last hour; the rate limit ' +
              'holds further orders until that window clears'
    };
  }
...
async function authorize(opts) {
  ...
  const vel = _velocityCheck(cfg, rows, _round(amount + planned));
  if (!vel.ok) return Object.assign(base, { remainingToday: remaining, reason: vel.reason });

  // ── real funds ──
  const funds = await _fundsCover(cfg, _round(amount + planned), opts.marketplace);
...
async function consumeApproved(opts) {
  ...
  const vel = _velocityCheck(cfg, rows, 0, row.id);
  if (!vel.ok) return Object.assign(base, { reason: vel.reason });

  const funds = await _fundsCover(cfg, row.amount, row.marketplace);
