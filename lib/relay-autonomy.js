...
async function authorize(opts) {
  ...
  if (cfg.requireFunds) {
    try {
      ...
      // Same accumulation as the ceiling: the balance has not moved for the other lines
      // of this cart yet, and it has to cover all of them.
      if (bal < _round(amount + planned)) {
        return Object.assign(base, {
          remainingToday: remaining,
          reason: 'funding balance $' + bal.toFixed(2) + ' does not cover $' +
                  _round(amount + planned).toFixed(2)
        });
      }
    } catch (e) {
