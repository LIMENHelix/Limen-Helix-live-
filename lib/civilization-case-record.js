...
var HOMOLOGY_SCHEMA = 'civilization-homology-context/1.0';
...
function build(packet, lane, opportunity) {
  ...
  var homology = packet.homologyContext && typeof packet.homologyContext === 'object' ? clone(packet.homologyContext) : null;
  ...
  return {
    ...
    recordVersion: 1,
    documentControl: { classification: 'INTERNAL_OPERATOR_REVIEW', status: 'DRAFT_OBSERVATIONAL', generatedAt: packet.generatedAt, retentionDays: 90, supersedes: null },
    status: 'OBSERVATIONAL',
    authorization: { status: 'NOT_AUTHORIZED', liveExecution: false, paperOnly: lane === 'investments', humanReviewRequired: true },
    parties: { originDomain: domain, accountableOwner: lane === 'research-papers' ? (domain === 'science' || domain === 'medicine' ? domain : null) : 'finance', externalCounterparty: null },
    ...
    basis: { treatments: treatments, directives: directives, phase: value(packet.truth && packet.truth.phase), phaseLabel: value(packet.truth && packet.truth.phaseLabel), confidence: value(packet.truth && packet.truth.confidence), homologyContext: homology, contextOnly: true },
    evidence: { citations: citations, evidenceIds: evidenceIds, sourceIdentity: clone(packet.sourceIdentity), independenceAssessment: { status: 'UNESTABLISHED', method: null, basis: 'handoff record alone does not establish independence' }, contradictions: [], retractions: [], homologyContext: homology, mappingCoverage: { neurology_to_business_homology: false, business_to_neurology_homology: false, kernel_dynamics: false, p0_p10_proof_and_effects: false } },
    laneTerms: lane === 'research-papers' ? {
      ...
      accountableOwner: 'finance', executionMode: 'paper-only', horizonsDays: [1, 3, 7, 14, 30, 60, 90], instrument: value(opportunity.ticker || opportunity.symbol || opportunity.instrument), quantity: null, budget: null, broker: null, account: null, order: null, riskLimits: { maxLoss: null, maxDrawdown: null, stop: null }, outcomeStatus: 'NOT_OBSERVED'
    },
    representations: { factualClaimStatus: 'UNVERIFIED', noLegalOpinion: true, noInvestmentAdvice: true, noAuthorization: true },
    reviewChecklist: [
      { item: 'source identity', status: 'PRESENT' },
      { item: 'independent corroboration', status: 'NOT_ESTABLISHED' },
      { item: 'contradictions and retractions', status: 'NOT_REVIEWED' },
      { item: 'homology and P0-P10 mappings', status: homology && homology.schemaVersion === HOMOLOGY_SCHEMA ? 'PRESENT_CONTEXT_ONLY' : 'MISSING' },
      { item: 'human approval', status: 'REQUIRED' }
    ],
...
module.exports = { SCHEMA: SCHEMA, HOMOLOGY_SCHEMA: HOMOLOGY_SCHEMA, MAPPINGS: MAPPINGS.slice(), ACTIVE_LANES: ACTIVE_LANES.slice(), build: build };
