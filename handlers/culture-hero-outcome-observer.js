...
function createHandler(deps) {
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      res.statusCode = 200;
      return res.end(JSON.stringify({ ok: true, schemaVersion: 'culture-hero-observer-cycle/1.0',
        inspected: commands.length, observations: observations, generationEndpointCalled: false, providerCalled: false, liveMoney: false }));
    } catch (error) {
      res.statusCode = 503;
