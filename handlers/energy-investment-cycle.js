  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      for (var i = 0; i < refs.length; i++) {
        ...
        if (result.status === 'COMMAND_RECEIPTED') { task.status = 'COMPLETED'; task.commandId = result.commandId; task.completedAt = Date.now(); await store.set(taskKey(task.taskId), task); var rb = await store.get(taskKey(task.taskId)); if (!rb || rb.status !== 'COMPLETED') throw new Error('energy investment task completion readback invalid'); }
      }
      return json(res, 200, { ok: true, schemaVersion: 'energy-investment-cycle/1.0', inspected: refs.length, accepted: accepted, results: results, executionMode: 'paper', liveMoney: false });
