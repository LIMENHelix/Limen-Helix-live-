...
var WORKLIST = 'infrastructure_real_estate_worklist';
var TASK_PREFIX = 'infrastructure_real_estate_task:';
...
function receivingAddress(actionId) { var domain = String(process.env.INFRASTRUCTURE_REAL_ESTATE_RECEIVING_DOMAIN || '').trim().toLowerCase(); return /^[a-z0-9.-]+\.[a-z]{2,}$/.test(domain) ? 'realestate+' + actionId + '@' + domain : null; }
async function send(candidate, actionId, idempotencyKey, deps) {
  ...
  if (!apiKey || !from || !replyTo) return { ok: false, providerCalled: false, definitiveFailure: true, error: 'infrastructure real-estate Resend/from/receiving-domain configuration incomplete' };
...
function createHandler(deps) {
  ...
  return async function handler(req, res) {
    ...
    if (method === 'POST') {
      var pass = Admin.reqKey(req); if (!Admin.hasDomain(pass, 'infrastructure')) return Admin.deny(res);
    ...
    try {
      ...
      for (var i = 0; i < refs.length; i++) {
        ...
        var decisionDeps = Object.assign({}, deps.decisionDeps || {}, { maxIndicationUsd: deps.maxIndicationUsd != null ? deps.maxIndicationUsd : numberEnv('INFRASTRUCTURE_REAL_ESTATE_MAX_INDICATION_USD') });
        ...
        var result = await (deps.executor || Executor).execute({ store: store, candidate: task.candidate, decision: decision, now: Date.now(), motorAuthorization: deps.motorAuthorization,
          emailCostUsd: deps.emailCostUsd != null ? deps.emailCostUsd : numberEnv('INFRASTRUCTURE_REAL_ESTATE_EMAIL_COST_USD'),
          dailyBudgetUsd: deps.dailyBudgetUsd != null ? deps.dailyBudgetUsd : numberEnv('INFRASTRUCTURE_REAL_ESTATE_DAILY_BUDGET_USD'),
          dailyRequestCap: deps.dailyRequestCap != null ? deps.dailyRequestCap : numberEnv('INFRASTRUCTURE_REAL_ESTATE_DAILY_REQUEST_CAP'),
      ...
      return json(res, 200, { ok: true, schemaVersion: 'infrastructure-real-estate-cycle/1.0', inspected: refs.length, accepted: accepted, providerCalls: providerCalls, results: results, liveMoney: false });
    } catch (error) { return json(res, 503, { ok: false, error: 'infrastructure-real-estate-cycle-unavailable', detail: String(error && error.message || error), providerCalls: 0, liveMoney: false }); }
  };
}
var handler = createHandler();
module.exports = require('../lib/heartbeat').wrap('infrastructure-real-estate-cycle', handler);
