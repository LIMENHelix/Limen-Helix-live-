  ...
  try {
    var decisions = (await Store.lrange(Decision.LOG_KEY, 0, 19)).filter(function (row) { return row && row.schemaVersion === Decision.SCHEMA; }).map(function (row) {
      return { decisionReceiptId: row.decisionReceiptId, actionId: row.actionId, status: row.status, reason: row.reason || null,
        blockers: row.blockers || [], governancePacketId: row.governancePacketId, decidedAt: row.decidedAt, providerCalled: false, liveMoney: false };
    });
    ...
    res.statusCode = 200;
    return res.end(JSON.stringify({ ok: true, schemaVersion: 'governance-publication-status/1.0', productDomain: 'governance', ownerDomain: 'governance', lane: 'publication',
      switchEnabled: process.env.GOVERNANCE_PUBLICATION_ENABLED === '1', decisions: decisions, commands: commands, observations: observations,
      readOnly: true, providerCalledByRead: false, liveMoney: false }));
  } catch (error) {
    res.statusCode = 503; return res.end(JSON.stringify({ ok: false, error: 'governance-publication-status-unavailable', readOnly: true }));
  }
};
