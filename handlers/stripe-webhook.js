...
module.exports = async function handler(req, res) {
  ...
  try {
    if (evt.type === 'checkout.session.completed') {
      ...
      /**
       * BOOK THE CASH TO THE DOMAIN'S TREASURY ACCOUNT.
       *
       * OUTSIDE the `act.ok` branch, and outside the activation guard entirely, because
       * the customer was charged whether or not we managed to activate them. Booking only
       * on successful activation would make the treasury under-report by exactly the
       * failures, which is the subset you would most want the money trail for.
       *
       * Guarded on `limen:'1'` instead: the Relay storefront books through this same
       * Stripe account and must never land in a product domain's account.
       */
      if (treasuryBridge.isLimenProductSale(meta)) {
        var booked = await treasuryBridge.bookCapturedSale({
          store: motorStore, domain: meta.domain,
          grossCents: obj.amount_total, eventId: evt.id
        });
        out.treasuryBooked = booked.booked;
        if (booked.receiptId) out.treasuryReceiptId = booked.receiptId;
        if (booked.reason) out.treasuryReason = booked.reason;
      }
      ...
      if (reason === 'subscription_cycle') {
        ...
        /**
         * RENEWAL CASH, booked from the SUBSCRIBER's domain rather than the invoice's
         * metadata. A renewal invoice is generated by Stripe from the subscription and
         * does not carry the `limen`/`domain` marks the original checkout session had, so
         * reading `meta.domain` here would find nothing and book nothing, every month,
         * silently. The subscriber record is the authority on which domain was bought.
         *
        ...
        if (renewalBooked.reason) out.treasuryReason = renewalBooked.reason;
      }
      out.handled = true;
    }
