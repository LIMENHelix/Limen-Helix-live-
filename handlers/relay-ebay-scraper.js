/**
 * api/relay-ebay-scraper — Scrape eBay listings and post to Relay
 *
...
module.exports = async function handler(req, res) {
  ...
  if (action === 'search') {
    const result = await ebay.searchEbay(query, { limit: maxItems });
    if (!result.ok) {
  ...
  if (action === 'scrape-and-post') {
    ...
    const result = await ebay.scrapeAndPost(query, marketplaceId, sellerId, {
      maxItems: maxItems,
