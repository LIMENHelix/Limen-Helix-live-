...
function createHandler(deps) {
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      var jar = cookies(req), token = jar.limen_governance_publication_visitor;
      if (!token || !/^[a-f0-9]{32}$/.test(token)) {
        token = (deps.randomToken ? deps.randomToken() : crypto.randomBytes(16).toString('hex'));
        res.setHeader('set-cookie', 'limen_governance_publication_visitor=' + token + '; Path=/; Max-Age=31536000; HttpOnly; Secure; SameSite=Lax');
      }
      ...
      res.statusCode = 503; res.setHeader('content-type', 'application/json'); return res.end(JSON.stringify({ ok: false, error: 'governance-publication-engagement-unavailable' }));
    }
  };
}
