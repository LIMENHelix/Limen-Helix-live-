...
function createHandler(deps) { deps = deps || {}; var store = deps.store || Store; return async function handler(req, res) {
  ...
  var raw = await rawBody(req), secret = deps.secret || process.env.AGRICULTURE_HOMESTEAD_RESEND_WEBHOOK_SECRET || '';
  ...
  catch (error) { return json(res, 503, { ok: false, error: 'agriculture-homestead-inbound-unavailable', detail: String(error && error.message || error), sendEndpointCalled: false, liveMoney: false }); }
}; }
