...
var Executor = require('../lib/law-automail-executor.js'); var Observer = require('../lib/law-automail-outcome-observer.js'); var Learning = require('../lib/law-automail-learning.js');
function createHandler(deps) { deps = deps || {}; var store = deps.store || Store, observer = deps.observer || Observer, auth = deps.cronAuth || CronAuth;
  return async function handler(req, res) { res.setHeader('content-type', 'application/json'); res.setHeader('cache-control', 'no-store');
    ...
    try { var commands = await store.lrange(Executor.LOG_KEY, 0, 99), observations = await observer.observeRecent(store, commands, { fetch: deps.fetch || global.fetch, apiKey: deps.apiKey });
      var learned = 0, failures = []; for (var i = 0; i < observations.length; i++) { if (!observations[i] || !observations[i].observationId) continue;
        var learnedResult = await Learning.recordObservation(store, observations[i]); if (learnedResult && learnedResult.ok) { if (!learnedResult.duplicate) learned++; }
        else failures.push({ observationId: observations[i].observationId, reason: learnedResult && learnedResult.reason || 'law-learning-failed' }); }
      res.statusCode = failures.length ? 207 : 200; return res.end(JSON.stringify({ ok: failures.length === 0, schemaVersion: 'law-automail-observer-cycle/1.0', inspectedCommands: commands.length,
        observations: observations, learning: { recorded: learned, failures: failures }, createEndpointCalled: false, liveMoney: false })); }
