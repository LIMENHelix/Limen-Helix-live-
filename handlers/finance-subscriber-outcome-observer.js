...
function createHandler(deps) {
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      res.statusCode = 200; return res.end(JSON.stringify({ ok: true, schemaVersion: 'finance-subscriber-observer-cycle/1.0',
    ...
    } catch (error) { res.statusCode = 503; return res.end(JSON.stringify({ ok: false, error: 'finance-subscriber-observer-unavailable', detail: String(error && error.message || error), sendEndpointCalled: false, liveMoney: false })); }
  };
}
var handler = createHandler(); module.exports = require('../lib/heartbeat').wrap('finance-subscriber-outcome-observer', handler); module.exports.createHandler = createHandler;
