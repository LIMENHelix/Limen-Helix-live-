/**
 * homestead-status.js — per-deal CRM state (Layer 5: what happened / outcomes).
 * The operator sets a status + optional note + revenue per deal; it persists so the
 * desk knows New → Contacted → Negotiating → Under contract → Assigned → Closed / Dead /
 * Review. Drives the TODAY counts, the human-review queue, and revenue tracking.
 *
 * GET  ?key=  -> { ok, statuses: { <dealKey>: {status,note,revenue,updatedMs,closedMs} } }
 * POST { key, dealKey, status?, note?, revenue? } -> upsert one deal's state
 * Admin-only (LEAD_ADMIN_KEY).
 */
var db = require('../lib/limen-db');
var SKEY = 'homestead:status';
