...
function createHandler(deps) {
  ...
  return async function handler(req, res) {
    ...
    var enabled = deps.enabled != null ? deps.enabled : process.env.GOVERNANCE_PUBLICATION_ENABLED === '1';
    if (enabled !== true) return json(res, 200, { ok: true, status: 'HELD', reason: 'governance-publication-switch-disabled', accepted: 0, providerCalls: 0, liveMoney: false });
    try {
      store.assertDurable();
      var now = Date.now();
      var titleSets = await (deps.readTitleSets || Db.lrangeStrict)('feedtitles:governance', 0, 7);
      var cognition = deps.cognition || await (deps.redisGet || Redis.redisGet)('limen:brain:cognition:governance');
      var candidate = (deps.source || Source).build(titleSets, cognition, now);
      if (!candidate) return json(res, 200, { ok: true, status: 'NO_ACTION', reason: 'governance-publication-source-or-brain-selection-insufficient', accepted: 0, providerCalls: 0, liveMoney: false });
      ...
      var result = await (deps.executor || Executor).execute({
        ...
        operationCostUsd: deps.operationCostUsd != null ? deps.operationCostUsd : envNumber('GOVERNANCE_PUBLICATION_OPERATION_COST_USD'),
        dailyBudgetUsd: deps.dailyBudgetUsd != null ? deps.dailyBudgetUsd : envNumber('GOVERNANCE_PUBLICATION_DAILY_BUDGET_USD'),
        dailyPublicationCap: deps.dailyPublicationCap != null ? deps.dailyPublicationCap : envNumber('GOVERNANCE_PUBLICATION_DAILY_CAP'),
        publisher: deps.publisher
      });
      return json(res, 200, { ok: true, schemaVersion: 'governance-publication-cycle/1.0', status: result.status,
      ...
      return json(res, 503, { ok: false, error: 'governance-publication-cycle-unavailable', detail: String(error && error.message || error), providerCalls: 0, liveMoney: false });
    }
  };
}

var handler = createHandler();
module.exports = require('../lib/heartbeat').wrap('governance-publication-cycle', handler);
