...
function createHandler(deps) {
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      res.statusCode = 200;
      return res.end(JSON.stringify({ ok: true, schemaVersion: 'defense-publication-observer-cycle/1.0',
      ...
      res.statusCode = 503;
      return res.end(JSON.stringify({ ok: false, error: 'defense-publication-observer-unavailable', detail: String(error && error.message || error), publishEndpointCalled: false, liveMoney: false }));
    }
  };
}

var handler = createHandler();
module.exports = require('../lib/heartbeat').wrap('defense-publication-outcome-observer', handler);
