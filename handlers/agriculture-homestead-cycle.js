...
var WORKLIST = 'agriculture_homestead_worklist';
var TASK_PREFIX = 'agriculture_homestead_task:';
...
function receivingAddress(actionId) { var domain = String(process.env.AGRICULTURE_HOMESTEAD_RECEIVING_DOMAIN || '').trim().toLowerCase(); return /^[a-z0-9.-]+\.[a-z]{2,}$/.test(domain) ? 'homestead+' + actionId + '@' + domain : null; }
async function send(candidate, actionId, idempotencyKey, deps) {
  ...
  if (!apiKey || !from || !replyTo) return { ok: false, providerCalled: false, definitiveFailure: true, error: 'agriculture homestead Resend/from/receiving-domain configuration incomplete' };
  var body = candidate.body + '\n\nWork order: ' + candidate.workOrderId + '\nOperation: ' + candidate.operationKind + '\nReply to this email so LIMEN can independently observe the service response.';
  var payload = { from: from, to: [candidate.providerEmail], reply_to: replyTo, subject: '[LIMEN ' + actionId + '] ' + candidate.subject,
...
function createHandler(deps) {
  ...
  return async function handler(req, res) {
    ...
    if (method === 'POST') {
      var pass = Admin.reqKey(req); if (!Admin.hasDomain(pass, 'agriculture')) return Admin.deny(res);
      var body = await readBody(req), candidate = Decision.candidate(body);
      if (!candidate) return json(res, 400, { ok: false, error: 'valid workOrderId, providerEmail, propertyRef, operationKind, subject, body, and evidenceId required', providerCalled: false, liveMoney: false });
      try { store.assertDurable(); var task = { schemaVersion: 'agriculture-homestead-task/1.0', taskId: candidate.workOrderId, candidate: candidate, enqueuedAt: Date.now(), status: 'QUEUED' }; var created = await store.setIfAbsent(taskKey(task.taskId), task); var restored = await store.get(taskKey(task.taskId)); if (!restored || restored.taskId !== task.taskId) throw new Error('task readback invalid'); if (created) { await store.lpush(WORKLIST, { taskId: task.taskId, enqueuedAt: task.enqueuedAt }); await store.ltrim(WORKLIST, 0, 999); } return json(res, 200, { ok: true, status: restored.status, taskId: task.taskId, duplicate: !created, providerCalled: false, liveMoney: false }); }
      catch (error) { return json(res, 503, { ok: false, error: 'agriculture-homestead-enqueue-unavailable', detail: String(error && error.message || error), providerCalled: false, liveMoney: false }); }
    }
    if (method !== 'GET') return json(res, 405, { ok: false, error: 'GET or POST only' });
    if (!cronAuth.enforce(req, res)) return;
    if ((deps.enabled != null ? deps.enabled : process.env.AGRICULTURE_HOMESTEAD_ENABLED === '1') !== true) return json(res, 200, { ok: true, status: 'HELD', reason: 'agriculture-homestead-switch-disabled', inspected: 0, accepted: 0, providerCalls: 0, liveMoney: false });
    try {
      ...
      for (var i = 0; i < refs.length; i++) {
        ...
        var decision = await (deps.decision || Decision).decide(store, task.candidate, Date.now(), deps.decisionDeps);
        ...
        var result = await (deps.executor || Executor).execute({ store: store, candidate: task.candidate, decision: decision, now: Date.now(), motorAuthorization: deps.motorAuthorization,
          emailCostUsd: deps.emailCostUsd != null ? deps.emailCostUsd : numberEnv('AGRICULTURE_HOMESTEAD_EMAIL_COST_USD'),
          dailyBudgetUsd: deps.dailyBudgetUsd != null ? deps.dailyBudgetUsd : numberEnv('AGRICULTURE_HOMESTEAD_DAILY_BUDGET_USD'),
          dailyRequestCap: deps.dailyRequestCap != null ? deps.dailyRequestCap : numberEnv('AGRICULTURE_HOMESTEAD_DAILY_REQUEST_CAP'),
        ...
        if (result.status === 'ACCEPTED') { task.status = 'COMPLETED'; task.commandId = result.commandId; task.completedAt = Date.now(); await store.set(taskKey(task.taskId), task); var taskReadback = await store.get(taskKey(task.taskId)); if (!taskReadback || taskReadback.status !== 'COMPLETED') throw new Error('task completion readback invalid'); }
      }
      return json(res, 200, { ok: true, schemaVersion: 'agriculture-homestead-cycle/1.0', inspected: refs.length, accepted: accepted, providerCalls: providerCalls, results: results, liveMoney: false });
    } catch (error) { return json(res, 503, { ok: false, error: 'agriculture-homestead-cycle-unavailable', detail: String(error && error.message || error), providerCalls: 0, liveMoney: false }); }
  };
}
var handler = createHandler();
module.exports = require('../lib/heartbeat').wrap('agriculture-homestead-cycle', handler);
