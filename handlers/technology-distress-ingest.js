/**
 * technology-distress-ingest.js — write/read bridge for the layoffs.fyi worker (scripts/tech-fetch.js).
 * Mirrors finance-distress-ingest. Gated by LEAD_ADMIN_KEY. Stores technology:distress + :meta.
 */
...
module.exports = async function handler(req, res) {
  ...
  if (method === 'GET') {
    ...
    return j(res, 200, { ok: true, count: ((await db.get('technology:distress')) || []).length, meta: (await db.get('technology:distress:meta')) || null, deals: (await db.get('technology:distress')) || [] });
  }
  if (method === 'POST') {
    ...
    await db.set('technology:distress', body.deals, TTL);
    await db.set('technology:distress:meta', meta, TTL);
