...
function createHandler(deps) {
  ...
  var run = deps.commission || Commissioning.commissionScope;
  return async function (req, res) {
    ...
    try {
      var result = cron
        ? await run(store, { env: env, now: Date.now() })
        : await run(store, { env: env, now: Date.now() });
      ...
      return send(res, 503, {
        ok: false, error: 'g0-domain-loop-commissioning-failed',
...
module.exports = require('../lib/heartbeat').wrap('g0-domain-loop-commissioning', handler);
