/**
 * api/infrastructure-markets.js — cached batch quotes for the curated infrastructure ticker set.
 * Civil mirror of energy-markets.js.
 *
 * GET /api/infrastructure-markets → { ok, updated, cached, quotes: { SYM: {price, changePct} } }
 *
 * Source: Yahoo Finance v8 chart API (public, no key). Quotes are DELAYED (~15 min), cached in
 * Redis (infrastructure:markets:v1) for 15 min; Yahoo hit only on a cache miss. Stale-on-failure.
 * Educational data only — NOT investment advice. Tickers are themes to research, never "buy".
 */
var db = require('../lib/limen-db');

var CACHE_KEY = 'infrastructure:markets:v1';
