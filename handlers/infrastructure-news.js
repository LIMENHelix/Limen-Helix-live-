/**
 * api/infrastructure-news.js — cached infrastructure/civil news headlines (free, aggregated).
 * Mirror of energy-news.js, civil identity.
 *
 * GET /api/infrastructure-news → { ok, updated, items:[{title, link, source, pubDate}] }
 *
 * Source: Google News RSS search (public, no key). HEADLINES + LINKS only (no bodies).
 * Fetched server-side (no CORS), cached in Redis 30 min. Stale-on-failure.
 */
var db = require('../lib/limen-db');
var CACHE_KEY = 'infrastructure:news:v1';
var TTL_MS = 30 * 60 * 1000;
var QUERY = '("infrastructure bill" OR "public works" OR "road construction" OR "bridge construction" OR "water infrastructure" OR "civil engineering" OR "infrastructure funding" OR "construction project" OR "transportation infrastructure" OR "grid infrastructure")';
function decode(s) {
  return String(s || '')
