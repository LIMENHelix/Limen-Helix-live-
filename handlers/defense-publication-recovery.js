...
module.exports = async function handler(req, res) {
  ...
  var pass = Gate.reqKey(req); if (!Gate.hasDomain(pass, 'defense')) return Gate.deny(res);
  try {
    ...
    res.statusCode = 503; return res.end(JSON.stringify({ ok: false, error: 'defense-publication-recovery-unavailable', detail: String(error && error.message || error) }));
  }
};
