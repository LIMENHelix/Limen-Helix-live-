...
function createHandler(deps) {
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      res.statusCode = 200; return res.end(JSON.stringify({ ok: true, schemaVersion: 'religion-revenue-fulfillment-cycle/1.0',
    ...
    } catch (error) { res.statusCode = 503; return res.end(JSON.stringify({ ok: false, error: 'religion-revenue-fulfillment-unavailable', detail: String(error && error.message || error), liveMoney: false })); }
  };
}
var handler = createHandler(); module.exports = require('../lib/heartbeat').wrap('religion-revenue-fulfillment', handler); module.exports.createHandler = createHandler;
