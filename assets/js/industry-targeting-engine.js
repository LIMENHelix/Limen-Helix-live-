/**
 * environment-targeting-engine.js — Context-aware targeting for promoted Industry directives
 *
 ...
 *   Tier 3: Example entities (high-confidence well-known industry sector players)
 *
 * ADDITIVE ONLY. Feature-flagged: window.LIMEN_ENABLE_DIRECTIVE_EXTRACTION
 * Exposes: window.LIMENIndustryTargetingEngine
 */
(function () {
  'use strict';
  ...
  var SEGMENTS = {
    ...
    research_universities: {
      label: 'R1 industry universities and university tech transfer offices',
    ...
    contract_research: {
      label: 'Contract industry organizations (CROs)',
    ...
    research_software: {
      label: 'Industry informatics, ELN, and lab software',
      keywords: ['ELN', 'electronic lab notebook', 'LIMS', 'lab informatics', 'industry data management', 'data repository', 'workflow', 'jupyter', 'notebook'],
      tier3: [
    ...
    research_integrity: {
      label: 'Industry integrity, replication, and reproducibility tools',
      ...
      tier3: [
        ...
        { name: 'Open Industry Framework (COS)', ticker: null },
    ...
    research_compute: {
      label: 'Industry computing, HPC, and AI-for-science',
      keywords: ['HPC', 'supercomput', 'GPU cluster', 'AI for science', 'foundation model', 'industry compute', 'cloud HPC', 'protein folding', 'molecular dynamics'],
      tier3: [
    ...
    foundations: {
      label: 'Private industry foundations and philanthropy',
  ...
  function resolveTargets(directive) {
    ...
    else if (path === 'PATENTABLE') executionTargets = ['Patent attorneys', 'University tech transfer offices', 'Scientific instrument licensors', 'Strategic acquirers in industry informatics'];
    else if (path === 'INVESTABLE') executionTargets = ['Life sciences VCs', 'Industry informatics PE', 'University endowment fund managers'];
  ...
  window.LIMENIndustryTargetingEngine = { resolveTargets: resolveTargets };
})();
