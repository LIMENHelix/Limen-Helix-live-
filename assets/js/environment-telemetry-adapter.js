/*
 * environment-telemetry-adapter.js  — wires runtime fields to the LIVE Environment telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/environment-pulse-engine.js  (window.LIMENEnvironmentPulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Environment issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to environment.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into environment.json (a
...
(function (root) {
   ...
   * inputs: { environment, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    inp = inp || {};
    var environment = inp.environment || {};
    ...
    var issuesOverlay = {};
    (environment.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (environment.activations || []).forEach(function (a) {
  ...
  /* Merge environment.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(environment, overlay) {
    var base = environment.runtime || {};
    ...
    var acts = {}; (environment.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (environment.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the environment definition + brainState. */
  function fromLive(environment, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENEnvironmentPulse) ? window.LIMENEnvironmentPulse : null;
    if (!P) return null;
    return fromPulse({ environment: environment, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN environment definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/environment.json', 'environment'),
      _load('/assets/data/environment-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ environment: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.EnvironmentTelemetryAdapter) root.EnvironmentTelemetryAdapter = API;
