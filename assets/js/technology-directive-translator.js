/**
 * technology-directive-translator.js — Convert ranked Technology directives into opportunity objects
 * Carries _omittedSiblingCount through. Exposes: window.LIMENTechnologyDirectiveTranslator
 */
(function () {
  'use strict';
  ...
  function _buildMonetizationPath(d) {
    ...
    if (type === 'INVESTABLE') {
      if (tickerStr) return 'Firms in this pathway (' + tickerStr + ') benefit from the technology condition. Position during stress window for repricing.';
    ...
    return 'Active diagnosis creates documented technology need. Grant proposals backed by live system evidence win at higher rates.';
  }
  ...
  function _buildNextAction(d) {
    ...
    if (type === 'INVESTABLE' && companies.length > 0) return 'Check ' + companies[0].ticker + ' (' + companies[0].name + ') current technology exposure.';
    if (type === 'PATENTABLE') return 'Search patents.google.com for prior art in "' + (d.nodeLabel || 'edtech').toLowerCase() + '" technology.';
    return 'Search grants.gov, ED.gov, IES grants for solicitations matching "' + (d.nodeLabel || 'technology').toLowerCase() + '".';
  }
  ...
  function _buildEvidence(d) {
    var parts = ['Domain: technology. Stress: ' + Math.round((d.stress || 0) * 100) + '%.'];
  ...
  function translate(ranked, limit) {
    ...
    for (var i = 0; i < Math.min(ranked.length, limit); i++) {
      ...
      if (primaryPath === 'GRANT-ELIGIBLE') targetStr += (targetStr ? ' ' : '') + 'Agencies: US Dept of Technology, IES, NSF EHR, Gates Foundation, Walton, Carnegie. Search grants.gov, ies.ed.gov.';
      ...
      opportunities.push({
        id: 'pdir_' + (d.portalDomainId || 'technology') + '_' + (d.nodeId || '') + '_' + i,
        title: _buildTitle(d),
        rank: d.rankScore || 0,
        path: primaryPath,
        urgency: urgency,
        ...
        valueRange: primaryPath === 'INVESTABLE' ? '10-25% sector premium' : primaryPath === 'PATENTABLE' ? '5-15% royalty' : '$250K-$5M technology grants',
    ...
    console.log('[TechnologyTranslator] Translated ' + opportunities.length);
  ...
  var REVENUE_MODELS = {
    'INVESTABLE': [
      { model: 'advisory', desc: 'Build a technology sector exposure scorecard. Sell to tech VCs, corporate CISOs, and enterprise CTOs. $50K-$250K per engagement.' },
      { model: 'trading', desc: 'Position in cybersecurity, AI infrastructure, cloud, or semiconductor names benefiting from technology stress. Target 15-25% repricing premium.' },
    ...
    'GRANT-ELIGIBLE': [
      { model: 'grant award', desc: 'Write a technology research or deployment proposal. Submit to NSF CISE, DARPA, IARPA, DoE ARPA-E, ARPA-H, SBIR/STTR. $250K-$5M per award.' },
  ...
  var TITLE_TEMPLATES = [
    ...
    { match: /assessment|diagnostic/i, titles: ['Build technology risk scorecard \u2014 sell to {target}'] }
  ];
  ...
  function _targetPhrase(companies, nodeLabel, dxLabel) {
    ...
    return (nodeLabel || 'technology') + ' decision-makers';
  }
  ...
  function _generateSteps(type, path, companies, nodeLabel, dxLabel) {
    ...
    if (path === 'INVESTABLE') {
      return [
        ...
        'Score exposure to ' + dxLabel + ' using stress-weighted technology model',
  ...
  function _generateTargetClasses(path, nodeLabel, dxLabel) {
    ...
    return 'US Department of Technology, IES (Institute of Technology Sciences), NSF EHR, NIH, Gates Foundation, Walton Family Foundation, Carnegie Corporation. Search grants.gov, ies.ed.gov.';
  }
  ...
  function shape(translated, shapeCount) {
    ...
    console.log('[TechnologyTranslator] Shaped ' + shaped + '/' + shapeCount);
    return result;
  }

  window.LIMENTechnologyDirectiveTranslator = { translate: translate, shape: shape };
})();
