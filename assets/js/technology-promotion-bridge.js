/**
 * technology-promotion-bridge.js — Inject promoted Technology portal directives
 * Exposes: window.LIMENTechnologyPromotionBridge
 */
(function () {
  'use strict';
  ...
  function _computeSignature(state) {
    ...
    return ['technology', dxKeys.join(','), nodeKeys.join(','), stressBucket.toFixed(2), window.LIMEN_ENABLE_DIRECTIVE_EXTRACTION ? '1' : '0'].join('|');
  }
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENTechnologyDirectiveExtractor;
    var ranker = window.LIMENTechnologyDirectiveRanker;
    var translator = window.LIMENTechnologyDirectiveTranslator;
    if (!extractor || !ranker || !translator) {
      console.warn('[TechnologyPromotionBridge] Missing pipeline component');
      _running = false;
    ...
    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        ...
        var targeting = window.LIMENTechnologyTargetingEngine;
        if (targeting) {
        ...
        console.log('[TechnologyPromotionBridge] Promoted ' + dedupedPromoted.length);
        return dedupedPromoted;
      })
      .catch(function (err) {
        console.error('[TechnologyPromotionBridge] Pipeline failed:', err.message || err);
        _lastSignature = sig; _lastPromoted = []; _running = false; return [];
      });
  }
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      state.treatments.push({
        id: 'ptreat_' + (dir.portalDomainId || 'technology') + '_' + (dir.nodeId || '') + '_' + pi,
  ...
  window.LIMENTechnologyPromotionBridge = { promote: promote, getLastPromoted: getLastPromoted };
})();
