/*
 * religion-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/religion-neuro-substrate.json
 * Prompts:       assets/prompts/religion-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/religion.json
 * or any existing religion-*.js. It reads existing Religion data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Religion brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Religion file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/religion-neuro-substrate.json';
  var RELIGION_URL = '/assets/data/domains/religion.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Religion issue id. */
     ...
     * Reads an Religion issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Religion issue. */
    auditAllIssues: function () {
      return loadJSON(RELIGION_URL).then(function (religion) {
        return (religion.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.ReligionNeuroSubstrate) root.ReligionNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Religion brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import ReligionNeuroSubstrate from '/assets/js/religion-neuro-substrate.js';
   *      // then, after issues render:
   *      ReligionNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
