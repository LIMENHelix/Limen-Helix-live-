/*
 * finance-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Finance's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (finance-offline-maintenance.js)
 *   - refractory absolute window       (finance-refractory-limiter.js)
 *   - prediction-error threshold       (finance-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Finance has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Finance has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.FinanceMetaplasticity) root.FinanceMetaplasticity = API;
