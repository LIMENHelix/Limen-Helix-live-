/*
 * industry-telemetry-adapter.js  — wires runtime fields to the LIVE Industry telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/industry-pulse-engine.js  (window.LIMENIndustryPulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Industry issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to industry.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into industry.json (a
...
(function (root) {
   ...
   * inputs: { industry, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (industry.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (industry.activations || []).forEach(function (a) {
  ...
  /* Merge industry.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(industry, overlay) {
    var base = industry.runtime || {};
    ...
    var acts = {}; (industry.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (industry.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the industry definition + brainState. */
  function fromLive(industry, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENIndustryPulse) ? window.LIMENIndustryPulse : null;
    if (!P) return null;
    return fromPulse({ industry: industry, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN industry definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/industry.json', 'industry'),
      _load('/assets/data/industry-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ industry: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.IndustryTelemetryAdapter) root.IndustryTelemetryAdapter = API;
