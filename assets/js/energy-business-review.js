/**
 * energy-business-review.js — Energy Business Assignment Review Surface
 *
 * ENERGY DOMAIN ONLY. Renders a review panel for proposed business mappings.
 * Requires energy-node-business-engine.js to be loaded first.
 *
 * Provides APPROVE / DENY / HOLD buttons per proposal.
 * Only APPROVED mappings become active in the Energy operator surface.
 *
...
(function () {
  ...
  function renderCard(entry) {
    ...
    if (entry.neuroTranslation) {
      ...
      h += '<div style="font-size:0.32rem;color:#b0a898;line-height:1.5"><b style="color:rgba(201,169,78,0.7)">In business (Energy systems):</b> ' + esc(entry.neuroTranslation.inBusiness) + '</div>';
      h += '</div>';
    }
    ...
    if (entry.existingCompanies && entry.existingCompanies.length > 0) {
      ...
      h += '<div class="ebr-card-existing" style="color:#C9A94E">Missing from current Energy system</div>';
    }
  ...
  function render() {
    var engine = window.LIMENEnergyBusinessEngine;
    if (!engine) return;
      ...
      bizBtns[bi].addEventListener('click', function (e) {
        ...
        try {
          sessionStorage.setItem('limen_exec_context', JSON.stringify({
            ...
            source: 'energy', returnTo: '/domain-console?domain=energy&mode=operator'
          }));
        } catch (ex) {}
        window.location.href = 'energy-workspace.html?track=business&opp=' + encodeURIComponent(oppKey) + '&returnTo=' + encodeURIComponent('/domain-console?domain=energy&mode=operator');
      });
    }
  ...
  window.LIMENEnergyBusinessReview = {
    render: render,
    mount: mount
  };

  console.log('[EnergyBusinessReview] Loaded — review surface ready');
