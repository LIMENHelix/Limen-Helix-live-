/*
 * intelligence-telemetry-adapter.js  — wires runtime fields to the LIVE Intelligence telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/intelligence-pulse-engine.js  (window.LIMENIntelligencePulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Intelligence issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to intelligence.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into intelligence.json (a
...
(function (root) {
   ...
   * inputs: { intelligence, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    inp = inp || {};
    var intelligence = inp.intelligence || {};
    ...
    var issuesOverlay = {};
    (intelligence.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (intelligence.activations || []).forEach(function (a) {
  ...
  /* Merge intelligence.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(intelligence, overlay) {
    var base = intelligence.runtime || {};
    ...
    var acts = {}; (intelligence.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (intelligence.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the intelligence definition + brainState. */
  function fromLive(intelligence, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENIntelligencePulse) ? window.LIMENIntelligencePulse : null;
    if (!P) return null;
    return fromPulse({ intelligence: intelligence, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN intelligence definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/intelligence.json', 'intelligence'),
      _load('/assets/data/intelligence-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ intelligence: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.IntelligenceTelemetryAdapter) root.IntelligenceTelemetryAdapter = API;
