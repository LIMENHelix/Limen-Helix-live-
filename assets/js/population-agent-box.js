/**
 * population-agent-box.js — the Population domain's operator AI box (client side).
 *
 * A floating "Talk to Population" panel on the population domain-console. It KNOWS its
 * domain (window.LIMENPopulationBrain.getStateSummary()) and can make bounded
 ...
 *    tries /api/population-agent first and falls back to LOCAL on any failure, so it
...
(function () {
  ...
  function parseLocal(prompt, state) {
    ...
    if (mF) {
      var topic = mF[1].replace(/\b(please|now|today|this week|domain|population|closely|carefully)\b/g, '').trim();
  ...
  if (typeof window !== 'undefined') window.__populationAgentLocal = { parse: parseLocal, query: answerQuery };

  ready(function () {
    if (window.__populationAgentBox) return; window.__populationAgentBox = true;
    var LS_KEY = 'limenPopulationAgentKey';
    var esc = function (s) { return String(s == null ? '' : s).replace(/[&<>"]/g, function (c) { return ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[c]; }); };
    function brain() { return window.LIMENPopulationBrain || null; }
    ...
    var toggle = document.createElement('div'); toggle.id = 'eab-toggle'; toggle.textContent = '⚡ Talk to Population';
    var panel = document.createElement('div'); panel.id = 'eab';
    panel.innerHTML =
      '<div class="eab-h"><span><b>Population</b> · ask or steer</span><span class="eab-x">✕</span></div>' +
    ...
    function renderResult(pending, result, mode, left) {
      ...
      if (applied) html += '<div class="eab-tool">✓ applied to population domain:\n' + esc(applied) + '</div>';
    ...
    function send() {
      var b = brain();
      if (!b) { add('<span class="eab-ai">Population brain not loaded on this page.</span>'); return; }
