/**
 * technology-agent-box.js — the Technology domain's operator AI box (client side).
 *
 * A floating "Talk to Technology" panel on the technology domain-console. It KNOWS its
 * domain (window.LIMENTechnologyBrain.getStateSummary()) and can make bounded
 ...
 *    tries /api/technology-agent first and falls back to LOCAL on any failure, so it
...
(function () {
  ...
  function parseLocal(prompt, state) {
    ...
    if (mF) {
      var topic = mF[1].replace(/\b(please|now|today|this week|domain|technology|closely|carefully)\b/g, '').trim();
  ...
  if (typeof window !== 'undefined') window.__technologyAgentLocal = { parse: parseLocal, query: answerQuery };

  ready(function () {
    if (window.__technologyAgentBox) return; window.__technologyAgentBox = true;
    var LS_KEY = 'limenTechnologyAgentKey';
    var esc = function (s) { return String(s == null ? '' : s).replace(/[&<>"]/g, function (c) { return ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[c]; }); };
    function brain() { return window.LIMENTechnologyBrain || null; }
    ...
    var toggle = document.createElement('div'); toggle.id = 'eab-toggle'; toggle.textContent = '⚡ Talk to Technology';
    var panel = document.createElement('div'); panel.id = 'eab';
    panel.innerHTML =
      '<div class="eab-h"><span><b>Technology</b> · ask or steer</span><span class="eab-x">✕</span></div>' +
    ...
    function renderResult(pending, result, mode, left) {
      ...
      if (applied) html += '<div class="eab-tool">✓ applied to technology domain:\n' + esc(applied) + '</div>';
    ...
    function send() {
      var b = brain();
      if (!b) { add('<span class="eab-ai">Technology brain not loaded on this page.</span>'); return; }
    ...
    add('<span class="eab-ai">Technology domain online (free local mode). Ask about it, or steer it: "focus on grid", "prefer research", "autonomy off", "how is technology doing", "show the forecast". I bias the domain, never force a finding, move capital, or touch code. Add a passcode above for conversational AI once billing is on.</span>');
  });
})();
