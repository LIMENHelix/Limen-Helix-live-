/*
 * governance-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/governance-neuro-substrate.json
 * Prompts:       assets/prompts/governance-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/governance.json
 * or any existing governance-*.js. It reads existing Governance data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Governance brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Governance file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/governance-neuro-substrate.json';
  var GOVERNANCE_URL = '/assets/data/domains/governance.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Governance issue id. */
     ...
     * Reads an Governance issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Governance issue. */
    auditAllIssues: function () {
      return loadJSON(GOVERNANCE_URL).then(function (governance) {
        return (governance.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.GovernanceNeuroSubstrate) root.GovernanceNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Governance brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import GovernanceNeuroSubstrate from '/assets/js/governance-neuro-substrate.js';
   *      // then, after issues render:
   *      GovernanceNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
