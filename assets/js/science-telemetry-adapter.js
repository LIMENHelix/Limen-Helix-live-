/*
 * science-telemetry-adapter.js  — wires runtime fields to the LIVE Science telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/science-pulse-engine.js  (window.LIMENSciencePulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Science issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to science.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into science.json (a
...
(function (root) {
   ...
   * inputs: { science, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (science.issues || []).forEach(function (is) {
  ...
  /* Merge science.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(science, overlay) {
    var base = science.runtime || {};
    ...
    var acts = {}; (science.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (science.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the science definition + brainState. */
  function fromLive(science, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENSciencePulse) ? window.LIMENSciencePulse : null;
    if (!P) return null;
    return fromPulse({ science: science, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN science definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/science.json', 'science'),
      _load('/assets/data/science-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ science: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.ScienceTelemetryAdapter) root.ScienceTelemetryAdapter = API;
