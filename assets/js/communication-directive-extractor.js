/**
 * communication-directive-extractor.js — Exhaustive Fractal Directive Extraction (Communication)
 *
 * Prebuilt-first, live fallback. Self-gates: ?domain=communication
 * Exposes: window.LIMENCommunicationDirectiveExtractor
 */
(function () {
  'use strict';
  ...
  function _loadPrebuiltPayload() {
    ...
    return fetch('/assets/data/deep/communication-deep-directives.json')
      .then(function (r) { if (!r.ok) throw new Error(r.status); return r.json(); })
      .then(function (data) {
        if (data && data.directives && data.directives.length > 0) {
          _prebuiltPayload = data;
          console.log('[CommunicationExtractor] Prebuilt: ' + data.directives.length + ' directives, maxDepth=' + (data.traversal && data.traversal.maxDepth));
        }
      ...
      if (prebuilt && prebuilt.directives) {
        ...
        for (var pi = 0; pi < prebuilt.directives.length; pi++) {
          ...
          if (dxCtx) {
            ...
            d.stress = stress; d.domain = 'communication';
            filtered.push(d);
        ...
        console.log('[CommunicationExtractor] Prebuilt: ' + filtered.length + ' candidates, ' + activeDx.length + ' active dx');
        return filtered;
      }
      ...
      (function (pid) {
        chain = chain.then(function () {
          return _fetchPortalJSON(pid).then(function (data) {
            if (!data) return;
            return _extractFromPortal(pid, data, activeNodeIds, stress, 1, ['communication'], seen, results);
          });
        });
      })(diagnosisMappedPortals[pi]);
    }
    return chain
      .then(function () { return _extractFromPortal('communication', portal, activeNodeIds, stress, 0, [], seen, results); })
      .then(function () { console.log('[CommunicationExtractor] Extracted ' + results.length); return results; });
  }

  window.LIMENCommunicationDirectiveExtractor = { extract: extract, _fetchPortalJSON: _fetchPortalJSON, _portalFileToId: _portalFileToId };
})();
