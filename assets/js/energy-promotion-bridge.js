/**
 * energy-promotion-bridge.js — Inject promoted portal directives into operator surfaces
 *
 ...
 *
 * Exposes: window.LIMENEnergyPromotionBridge
 */
(function () {
  'use strict';
   ...
   * @param {Object} portal - L0 energy portal JSON
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENEnergyDirectiveExtractor;
    var ranker = window.LIMENEnergyDirectiveRanker;
    var translator = window.LIMENEnergyDirectiveTranslator;

    if (!extractor || !ranker || !translator) {
      console.warn('[PromotionBridge] Missing module(s) — extractor:', !!extractor, 'ranker:', !!ranker, 'translator:', !!translator);
      _running = false;
      return Promise.resolve([]);
    }

    console.log('[PromotionBridge] Input signature changed — running extraction pipeline');
    console.log('[PromotionBridge] Signature: ' + sig);
    var t0 = Date.now();

    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        if (!candidates || candidates.length === 0) {
          console.log('[PromotionBridge] No candidates extracted');
          _lastSignature = sig;
          _lastPromoted = [];
          _running = false;
          return [];
        ...
        console.log('[PromotionBridge] Extracted ' + candidates.length + ' candidates in ' + (Date.now() - t0) + 'ms');
        ...
        if (bestProofIdx > limit - 1 && bestProofIdx !== -1) {
          ...
          console.log('[PromotionBridge] Elevated proof directive from position ' + bestProofIdx + ': ' + proofItem.treatmentLabel.substring(0, 50));
        }
        ...
        if (bestDeepIdx !== -1 && bestDeepIdx >= limit) {
          ...
          console.log('[PromotionBridge] Deep proof slot: L' + deepItem.depth + ' ' + deepItem.treatmentLabel.substring(0, 50) + ' (deepScore=' + bestDeepScore.toFixed(3) + ')');
        } else if (bestDeepIdx !== -1 && bestDeepIdx < limit) {
          // Already in top set — just mark it
          ranked[bestDeepIdx]._isDeepProofSlot = true;
          console.log('[PromotionBridge] Deep proof already in top set: L' + ranked[bestDeepIdx].depth);
        }

        console.log('[PromotionBridge] Top 5 after depth-aware selection:');
        ...
        var targeting = window.LIMENEnergyTargetingEngine;
        if (targeting) {
        ...
        console.log('[PromotionBridge] Promoted ' + dedupedPromoted.length + ' directives (' + (Date.now() - t0) + 'ms total)');
        return dedupedPromoted;
      })
      .catch(function (err) {
        console.error('[PromotionBridge] Pipeline failed — existing opportunities untouched:', err.message || err);
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      var overlay = {
        id: 'ptreat_' + (dir.portalDomainId || 'energy') + '_' + (dir.nodeId || '') + '_' + pi,
  ...
  window.LIMENEnergyPromotionBridge = {
    promote: promote,
