/*
 * finance-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/finance-neuro-substrate.json
 * Prompts:       assets/prompts/finance-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/finance.json
 * or any existing finance-*.js. It reads existing Finance data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Finance brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Finance file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/finance-neuro-substrate.json';
  var FINANCE_URL = '/assets/data/domains/finance.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Finance issue id. */
     ...
     * Reads an Finance issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Finance issue. */
    auditAllIssues: function () {
      return loadJSON(FINANCE_URL).then(function (finance) {
        return (finance.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.FinanceNeuroSubstrate) root.FinanceNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Finance brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import FinanceNeuroSubstrate from '/assets/js/finance-neuro-substrate.js';
   *      // then, after issues render:
   *      FinanceNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
