/*
 * economy-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes economy.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Economy has no runtime load field). { nodeId: loadFraction }
  ...
  /* economy: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(economy, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (economy.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. economy.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Economy has no runtime load field). throttleGain operator-set. Never writes economy.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.EconomyRetrogradeThrottle) root.EconomyRetrogradeThrottle = API;
