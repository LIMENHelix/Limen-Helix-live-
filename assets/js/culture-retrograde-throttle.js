/*
 * culture-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes culture.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Culture has no runtime load field). { nodeId: loadFraction }
  ...
  /* culture: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(culture, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (culture.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. culture.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Culture has no runtime load field). throttleGain operator-set. Never writes culture.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.CultureRetrogradeThrottle) root.CultureRetrogradeThrottle = API;
