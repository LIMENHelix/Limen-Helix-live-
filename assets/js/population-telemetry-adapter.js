/*
 * population-telemetry-adapter.js  — wires runtime fields to the LIVE Population telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/population-pulse-engine.js  (window.LIMENPopulationPulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Population issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to population.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into population.json (a
...
(function (root) {
   ...
   * inputs: { population, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (population.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (population.activations || []).forEach(function (a) {
  ...
  /* Merge population.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(population, overlay) {
    var base = population.runtime || {};
    ...
    var acts = {}; (population.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (population.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the population definition + brainState. */
  function fromLive(population, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENPopulationPulse) ? window.LIMENPopulationPulse : null;
    if (!P) return null;
    return fromPulse({ population: population, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN population definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/population.json', 'population'),
      _load('/assets/data/population-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ population: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.PopulationTelemetryAdapter) root.PopulationTelemetryAdapter = API;
