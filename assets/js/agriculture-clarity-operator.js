...
(function () {
  ...
  var VIEW_ID = 'aos-operator-view';
  var STATUS_KEY = 'limen_agriculture_operator_status';
  var COLLAPSE_KEY = 'limen_agriculture_collapse_state';
  ...
  function getState() {
    ...
    var brain = brains.get('agriculture');
  ...
  var DX_CONTEXT = {
    'CASH_FLOW_CRISIS': {
      what: 'Farm operating margins are collapsing',
      money: 'Input costs exceeding revenue creates distressed asset acquisition, agri-finance restructuring, and crop insurance product opportunities.',
      step: 'Check USDA ERS farm income data. Position in farm credit (FCS) or distressed agri-land funds (FPI).',
      outcome: '10-25% returns on farm finance restructuring or insurance products'
    },
    'SUPPLY_CHAIN_BREAKDOWN': {
      what: 'Food logistics and cold chain are disrupted',
      money: 'Cold chain failures and distribution gaps create demand for resilient supply infrastructure, alternative routing, and logistics technology.',
      step: 'Search sam.gov for USDA supply chain resilience grants. Check cold chain providers (LW, COLD).',
      outcome: '$250K-$5M government contracts or logistics platform licensing'
    },
    'DROUGHT': {
      what: 'Water stress is compressing crop yields',
      money: 'Drought conditions drive demand for irrigation technology, drought-resistant genetics, crop insurance products, and water management patents.',
      step: 'Check NOAA drought monitor. Position in irrigation (LNN) or crop insurance. File precision irrigation patents.',
      outcome: '20-40% agtech growth or $500K-$5M patent value in water management'
    },
    'MARKET_COLLAPSE': {
      what: 'Commodity prices are in freefall',
      money: 'Demand destruction and export restrictions create distressed positioning in farmland, grain storage, and substitution acceleration opportunities.',
      step: 'Check CME grain futures. Position in grain storage (ADM, BG) or farmland REITs (FPI).',
      outcome: '15-30% recovery premium on distressed agricultural assets'
    },
    'EQUIPMENT_FAILURE': {
      what: 'Farm equipment capacity is constrained',
      money: 'Parts shortages and maintenance backlogs create demand for autonomous systems, predictive maintenance platforms, and equipment leasing.',
      step: 'Check Deere dealer inventory data. Position in equipment (DE, AGCO, CNHI) or precision ag platforms.',
      outcome: '10-20% equipment sector premium during shortage'
    },
    'PEST_OUTBREAK': {
      what: 'Biological threat is spreading across crops',
      money: 'Pest/disease outbreak drives demand for biocontrol products, monitoring systems, containment infrastructure, and resistant seed genetics.',
      step: 'Check USDA APHIS alerts. Position in crop protection (FMC, CTVA) or biological control firms.',
      outcome: '20-35% crop protection sector premium or $1M-$10M biocontrol patent value'
    }
  };
  ...
  // Diagnosis → playbook ID for Agriculture-domain invest opportunities
  var DX_TO_PLAYBOOK = {
  ...
  var SOURCE_TO_PLAYBOOK = {
    'company_terminal':  'agri_finance',
    'company_stressed':  'agri_finance',
    'convergence':       'climate_resilience',
    'cross_domain':      'agri_supply'
  };

  // Registered playbook definitions for sessionStorage handoff
  var PLAYBOOK_DEFS = {
    'agri_finance': { title: 'Agricultural Finance & Risk', domains: ['agriculture', 'economy'], type: 'invest' },
    'agri_supply': { title: 'Agricultural Supply Chain', domains: ['agriculture', 'supplyChain'], type: 'invest' },
    'climate_resilience': { title: 'Climate Resilience & AgTech', domains: ['agriculture', 'environment'], type: 'invest' },
  ...
  // validation: HELIX_VALIDATED (in command-board-data, phase-scored), NODE_MAPPED (in p2_agri.json portal),
  //             DOMAIN_MAPPED (in agriculture command board), ETF_PROXY (sector proxy, no company-level validation)
  var INVEST_TARGETS = {
    'agri_finance': [
      { ticker: 'MOO',  name: 'VanEck Agribusiness ETF',    cik: null,      validation: 'ETF_PROXY',       reason: 'Broad agribusiness ETF — thematic proxy for agricultural finance and supply' },
      { ticker: 'ADM',  name: 'Archer-Daniels-Midland',     cik: '7084',    validation: 'HELIX_VALIDATED', reason: 'Global grain trader and processor; captures margin on commodity flow disruption' },
      { ticker: 'BG',   name: 'Bunge Global',               cik: '1144519', validation: 'HELIX_VALIDATED', reason: 'Global agribusiness; oilseed processing and grain origination' },
      { ticker: 'NTR',  name: 'Nutrien',                    cik: '1725964', validation: 'HELIX_VALIDATED', reason: 'Largest fertilizer producer; input cost dynamics drive revenue' },
      { ticker: 'DE',   name: 'Deere & Company',            cik: '315189',  validation: 'HELIX_VALIDATED', reason: 'Farm equipment leader; precision agriculture technology platform' },
      { ticker: 'FPI',  name: 'Farmland Partners',          cik: '1591670', validation: 'DOMAIN_MAPPED',   reason: 'Farmland REIT — direct exposure to agricultural land value' }
    ],
    'agri_supply': [
      { ticker: 'DE',   name: 'Deere & Company',            cik: '315189',  validation: 'HELIX_VALIDATED', reason: 'Farm equipment and precision ag; autonomous systems and logistics' },
      { ticker: 'AGCO', name: 'AGCO Corporation',           cik: '880266',  validation: 'HELIX_VALIDATED', reason: 'Global farm equipment manufacturer; Fendt, Massey Ferguson brands' },
      { ticker: 'CNHI', name: 'CNH Industrial',             cik: '1567094', validation: 'HELIX_VALIDATED', reason: 'Case IH and New Holland equipment; parts and service network' },
      { ticker: 'LNN',  name: 'Lindsay Corporation',        cik: '60714',   validation: 'NODE_MAPPED',     reason: 'Irrigation systems manufacturer; mapped to water management node' },
      { ticker: 'LW',   name: 'Lamb Weston',                cik: '1679273', validation: 'HELIX_VALIDATED', reason: 'Frozen potato processing; cold chain infrastructure operator' },
      { ticker: 'COLD', name: 'Americold Realty',           cik: '1455863', validation: 'DOMAIN_MAPPED',   reason: 'Temperature-controlled warehouse REIT; cold chain infrastructure' }
    ],
    'climate_resilience': [
      { ticker: 'CTVA', name: 'Corteva Agriscience',        cik: '1755672', validation: 'HELIX_VALIDATED', reason: 'Seed genetics and crop protection; drought-tolerant trait development' },
      { ticker: 'FMC',  name: 'FMC Corporation',            cik: '37996',   validation: 'HELIX_VALIDATED', reason: 'Crop protection chemicals and biologicals; pest/disease management' },
      { ticker: 'SMG',  name: 'Scotts Miracle-Gro',         cik: '1173514', validation: 'HELIX_VALIDATED', reason: 'Lawn and garden; hydroponics and controlled environment agriculture' },
      { ticker: 'ANDE', name: 'Andersons Inc',              cik: '821026',  validation: 'HELIX_VALIDATED', reason: 'Grain storage, ethanol, and plant nutrient operations' },
      { ticker: 'DBA',  name: 'Invesco DB Agriculture',     cik: null,      validation: 'ETF_PROXY',       reason: 'Agriculture commodity ETF — tracks grain, livestock, softs futures' },
      { ticker: 'PAGG', name: 'Invesco Global Agriculture',  cik: null,      validation: 'ETF_PROXY',       reason: 'Global agriculture equities ETF — broad sector exposure' }
    ]
  };
  ...
  function renderTargets(pbId) {
    ...
    h += '<div class="eos-invest-meaning">INVEST means: take a position in companies or ETFs expected to benefit from this Agriculture condition. These are not buy recommendations \u2014 candidates for your own due diligence.</div>';
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if (t.validation === 'NODE_MAPPED') h += '<div style="margin-bottom:4px;color:#4a8fd4">Mapped to an Agriculture portal node. Company-level Helix validation pending.</div>';
      // Action links
      h += '<div style="margin-top:4px">';
      if (t.cik) h += '<a class="eos-target-link" href="helix-report.html?cik=' + esc(t.cik) + '&company=' + esc(t.ticker.toLowerCase()) + '&source_surface=domain_clarity_operator&domain=agriculture&requested_report_type=partial_phase_snapshot" target="_blank">HELIX REPORT</a>';
  ...
  var MECH_EXPLAIN = {
    'CASH_FLOW_CRISIS': {
      'input_cost_spike':     { why: 'Fertilizer, seed, and fuel costs surged past revenue capacity. Farms are burning cash. Lenders are tightening.', move: 'Build a farm financial stress scorecard. Sell it to ag lenders, crop insurance adjusters, and USDA field offices managing loan portfolios.' },
      'margin_compression':   { why: 'Revenue per acre is flat or declining while costs rise. Operating margins are collapsing across grain and livestock.', move: 'Build a margin recovery model. Sell it to farm management companies and agricultural consultants advising distressed operations.' },
      'debt_service':         { why: 'Farm debt payments exceed cash flow. Equipment loans, land mortgages, and operating lines are at risk of default.', move: 'Build a debt restructuring package. Sell it to Farm Credit System banks and USDA FSA offices managing loan workouts.' },
      'price_dislocation':    { why: 'Commodity prices disconnected from production costs. Farmers are producing at a loss. Markets are oversupplied or export-blocked.', move: 'Build a price-cost spread model. Sell it to commodity traders, grain elevators, and farm managers timing sales.' }
    },
    'SUPPLY_CHAIN_BREAKDOWN': {
      'logistics_constraint': { why: 'Trucks, rail, or barge capacity is unavailable. Grain is piling up at elevators. Perishables are spoiling. Cold chain is broken.', move: 'Build a logistics bottleneck map. Sell it to food distributors, grocery chains, and USDA agencies managing emergency food routing.' },
      'supply_disruption':    { why: 'Input supplies — seed, fertilizer, crop protection — are not reaching farms on time. Planting windows are closing.', move: 'Build an input delivery tracker. Sell it to agricultural retailers, cooperatives, and farm managers managing planting schedules.' },
      'infrastructure_stress':{ why: 'Storage facilities, processing plants, or distribution centers are at capacity or failing. The physical food system is constrained.', move: 'Build an infrastructure capacity model. Sell it to food processors, cold storage operators, and USDA emergency planners.' },
      'market_structure':     { why: 'Normal buyer-seller relationships are disrupted. Contract defaults, force majeure, and spot market chaos.', move: 'Build a contract renegotiation toolkit. Sell it to grain traders, food processors, and cooperatives managing disrupted agreements.' }
    },
    'DROUGHT': {
      'supply_disruption':    { why: 'Crop yields are collapsing across drought-affected regions. Food supply will tighten in 3-6 months.', move: 'Build a yield impact model by county and crop. Sell it to commodity traders, food companies, and USDA crop insurance adjusters.' },
      'infrastructure_stress':{ why: 'Irrigation systems are failing or running dry. Wells are dropping. Water rights are being contested.', move: 'Build a water availability scorecard. Sell it to irrigation districts, state water agencies, and agricultural lenders assessing collateral risk.' },
      'production_impact':    { why: 'Fields are being abandoned or switched to drought-tolerant crops. Production mix is shifting in real time.', move: 'Build a crop switching model. Sell it to seed companies, agricultural insurers, and commodity traders positioning for changed supply.' },
      'policy_response':      { why: 'USDA is activating disaster designations. Emergency programs are opening. Water use restrictions are being imposed.', move: 'Build a disaster program eligibility tracker. Sell it to farm managers, extension agents, and USDA county offices processing claims.' }
    },
    'MARKET_COLLAPSE': {
      'price_dislocation':    { why: 'Commodity prices dropped below production cost. Grain elevators are full. Export markets are closed or restricted.', move: 'Build a distressed asset identification model. Sell it to agricultural investors, farmland funds, and grain trading firms.' },
      'supply_disruption':    { why: 'Oversupply is backing up through the system. Storage is full. Processing capacity is idle. Farmers cannot sell.', move: 'Build a surplus routing model. Sell it to food banks, government commodity programs, and alternative market developers.' },
      'market_structure':     { why: 'Export restrictions, tariffs, or sanctions have closed major markets. Trade flows are rerouting. Contract values are shifting.', move: 'Build a trade flow rerouting model. Sell it to grain exporters, trading companies, and government trade agencies.' },
      'contract_mispricing':  { why: 'Forward contracts written at higher prices are now underwater. Counterparties are defaulting or renegotiating.', move: 'Build a contract exposure audit. Sell it to grain merchandisers, food companies, and cooperatives managing default risk.' }
    },
    'EQUIPMENT_FAILURE': {
      'infrastructure_stress':{ why: 'Farm equipment is breaking down during critical windows. Parts are backordered. Dealers have no inventory.', move: 'Build an equipment availability dashboard. Sell it to farm managers, equipment dealers, and agricultural lenders monitoring operational risk.' },
      'logistics_constraint': { why: 'Equipment parts supply chains are disrupted. Manufacturing delays ripple through to farms missing planting or harvest windows.', move: 'Build a parts supply tracker. Sell it to equipment dealers, OEM service networks, and farm cooperatives managing fleet maintenance.' },
      'production_impact':    { why: 'Fields cannot be planted or harvested because equipment is unavailable. Yield losses are escalating daily.', move: 'Build a harvest timing risk model. Sell it to crop insurance adjusters, farm managers, and commodity traders pricing delayed harvest.' },
      'supply_disruption':    { why: 'Downstream food processing is affected as harvest delays reduce raw material availability.', move: 'Build a processing capacity planner. Sell it to food processors, ethanol plants, and feed mills managing reduced throughput.' }
    },
    'PEST_OUTBREAK': {
      'supply_disruption':    { why: 'Pest damage is destroying standing crops. Yield estimates are being revised down. Food supply will tighten.', move: 'Build a pest damage assessment model. Sell it to crop insurance adjusters, commodity traders, and food companies repricing supply.' },
      'production_impact':    { why: 'Farmers are emergency-spraying, quarantining fields, or abandoning crops. Production costs are spiking.', move: 'Build a treatment cost model. Sell it to crop protection companies, agricultural consultants, and farm lenders assessing exposure.' },
      'infrastructure_stress':{ why: 'Quarantine zones are restricting crop movement. Processing plants in affected areas may need to source from elsewhere.', move: 'Build a quarantine impact map. Sell it to food processors, grain traders, and state departments of agriculture managing containment.' },
      'policy_response':      { why: 'USDA APHIS is declaring pest emergencies. New restrictions and eradication programs are being funded.', move: 'Build a federal response tracker. Sell it to crop protection companies, biocontrol firms, and agricultural consultants positioning for new program funding.' }
    }
  };
  ...
  function _loadBranchIndex() { if (_branchIndex) return Promise.resolve(_branchIndex); if (_branchIndexFailed) return Promise.resolve(null); return fetch('/assets/data/deep/p2_agri-branch-index.json').then(function(r){if(!r.ok)throw new Error(r.status);return r.json();}).then(function(d){_branchIndex=d;return d;}).catch(function(){_branchIndexFailed=true;return null;}); }
  ...
  function buildMoneySummary(state) {
    ...
    if (stress > 0.70) {
      text = '<b>Agriculture stress at ' + stressPct + '% \u2014 crisis level.</b> Multiple agricultural systems under acute pressure. Farm margins collapsing, supply chains disrupted, or biological threats spreading. ';
      if (activeDx.length > 0) {
        text += activeDx.length + ' active diagnosis pathway' + (activeDx.length > 1 ? 's' : '') + ' confirmed by live evidence.';
      }
    } else if (stress > 0.50) {
      text = '<b>Agriculture stress at ' + stressPct + '% \u2014 elevated.</b> Sustained pressure on agricultural operations. Input costs, logistics, or weather conditions creating actionable stress. ';
      if (activeDx.length > 0) text += activeDx.length + ' active diagnosis pathway' + (activeDx.length > 1 ? 's' : '') + '.';
    } else if (stress > 0.30) {
      text = '<b>Agriculture stress at ' + stressPct + '%.</b> Above baseline but within manageable range. ';
      if (activeDx.length > 0) {
      ...
      text = '<b>Agriculture stress at ' + stressPct + '%.</b> Within normal range. No acute pressure. ';
      if (activeDx.length === 0) text += 'No immediate money on the table from stress alone.';
    } else {
      // No live feed data
      text = '<b>Agriculture feeds loading.</b> Waiting for live agricultural data. ';
  ...
  function buildTopPlays(state) {
    ...
    for (var i = 0; i < top.length; i++) {
      ...
      if ((o.path === 'GRANT-ELIGIBLE' || o.path === 'PATENTABLE') && window.LIMENAgricultureExecutionPanels) {
        h += window.LIMENAgricultureExecutionPanels.renderForOpportunity(oppKey(o), o.path, o);
      }
      h += '</div>';
    }
    h += '</div>';
  ...
  function buildActionQueue(state) {
    ...
    if (ledger) {
      var claims = ledger.getClaimsByDomain('agriculture');
      var oppIds = {};
    ...
    for (var i = 0; i < opps.length; i++) {
      ...
      var _claimExisting = window.LIMENClaimLedger ? window.LIMENClaimLedger.isOpportunityClaimed(o.id || key, 'agriculture') : null;
      if (_claimExisting) {
  ...
  function renderOperator() {
    ...
    var bridge = window.LIMENAgriculturePromotionBridge;
    if (bridge && window.LIMEN_ENABLE_DIRECTIVE_EXTRACTION) {
      var brains = window.LIMENDomainBrains;
      var brain = brains ? brains.get('agriculture') : null;
      ...
      if (portalCache) {
        ...
        console.log('[AgricultureOperator] Portal cache not ready — scheduling retry');
        setTimeout(function () {
          var rb = brains ? brains.get('agriculture') : null;
  ...
  function _renderOperatorDOM(state) {
    ...
    for (var ib = 0; ib < investBtns.length; ib++) {
      investBtns[ib].addEventListener('click', function (e) {
        ...
        var def = PLAYBOOK_DEFS[pbId] || { title: oppTitle, domains: ['agriculture'], type: 'invest' };
        ...
        if (!outcome) outcome = 'Linked agriculture companies capture sector premium during sustained stress. Monitor for confirmation and position sizing.';
        ...
        window.location.href = 'investment-console.html?opp=' + encodeURIComponent(pbId) + '&source=agriculture&returnTo=' + encodeURIComponent('/domain-console?domain=agriculture');
      });
    }
      ...
      execBtns[eb].addEventListener('click', function (e) {
        ...
        try {
          ...
          sessionStorage.setItem('limen_exec_context', JSON.stringify({
            ...
            source: 'agriculture',
            returnTo: '/domain-console?domain=agriculture&mode=operator'
          }));
        } catch (ex) {}
        var trackName = trackMap[execPath] || 'grant';
        window.location.href = 'agriculture-workspace.html?track=' + trackName + '&opp=' + encodeURIComponent(execKey) + '&returnTo=' + encodeURIComponent('/domain-console?domain=agriculture&mode=operator');
      });
    }
    ...
    for (var cb = 0; cb < claimBtns.length; cb++) {
      claimBtns[cb].addEventListener('click', function (e) {
        ...
        // Open claim modal
        window.LIMENClaimFlow.openClaimModal(opp, 'agriculture', function (confirmedOpp, estimate) {
          // Create claim in ledger
          if (window.LIMENClaimLedger) {
            window.LIMENClaimLedger.createClaim(confirmedOpp, 'agriculture', estimate);
          }
    ...
    if (window.LIMENOperatorPanel) {
      window.LIMENOperatorPanel.mount(_operatorView, 'agriculture');
    }
    ...
    if (window.LIMENAgricultureBusinessReview) {
      window.LIMENAgricultureBusinessReview.mount(_operatorView);
    }
  }
  ...
  function boot() {
    ...
    console.log('[AgricultureOperator] Booted — operator view created, toggle wired');
  }
  ...
  var _bootCheck = setInterval(function () {
    ...
    if (brainRendered && state && state.updated > 0) {
      ...
      // One-shot re-render after next brain cycle — ensures diagnoses + directives
      // are populated before the operator surface finalizes. The first boot render
      // often fires before the brain's portal fetch + deriveDiagnoses completes.
      // This listens for exactly one brain-update, then re-renders and detaches.
      ...
      // Safety: remove listener after 90s regardless (prevents zombie listener)
      setTimeout(function () { window.removeEventListener('limen:domain-brain-update', _onBrainUpdate); }, 90000);
    }
  }, 300);

  // Brain updates do NOT trigger full operator re-render (no scroll-destroying repaints).
  // The one-shot listener above handles the initial diagnosis-population race only.
  // After that, data refreshes happen only when the user switches views or claims.
