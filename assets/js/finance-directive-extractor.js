/**
 * finance-directive-extractor.js — Exhaustive Fractal Directive Extraction
 *
 * Traverses the finance portal fractal tree starting from active diagnosis
 ...
 *
 * Exposes: window.LIMENFinanceDirectiveExtractor
 */
(function () {
  'use strict';
  ...
  function _loadPrebuiltPayload() {
    ...
    return fetch('/assets/data/deep/finance-deep-directives.json')
      .then(function (r) {
      ...
      .then(function (data) {
        if (data && data.directives && data.directives.length > 0) {
          ...
          console.log('[FinanceDirectiveExtractor] Prebuilt payload loaded: ' + data.directives.length + ' deep directives, maxDepth=' + data.traversal.maxDepth);
        }
      ...
      .catch(function () {
        ...
        _prebuiltLoading = false;
        console.log('[FinanceDirectiveExtractor] No prebuilt payload — falling back to live extraction');
        return null;
      });
  }
   ...
   * e.g. "finance_banking_retail_portal.html" → "finance_banking_retail"
   */
   ...
   * @param {Object} portal - L0 finance portal JSON (finance.json)
  ...
  function extract(state, portal, opts) {
    ...
    return _loadPrebuiltPayload().then(function (prebuilt) {
      if (prebuilt && prebuilt.directives) {
        ...
        console.log('[FinanceDirectiveExtractor] Prebuilt: ' + filtered.length + ' candidates from ' + prebuilt.directives.length + ' total, ' + activeDx.length + ' active diagnoses');
        return filtered;
      }

      // ── FALLBACK: live portal extraction (original path) ──
      console.log('[FinanceDirectiveExtractor] Using live extraction fallback');
  ...
  function _liveExtract(state, portal, activeNodeIds, activeDx, stress, opts) {
      ...
      (function (pid) {
        chain = chain.then(function () {
          return _fetchPortalJSON(pid).then(function (data) {
            if (!data) return;
            return _extractFromPortal(pid, data, activeNodeIds, stress, 1, ['finance'], seen, results);
          });
        });
    ...
    return chain
      .then(function () {
        ...
        return _extractFromPortal('finance', portal, activeNodeIds, stress, 0, [], seen, results);
      })
      .then(function () {
        console.log('[FinanceDirectiveExtractor] Extracted ' + results.length + ' candidates from ' +
  ...
  window.LIMENFinanceDirectiveExtractor = {
    extract: extract,
    // Exposed for testing
