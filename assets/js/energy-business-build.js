/**
 * energy-business-build.js — BUSINESS BUILD Execution Workspace v2
 *
 ...
 *
 * Energy domain only.
 * Exposes: window.LIMENEnergyBusinessBuild
 */
(function () {
  'use strict';

  var params = new URLSearchParams(window.location.search);
  var isEnergy = params.get('domain') === 'energy';
  var isWorkspace = window.location.pathname.indexOf('energy-workspace') !== -1;
  if (!isEnergy && !isWorkspace) return;

  var STORE_KEY = 'limen_energy_business_build';
  ...
  var TEMPLATES = {
    ...
    'energy_operator': {
      label: 'Energy Operator / Infrastructure Business',
      description: 'A business that operates, maintains, or manages energy infrastructure. Examples: solar farm operator, grid monitoring service, battery storage operator, pipeline inspection.',
      capitalIntensity: 'high', regulated: true, typicalStartup: '$100K-$5M+',
      legalNotes: 'LLC or C-Corp. Significant regulatory requirements: FERC, state utility commission, NERC compliance, environmental permits. May need specialized insurance and bonding.',
      fundingNotes: 'Requires significant capital. Likely SBA 7(a) or conventional term loan. Equipment financing essential. May qualify for DOE grants or state energy incentives.',
      operatingNotes: 'Revenue often from long-term contracts (PPA, maintenance agreements). Regulatory compliance is ongoing. Equipment reliability is critical.',
      first90: [
        'DAYS 1-7: Hire an attorney with energy regulatory experience. Ask for referrals from your state bar association or search Martindale-Hubbell. Schedule a 1-hour consultation to discuss entity structure, permits, and compliance. Budget $300-$500 for initial consult. OUTPUT: Attorney retained, legal roadmap documented.',
        'DAYS 1-7: Form entity (LLC or C-Corp based on attorney advice). File with Secretary of State. Apply for EIN at irs.gov/ein. If planning to raise investment, attorney should advise on C-Corp with Delaware incorporation. OUTPUT: Entity formed, EIN obtained.',
        'DAYS 8-14: Identify all federal, state, and local permits required. Contact your state public utility commission. Check FERC requirements if operating interstate. Check NERC compliance if touching the grid. Make a list with application deadlines and fees. OUTPUT: Complete permit requirements list with deadlines.',
        'DAYS 8-14: Begin permit applications that have long lead times. Some environmental permits take 6-12 months. File early. Budget $5,000-$50,000 for permit fees and environmental assessments depending on project scope. OUTPUT: Priority permit applications submitted.',
        'DAYS 15-30: Identify your target site or facility. Verify zoning compliance with local planning department. Request quotes from 3 potential landlords or sellers. Do not sign a lease until permits are confirmed for that location. OUTPUT: Target site identified, zoning confirmed.',
        'DAYS 15-30: Contact equipment vendors (3 minimum). Request formal quotes with delivery timelines, warranty terms, and installation support. Compare total cost of ownership, not just purchase price. OUTPUT: Equipment vendor comparison table.',
        'DAYS 31-45: Begin SBA 7(a) loan application or alternative financing. Bring: business plan, pro forma projections, personal financial statement, tax returns, collateral documentation. Allow 30-60 days for SBA approval. OUTPUT: Loan application submitted.',
        'DAYS 31-45: Search grants.gov and sam.gov for DOE energy infrastructure grants. Check your state energy office for incentive programs. Apply to any open solicitations with matching scope. OUTPUT: Grant applications submitted.',
        'DAYS 46-60: Hire an operations manager with direct experience in your infrastructure type. Write a job description with specific technical requirements. Post on LinkedIn and industry job boards. Budget $60K-$100K annual salary. OUTPUT: Operations manager hired.',
        'DAYS 46-60: Write safety and compliance protocols. Include: emergency response procedures, maintenance schedules, reporting requirements, OSHA compliance checklist. Have your attorney and operations manager review. OUTPUT: Safety manual completed.',
        'DAYS 61-90: Begin outreach to first customers. For energy infrastructure, this means utilities, municipalities, or commercial offtakers. Prepare a capabilities presentation. Offer a pilot or introductory contract. OUTPUT: First customer or offtake agreement signed.'
      ]
    },
    'product_equipment': {
  ...
  var STAGES = [
    ...
    { id: 'stage1', title: 'STAGE 1 — BUSINESS DEFINITION', sections: [
      { id: 'concept', title: 'Business Concept', type: 'fields',
        ...
        fields: [
          { id: 'name', label: 'Business name (draft)', type: 'text', placeholder: 'e.g., GridWatch Monitoring LLC' },
      ...
      { id: 'biz_type', title: 'Business Type & Mode', type: 'fields',
        ...
        fields: [
          { id: 'template', label: 'Business type template', type: 'select', options: ['local_service', 'advisory', 'energy_operator', 'product_equipment', 'franchise_ready', 'facility'] },
  ...
  function autoGenerate(ws, prefill, tmpl) {
    ...
    var bizName = (prefill && prefill.businessType) ? prefill.businessType : 'New Energy Business';
    ...
    ws.fields['concept']['one_liner'] = ws.fields['concept']['one_liner'] || 'We provide ' + bizName.toLowerCase() + ' services to the energy sector.';
  ...
  function renderPanel(oppKey, prefill) {
    ...
    if (!tmplKey && prefill && prefill.businessType) {
      ...
      else if (bt.indexOf('operator') !== -1 || bt.indexOf('grid') !== -1 || bt.indexOf('plant') !== -1) tmplKey = 'energy_operator';
  ...
  window.LIMENEnergyBusinessBuild = { renderPanel: renderPanel, wirePanel: wirePanel, STAGES: STAGES, TEMPLATES: TEMPLATES };
  console.log('[EnergyBusinessBuild] v2 loaded — 7 stages, 6 templates, 40+ subsections');
})();
