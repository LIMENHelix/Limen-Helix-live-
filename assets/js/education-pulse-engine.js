/**
 * education-pulse-engine.js — Live Feed Pulse Engine for Education Domain
 * Self-gates: only runs when ?domain=education
 * Exposes: window.LIMENEducationPulse
 */
(function () {
  'use strict';
  ...
  var FRESHNESS_TTL = {
    'education_indicator': 24 * 60 * 60 * 1000,
  ...
  var SOURCE_TYPES = {
    'World Bank Education':       'education_indicator',
    'OpenAlex Institutions':      'institutional_index',
    'NCES News':                  'policy_feed',
    'ED.gov News':                'policy_feed',
    'EdWeek':                     'news_feed',
    'OECD Education':             'education_indicator',
    'IES News':                   'policy_feed'
  };

  var EVIDENCE_CONTRACTS = {
    'FUNDING_CRISIS': {
      label: 'Education Funding Crisis',
      requiredFamilies: ['policy_event', 'institutional_event'],
      minimumEvidence: 1,
      freshnessThreshold: FRESHNESS_TTL.policy_feed,
      primaryIndicators: ['budget_shortfall', 'resource_scarcity', 'institutional_decline'],
      secondaryConfirmation: ['infrastructure_degradation'],
      catchAllBlocked: true
    },
    'TEACHER_SHORTAGE': {
      label: 'Teacher Shortage Emergency',
      requiredFamilies: ['workforce_event', 'policy_event'],
      minimumEvidence: 1,
      freshnessThreshold: FRESHNESS_TTL.workforce_feed,
      primaryIndicators: ['workforce_gap', 'retention_failure', 'recruitment_deficit', 'burnout_epidemic'],
      secondaryConfirmation: [],
      catchAllBlocked: true
    },
    'ACHIEVEMENT_GAP': {
      label: 'Achievement Gap Crisis',
      requiredFamilies: ['indicator_event', 'policy_event'],
      minimumEvidence: 1,
      freshnessThreshold: FRESHNESS_TTL.education_indicator,
      primaryIndicators: ['outcome_disparity', 'access_inequality', 'performance_decline', 'equity_failure'],
      secondaryConfirmation: [],
      catchAllBlocked: false
    },
    'ACCREDITATION_FAILURE': {
      label: 'Accreditation & Quality Failure',
      requiredFamilies: ['institutional_event', 'policy_event'],
      minimumEvidence: 1,
      freshnessThreshold: FRESHNESS_TTL.event_cluster,
      primaryIndicators: ['quality_degradation', 'standards_erosion', 'oversight_failure', 'credential_devaluation'],
      secondaryConfirmation: ['institutional_decline'],
      catchAllBlocked: true
    },
    'TECHNOLOGY_DISRUPTION': {
      label: 'Technology Disruption of Education',
      requiredFamilies: ['news_event', 'institutional_event'],
      minimumEvidence: 1,
      freshnessThreshold: FRESHNESS_TTL.news_feed,
      primaryIndicators: ['digital_divide', 'platform_transition', 'pedagogical_mismatch', 'automation_displacement'],
      secondaryConfirmation: ['adaptive_failure'],
      catchAllBlocked: true
    }
  };
  ...
  function computePulse(brainState) {
    ...
    for (var ci = 0; ci < conditions.length; ci++) {
      ...
      if (cond === 'education_high_stress' || cond === 'structural_stress') addFamily('structural_event');
  ...
  console.log('[EducationPulse] Loaded');
})();
