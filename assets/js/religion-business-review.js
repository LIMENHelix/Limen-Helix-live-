/**
 * religion-business-review.js — Religion Business Assignment Review Surface
 *
 * RELIGION DOMAIN ONLY. Renders a review panel for proposed business mappings
 * across the full 103-node hierarchy.
 * Requires religion-node-business-engine.js to be loaded first.
 *
 * Self-gates: only runs when ?domain=religion or ?domain=research
 * Exposes: window.LIMENReligionBusinessReview
 */
(function () {
  'use strict';
  ...
  function renderCard(entry) {
    ...
    // Business type + badges (deduped)
    h += '<div class="sbr-card-header">';
    h += '<div class="sbr-card-type">' + esc(entry.businessType) + '</div>';
    h += '<div>';
    var _badgeSeen = {};
    if (entry.tier) {
      var _tierKey = entry.tier.toUpperCase();
      if (!_badgeSeen[_tierKey]) { _badgeSeen[_tierKey] = true; h += tierBadge(entry.tier); }
    }
    ...
    if (entry.variantState) {
      ...
      if (!_badgeSeen[_vs]) {
        ...
        h += ' <span class="sbr-badge" style="' + (_vsColors[entry.variantState] || '') + '">' + entry.variantState + '</span>';
      }
    }
    if (entry.nodeActive && !_badgeSeen['ACTIVE']) { _badgeSeen['ACTIVE'] = true; h += ' <span class="sbr-badge" style="color:#e85454;border:1px solid rgba(232,84,84,0.2)">ACTIVE</span>'; }
    h += '</div></div>';
    ...
    // Neurology / Religion translation
    if (entry.neuroTranslation) {
      ...
      h += '<div style="font-size:0.26rem;letter-spacing:2px;color:rgba(74,143,212,0.5);margin-bottom:4px">IN NEUROLOGY / IN RELIGION TRANSLATION</div>';
      ...
      h += '<div style="font-size:0.32rem;color:#b0a898;line-height:1.5"><b style="color:rgba(201,169,78,0.7)">In religion:</b> ' + esc(entry.neuroTranslation.inBusiness) + '</div>';
      h += '</div>';
    }
    ...
    h += '<div style="margin-bottom:4px"><b style="color:#b0a898">What dysfunction looks like:</b> ' + esc(entry.plainDysregulation || entry.dysregulation || 'Institutional failure, community fracture, or loss of function in this area') + '</div>';
    ...
    if (entry.existingCompanies && entry.existingCompanies.length > 0) {
      ...
      h += '<div class="sbr-card-existing" style="color:#C9A94E">Missing from current Religion system</div>';
    }
  ...
  function render() {
    var engine = window.LIMENReligionBusinessEngine;
    if (!engine) return;
    injectStyles();
    ...
    h += '<div class="sbr-title" style="margin-bottom:0">NODE-TO-BUSINESS ASSIGNMENT REVIEW <a href="/execution-framework" target="_blank" style="font-size:0.22rem;color:rgba(201,169,78,0.4);text-decoration:none;margin-left:8px;border:1px solid rgba(201,169,78,0.1);padding:1px 5px;border-radius:2px" onclick="event.stopPropagation()">LEGAL</a></div>';
    ...
    var filters = [
      ...
      { id: 'denied', label: 'DENIED (' + deniedCount + ')' },
      ...
      bizBtns[bi].addEventListener('click', function (e) {
        ...
        try {
          sessionStorage.setItem('limen_exec_context', JSON.stringify({
            ...
            source: 'religion', returnTo: '/domain-console?domain=religion&mode=operator'
          }));
        } catch (ex) {}
        window.location.href = 'religion-workspace.html?track=business&opp=' + encodeURIComponent(oppKey) + '&returnTo=' + encodeURIComponent('/domain-console?domain=religion&mode=operator');
      });
    }
  ...
  window.LIMENReligionBusinessReview = { render: render, mount: mount };
  console.log('[ReligionBusinessReview] Loaded \u2014 review surface ready');
})();
