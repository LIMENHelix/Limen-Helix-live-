/**
 * environment-promotion-bridge.js — Inject promoted Religion portal directives into operator surfaces
 *
 ...
 * Exposes: window.LIMENReligionPromotionBridge
 */
(function () {
  'use strict';
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENReligionDirectiveExtractor;
    var ranker = window.LIMENReligionDirectiveRanker;
    var translator = window.LIMENReligionDirectiveTranslator;
    ...
    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        ...
        var targeting = window.LIMENReligionTargetingEngine;
        if (targeting) {
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      state.treatments.push({
        id: 'ptreat_' + (dir.portalDomainId || 'religion') + '_' + (dir.nodeId || '') + '_' + pi,
  ...
  window.LIMENReligionPromotionBridge = {
    promote: promote,
