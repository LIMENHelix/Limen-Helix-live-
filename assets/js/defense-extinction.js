/*
 * defense-extinction.js  — ADDITIVE mechanism (completes EXTINCTION_LEARNING)
 * ----------------------------------------------------------------------------
 ...
 * (complements the GLOBAL down-scaling in defense-offline-maintenance.js).
 *
 * CONTRACT: read-only / copy-only. NEVER writes defense.json. Proposes which acquired responses to
...
(function (root) {
  ...
  /* Extinction over Defense activations: an activation whose diagnosticTriggers are ALL absent
   * from the caller-supplied activeTriggers is a candidate to retire (its trigger is gone).
   * activeTriggers: array of currently-firing trigger strings (runtime state; Defense has none baked in). */
  function proposeExtinction(defense, activeTriggers, opts) {
    ...
    return {
      ...
      note: 'Read-only PROPOSAL. Applies nothing; retiring a node would edit defense.json. activeTriggers is caller-supplied runtime state.'
    };
  }

  var API = { proposeExtinction: proposeExtinction };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.DefenseExtinction) root.DefenseExtinction = API;
