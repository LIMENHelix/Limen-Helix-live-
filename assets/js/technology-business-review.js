/**
 * technology-business-review.js — Technology Business Assignment Review Surface
 *
 * TECHNOLOGY DOMAIN ONLY. Renders a review panel for proposed business mappings
 * across the full 103-node hierarchy.
 * Requires technology-node-business-engine.js to be loaded first.
 *
 * Self-gates: only runs when ?domain=technology
 * Exposes: window.LIMENTechnologyBusinessReview
 */
(function () {
  'use strict';
  ...
  var PANEL_ID = 'tbr-business-review';
  ...
  function statusBadge(approval) {
    ...
    var cls = { 'PROPOSED': 'tbr-badge-proposed', 'APPROVED': 'tbr-badge-approved', 'DENIED': 'tbr-badge-denied', 'NEEDS_REVIEW': 'tbr-badge-proposed' };
  ...
  function renderCard(entry) {
    ...
    if (entry.nodeActive) h += ' <span class="tbr-badge" style="color:#e85454;border:1px solid rgba(232,84,84,0.2)">ACTIVE</span>';
    h += '</div></div>';
    ...
    if (entry.neuroTranslation) {
      ...
      h += '<div style="font-size:0.26rem;letter-spacing:2px;color:rgba(74,143,212,0.5);margin-bottom:4px">IN NEUROLOGY / IN TECHNOLOGY TRANSLATION</div>';
      ...
      h += '<div style="font-size:0.32rem;color:#b0a898;line-height:1.5"><b style="color:rgba(201,169,78,0.7)">In technology (learning systems):</b> ' + esc(entry.neuroTranslation.inBusiness) + '</div>';
      h += '</div>';
    }
    ...
    if (entry.existingCompanies && entry.existingCompanies.length > 0) {
      ...
      h += '<div class="tbr-card-existing" style="color:#C9A94E">Missing from current Technology system</div>';
    }
  ...
  function render() {
    var engine = window.LIMENTechnologyBusinessEngine;
    if (!engine) return;
    injectStyles();
    ...
    h += '<div class="tbr-title" style="margin-bottom:0">NODE-TO-BUSINESS ASSIGNMENT REVIEW</div>';
  ...
  window.LIMENTechnologyBusinessReview = { render: render, mount: mount };
  console.log('[TechnologyBusinessReview] Loaded \u2014 review surface ready');
})();
