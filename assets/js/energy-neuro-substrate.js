/*
 * energy-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/energy-neuro-substrate.json
 * Prompts:       assets/prompts/energy-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/energy.json
 * or any existing energy-*.js. It reads existing Energy data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Energy brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Energy file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/energy-neuro-substrate.json';
  var ENERGY_URL = '/assets/data/domains/energy.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Energy issue id. */
     ...
     * Reads an Energy issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Energy issue. */
    auditAllIssues: function () {
      return loadJSON(ENERGY_URL).then(function (energy) {
        return (energy.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.EnergyNeuroSubstrate) root.EnergyNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Energy brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import EnergyNeuroSubstrate from '/assets/js/energy-neuro-substrate.js';
   *      // then, after issues render:
   *      EnergyNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
