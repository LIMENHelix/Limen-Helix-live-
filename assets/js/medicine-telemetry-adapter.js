/*
 * medicine-telemetry-adapter.js  — wires runtime fields to the LIVE Medicine telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/medicine-pulse-engine.js  (window.LIMENMedicinePulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Medicine issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to medicine.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into medicine.json (a
...
(function (root) {
   ...
   * inputs: { medicine, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (medicine.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (medicine.activations || []).forEach(function (a) {
  ...
  /* Merge medicine.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(medicine, overlay) {
    var base = medicine.runtime || {};
    ...
    var acts = {}; (medicine.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (medicine.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the medicine definition + brainState. */
  function fromLive(medicine, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENMedicinePulse) ? window.LIMENMedicinePulse : null;
    if (!P) return null;
    return fromPulse({ medicine: medicine, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN medicine definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/medicine.json', 'medicine'),
      _load('/assets/data/medicine-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ medicine: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.MedicineTelemetryAdapter) root.MedicineTelemetryAdapter = API;
