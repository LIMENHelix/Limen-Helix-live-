/*
 * trade-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/trade-neuro-substrate.json
 * Prompts:       assets/prompts/trade-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/trade.json
 * or any existing trade-*.js. It reads existing Trade data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Trade brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Trade file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/trade-neuro-substrate.json';
  var TRADE_URL = '/assets/data/domains/trade.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Trade issue id. */
     ...
     * Reads an Trade issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Trade issue. */
    auditAllIssues: function () {
      return loadJSON(TRADE_URL).then(function (trade) {
        return (trade.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.TradeNeuroSubstrate) root.TradeNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Trade brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import TradeNeuroSubstrate from '/assets/js/trade-neuro-substrate.js';
   *      // then, after issues render:
   *      TradeNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
