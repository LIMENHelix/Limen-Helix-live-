/**
 * science-claim-flow.js — Science Domain Claim Acceptance Flow
 *
 * Adds CLAIM button to Science opportunity cards. On click, opens
 * modal requiring acceptance of framework terms before creating
 * a claim record. Science-scoped only. No backend.
 *
 * Depends on: science-compensation.js, science-claim-ledger.js
 * Namespace: window.LIMENScience.economy.claimFlow
 */
(function () {
  'use strict';
  ...
  function openClaimModal(opportunity) {
    ...
    var comp = window.LIMENScience.economy.compensation;
    ...
    h += '<div class="scf-title">Claim Science Opportunity</div>';
    ...
    h += '<div>\u2022 Operator is responsible for research integrity, data protection, and authorship ethics</div>';
    h += '</div></div>';

    h += '<label class="scf-accept">';
    h += '<input type="checkbox" id="scfAcceptCheck">';
    h += '<span class="scf-accept-text">I understand this is a Science-domain execution claim, I am responsible for truthful submission, documented progress, and compliance with applicable research integrity, IRB, and data protection rules.</span>';
    h += '</label>';
  ...
  window.addEventListener('limen:science-claim-update', function () { setTimeout(injectIntoCards, 100); });

  window.LIMENScience.economy.claimFlow = {
