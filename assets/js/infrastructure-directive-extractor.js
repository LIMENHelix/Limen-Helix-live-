/**
 * infrastructure-directive-extractor.js — Exhaustive Fractal Directive Extraction
 *
 * Traverses the infrastructure portal fractal tree starting from active diagnosis
 ...
 *
 * Exposes: window.LIMENInfrastructureDirectiveExtractor
 */
(function () {
  'use strict';
  ...
  function _loadPrebuiltPayload() {
    ...
    return fetch('/assets/data/deep/infrastructure-deep-directives.json')
      .then(function (r) { if (!r.ok) throw new Error(r.status); return r.json(); })
      .then(function (data) { if (data && data.directives && data.directives.length > 0) { _prebuiltPayload = data; console.log('[InfraExtractor] Prebuilt: ' + data.directives.length + ' directives, maxDepth=' + data.traversal.maxDepth); } return _prebuiltPayload; })
   ...
   * e.g. "infrastructure_bridge_steel_portal.html" → "infrastructure_bridge_steel"
   */
   ...
   * @param {Object} portal - L0 infrastructure portal JSON (infrastructure.json)
    ...
    return _loadPrebuiltPayload().then(function (prebuilt) {
      if (prebuilt && prebuilt.directives) {
        ...
        for (var pi = 0; pi < prebuilt.directives.length; pi++) {
          ...
          if (dxCtx) { d.diagnosisId = dxCtx.diagnosisId; d.diagnosisLabel = dxCtx.diagnosisLabel; d.circuitDir = dxCtx.dir; d.circuitDetail = dxCtx.detail; d.circuitEvidence = dxCtx.evidence; d.stress = stress; d.domain = 'infrastructure'; filtered.push(d); }
          else if (d.treatmentEvidence === 'Strong' || d.treatmentEvidence === 'A') { d.stress = stress; d.domain = 'infrastructure'; filtered.push(d); }
        }
        console.log('[InfraExtractor] Prebuilt: ' + filtered.length + ' candidates, ' + activeDx.length + ' active dx');
        return filtered;
      }
  ...
  function _liveExtract(activeNodeIds, activeDx, stress, portal) {
      ...
      (function (pid) {
        chain = chain.then(function () {
          return _fetchPortalJSON(pid).then(function (data) {
            if (!data) return;
            return _extractFromPortal(pid, data, activeNodeIds, stress, 1, ['infrastructure'], seen, results);
          });
        });
    ...
    return chain
      .then(function () {
        ...
        return _extractFromPortal('infrastructure', portal, activeNodeIds, stress, 0, [], seen, results);
      })
      .then(function () {
        console.log('[InfrastructureDirectiveExtractor] Extracted ' + results.length + ' candidates from ' +
  ...
  window.LIMENInfrastructureDirectiveExtractor = {
    extract: extract,
    // Exposed for testing
