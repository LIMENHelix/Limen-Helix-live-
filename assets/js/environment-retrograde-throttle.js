/*
 * environment-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes environment.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Environment has no runtime load field). { nodeId: loadFraction }
  ...
  /* environment: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(environment, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (environment.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. environment.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Environment has no runtime load field). throttleGain operator-set. Never writes environment.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.EnvironmentRetrogradeThrottle) root.EnvironmentRetrogradeThrottle = API;
