/*
 * science-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Science's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (science-offline-maintenance.js)
 *   - refractory absolute window       (science-refractory-limiter.js)
 *   - prediction-error threshold       (science-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Science has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Science has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.ScienceMetaplasticity) root.ScienceMetaplasticity = API;
