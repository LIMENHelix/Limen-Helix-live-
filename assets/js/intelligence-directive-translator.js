/**
 * intelligence-directive-translator.js — Convert ranked Intelligence directives into opportunity objects
 * Carries _omittedSiblingCount through. Exposes: window.LIMENIntelligenceDirectiveTranslator
 */
(function () {
  'use strict';
  ...
  function _buildMonetizationPath(d) {
    ...
    if (type === 'INVESTABLE') {
      if (tickerStr) return 'Firms in this pathway (' + tickerStr + ') benefit from the intelligence condition. Position during stress window for repricing.';
      return 'Firms serving ' + (d.nodeLabel || 'this function').toLowerCase() + ' capture demand created by the active diagnosis.';
    }
    if (type === 'PATENTABLE') return 'Intelligence gap in ' + (d.nodeLabel || 'this area').toLowerCase() + ' has no dominant IP holder. First filing creates defensible edtech position.';
    return 'Active diagnosis creates documented intelligence need. Grant proposals backed by live system evidence win at higher rates.';
  }
  ...
  function _buildNextAction(d) {
    ...
    if (type === 'INVESTABLE' && companies.length > 0) return 'Check ' + companies[0].ticker + ' (' + companies[0].name + ') current intelligence exposure.';
    if (type === 'PATENTABLE') return 'Search patents.google.com for prior art in "' + (d.nodeLabel || 'edtech').toLowerCase() + '" intelligence.';
    return 'Search grants.gov, ED.gov, IES grants for solicitations matching "' + (d.nodeLabel || 'intelligence').toLowerCase() + '".';
  }
  ...
  function _buildEvidence(d) {
    var parts = ['Domain: intelligence. Stress: ' + Math.round((d.stress || 0) * 100) + '%.'];
  ...
  function translate(ranked, limit) {
    ...
    for (var i = 0; i < Math.min(ranked.length, limit); i++) {
      ...
      if (primaryPath === 'GRANT-ELIGIBLE') targetStr += (targetStr ? ' ' : '') + 'Agencies: US Dept of Intelligence, IES, NSF EHR, Gates Foundation, Walton, Carnegie. Search grants.gov, ies.ed.gov.';
      ...
      opportunities.push({
        id: 'pdir_' + (d.portalDomainId || 'intelligence') + '_' + (d.nodeId || '') + '_' + i,
        title: _buildTitle(d),
        rank: d.rankScore || 0,
        path: primaryPath,
        urgency: urgency,
        ...
        valueRange: primaryPath === 'INVESTABLE' ? '10-25% sector premium' : primaryPath === 'PATENTABLE' ? '5-15% royalty' : '$250K-$5M intelligence grants',
  ...
  var REVENUE_MODELS = {
    'INVESTABLE': [
      { model: 'advisory', desc: 'Build a intelligence sector exposure scorecard. Sell to intelligence primes, sovereign wealth funds, and family offices with global security exposure. $100K-$1M per engagement.' },
      { model: 'trading', desc: 'Position in intelligence primes (LMT, RTX, NOC, GD), European intelligence (BA., HO, RHM), drone / autonomous systems, and uranium / nuclear. Target 20-40% sector premium during sustained tension.' },
      { model: 'intelligence', desc: 'Build a global threat-monitoring and intelligence procurement intelligence dashboard. Sell to intelligence ministries, primes, and sovereign procurement offices worldwide.' }
    ],
    'GRANT-ELIGIBLE': [
      { model: 'grant award', desc: 'Write a intelligence research or capability proposal. Submit to DARPA, IARPA, AFRL, ONR, ARO, ARL, NATO STO, EU EDF, AUKUS Pillar 2, UK DASA. $500K-$50M per award.' },
      { model: 'consulting', desc: 'Win the contract. Deliver the funded work. Bill against cost-plus or fixed-price terms. Intelligence contracts typically 1-5 year base + options.' },
      { model: 'cost recovery', desc: 'Deploy the capability at a intelligence ministry or NATO command. Bill through Foreign Military Sales (FMS), Direct Commercial Sales (DCS), or sustainment contracts.' }
    ],
    'PATENTABLE': [
      { model: 'licensing', desc: 'File a intelligence patent (be aware of secrecy orders under 35 USC 181). License to intelligence primes (LMT, RTX, NOC, GD, BAE, Thales, Leonardo, Saab, MBDA, Rafael).' },
      { model: 'SaaS / dual-use', desc: 'Turn the technique into a dual-use platform. Sell to both intelligence and adjacent commercial markets (logistics, security, autonomy).' },
      { model: 'acquisition', desc: 'Build the IP portfolio. Position for acquisition by a intelligence prime or intelligence-tech investor (Anduril, Shield AI, Palantir).' }
    ]
  };

  var TITLE_TEMPLATES = [
    { match: /invasion|incursion|territorial|force projection|tank|artillery|amphibious/i, titles: ['Build battlespace awareness + sustainment platform \u2014 sell to allied intelligence ministries and primes'] },
    ...
    { match: /intelligence|sigint|humint|osint|surveillance|reconnaissance|estimate/i, titles: ['Build OSINT / multi-INT analysis platform \u2014 sell to IC, allied intel services, intelligence primes'] },
    { match: /logistics|supply chain|munitions|sustainment|sealift|airlift|spare parts/i, titles: ['Build intelligence logistics / sustainment platform \u2014 sell to DLA, NATO NSPA, allied support commands'] },
    ...
    { match: /arms sale|fms|export license|itar|intelligence contract|procurement|rfp/i, titles: ['Build intelligence procurement intelligence + bid support \u2014 sell to primes and exporters'] },
    ...
    { match: /assessment|diagnostic/i, titles: ['Build intelligence risk scorecard \u2014 sell to {target}'] }
  ];
  ...
  function _targetPhrase(companies, nodeLabel, dxLabel) {
    ...
    return (nodeLabel || 'intelligence') + ' decision-makers';
  }
  ...
  function _generateSteps(type, path, companies, nodeLabel, dxLabel) {
    ...
    if (path === 'INVESTABLE') {
      return [
        ...
        'Score exposure to ' + dxLabel + ' using stress-weighted intelligence model',
    ...
    return [
      ...
      'Prepare budget narrative: personnel, intelligence, indirect costs',
  ...
  function _generateTargetClasses(path, nodeLabel, dxLabel) {
    ...
    return 'US Department of Intelligence, IES (Institute of Intelligence Sciences), NSF EHR, NIH, Gates Foundation, Walton Family Foundation, Carnegie Corporation. Search grants.gov, ies.ed.gov.';
  }
  ...
  window.LIMENIntelligenceDirectiveTranslator = { translate: translate, shape: shape };
})();
