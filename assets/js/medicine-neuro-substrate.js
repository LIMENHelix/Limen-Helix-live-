/*
 * medicine-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/medicine-neuro-substrate.json
 * Prompts:       assets/prompts/medicine-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/medicine.json
 * or any existing medicine-*.js. It reads existing Medicine data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Medicine brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Medicine file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/medicine-neuro-substrate.json';
  var MEDICINE_URL = '/assets/data/domains/medicine.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Medicine issue id. */
     ...
     * Reads an Medicine issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Medicine issue. */
    auditAllIssues: function () {
      return loadJSON(MEDICINE_URL).then(function (medicine) {
        return (medicine.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.MedicineNeuroSubstrate) root.MedicineNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Medicine brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import MedicineNeuroSubstrate from '/assets/js/medicine-neuro-substrate.js';
   *      // then, after issues render:
   *      MedicineNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
