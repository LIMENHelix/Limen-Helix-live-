/*
 * education-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes education.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Education has no runtime load field). { nodeId: loadFraction }
  ...
  /* education: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(education, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (education.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. education.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Education has no runtime load field). throttleGain operator-set. Never writes education.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.EducationRetrogradeThrottle) root.EducationRetrogradeThrottle = API;
