/**
 * energy-claim-ledger.js — Energy Domain Claim Ledger (Local Only)
 *
 * Stores claim records in localStorage. Tracks opportunity claims
 * from acceptance through outcome. Energy-scoped only.
 *
 * Namespace: window.LIMENEnergy.economy.claims
 */
(function () {
  'use strict';
  ...
  var STORE_KEY = 'LIMEN_ENERGY_CLAIMS';
  ...
  function createClaimRecord(opportunity) {
    ...
    try { window.dispatchEvent(new CustomEvent('limen:energy-claim-update')); } catch (e) {}
  ...
  window.LIMENEnergy.economy.claims = {
  ...
  console.log('[EnergyClaimLedger] Loaded — claim ledger ready');
})();
