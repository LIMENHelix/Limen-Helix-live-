/**
 * energy-directive-extractor.js — Exhaustive Fractal Directive Extraction
 *
 * Traverses the energy portal fractal tree starting from active diagnosis
 ...
 *
 * Exposes: window.LIMENEnergyDirectiveExtractor
 */
(function () {
  'use strict';
  ...
  function _loadPrebuiltPayload() {
    ...
    return fetch('/assets/data/deep/energy-deep-directives.json')
      .then(function (r) {
      ...
      .then(function (data) {
        if (data && data.directives && data.directives.length > 0) {
          ...
          console.log('[DirectiveExtractor] Prebuilt payload loaded: ' + data.directives.length + ' deep directives, maxDepth=' + data.traversal.maxDepth);
        }
      ...
      .catch(function () {
        ...
        _prebuiltLoading = false;
        console.log('[DirectiveExtractor] No prebuilt payload — falling back to live extraction');
        return null;
      });
  }
  ...
  function _fetchPortalJSON(domainId) {
    ...
    return fetch(staticUrl)
      .then(function (r) {
        if (!r.ok) {
          _notFoundCache[domainId] = true; // Remember this 404
   ...
   * e.g. "energy_battery_lithium_portal.html" → "energy_battery_lithium"
   */
  ...
  function _extractFromPortal(domainId, portalData, activeNodeIds, stress, depth, path, seen, results) {
    ...
    // Fetch child portals in parallel — 404s resolve instantly from cache,
    // live files return data for recursive extraction
   ...
   *
   * @param {Object} state - energy brain state
   * @param {Object} portal - L0 energy portal JSON (energy.json)
  ...
  function extract(state, portal, opts) {
    ...
    // activeDx gate removed from prebuilt path — ranked pool serves even in calm markets.
    // Gate preserved in _liveExtract fallback only.
    ...
    return _loadPrebuiltPayload().then(function (prebuilt) {
      if (prebuilt && prebuilt.directives) {
        ...
        console.log('[DirectiveExtractor] Prebuilt: ' + filtered.length + ' candidates from ' + prebuilt.directives.length + ' total, ' + activeDx.length + ' active diagnoses');
        return filtered;
      }

      // ── FALLBACK: live portal extraction (original path) ──
      console.log('[DirectiveExtractor] Using live extraction fallback');
  ...
  function _liveExtract(state, portal, activeNodeIds, activeDx, stress, opts) {
    ...
    // Fetch portals that the resolver maps to the active diagnosis.
    // These are the highest-relevance branches for the current condition.
    // By visiting them first, they claim extraction budget before generic
    // L0 childPortal descent can exhaust it on lower-relevance branches
    // (e.g. biomass filling budget before grid/transmission for GRID_COLLAPSE).
      ...
      (function (pid) {
        chain = chain.then(function () {
          return _fetchPortalJSON(pid).then(function (data) {
            if (!data) return;
            return _extractFromPortal(pid, data, activeNodeIds, stress, 1, ['energy'], seen, results);
          });
        });
    ...
    return chain
      .then(function () {
        ...
        return _extractFromPortal('energy', portal, activeNodeIds, stress, 0, [], seen, results);
      })
      .then(function () {
        console.log('[DirectiveExtractor] Extracted ' + results.length + ' candidates from ' +
  ...
  window.LIMENEnergyDirectiveExtractor = {
    extract: extract,
    // Exposed for testing
