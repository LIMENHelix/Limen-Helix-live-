...
(function () {
  ...
  function _renderTabCivilization() {
    ...
    // one domain row was actually added (no empty "DETECTED DIAGNOSES & TREATMENTS"
    // header with no content).
    if (report.registryDiagnoses) {
      var _regCardRendered = false;
      var regCard = null;
      var _pfDomains = window.LIMENDomains || {};
      var regDk = Object.keys(report.registryDiagnoses);
      function _ensureRegCard() {
        if (regCard) return regCard;
        regCard = _subCard('DETECTED DIAGNOSES & TREATMENTS');
        _regCardRendered = true;
        return regCard;
      }
      for (var ri = 0; ri < regDk.length; ri++) {
        // ── Prefer brain diagnoses if active ──
        var _pfBrain = _pfDomains[regDk[ri]];
        var _pfBrainDx = (_pfBrain && Array.isArray(_pfBrain.brainDiagnoses))
          ? _pfBrain.brainDiagnoses.filter(function (dx) { return dx.active; })
          : [];

        var domHeader = document.createElement('div');
        domHeader.style.cssText = 'font-size:0.5rem;letter-spacing:1.5px;text-transform:uppercase;color:#C9A94E;margin:8px 0 3px;';

        if (_pfBrainDx.length > 0) {
          // Brain-preferred path
          var rc = _ensureRegCard();
          domHeader.textContent = regDk[ri];
          domHeader.innerHTML += ' <span style="font-size:0.28rem;letter-spacing:0.8px;padding:1px 4px;border-radius:2px;background:rgba(90,181,160,0.1);color:rgba(90,181,160,0.7);vertical-align:middle">BRAIN</span>';
          rc.appendChild(domHeader);

          for (var _pfdi = 0; _pfdi < Math.min(_pfBrainDx.length, 5); _pfdi++) {
            var _pfDx = _pfBrainDx[_pfdi];
            var _dxRow = document.createElement('div');
            _dxRow.className = 'clr-dx-row';
            var _dxLabel = document.createElement('div');
            _dxLabel.className = 'clr-dx-label';
            _dxLabel.textContent = _pfDx.label || _pfDx.id;
            _dxRow.appendChild(_dxLabel);
            var _dxExplain = document.createElement('div');
            _dxExplain.className = 'clr-dx-explain';
            var _pfMeta = 'Relevance ' + Math.round((_pfDx.relevance || 0) * 100) + '%';
            if (typeof _pfDx.matchedConditions === 'number' && typeof _pfDx.totalTriggers === 'number' && _pfDx.totalTriggers > 0) {
              _pfMeta += ' \u00b7 ' + _pfDx.matchedConditions + '/' + _pfDx.totalTriggers + ' triggers matched';
            }
            if (_pfDx.summary) _pfMeta += ' \u00b7 ' + _pfDx.summary;
            _dxExplain.textContent = _pfMeta;
            _dxRow.appendChild(_dxExplain);
            rc.appendChild(_dxRow);
          }
          // Prefer brain treatments when available, fall back to registry
          var _pfBrainTx = (_pfBrain && Array.isArray(_pfBrain.brainTreatments) && _pfBrain.brainTreatments.length > 0)
            ? _pfBrain.brainTreatments : null;

          if (_pfBrainTx) {
            for (var _pfti = 0; _pfti < Math.min(_pfBrainTx.length, 4); _pfti++) {
              var _pfTx = _pfBrainTx[_pfti];
              if (!_pfTx || !_pfTx.label) continue;
              var _pfTxEl = document.createElement('div');
              _pfTxEl.className = 'clr-tx-item';
              var _txText = _pfTx.label;
              if (_pfTx.evidence) _txText += ' [' + _pfTx.evidence + ']';
              _pfTxEl.textContent = _txText;
              rc.appendChild(_pfTxEl);
            }
          } else {
            var txList = report.registryTreatments ? (report.registryTreatments[regDk[ri]] || []) : [];
            for (var txi = 0; txi < Math.min(txList.length, 2); txi++) {
              var txEl = document.createElement('div');
              txEl.className = 'clr-tx-item';
              txEl.textContent = txList[txi].title;
              rc.appendChild(txEl);
            }
          }
        } else {
          // Fallback: legacy registry path — stacked layout to match brain path
          var dxList = report.registryDiagnoses[regDk[ri]] || [];
          var txList2 = report.registryTreatments ? (report.registryTreatments[regDk[ri]] || []) : [];
          if (dxList.length === 0 && txList2.length === 0) continue;

          var rc2 = _ensureRegCard();
          domHeader.textContent = regDk[ri];
          rc2.appendChild(domHeader);

          for (var dxi = 0; dxi < Math.min(dxList.length, 3); dxi++) {
            var _dxRow2 = document.createElement('div');
            _dxRow2.className = 'clr-dx-row';
            var _dxLabel2 = document.createElement('div');
            _dxLabel2.className = 'clr-dx-label';
            _dxLabel2.textContent = dxList[dxi].title;
            _dxRow2.appendChild(_dxLabel2);
            var _dxExplain2 = document.createElement('div');
            _dxExplain2.className = 'clr-dx-explain';
            _dxExplain2.textContent = (dxList[dxi].severity || 'unknown') + ' \u00b7 confidence ' + Math.round((dxList[dxi].confidence || 0) * 100) + '%';
            _dxRow2.appendChild(_dxExplain2);
            rc2.appendChild(_dxRow2);
          }
          for (var txi2 = 0; txi2 < Math.min(txList2.length, 2); txi2++) {
            var txEl2 = document.createElement('div');
            txEl2.className = 'clr-tx-item';
            txEl2.textContent = txList2[txi2].title;
            rc2.appendChild(txEl2);
          }
        }
      }
      // Only mount the card if at least one domain row was added
      if (_regCardRendered && regCard) _tabContentEl.appendChild(regCard);
    }
  }
