/**
 * agriculture-promotion-bridge.js — Inject promoted portal directives into operator surfaces
 *
 ...
 *
 * Exposes: window.LIMENAgriculturePromotionBridge
 */
(function () {
  'use strict';
   ...
   * @param {Object} portal - L0 agriculture portal JSON
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENAgricultureDirectiveExtractor;
    var ranker = window.LIMENAgricultureDirectiveRanker;
    var translator = window.LIMENAgricultureDirectiveTranslator;

    if (!extractor || !ranker || !translator) {
      console.warn('[AgriPromotionBridge] Missing module(s) — extractor:', !!extractor, 'ranker:', !!ranker, 'translator:', !!translator);
      _running = false;
      return Promise.resolve([]);
    }

    console.log('[AgriPromotionBridge] Input signature changed — running extraction pipeline');
    console.log('[AgriPromotionBridge] Signature: ' + sig);
    var t0 = Date.now();

    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        if (!candidates || candidates.length === 0) {
          console.log('[AgriPromotionBridge] No candidates extracted');
          _lastSignature = sig;
          _lastPromoted = [];
          _running = false;
          return [];
        ...
        console.log('[AgriPromotionBridge] Extracted ' + candidates.length + ' candidates in ' + (Date.now() - t0) + 'ms');
        ...
        if (bestProofIdx > limit - 1 && bestProofIdx !== -1) {
          ...
          console.log('[AgriPromotionBridge] Elevated proof directive from position ' + bestProofIdx + ': ' + proofItem.treatmentLabel.substring(0, 50));
        }
        ...
        if (bestDeepIdx !== -1 && bestDeepIdx >= limit) {
          ...
          console.log('[AgriPromotionBridge] Deep proof slot: L' + deepItem.depth + ' ' + deepItem.treatmentLabel.substring(0, 50) + ' (deepScore=' + bestDeepScore.toFixed(3) + ')');
        } else if (bestDeepIdx !== -1 && bestDeepIdx < limit) {
          // Already in top set — just mark it
          ranked[bestDeepIdx]._isDeepProofSlot = true;
          console.log('[AgriPromotionBridge] Deep proof already in top set: L' + ranked[bestDeepIdx].depth);
        }

        console.log('[AgriPromotionBridge] Top 5 after depth-aware selection:');
        ...
        var targeting = window.LIMENAgricultureTargetingEngine;
        if (targeting) {
        ...
        console.log('[AgriPromotionBridge] Promoted ' + dedupedPromoted.length + ' directives (' + (Date.now() - t0) + 'ms total)');
        return dedupedPromoted;
      })
      .catch(function (err) {
        console.error('[AgriPromotionBridge] Pipeline failed — existing opportunities untouched:', err.message || err);
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      var overlay = {
        id: 'ptreat_' + (dir.portalDomainId || 'agriculture') + '_' + (dir.nodeId || '') + '_' + pi,
  ...
  window.LIMENAgriculturePromotionBridge = {
    promote: promote,
