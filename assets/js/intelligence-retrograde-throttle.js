/*
 * intelligence-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes intelligence.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Intelligence has no runtime load field). { nodeId: loadFraction }
  ...
  /* intelligence: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(intelligence, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (intelligence.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. intelligence.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Intelligence has no runtime load field). throttleGain operator-set. Never writes intelligence.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.IntelligenceRetrogradeThrottle) root.IntelligenceRetrogradeThrottle = API;
