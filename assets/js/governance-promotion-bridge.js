/**
 * governance-promotion-bridge.js — Inject promoted Governance portal directives
 * Exposes: window.LIMENGovernancePromotionBridge
 */
(function () {
  'use strict';
  ...
  function _computeSignature(state) {
    ...
    return ['governance', dxKeys.join(','), nodeKeys.join(','), stressBucket.toFixed(2), window.LIMEN_ENABLE_DIRECTIVE_EXTRACTION ? '1' : '0'].join('|');
  }
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENGovernanceDirectiveExtractor;
    var ranker = window.LIMENGovernanceDirectiveRanker;
    var translator = window.LIMENGovernanceDirectiveTranslator;
    if (!extractor || !ranker || !translator) {
      console.warn('[GovernancePromotionBridge] Missing pipeline component');
      _running = false;
    ...
    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        ...
        var targeting = window.LIMENGovernanceTargetingEngine;
        if (targeting) {
        ...
        console.log('[GovernancePromotionBridge] Promoted ' + dedupedPromoted.length);
        return dedupedPromoted;
      })
      .catch(function (err) {
        console.error('[GovernancePromotionBridge] Pipeline failed:', err.message || err);
        _lastSignature = sig; _lastPromoted = []; _running = false; return [];
      });
  }
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      state.treatments.push({
        id: 'ptreat_' + (dir.portalDomainId || 'governance') + '_' + (dir.nodeId || '') + '_' + pi,
  ...
  window.LIMENGovernancePromotionBridge = { promote: promote, getLastPromoted: getLastPromoted };
})();
