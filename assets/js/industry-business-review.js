/**
 * environment-business-review.js — Industry Business Assignment Review Surface
 *
 * INDUSTRY DOMAIN ONLY. Renders a review panel for proposed business mappings
 ...
 * Exposes: window.LIMENIndustryBusinessReview
 */
(function () {
  'use strict';
  ...
  function renderCard(entry) {
    ...
    if (entry.neuroTranslation) {
      ...
      h += '<div style="font-size:0.26rem;letter-spacing:2px;color:rgba(74,143,212,0.5);margin-bottom:4px">IN NEUROLOGY / IN INDUSTRY TRANSLATION</div>';
      ...
      h += '<div style="font-size:0.32rem;color:#b0a898;line-height:1.5"><b style="color:rgba(201,169,78,0.7)">In industry (industry systems):</b> ' + esc(entry.neuroTranslation.inBusiness) + '</div>';
      h += '</div>';
    }
    ...
    if (entry.existingCompanies && entry.existingCompanies.length > 0) {
      ...
      h += '<div class="sbr-card-existing" style="color:#C9A94E">Missing from current Industry system</div>';
    }
  ...
  function render() {
    var engine = window.LIMENIndustryBusinessEngine;
    if (!engine) return;
    injectStyles();
      ...
      bizBtns[bi].addEventListener('click', function (e) {
        ...
        try {
          sessionStorage.setItem('limen_exec_context', JSON.stringify({
            ...
            source: 'industry', returnTo: '/domain-console?domain=environment&mode=operator'
          }));
        } catch (ex) {}
        window.location.href = 'industry-workspace.html?track=business&opp=' + encodeURIComponent(oppKey) + '&returnTo=' + encodeURIComponent('/domain-console?domain=environment&mode=operator');
      });
    }
  ...
  window.LIMENIndustryBusinessReview = { render: render, mount: mount };
