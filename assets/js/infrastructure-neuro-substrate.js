/*
 * infrastructure-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/infrastructure-neuro-substrate.json
 * Prompts:       assets/prompts/infrastructure-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/infrastructure.json
 * or any existing infrastructure-*.js. It reads existing Infrastructure data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Infrastructure brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Infrastructure file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/infrastructure-neuro-substrate.json';
  var INFRASTRUCTURE_URL = '/assets/data/domains/infrastructure.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Infrastructure issue id. */
     ...
     * Reads an Infrastructure issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Infrastructure issue. */
    auditAllIssues: function () {
      return loadJSON(INFRASTRUCTURE_URL).then(function (infrastructure) {
        return (infrastructure.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.InfrastructureNeuroSubstrate) root.InfrastructureNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Infrastructure brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import InfrastructureNeuroSubstrate from '/assets/js/infrastructure-neuro-substrate.js';
   *      // then, after issues render:
   *      InfrastructureNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
