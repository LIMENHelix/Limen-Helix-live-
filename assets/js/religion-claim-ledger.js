/**
 * environment-claim-ledger.js — Religion Domain Claim Ledger (Local Only)
 *
 * Stores claim records in localStorage. Tracks opportunity claims
 * from acceptance through outcome. Science-scoped only.
 *
 * Namespace: window.LIMENReligion.economy.claims
 */
(function () {
  'use strict';
  ...
  var STORE_KEY = 'LIMEN_RELIGION_CLAIMS';
  ...
  function createClaimRecord(opportunity) {
    ...
    try { window.dispatchEvent(new CustomEvent('limen:religion-claim-update')); } catch (e) {}
  ...
  function getClaimByOppId(oppId) {
    var all = loadAll();
    for (var i = 0; i < all.length; i++) if (all[i].opportunityId === oppId) return all[i];
    return null;
  }
  ...
  window.LIMENReligion.economy.claims = {
  ...
  console.log('[ScienceClaimLedger] Loaded \u2014 claim ledger ready');
})();
