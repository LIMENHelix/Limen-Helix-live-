/*
 * culture-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/culture-neuro-substrate.json
 * Prompts:       assets/prompts/culture-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/culture.json
 * or any existing culture-*.js. It reads existing Culture data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Culture brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Culture file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/culture-neuro-substrate.json';
  var CULTURE_URL = '/assets/data/domains/culture.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Culture issue id. */
     ...
     * Reads an Culture issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Culture issue. */
    auditAllIssues: function () {
      return loadJSON(CULTURE_URL).then(function (culture) {
        return (culture.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.CultureNeuroSubstrate) root.CultureNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Culture brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import CultureNeuroSubstrate from '/assets/js/culture-neuro-substrate.js';
   *      // then, after issues render:
   *      CultureNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
