/**
 * education-agent-box.js — the Education domain's operator AI box (client side).
 *
 * A floating "Talk to Education" panel on the education domain-console. It KNOWS its
 * domain (window.LIMENEducationBrain.getStateSummary()) and can make bounded
 ...
 *    tries /api/education-agent first and falls back to LOCAL on any failure, so it
...
(function () {
  ...
  function parseLocal(prompt, state) {
    ...
    if (mF) {
      var topic = mF[1].replace(/\b(please|now|today|this week|domain|education|closely|carefully)\b/g, '').trim();
  ...
  if (typeof window !== 'undefined') window.__educationAgentLocal = { parse: parseLocal, query: answerQuery };

  ready(function () {
    if (window.__educationAgentBox) return; window.__educationAgentBox = true;
    var LS_KEY = 'limenEducationAgentKey';
    var esc = function (s) { return String(s == null ? '' : s).replace(/[&<>"]/g, function (c) { return ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[c]; }); };
    function brain() { return window.LIMENEducationBrain || null; }
    ...
    var toggle = document.createElement('div'); toggle.id = 'eab-toggle'; toggle.textContent = '⚡ Talk to Education';
    var panel = document.createElement('div'); panel.id = 'eab';
    panel.innerHTML =
      '<div class="eab-h"><span><b>Education</b> · ask or steer</span><span class="eab-x">✕</span></div>' +
    ...
    function renderResult(pending, result, mode, left) {
      ...
      if (applied) html += '<div class="eab-tool">✓ applied to education domain:\n' + esc(applied) + '</div>';
    ...
    function send() {
      var b = brain();
      if (!b) { add('<span class="eab-ai">Education brain not loaded on this page.</span>'); return; }
    ...
    add('<span class="eab-ai">Education domain online (free local mode). Ask about it, or steer it: "focus on grid", "prefer research", "autonomy off", "how is education doing", "show the forecast". I bias the domain, never force a finding, move capital, or touch code. Add a passcode above for conversational AI once billing is on.</span>');
  });
})();
