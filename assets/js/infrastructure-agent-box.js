/**
 * infrastructure-agent-box.js — the Infrastructure domain's operator AI box (client side).
 *
 * A floating "Talk to Infrastructure" panel on the infrastructure domain-console. It KNOWS its
 * domain (window.LIMENInfrastructureBrain.getStateSummary()) and can make bounded
 * CHANGES to it: STEER (bias) + CONFIG, applied through the brain's clamped methods
 * (applyRequestBias / setDomainConfig) — never code, capital, or an effector.
 *
 ...
 *    tries /api/infrastructure-agent first and falls back to LOCAL on any failure, so it
...
(function () {
  ...
  function parseLocal(prompt, state) {
    ...
    if (mF) {
      var topic = mF[1].replace(/\b(please|now|today|this week|domain|infrastructure|closely|carefully)\b/g, '').trim();
  ...
  if (typeof window !== 'undefined') window.__infrastructureAgentLocal = { parse: parseLocal, query: answerQuery };

  ready(function () {
    if (window.__infrastructureAgentBox) return; window.__infrastructureAgentBox = true;
    var LS_KEY = 'limenInfrastructureAgentKey';
    var esc = function (s) { return String(s == null ? '' : s).replace(/[&<>"]/g, function (c) { return ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[c]; }); };
    function brain() { return window.LIMENInfrastructureBrain || null; }
    ...
    var toggle = document.createElement('div'); toggle.id = 'eab-toggle'; toggle.textContent = '⚡ Talk to Infrastructure';
    var panel = document.createElement('div'); panel.id = 'eab';
    panel.innerHTML =
      '<div class="eab-h"><span><b>Infrastructure</b> · ask or steer</span><span class="eab-x">✕</span></div>' +
    ...
    function applyTools(tools) {
      ...
      tools.forEach(function (t) {
        ...
        } else if (t.type === 'config' && typeof b.setDomainConfig === 'function') {
          var c = b.setDomainConfig(t);
    ...
    function stampLine(mode, left) {
      ...
      if (!b || typeof b.getStateSummary !== 'function') return extra;
      var s = b.getStateSummary();
    ...
    function renderResult(pending, result, mode, left) {
      ...
      if (applied) html += '<div class="eab-tool">✓ applied to infrastructure domain:\n' + esc(applied) + '</div>';
    ...
    function send() {
      var b = brain();
      if (!b) { add('<span class="eab-ai">Infrastructure brain not loaded on this page.</span>'); return; }
      ...
      var state = {}; try { state = b.getStateSummary(); } catch (e) {}
      ...
      fetch('/api/infrastructure-agent', {
    ...
    add('<span class="eab-ai">Infrastructure domain online (free local mode). Ask about it, or steer it: "focus on grid", "prefer research", "autonomy off", "how is infrastructure doing", "show the forecast". I bias the domain, never force a finding, move capital, or touch code. Add a passcode above for conversational AI once billing is on.</span>');
  });
})();
