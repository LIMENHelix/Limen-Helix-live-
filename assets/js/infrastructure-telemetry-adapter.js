/*
 * infrastructure-telemetry-adapter.js  — wires runtime fields to the LIVE Infrastructure telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/infrastructure-pulse-engine.js  (window.LIMENInfrastructurePulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Infrastructure issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to infrastructure.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into infrastructure.json (a
...
(function (root) {
   ...
   * inputs: { infrastructure, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    inp = inp || {};
    var infrastructure = inp.infrastructure || {};
    ...
    var issuesOverlay = {};
    (infrastructure.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (infrastructure.activations || []).forEach(function (a) {
  ...
  /* Merge infrastructure.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(infrastructure, overlay) {
    var base = infrastructure.runtime || {};
    ...
    var acts = {}; (infrastructure.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (infrastructure.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the infrastructure definition + brainState. */
  function fromLive(infrastructure, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENInfrastructurePulse) ? window.LIMENInfrastructurePulse : null;
    if (!P) return null;
    return fromPulse({ infrastructure: infrastructure, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN infrastructure definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/infrastructure.json', 'infrastructure'),
      _load('/assets/data/infrastructure-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ infrastructure: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.InfrastructureTelemetryAdapter) root.InfrastructureTelemetryAdapter = API;
