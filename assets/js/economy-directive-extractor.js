/**
 * economy-directive-extractor.js — Exhaustive Fractal Directive Extraction
 *
 * Traverses the economy portal fractal tree starting from active diagnosis
 ...
 *
 * Exposes: window.LIMENEconomyDirectiveExtractor
 */
(function () {
  'use strict';
  ...
  function _loadPrebuiltPayload() {
    ...
    return fetch('/assets/data/deep/economy-deep-directives.json')
      .then(function (r) {
      ...
      .then(function (data) {
        if (data && data.directives && data.directives.length > 0) {
          ...
          console.log('[EconomyDirectiveExtractor] Prebuilt payload loaded: ' + data.directives.length + ' deep directives, maxDepth=' + data.traversal.maxDepth);
        }
      ...
      .catch(function () {
        ...
        _prebuiltLoading = false;
        console.log('[EconomyDirectiveExtractor] No prebuilt payload \u2014 falling back to live extraction');
        return null;
      });
  }
   ...
   * e.g. "economy_labor_market_portal.html" → "economy_labor_market"
   */
   ...
   *
   * @param {Object} state - economy brain state
   * @param {Object} portal - L0 economy portal JSON (economy.json)
  ...
  function extract(state, portal, opts) {
    ...
    return _loadPrebuiltPayload().then(function (prebuilt) {
      if (prebuilt && prebuilt.directives) {
        ...
        console.log('[EconomyDirectiveExtractor] Prebuilt: ' + filtered.length + ' candidates from ' + prebuilt.directives.length + ' total, ' + activeDx.length + ' active diagnoses');
        return filtered;
      }

      // ── FALLBACK: live portal extraction (original path) ──
      console.log('[EconomyDirectiveExtractor] Using live extraction fallback');
  ...
  function _liveExtract(state, portal, activeNodeIds, activeDx, stress, opts) {
      ...
      (function (pid) {
        chain = chain.then(function () {
          return _fetchPortalJSON(pid).then(function (data) {
            if (!data) return;
            return _extractFromPortal(pid, data, activeNodeIds, stress, 1, ['economy'], seen, results);
          });
        });
    ...
    return chain
      .then(function () {
        // ── STAGE 2: L0 root portal traversal (fills remaining budget) ──
        return _extractFromPortal('economy', portal, activeNodeIds, stress, 0, [], seen, results);
      })
      .then(function () {
        console.log('[EconomyDirectiveExtractor] Extracted ' + results.length + ' candidates from ' +
  ...
  window.LIMENEconomyDirectiveExtractor = {
    extract: extract,
    // Exposed for testing
