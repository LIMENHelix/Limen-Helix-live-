/*
 * defense-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes defense.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Defense has no runtime load field). { nodeId: loadFraction }
  ...
  /* defense: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(defense, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (defense.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. defense.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Defense has no runtime load field). throttleGain operator-set. Never writes defense.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.DefenseRetrogradeThrottle) root.DefenseRetrogradeThrottle = API;
