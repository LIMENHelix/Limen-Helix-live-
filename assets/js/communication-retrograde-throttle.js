/*
 * communication-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes communication.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Communication has no runtime load field). { nodeId: loadFraction }
  ...
  /* communication: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(communication, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (communication.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. communication.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Communication has no runtime load field). throttleGain operator-set. Never writes communication.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.CommunicationRetrogradeThrottle) root.CommunicationRetrogradeThrottle = API;
