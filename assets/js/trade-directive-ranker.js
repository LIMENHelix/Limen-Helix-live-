/**
 * trade-directive-ranker.js — Score and rank extracted directive candidates
 *
 * Trade-specific mechanism classification and depth-aware ranking.
 ...
 * Exposes: window.LIMENTradeDirectiveRanker
 */
(function () {
  'use strict';
  ...
  // ══════════════════════════════════════════════════════════════════════
  // TRADE-SPECIFIC MECHANISM CLASSIFICATION
  // ══════════════════════════════════════════════════════════════════════

  var MECHANISMS = {
    freight_cost_spike:   { label: 'Freight Cost Spike',     signals: ['freight', 'shipping cost', 'rate spike', 'carrier rate', 'fuel surcharge', 'spot rate', 'contract rate', 'drayage', 'demurrage', 'detention', 'accessorial', 'bunker'] },
    logistics_constraint: { label: 'Logistics Constraint',   signals: ['logistics', 'transport', 'trucking', 'rail', 'barge', 'intermodal', 'dwell time', 'transit time', 'lead time', 'bottleneck', 'congestion', 'throughput', 'capacity', 'backlog'] },
    route_disruption:     { label: 'Route Disruption',       signals: ['route', 'strait', 'canal', 'chokepoint', 'diversion', 'reroute', 'blockage', 'closure', 'weather delay', 'piracy', 'embargo', 'exclusion zone', 'war risk'] },
    port_congestion:      { label: 'Port Congestion',        signals: ['port', 'terminal', 'berth', 'anchorage', 'vessel queue', 'container yard', 'chassis', 'crane', 'stevedore', 'longshoreman', 'labor dispute', 'strike'] },
    capacity_shortage:    { label: 'Capacity Shortage',      signals: ['capacity', 'shortage', 'equipment', 'container', 'chassis', 'vessel', 'fleet', 'allocation', 'blank sailing', 'rolled cargo', 'space', 'booking'] },
    intermodal_breakdown: { label: 'Intermodal Breakdown',   signals: ['intermodal', 'transload', 'cross-dock', 'warehouse', 'distribution', 'fulfillment', 'last mile', 'cold chain', 'temperature', 'perishable', 'reefer'] },
    trade_policy:         { label: 'Trade Policy Shock',     signals: ['tariff', 'sanction', 'quota', 'ban', 'restriction', 'customs', 'duty', 'compliance', 'origin', 'certificate', 'clearance', 'inspection', 'regulatory'] },
    contract_mispricing:  { label: 'Contract Mispricing',    signals: ['contract', 'renegotiat', 'force majeure', 'penalty', 'SLA', 'service level', 'claim', 'dispute', 'settlement', 'counterparty', 'credit', 'default'] },
    inventory_imbalance:  { label: 'Inventory Imbalance',    signals: ['inventory', 'stock', 'buffer', 'safety stock', 'reorder', 'stockout', 'overstock', 'obsolescence', 'write-down', 'carrying cost', 'warehouse', 'storage'] },
    supply_disruption:    { label: 'Supply Disruption',      signals: ['supply', 'shortage', 'disruption', 'sourcing', 'procurement', 'raw material', 'component', 'critical', 'single source', 'dependency', 'nearshoring', 'reshoring'] }
  };

  var DX_VALID_MECHANISMS = {
    'SHIPPING_CRISIS':        ['freight_cost_spike', 'capacity_shortage', 'route_disruption', 'logistics_constraint', 'port_congestion', 'contract_mispricing'],
    'PORT_BLOCKADE':          ['port_congestion', 'route_disruption', 'capacity_shortage', 'logistics_constraint', 'intermodal_breakdown', 'inventory_imbalance'],
    'SUPPLY_CHAIN_COLLAPSE':  ['supply_disruption', 'logistics_constraint', 'capacity_shortage', 'inventory_imbalance', 'intermodal_breakdown', 'contract_mispricing'],
    'TRADE_WAR':              ['trade_policy', 'contract_mispricing', 'supply_disruption', 'route_disruption', 'inventory_imbalance'],
    'CUSTOMS_DISRUPTION':     ['trade_policy', 'logistics_constraint', 'contract_mispricing', 'inventory_imbalance']
  };
  ...
  function rank(directives) {
    ...
    console.log('[TradeRanker] Ranked ' + directives.length + '. Top: ' +
  ...
  window.LIMENTradeDirectiveRanker = { rank: rank };
})();
