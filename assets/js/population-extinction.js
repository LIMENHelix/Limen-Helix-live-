/*
 * population-extinction.js  — ADDITIVE mechanism (completes EXTINCTION_LEARNING)
 * ----------------------------------------------------------------------------
 ...
 * (complements the GLOBAL down-scaling in population-offline-maintenance.js).
 *
 * CONTRACT: read-only / copy-only. NEVER writes population.json. Proposes which acquired responses to
...
(function (root) {
  ...
  /* Extinction over Population activations: an activation whose diagnosticTriggers are ALL absent
   * from the caller-supplied activeTriggers is a candidate to retire (its trigger is gone).
   * activeTriggers: array of currently-firing trigger strings (runtime state; Population has none baked in). */
  function proposeExtinction(population, activeTriggers, opts) {
    ...
    return {
      ...
      note: 'Read-only PROPOSAL. Applies nothing; retiring a node would edit population.json. activeTriggers is caller-supplied runtime state.'
    };
  }

  var API = { proposeExtinction: proposeExtinction };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.PopulationExtinction) root.PopulationExtinction = API;
