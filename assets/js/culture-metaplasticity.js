/*
 * culture-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Culture's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (culture-offline-maintenance.js)
 *   - refractory absolute window       (culture-refractory-limiter.js)
 *   - prediction-error threshold       (culture-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Culture has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Culture has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.CultureMetaplasticity) root.CultureMetaplasticity = API;
