/**
 * infrastructure-directive-translator.js — Convert ranked directives into canonical opportunity objects
 *
 * Takes ranked directives from infrastructure-directive-ranker and translates them into
 * the same opportunity object shape that infrastructure-brain.js produces, so they can
 ...
 *
 * Exposes: window.LIMENInfrastructureDirectiveTranslator
 */
(function () {
  'use strict';
  ...
  function _buildNextAction(d) {
    ...
    if (type === 'INVESTABLE' && companies.length > 0) {
      ...
      return 'Search patents.google.com for prior art in "' + (d.nodeLabel || 'infrastructure').toLowerCase() + '" technology. Draft provisional claim outline.';
    } else {
      return 'Search sam.gov for open solicitations matching "' + (d.nodeLabel || 'infrastructure').toLowerCase() + '". Draft 1-page concept note with system evidence.';
    }
  }
  ...
  function _buildEvidence(d) {
    var parts = ['Domain: infrastructure. Stress: ' + Math.round((d.stress || 0) * 100) + '%.'];
  ...
  function translate(ranked, limit) {
    ...
    for (var i = 0; i < Math.min(ranked.length, limit); i++) {
      ...
      if (primaryPath === 'GRANT-ELIGIBLE') targetStr += (targetStr ? ' ' : '') + 'Agencies: DOT, EPA, FEMA, state infrastructure offices. Search sam.gov.';
      if (primaryPath === 'PATENTABLE') targetStr += (targetStr ? ' ' : '') + 'Licensees: utilities, municipalities, EPC firms, infrastructure operators.';
      if (!targetStr) targetStr = 'Identify counterparties affected by ' + (d.nodeLabel || 'this condition') + '.';

      var opp = {
        // Core identity — matches infrastructure-brain.js opportunity shape
        id: 'pdir_' + (d.portalDomainId || 'infrastructure') + '_' + (d.nodeId || '') + '_' + i,
        title: _buildTitle(d),
        rank: d.rankScore || 0,
        path: primaryPath,
        urgency: urgency,
        ...
        // Enriched fields (same as infrastructure-brain enrichment)
        ...
        // Compensation model (same as infrastructure-brain)
    ...
    console.log('[InfrastructureDirectiveTranslator] Translated ' + opportunities.length + ' opportunities from ' + ranked.length + ' ranked directives');
  ...
  var REVENUE_MODELS = {
    ...
    'GRANT-ELIGIBLE': [
      { model: 'grant award', desc: 'Write a grant proposal backed by live stress data. Submit to DOT/EPA/FEMA/state infrastructure office. $250K-$5M per award. 60-180 day cycle.' },
      ...
      { model: 'cost recovery', desc: 'Implement the solution for a municipality or utility. Bill costs through their approved capital budget. Guaranteed recovery.' }
    ],
    'PATENTABLE': [
  ...
  var TITLE_TEMPLATES = [
    { match: /bridge|overpass|structural/i, titles: ['Build bridge condition assessment model \u2014 sell to state DOTs and municipalities'] },
    { match: /water main|water system|pipe/i, titles: ['Build water infrastructure risk scorecard \u2014 sell to water utilities'] },
    { match: /broadband|fiber|telecom/i, titles: ['Build broadband gap analysis model \u2014 sell to ISPs and municipalities'] },
    { match: /grid modernization|smart grid/i, titles: ['Build grid modernization roadmap \u2014 sell to distribution utilities'] },
    { match: /deferred maintenance|maintenance backlog/i, titles: ['Build maintenance prioritization model \u2014 sell to asset operators'] },
    { match: /transportation|highway|road/i, titles: ['Build transportation capacity model \u2014 sell to state DOTs'] },
    { match: /cyber|security|resilience/i, titles: ['Build infrastructure cybersecurity assessment \u2014 sell to utility operators'] },
    { match: /dam|levee|flood/i, titles: ['Build flood infrastructure risk model \u2014 sell to FEMA and municipalities'] },
    { match: /pppp|public.private|partnership/i, titles: ['Build PPP feasibility model \u2014 sell to government agencies'] },
    { match: /permitting|regulatory/i, titles: ['Build permitting acceleration toolkit \u2014 sell to infrastructure developers'] },
    { match: /substation|transformer/i, titles: ['Build substation upgrade priority model \u2014 sell to T&D utilities'] },
    { match: /rail|transit|station/i, titles: ['Build transit system diagnostic \u2014 sell to transit authorities'] },
    { match: /waste|sanitation/i, titles: ['Build waste infrastructure capacity model \u2014 sell to municipalities'] },
    { match: /airport|port|terminal/i, titles: ['Build port/terminal throughput model \u2014 sell to port authorities'] }
  ];
  ...
  function _targetPhrase(companies, nodeLabel, dxLabel) {
    ...
    return (nodeLabel || 'infrastructure') + ' buyers';
  }
  ...
  function _sharpenStep(raw, index, companies, nodeLabel) {
    ...
    raw = raw.replace(/\busing (IEEE|ASCE|AASHTO|EPA|FEMA|FHWA|DOT|NERC) [^,.]+ (framework|methodology|analysis|standards|requirements)/gi, '');
  ...
  function _generateTargetClasses(path, nodeLabel, dxLabel) {
    if (path === 'INVESTABLE') return 'Utilities, municipalities, EPC contractors, and infrastructure service companies with direct ' + (nodeLabel || 'sector') + ' exposure.';
    if (path === 'PATENTABLE') return 'DOTs, transit authorities, water utilities, infrastructure management platforms, and technology vendors serving ' + (nodeLabel || 'the sector') + '.';
    return 'DOT, EPA, FEMA, state infrastructure offices, USDOT. Municipalities with ' + (nodeLabel || 'infrastructure') + ' mandates.';
  }
  ...
  function shape(translated, shapeCount) {
    ...
    console.log('[InfrastructureDirectiveTranslator] Shaped ' + shaped + '/' + shapeCount + ' directives into deal-grade outputs');
    return result;
  }

  window.LIMENInfrastructureDirectiveTranslator = { translate: translate, shape: shape };
})();
