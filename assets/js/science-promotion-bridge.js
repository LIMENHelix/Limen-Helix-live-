/**
 * science-promotion-bridge.js — Inject promoted Science portal directives into operator surfaces
 *
 ...
 * Exposes: window.LIMENSciencePromotionBridge
 */
(function () {
  'use strict';
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENScienceDirectiveExtractor;
    var ranker = window.LIMENScienceDirectiveRanker;
    var translator = window.LIMENScienceDirectiveTranslator;
    ...
    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        ...
        var targeting = window.LIMENScienceTargetingEngine;
        if (targeting) {
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      state.treatments.push({
        id: 'ptreat_' + (dir.portalDomainId || 'science') + '_' + (dir.nodeId || '') + '_' + pi,
  ...
  window.LIMENSciencePromotionBridge = {
    promote: promote,
