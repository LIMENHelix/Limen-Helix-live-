/*
 * education-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/education-neuro-substrate.json
 * Prompts:       assets/prompts/education-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/education.json
 * or any existing education-*.js. It reads existing Education data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Education brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Education file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/education-neuro-substrate.json';
  var EDUCATION_URL = '/assets/data/domains/education.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Education issue id. */
     ...
     * Reads an Education issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Education issue. */
    auditAllIssues: function () {
      return loadJSON(EDUCATION_URL).then(function (education) {
        return (education.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.EducationNeuroSubstrate) root.EducationNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Education brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import EducationNeuroSubstrate from '/assets/js/education-neuro-substrate.js';
   *      // then, after issues render:
   *      EducationNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
