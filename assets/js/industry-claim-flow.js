/**
 * environment-claim-flow.js — Industry Domain Claim Acceptance Flow
 *
 * Adds CLAIM button to Industry opportunity cards. On click, opens
 ...
 * Namespace: window.LIMENIndustry.economy.claimFlow
 */
(function () {
  'use strict';
  ...
  function openClaimModal(opportunity) {
    ...
    var comp = window.LIMENIndustry.economy.compensation;
    ...
    h += '<div class="scf-title">Claim Industry Opportunity</div>';
    ...
    h += '<div>\u2022 Operator is responsible for industry integrity, data protection, and authorship ethics</div>';
    h += '</div></div>';

    h += '<label class="scf-accept">';
    h += '<input type="checkbox" id="scfAcceptCheck">';
    h += '<span class="scf-accept-text">I understand this is a Science-domain execution claim, I am responsible for truthful submission, documented progress, and compliance with applicable industry integrity, IRB, and data protection rules.</span>';
    h += '</label>';
  ...
  window.addEventListener('limen:industry-claim-update', function () { setTimeout(injectIntoCards, 100); });

  window.LIMENIndustry.economy.claimFlow = {
