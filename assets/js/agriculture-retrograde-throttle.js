/*
 * agriculture-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes agriculture.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Agriculture has no runtime load field). { nodeId: loadFraction }
  ...
  /* agriculture: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(agriculture, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (agriculture.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. agriculture.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Agriculture has no runtime load field). throttleGain operator-set. Never writes agriculture.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.AgricultureRetrogradeThrottle) root.AgricultureRetrogradeThrottle = API;
