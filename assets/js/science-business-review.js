/**
 * science-business-review.js — Science Business Assignment Review Surface
 *
 * SCIENCE DOMAIN ONLY. Renders a review panel for proposed business mappings
 * across the full 103-node hierarchy.
 * Requires science-node-business-engine.js to be loaded first.
 *
 * Self-gates: only runs when ?domain=science or ?domain=research
 * Exposes: window.LIMENScienceBusinessReview
 */
(function () {
  'use strict';
  ...
  function renderCard(entry) {
    ...
    if (entry.neuroTranslation) {
      ...
      h += '<div style="font-size:0.26rem;letter-spacing:2px;color:rgba(74,143,212,0.5);margin-bottom:4px">IN NEUROLOGY / IN SCIENCE TRANSLATION</div>';
      ...
      h += '<div style="font-size:0.32rem;color:#b0a898;line-height:1.5"><b style="color:rgba(201,169,78,0.7)">In science (research systems):</b> ' + esc(entry.neuroTranslation.inBusiness) + '</div>';
      h += '</div>';
    }
    ...
    if (entry.existingCompanies && entry.existingCompanies.length > 0) {
      ...
      h += '<div class="sbr-card-existing" style="color:#C9A94E">Missing from current Science system</div>';
    }
  ...
  function render() {
    var engine = window.LIMENScienceBusinessEngine;
    if (!engine) return;
    injectStyles();
      ...
      bizBtns[bi].addEventListener('click', function (e) {
        ...
        try {
          sessionStorage.setItem('limen_exec_context', JSON.stringify({
            ...
            source: 'science', returnTo: '/domain-console?domain=science&mode=operator'
          }));
        } catch (ex) {}
        window.location.href = 'science-workspace.html?track=business&opp=' + encodeURIComponent(oppKey) + '&returnTo=' + encodeURIComponent('/domain-console?domain=science&mode=operator');
      });
    }
  ...
  window.LIMENScienceBusinessReview = { render: render, mount: mount };
