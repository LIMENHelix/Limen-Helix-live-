/**
 * communication-execution-panels.js — GRANT and PATENT Execution Workspaces (Communication)
 * Self-gates: ?domain=communication
 * Exposes: window.LIMENCommunicationExecutionPanels
 */
(function () {
  'use strict';
  ...
  var isCommunicationDomain = _dom === 'communication';
  var isCommunicationWorkspace = window.location.pathname.indexOf('communication-workspace') !== -1;
  if (!isCommunicationDomain && !isCommunicationWorkspace) return;

  var STORE_KEY = 'limen_communication_exec_panels';
  ...
  var GRANT_SECTIONS = [
    { id: 'what', title: 'What This Grant Is', type: 'info', guidance: 'An communication grant is non-repayable funding from a federal agency, state, or private foundation. Major funders: US Dept of Communication, IES, NSF EHR, Gates Foundation, Walton, Carnegie, Lumina, Spencer.', prompt: 'No input needed.' },
    { id: 'why', title: 'Why This Opportunity Qualifies', type: 'textarea', guidance: 'Explain why this Communication opportunity is a good fit for grant funding. Address: communicational need, intervention strategy, evidence base, equity impact, alignment with funder priorities.', prompt: 'Write 3-5 sentences explaining need, strategy, and alignment.', prefillFrom: 'whyNow' },
    { id: 'eligibility', title: 'Eligibility', type: 'checklist', guidance: 'Most communication grants have eligibility rules.', items: ['Organization is an LEA, IHE, or 501(c)(3)', 'Organization has a UEI number', 'Organization is registered on SAM.gov / Grants.gov', 'No debarment or exclusion status', 'Has IRB approval if research with human subjects', 'Has financial management compliant with Uniform Guidance', 'Has data privacy and FERPA compliance documentation'] },
    ...
    { id: 'problem', title: 'Need Statement', type: 'textarea', guidance: 'What communicational problem does this address? Quantify with data: enrollment, achievement, dropout rates, workforce gaps.', prompt: 'Describe the communicational problem with data.', prefillFrom: 'diagnosis' },
  ...
  var PATENT_SECTIONS = [
    { id: 'what', title: 'What This Patent Is', type: 'info', guidance: 'For communication / edtech, patents typically protect software methods, instructional system designs, and assessment workflows. Provisional ~$320; non-provisional much more. Subject-matter eligibility (Alice/Mayo) is the main hurdle for ed software claims.', prompt: 'No input needed.' },
    ...
    { id: 'problem_solved', title: 'Problem Being Solved', type: 'textarea', guidance: 'What communicational or instructional problem does this address?', prompt: 'What problem exists today that this fixes?', prefillFrom: 'diagnosis' },
    ...
    if (!ws._prefilled && opp) {
      ...
      for (var si = 0; si < sections.length; si++) { var sec = sections[si];
        if (sec.prefillFrom && !ws.fields[sec.id]) {
          ...
          if (sec.prefillFrom === 'whyNow') ws.fields[sec.id] = (opp.title || '') + '. This Communication condition creates demand because: ' + (opp.reason || opp.whyNow || 'active diagnosis pathway detected.');
          if (sec.prefillFrom === 'diagnosis') ws.fields[sec.id] = 'Communication domain diagnosis: ' + (opp.diagnosisId || 'active').replace(/_/g, ' ') + '. ' + (opp.title || '');
        }
      }
  ...
  window.LIMENCommunicationExecutionPanels = { renderForOpportunity: renderForOpportunity, wireForOpportunity: wireForOpportunity, GRANT_SECTIONS: GRANT_SECTIONS, PATENT_SECTIONS: PATENT_SECTIONS };
  console.log('[CommunicationExecutionPanels] Loaded');
})();
