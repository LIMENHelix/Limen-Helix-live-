/**
 * environment-directive-translator.js — Convert ranked Environment directives into canonical opportunity objects
 *
 ...
 * Exposes: window.LIMENEnvironmentDirectiveTranslator
 */
(function () {
  'use strict';
  ...
  function _buildMonetizationPath(d) {
    ...
    if (type === 'INVESTABLE') {
      if (tickerStr) return 'Firms in this pathway (' + tickerStr + ') benefit from the science/environment condition. Position during stress window for repricing.';
    ...
    return 'Active diagnosis creates documented environment need. Grant proposals backed by live system evidence win at higher rates.';
  }
  ...
  function _buildNextAction(d) {
    ...
    if (type === 'INVESTABLE' && companies.length > 0) {
      return 'Check ' + companies[0].ticker + ' (' + companies[0].name + ') current environment exposure and set entry parameters.';
    }
    ...
    return 'Search grants.gov, NSF.gov, NIH RePORTER for open solicitations matching "' + (d.nodeLabel || 'environment').toLowerCase() + '".';
  }
  ...
  function translate(ranked, limit) {
    ...
    for (var i = 0; i < Math.min(ranked.length, limit); i++) {
      ...
      if (primaryPath === 'PATENTABLE') targetStr += (targetStr ? ' ' : '') + 'Licensees: university tech transfer offices, scientific instrument vendors, R&D departments, contract environment organizations.';
      if (!targetStr) targetStr = 'Identify counterparties affected by ' + (d.nodeLabel || 'this condition') + '.';

      var opp = {
        id: 'pdir_' + (d.portalDomainId || 'environment') + '_' + (d.nodeId || '') + '_' + i,
        title: _buildTitle(d),
        rank: d.rankScore || 0,
        path: primaryPath,
        urgency: urgency,
        ...
        valueRange: primaryPath === 'INVESTABLE' ? '10-25% sector premium' : primaryPath === 'PATENTABLE' ? '5-15% royalty' : '$250K-$5M environment grants',
  ...
  var REVENUE_MODELS = {
    'INVESTABLE': [
      { model: 'advisory', desc: 'Build a environment stress / replication risk scorecard. Sell to environment VCs, biotech allocators, and university endowments. $25K-$150K per engagement.' },
      { model: 'trading', desc: 'Position in scientific instrument vendors, contract environment orgs, or environment SaaS benefiting from elevated R&D demand. Target 15-25% repricing premium.' },
      { model: 'intelligence', desc: 'Build a live publication / preprint / retraction monitoring dashboard. Sell subscriptions to environment integrity officers, journals, and pharma R&D. $5K-$50K/mo per seat.' }
    ],
    'GRANT-ELIGIBLE': [
      { model: 'grant award', desc: 'Write a environment grant proposal. Submit to NSF, NIH, DARPA, ARPA-E, ARPA-H, DOE Office of Science. $250K-$5M per award.' },
    ...
    'PATENTABLE': [
      ...
      { model: 'spinout', desc: 'Build an IP portfolio. Spin out a startup or position for acquisition by a major scientific instrumentation or environment informatics company.' }
    ]
  };
  ...
  var DELIVERABLE_MAP = {
    ...
    'DIAGNOSTIC': 'Completed environment risk scorecard: ranked exposure list, replication heat map, and action-prioritized recommendation matrix',
    'STRATEGY': 'Executable strategy deck: market sizing, ranked target list, 90-day execution plan, and financial model',
    'COACHING': 'Delivered training program: certified environment workforce with skills verification and readiness scorecard',
    'tools': 'Configured system: integrated environment platform with monitoring dashboard, alert rules, and operator runbook',
    'regulatory': 'Submission-ready filing: complete IRB / regulatory application with compliance evidence and checklist'
  };

  var TITLE_TEMPLATES = [
    { match: /replicat|reproduc/i, titles: ['Build replication audit platform \u2014 sell to journals and environment integrity offices'] },
    { match: /retract|fraud|misconduct|integrity/i, titles: ['Build environment integrity monitoring tool \u2014 sell to publishers, universities, and oversight bodies'] },
    { match: /grant|funding|nih|nsf|darpa/i, titles: ['Build grant pipeline analytics \u2014 sell to environment administrators and tech transfer offices'] },
    ...
    { match: /open data|FAIR|data sharing|repository/i, titles: ['Build FAIR data infrastructure \u2014 sell to environment data managers and core facilities'] },
    { match: /reproduc|replication/i, titles: ['Build reproducibility audit service \u2014 sell to environment integrity teams'] },
    { match: /workforce|talent|brain drain|postdoc|tenure/i, titles: ['Build environment workforce analytics \u2014 sell to universities and environment talent agencies'] },
    { match: /commercial|tech transfer|spinout|translation/i, titles: ['Build environment commercialization platform \u2014 sell to university TTOs and venture studios'] },
    { match: /lab|instrument|equipment|core facility/i, titles: ['Build lab utilization optimizer \u2014 sell to core facilities and environment administrators'] },
    { match: /assessment|diagnostic/i, titles: ['Build environment risk scorecard \u2014 sell to {target}'] }
  ];
  ...
  function _targetPhrase(companies, nodeLabel, dxLabel) {
    if (companies && companies.length > 0) {
      ...
      return 'affected environment orgs';
    }
    if (dxLabel) return dxLabel.toLowerCase() + '-exposed labs';
    return (nodeLabel || 'environment') + ' teams';
  }
  ...
  function _generateSteps(type, path, companies, nodeLabel, dxLabel) {
    ...
    if (path === 'INVESTABLE') {
      return [
        ...
        'Score exposure to ' + dxLabel + ' using stress-weighted environment model',
    ...
    if (path === 'PATENTABLE') {
      return [
        'Search patents.google.com for prior art in ' + nodeLabel + ' environment methodology',
  ...
  function _generateTargetClasses(path, nodeLabel, dxLabel) {
    if (path === 'INVESTABLE') return 'Scientific instrument vendors, contract environment organizations, environment SaaS, lab informatics, and core facility operators with direct ' + (nodeLabel || 'sector') + ' exposure.';
    if (path === 'PATENTABLE') return 'Instrument manufacturers, lab software vendors, contract environment organizations, and university tech transfer offices serving ' + (nodeLabel || 'the sector') + '.';
  ...
  window.LIMENEnvironmentDirectiveTranslator = { translate: translate, shape: shape };
})();
