/**
 * law-directive-extractor.js — Exhaustive Fractal Directive Extraction (Law)
 *
 * Traverses the Law portal fractal tree starting from active diagnosis circuits,
 * following ALL childPortal references recursively until terminal leaf nodes
 * are reached. No depth caps.
 *
 * Architecture:
 *   - Prebuilt-first: loads /assets/data/deep/law-deep-directives.json if available
 ...
 * Exposes: window.LIMENLawDirectiveExtractor
 */
(function () {
  'use strict';
  ...
  function _loadPrebuiltPayload() {
    ...
    return fetch('/assets/data/deep/law-deep-directives.json')
      .then(function (r) { if (!r.ok) throw new Error(r.status); return r.json(); })
      .then(function (data) {
        if (data && data.directives && data.directives.length > 0) {
          _prebuiltPayload = data;
          console.log('[LawExtractor] Prebuilt: ' + data.directives.length + ' directives, maxDepth=' + (data.traversal && data.traversal.maxDepth));
        }
  ...
  function extract(state, portal, opts) {
    ...
    return _loadPrebuiltPayload().then(function (prebuilt) {
      if (prebuilt && prebuilt.directives) {
        ...
        console.log('[LawExtractor] Prebuilt: ' + filtered.length + ' candidates, ' + activeDx.length + ' active dx');
        return filtered;
      }
  ...
  function _liveExtract(activeNodeIds, activeDx, stress, portal) {
      ...
      (function (pid) {
        chain = chain.then(function () {
          return _fetchPortalJSON(pid).then(function (data) {
            if (!data) return;
            return _extractFromPortal(pid, data, activeNodeIds, stress, 1, ['law'], seen, results);
          });
        });
    ...
    return chain
      .then(function () {
        return _extractFromPortal('law', portal, activeNodeIds, stress, 0, [], seen, results);
      })
      .then(function () {
        console.log('[LawExtractor] Extracted ' + results.length + ' candidates from ' +
  ...
  window.LIMENLawDirectiveExtractor = {
    extract: extract,
