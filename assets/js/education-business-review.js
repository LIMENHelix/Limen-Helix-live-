/**
 * education-business-review.js — Education Business Assignment Review Surface
 *
 * EDUCATION DOMAIN ONLY. Renders a review panel for proposed business mappings
 * across the full 103-node hierarchy.
 * Requires education-node-business-engine.js to be loaded first.
 *
 * Self-gates: only runs when ?domain=education
 * Exposes: window.LIMENEducationBusinessReview
 */
(function () {
  'use strict';
  ...
  var PANEL_ID = 'ebr-business-review';
  ...
  function injectStyles() {
    ...
    s.textContent = [
      ...
      '.ebr-badge-top{color:#5ab5a0;border:1px solid rgba(90,181,160,0.15);background:rgba(90,181,160,0.04)}',
      '.ebr-badge-operational{color:#4a8fd4;border:1px solid rgba(74,143,212,0.15);background:rgba(74,143,212,0.04)}'
    ].join('\n');
  ...
  function statusBadge(approval) {
    ...
    var cls = { 'PROPOSED': 'ebr-badge-proposed', 'APPROVED': 'ebr-badge-approved', 'DENIED': 'ebr-badge-denied', 'NEEDS_REVIEW': 'ebr-badge-proposed' };
  ...
  function tierBadge(tier) {
    if (!tier) return '';
    var cls = { 'top': 'ebr-badge-top', 'operational': 'ebr-badge-operational' };
    return '<span class="ebr-badge ' + (cls[tier] || '') + '">' + (tier === 'top' ? 'TOP-LEVEL' : 'HIERARCHY') + '</span>';
  }
  ...
  function renderCard(entry) {
    ...
    if (entry.nodeActive) h += ' <span class="ebr-badge" style="color:#e85454;border:1px solid rgba(232,84,84,0.2)">ACTIVE</span>';
    h += '</div></div>';
    ...
    if (entry.neuroTranslation) {
      ...
      h += '<div style="font-size:0.26rem;letter-spacing:2px;color:rgba(74,143,212,0.5);margin-bottom:4px">IN NEUROLOGY / IN EDUCATION TRANSLATION</div>';
      ...
      h += '<div style="font-size:0.32rem;color:#b0a898;line-height:1.5"><b style="color:rgba(201,169,78,0.7)">In education (learning systems):</b> ' + esc(entry.neuroTranslation.inBusiness) + '</div>';
      h += '</div>';
    }
    ...
    if (entry.existingCompanies && entry.existingCompanies.length > 0) {
      ...
      h += '<div class="ebr-card-existing" style="color:#C9A94E">Missing from current Education system</div>';
    }
  ...
  function render() {
    var engine = window.LIMENEducationBusinessEngine;
    if (!engine) return;
    injectStyles();
    ...
    h += '<div class="ebr-title" style="margin-bottom:0">NODE-TO-BUSINESS ASSIGNMENT REVIEW</div>';
      ...
      bizBtns[bi].addEventListener('click', function (e) {
        ...
        try {
          sessionStorage.setItem('limen_exec_context', JSON.stringify({
            ...
            source: 'education', returnTo: '/domain-console?domain=education&mode=operator'
          }));
        } catch (ex) {}
        window.location.href = 'education-workspace.html?track=business&opp=' + encodeURIComponent(oppKey) + '&returnTo=' + encodeURIComponent('/domain-console?domain=education&mode=operator');
      });
    }
  ...
  window.LIMENEducationBusinessReview = { render: render, mount: mount };
  console.log('[EducationBusinessReview] Loaded \u2014 review surface ready');
})();
