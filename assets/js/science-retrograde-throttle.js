/*
 * science-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes science.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Science has no runtime load field). { nodeId: loadFraction }
  ...
  /* science: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(science, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (science.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. science.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Science has no runtime load field). throttleGain operator-set. Never writes science.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.ScienceRetrogradeThrottle) root.ScienceRetrogradeThrottle = API;
