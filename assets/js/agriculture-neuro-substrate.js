/*
 * agriculture-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/agriculture-neuro-substrate.json
 * Prompts:       assets/prompts/agriculture-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/p2_agri.json
 * or any existing agriculture-*.js. It reads existing Agriculture data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Agriculture brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Agriculture file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/agriculture-neuro-substrate.json';
  var AGRICULTURE_URL = '/assets/data/domains/p2_agri.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Agriculture issue id. */
     ...
     * Reads an Agriculture issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Agriculture issue. */
    auditAllIssues: function () {
      return loadJSON(AGRICULTURE_URL).then(function (agriculture) {
        return (agriculture.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.AgricultureNeuroSubstrate) root.AgricultureNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Agriculture brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import AgricultureNeuroSubstrate from '/assets/js/agriculture-neuro-substrate.js';
   *      // then, after issues render:
   *      AgricultureNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
