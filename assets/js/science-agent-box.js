/**
 * science-agent-box.js — the Science domain's operator AI box (client side).
 *
 * A floating "Talk to Science" panel on the science domain-console. It KNOWS its
 * domain (window.LIMENScienceBrain.getStateSummary()) and can make bounded
 ...
 *    tries /api/science-agent first and falls back to LOCAL on any failure, so it
...
(function () {
  ...
  function parseLocal(prompt, state) {
    ...
    if (mF) {
      var topic = mF[1].replace(/\b(please|now|today|this week|domain|science|closely|carefully)\b/g, '').trim();
  ...
  if (typeof window !== 'undefined') window.__scienceAgentLocal = { parse: parseLocal, query: answerQuery };

  ready(function () {
    if (window.__scienceAgentBox) return; window.__scienceAgentBox = true;
    var LS_KEY = 'limenScienceAgentKey';
    var esc = function (s) { return String(s == null ? '' : s).replace(/[&<>"]/g, function (c) { return ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[c]; }); };
    function brain() { return window.LIMENScienceBrain || null; }
    ...
    var toggle = document.createElement('div'); toggle.id = 'eab-toggle'; toggle.textContent = '⚡ Talk to Science';
    var panel = document.createElement('div'); panel.id = 'eab';
    panel.innerHTML =
      '<div class="eab-h"><span><b>Science</b> · ask or steer</span><span class="eab-x">✕</span></div>' +
    ...
    function renderResult(pending, result, mode, left) {
      ...
      if (applied) html += '<div class="eab-tool">✓ applied to science domain:\n' + esc(applied) + '</div>';
    ...
    function send() {
      var b = brain();
      if (!b) { add('<span class="eab-ai">Science brain not loaded on this page.</span>'); return; }
    ...
    add('<span class="eab-ai">Science domain online (free local mode). Ask about it, or steer it: "focus on grid", "prefer research", "autonomy off", "how is science doing", "show the forecast". I bias the domain, never force a finding, move capital, or touch code. Add a passcode above for conversational AI once billing is on.</span>');
  });
})();
