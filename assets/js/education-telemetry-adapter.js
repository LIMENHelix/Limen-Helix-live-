/*
 * education-telemetry-adapter.js  — wires runtime fields to the LIVE Education telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/education-pulse-engine.js  (window.LIMENEducationPulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Education issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to education.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into education.json (a
...
(function (root) {
   ...
   * inputs: { education, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (education.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (education.activations || []).forEach(function (a) {
  ...
  /* Merge education.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(education, overlay) {
    var base = education.runtime || {};
    ...
    var acts = {}; (education.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (education.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the education definition + brainState. */
  function fromLive(education, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENEducationPulse) ? window.LIMENEducationPulse : null;
    if (!P) return null;
    return fromPulse({ education: education, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN education definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/education.json', 'education'),
      _load('/assets/data/education-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ education: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.EducationTelemetryAdapter) root.EducationTelemetryAdapter = API;
