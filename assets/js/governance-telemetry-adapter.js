/*
 * governance-telemetry-adapter.js  — wires runtime fields to the LIVE Governance telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/governance-pulse-engine.js  (window.LIMENGovernancePulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Governance issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to governance.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into governance.json (a
...
(function (root) {
   ...
   * inputs: { governance, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (governance.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (governance.activations || []).forEach(function (a) {
  ...
  /* Merge governance.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(governance, overlay) {
    var base = governance.runtime || {};
    ...
    var acts = {}; (governance.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (governance.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the governance definition + brainState. */
  function fromLive(governance, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENGovernancePulse) ? window.LIMENGovernancePulse : null;
    if (!P) return null;
    return fromPulse({ governance: governance, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN governance definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/governance.json', 'governance'),
      _load('/assets/data/governance-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ governance: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.GovernanceTelemetryAdapter) root.GovernanceTelemetryAdapter = API;
