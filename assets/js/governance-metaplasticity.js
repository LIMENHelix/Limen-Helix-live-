/*
 * governance-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Governance's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (governance-offline-maintenance.js)
 *   - refractory absolute window       (governance-refractory-limiter.js)
 *   - prediction-error threshold       (governance-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Governance has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Governance has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.GovernanceMetaplasticity) root.GovernanceMetaplasticity = API;
