/*
 * law-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes law.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Law has no runtime load field). { nodeId: loadFraction }
  ...
  /* law: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(law, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (law.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. law.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Law has no runtime load field). throttleGain operator-set. Never writes law.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.LawRetrogradeThrottle) root.LawRetrogradeThrottle = API;
