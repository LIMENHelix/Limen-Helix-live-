/*
 * communication-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/communication-neuro-substrate.json
 * Prompts:       assets/prompts/communication-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/communication.json
 * or any existing communication-*.js. It reads existing Communication data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Communication brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Communication file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/communication-neuro-substrate.json';
  var COMMUNICATION_URL = '/assets/data/domains/communication.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Communication issue id. */
     ...
     * Reads an Communication issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Communication issue. */
    auditAllIssues: function () {
      return loadJSON(COMMUNICATION_URL).then(function (communication) {
        return (communication.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.CommunicationNeuroSubstrate) root.CommunicationNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Communication brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import CommunicationNeuroSubstrate from '/assets/js/communication-neuro-substrate.js';
   *      // then, after issues render:
   *      CommunicationNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
