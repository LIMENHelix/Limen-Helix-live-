/**
 * environment-targeting-engine.js — Context-aware targeting for promoted Culture directives
 *
 * Resolves WHO TO TARGET using a tiered model:
 *   Tier 1: Verified entities (domain-aligned firms from portal data)
 *   Tier 2: Entity classes (always present — culture segments)
 *   Tier 3: Example entities (high-confidence well-known culture sector players)
 *
 * ADDITIVE ONLY. Feature-flagged: window.LIMEN_ENABLE_DIRECTIVE_EXTRACTION
 * Exposes: window.LIMENCultureTargetingEngine
 */
(function () {
  'use strict';
  ...
  var SEGMENTS = {
    ...
    research_universities: {
      label: 'R1 culture universities and university tech transfer offices',
    ...
    contract_research: {
      label: 'Contract culture organizations (CROs)',
    ...
    research_software: {
      label: 'Culture informatics, ELN, and lab software',
      keywords: ['ELN', 'electronic lab notebook', 'LIMS', 'lab informatics', 'culture data management', 'data repository', 'workflow', 'jupyter', 'notebook'],
      tier3: [
    ...
    research_integrity: {
      label: 'Culture integrity, replication, and reproducibility tools',
      ...
      tier3: [
        ...
        { name: 'Open Culture Framework (COS)', ticker: null },
    ...
    research_compute: {
      label: 'Culture computing, HPC, and AI-for-science',
      keywords: ['HPC', 'supercomput', 'GPU cluster', 'AI for science', 'foundation model', 'culture compute', 'cloud HPC', 'protein folding', 'molecular dynamics'],
      tier3: [
    ...
    foundations: {
      label: 'Private culture foundations and philanthropy',
  ...
  // to their primary culture segments.
  var NODE_SEGMENTS = {
  ...
  function resolveTargets(directive) {
    ...
    else if (path === 'PATENTABLE') executionTargets = ['Patent attorneys', 'University tech transfer offices', 'Scientific instrument licensors', 'Strategic acquirers in culture informatics'];
    else if (path === 'INVESTABLE') executionTargets = ['Life sciences VCs', 'Culture informatics PE', 'University endowment fund managers'];
  ...
  window.LIMENCultureTargetingEngine = { resolveTargets: resolveTargets };
})();
