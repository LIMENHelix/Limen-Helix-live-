/*
 * infrastructure-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Infrastructure's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (infrastructure-offline-maintenance.js)
 *   - refractory absolute window       (infrastructure-refractory-limiter.js)
 *   - prediction-error threshold       (infrastructure-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Infrastructure has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Infrastructure has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.InfrastructureMetaplasticity) root.InfrastructureMetaplasticity = API;
