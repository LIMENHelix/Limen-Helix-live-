/*
 * medicine-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Medicine's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (medicine-offline-maintenance.js)
 *   - refractory absolute window       (medicine-refractory-limiter.js)
 *   - prediction-error threshold       (medicine-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Medicine has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Medicine has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.MedicineMetaplasticity) root.MedicineMetaplasticity = API;
