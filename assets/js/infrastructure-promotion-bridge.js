/**
 * infrastructure-promotion-bridge.js — Inject promoted portal directives into operator surfaces
 *
 ...
 *
 * Exposes: window.LIMENInfrastructurePromotionBridge
 */
(function () {
  'use strict';
   ...
   *
   * @param {Object} state - infrastructure brain state
   * @param {Object} portal - L0 infrastructure portal JSON
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENInfrastructureDirectiveExtractor;
    var ranker = window.LIMENInfrastructureDirectiveRanker;
    var translator = window.LIMENInfrastructureDirectiveTranslator;

    if (!extractor || !ranker || !translator) {
      console.warn('[InfrastructurePromotionBridge] Missing module(s) — extractor:', !!extractor, 'ranker:', !!ranker, 'translator:', !!translator);
      _running = false;
      return Promise.resolve([]);
    }

    console.log('[InfrastructurePromotionBridge] Input signature changed — running extraction pipeline');
    console.log('[InfrastructurePromotionBridge] Signature: ' + sig);
    var t0 = Date.now();

    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        if (!candidates || candidates.length === 0) {
          console.log('[InfrastructurePromotionBridge] No candidates extracted');
          _lastSignature = sig;
          _lastPromoted = [];
          _running = false;
          return [];
        ...
        console.log('[InfrastructurePromotionBridge] Extracted ' + candidates.length + ' candidates in ' + (Date.now() - t0) + 'ms');
        ...
        if (bestProofIdx > limit - 1 && bestProofIdx !== -1) {
          ...
          console.log('[InfrastructurePromotionBridge] Elevated proof directive from position ' + bestProofIdx + ': ' + proofItem.treatmentLabel.substring(0, 50));
        }

        // Pass 2: Force a DEEP PROOF slot — best L2+ directive into the promoted set
        // This ensures we always have visible proof of deeper fractal extraction
        var bestDeepIdx = -1;
        var bestDeepScore = 0;
        ...
        if (bestDeepIdx !== -1 && bestDeepIdx >= limit) {
          ...
          console.log('[InfrastructurePromotionBridge] Deep proof slot: L' + deepItem.depth + ' ' + deepItem.treatmentLabel.substring(0, 50) + ' (deepScore=' + bestDeepScore.toFixed(3) + ')');
        } else if (bestDeepIdx !== -1 && bestDeepIdx < limit) {
          // Already in top set — just mark it
          ranked[bestDeepIdx]._isDeepProofSlot = true;
          console.log('[InfrastructurePromotionBridge] Deep proof already in top set: L' + ranked[bestDeepIdx].depth);
        }

        console.log('[InfrastructurePromotionBridge] Top 5 after depth-aware selection:');
        ...
        var targeting = window.LIMENInfrastructureTargetingEngine;
        if (targeting) {
        ...
        console.log('[InfrastructurePromotionBridge] Promoted ' + dedupedPromoted.length + ' directives (' + (Date.now() - t0) + 'ms total)');
        return dedupedPromoted;
      })
      .catch(function (err) {
        console.error('[InfrastructurePromotionBridge] Pipeline failed — existing opportunities untouched:', err.message || err);
  ...
  window.LIMENInfrastructurePromotionBridge = {
    promote: promote,
