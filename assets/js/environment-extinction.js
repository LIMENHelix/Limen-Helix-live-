/*
 * environment-extinction.js  — ADDITIVE mechanism (completes EXTINCTION_LEARNING)
 * ----------------------------------------------------------------------------
 ...
 * (complements the GLOBAL down-scaling in environment-offline-maintenance.js).
 *
 * CONTRACT: read-only / copy-only. NEVER writes environment.json. Proposes which acquired responses to
...
(function (root) {
  ...
  /* Extinction over Environment activations: an activation whose diagnosticTriggers are ALL absent
   * from the caller-supplied activeTriggers is a candidate to retire (its trigger is gone).
   * activeTriggers: array of currently-firing trigger strings (runtime state; Environment has none baked in). */
  function proposeExtinction(environment, activeTriggers, opts) {
    ...
    return {
      ...
      note: 'Read-only PROPOSAL. Applies nothing; retiring a node would edit environment.json. activeTriggers is caller-supplied runtime state.'
    };
  }

  var API = { proposeExtinction: proposeExtinction };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.EnvironmentExtinction) root.EnvironmentExtinction = API;
