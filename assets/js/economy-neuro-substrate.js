/*
 * economy-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/economy-neuro-substrate.json
 * Prompts:       assets/prompts/economy-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/economy.json
 * or any existing economy-*.js. It reads existing Economy data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Economy brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Economy file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/economy-neuro-substrate.json';
  var ECONOMY_URL = '/assets/data/domains/economy.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Economy issue id. */
     ...
     * Reads an Economy issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Economy issue. */
    auditAllIssues: function () {
      return loadJSON(ECONOMY_URL).then(function (economy) {
        return (economy.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.EconomyNeuroSubstrate) root.EconomyNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Economy brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import EconomyNeuroSubstrate from '/assets/js/economy-neuro-substrate.js';
   *      // then, after issues render:
   *      EconomyNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
