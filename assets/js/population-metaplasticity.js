/*
 * population-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Population's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (population-offline-maintenance.js)
 *   - refractory absolute window       (population-refractory-limiter.js)
 *   - prediction-error threshold       (population-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Population has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Population has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.PopulationMetaplasticity) root.PopulationMetaplasticity = API;
