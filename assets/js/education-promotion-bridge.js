/**
 * education-promotion-bridge.js — Inject promoted Education portal directives
 * Exposes: window.LIMENEducationPromotionBridge
 */
(function () {
  'use strict';
  ...
  function _computeSignature(state) {
    ...
    return ['education', dxKeys.join(','), nodeKeys.join(','), stressBucket.toFixed(2), window.LIMEN_ENABLE_DIRECTIVE_EXTRACTION ? '1' : '0'].join('|');
  }
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENEducationDirectiveExtractor;
    var ranker = window.LIMENEducationDirectiveRanker;
    var translator = window.LIMENEducationDirectiveTranslator;
    if (!extractor || !ranker || !translator) {
      console.warn('[EducationPromotionBridge] Missing pipeline component');
      _running = false;
    ...
    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        ...
        var targeting = window.LIMENEducationTargetingEngine;
        if (targeting) {
        ...
        console.log('[EducationPromotionBridge] Promoted ' + dedupedPromoted.length);
        return dedupedPromoted;
      })
      .catch(function (err) {
        console.error('[EducationPromotionBridge] Pipeline failed:', err.message || err);
        _lastSignature = sig; _lastPromoted = []; _running = false; return [];
      });
  }
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      state.treatments.push({
        id: 'ptreat_' + (dir.portalDomainId || 'education') + '_' + (dir.nodeId || '') + '_' + pi,
  ...
  window.LIMENEducationPromotionBridge = { promote: promote, getLastPromoted: getLastPromoted };
})();
