/*
 * law-telemetry-adapter.js  — wires runtime fields to the LIVE Law telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/law-pulse-engine.js  (window.LIMENLawPulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Law issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to law.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into law.json (a
...
(function (root) {
   ...
   * inputs: { law, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (law.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (law.activations || []).forEach(function (a) {
  ...
  /* Merge law.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(law, overlay) {
    var base = law.runtime || {};
    ...
    var acts = {}; (law.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (law.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the law definition + brainState. */
  function fromLive(law, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENLawPulse) ? window.LIMENLawPulse : null;
    if (!P) return null;
    return fromPulse({ law: law, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN law definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/law.json', 'law'),
      _load('/assets/data/law-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ law: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.LawTelemetryAdapter) root.LawTelemetryAdapter = API;
