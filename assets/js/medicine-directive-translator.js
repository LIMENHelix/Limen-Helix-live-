/**
 * medicine-directive-translator.js — Convert ranked directives into canonical opportunity objects
 *
 * Takes ranked directives from medicine-directive-ranker and translates them into
 * the same opportunity object shape that medicine-brain.js produces, so they can
 ...
 *
 * Exposes: window.LIMENMedicineDirectiveTranslator
 */
(function () {
  'use strict';
  ...
  function _buildMonetizationPath(d) {
    ...
    if (type === 'INVESTABLE') {
      ...
      return 'Technology gap in ' + (d.nodeLabel || 'this area').toLowerCase() + ' has no dominant IP holder. First patent filing creates defensible position licensable to hospitals, payers, and pharma.';
    } else {
  ...
  function _buildNextAction(d) {
    ...
    if (type === 'INVESTABLE' && companies.length > 0) {
      ...
      return 'Search patents.google.com for prior art in "' + (d.nodeLabel || 'medicine').toLowerCase() + '" technology. Draft provisional claim outline.';
    } else {
      return 'Search grants.gov for open solicitations matching "' + (d.nodeLabel || 'medicine').toLowerCase() + '". Draft 1-page concept note with system evidence.';
    }
  }
  ...
  function _buildEvidence(d) {
    var parts = ['Domain: medicine. Stress: ' + Math.round((d.stress || 0) * 100) + '%.'];
  ...
  function translate(ranked, limit) {
    ...
    for (var i = 0; i < Math.min(ranked.length, limit); i++) {
      ...
      if (primaryPath === 'GRANT-ELIGIBLE') targetStr += (targetStr ? ' ' : '') + 'Agencies: NIH, BARDA, CMS Innovation Center, HRSA, state health departments. Search grants.gov.';
      if (primaryPath === 'PATENTABLE') targetStr += (targetStr ? ' ' : '') + 'Licensees: hospital systems, pharma companies, medical device firms, health IT vendors, payers.';
      if (!targetStr) targetStr = 'Identify counterparties affected by ' + (d.nodeLabel || 'this condition') + '.';

      var opp = {
        // Core identity
        id: 'pdir_' + (d.portalDomainId || 'medicine') + '_' + (d.nodeId || '') + '_' + i,
        title: _buildTitle(d),
        rank: d.rankScore || 0,
        path: primaryPath,
        urgency: urgency,
    ...
    console.log('[MedicineDirectiveTranslator] Translated ' + opportunities.length + ' opportunities from ' + ranked.length + ' ranked directives');
  ...
  var ACTION_VERBS = {
    ...
    'clinical':    ['Implement', 'Standardize', 'Deploy', 'Validate'],
    'pharma':      ['License', 'Distribute', 'Formulate', 'Certify']
  };

  var REVENUE_MODELS = {
    'INVESTABLE': [
      { model: 'advisory', desc: 'Build a clinical disruption-exposure scorecard for health systems and payers. Sell to hospital networks, insurers, and pharma. $50K-$250K per engagement.' },
      { model: 'positioning', desc: 'Identify underpriced assets in the disrupted healthcare segment. Position through stress window. Target 15-30% sector premium on recovery.' },
      { model: 'intelligence', desc: 'Build a live healthcare stress monitoring dashboard. Sell subscriptions to hospital systems, insurers, and biotech investors. $10K-$100K/mo per seat.' }
    ],
    'GRANT-ELIGIBLE': [
      { model: 'grant award', desc: 'Write a grant proposal backed by live clinical stress data. Submit to NIH, BARDA, CMS Innovation Center, or HRSA. $250K-$10M per award. 60-180 day cycle.' },
      { model: 'consulting', desc: 'Win the grant. Deliver the funded clinical or public health project. Bill implementation hours at $300-$600/hr against the award budget.' },
      { model: 'cost recovery', desc: 'Implement the solution for a hospital system or public health agency. Bill costs through their approved healthcare budget. Guaranteed recovery.' }
    ],
    'PATENTABLE': [
      { model: 'licensing', desc: 'File a provisional patent on the clinical technology method. License it to every hospital system and health IT vendor in the affected sector. 5-15% royalty per implementation.' },
      { model: 'SaaS', desc: 'Turn the patented method into a clinical intelligence platform. Sell subscriptions. $10K-$100K/mo per institution.' },
      { model: 'acquisition', desc: 'Build the IP portfolio around clinical analytics. Position for acquisition by a health IT company or pharma firm. $5M-$100M exit depending on coverage.' }
    ]
  };

  var DELIVERABLE_MAP = {
    'STRUCTURAL': 'Finished deployment plan: vendor-scored shortlist, clinical integration timeline, cost model, and procurement-ready RFP',
    'DIAGNOSTIC': 'Completed clinical risk scorecard: ranked exposure list, patient outcome heat map, and action-prioritized recommendation matrix',
    'STRATEGY': 'Executable strategy deck: market sizing, ranked target list, 90-day execution plan, and financial model',
    'COACHING': 'Delivered training program: certified clinical workforce with skills verification and readiness scorecard',
    'tools': 'Configured system: integrated clinical monitoring platform with dashboard, alert rules, and operator runbook',
    'regulatory': 'Submission-ready filing: complete FDA/CMS application with clinical evidence, budget narrative, and compliance checklist',
    'clinical': 'Clinical pathway package: standardized protocol, outcome metrics, monitoring plan, and escalation triggers',
    'pharma': 'Drug pipeline assessment: efficacy data, regulatory pathway map, market sizing, and commercialization timeline'
  };
  ...
  var TITLE_TEMPLATES = [
    { match: /clinical trial|phase [123]|randomized|placebo/i, titles: ['Build clinical trial disruption model \u2014 sell to CROs and pharma sponsors'] },
    { match: /drug|pharmaceutical|medication|prescription|formulary/i, titles: ['Build drug supply stress model \u2014 sell to pharma distributors and hospital pharmacies'] },
    { match: /hospital|inpatient|icu|emergency department|bed capacity/i, titles: ['Build hospital capacity stress model \u2014 sell to health systems and CMS'] },
    { match: /telemedicine|telehealth|remote care|virtual visit|digital health/i, titles: ['Build telemedicine adoption model \u2014 sell to health IT vendors and payers'] },
    { match: /pharmacy|dispensing|medication management|adherence/i, titles: ['Build pharmacy operations model \u2014 sell to PBMs and retail pharmacy chains'] },
    { match: /diagnostic|imaging|radiology|pathology|lab test|screening/i, titles: ['Build diagnostic bottleneck model \u2014 sell to reference labs and imaging centers'] },
    { match: /surgery|surgical|procedure|operating room|anesthesia/i, titles: ['Build surgical capacity model \u2014 sell to ASCs and hospital systems'] },
    { match: /oncology|cancer|tumor|chemotherapy|radiation therapy/i, titles: ['Build oncology treatment pathway model \u2014 sell to cancer centers and pharma'] },
    { match: /cardiology|cardiac|heart|cardiovascular|arrhythmia/i, titles: ['Build cardiac care disruption model \u2014 sell to cardiology practices and device makers'] },
    { match: /mental health|psychiatric|behavioral|depression|anxiety|substance/i, titles: ['Build mental health access model \u2014 sell to behavioral health networks and payers'] },
    { match: /pediatric|child|neonatal|infant|maternal/i, titles: ['Build pediatric care stress model \u2014 sell to children\'s hospitals and MCOs'] },
    { match: /public health|epidemiology|surveillance|outbreak|cdc|prevention/i, titles: ['Build public health surveillance model \u2014 sell to state health departments and CDC'] },
    { match: /payer|insurance|coverage|reimbursement|claims|managed care/i, titles: ['Build payer risk exposure model \u2014 sell to health insurers and actuarial firms'] },
    { match: /medical device|implant|prosthetic|wearable|sensor/i, titles: ['Build medical device market disruption model \u2014 sell to device manufacturers and GPOs'] },
    { match: /fda|approval|regulatory|compliance|post-market/i, titles: ['Build FDA regulatory pathway model \u2014 sell to pharma and medtech companies'] },
    { match: /cdc|infectious disease|vaccine|immunization|epidemic/i, titles: ['Build infectious disease response model \u2014 sell to public health agencies and vaccine makers'] },
    { match: /patient safety|adverse event|medical error|sentinel|quality/i, titles: ['Build patient safety analytics model \u2014 sell to hospital systems and accreditation bodies'] },
    { match: /chronic disease|diabetes|hypertension|copd|asthma|obesity/i, titles: ['Build chronic disease management model \u2014 sell to health systems and disease management orgs'] },
    { match: /emergency|trauma|ems|ambulance|urgent care|disaster/i, titles: ['Build emergency care capacity model \u2014 sell to EMS systems and trauma centers'] },
    { match: /rehabilitation|physical therapy|occupational therapy|recovery|post-acute/i, titles: ['Build rehabilitation pathway model \u2014 sell to post-acute networks and payers'] },
    { match: /assessment|diagnostic/i, titles: ['Build clinical sector risk scorecard \u2014 sell to {target}'] }
  ];
  ...
  function _targetPhrase(companies, nodeLabel, dxLabel) {
    ...
    return (nodeLabel || 'medicine') + ' buyers';
  }
  ...
  function _sharpenStep(raw, index, companies, nodeLabel) {
    ...
    raw = raw.replace(/\busing (FDA|CMS|NIH|WHO|CDC|JCAHO|AHRQ|AMA|HIPAA) [^,.]+ (framework|methodology|analysis|standards|requirements)/gi, '');
  ...
  function _generateSteps(type, path, companies, nodeLabel, dxLabel) {
    ...
    if (path === 'INVESTABLE') {
      return [
        'Pull ' + target + ' clinical exposure data and Helix phase status',
        'Score vulnerability to ' + dxLabel + ' disruption using stress-weighted model',
        'Identify entry price and position size based on healthcare stress confidence',
      ...
      return [
        'Search patents.google.com for prior art in ' + nodeLabel + ' clinical technology',
        'Draft provisional patent claim around ' + (nodeLabel || 'diagnostic') + ' methodology',
        'Identify 5 potential licensees in the ' + nodeLabel + ' healthcare space',
      ...
      return [
        ...
        'Draft 1-page concept note with live clinical stress evidence',
        'Prepare budget narrative: personnel, technology, indirect costs',
        'Assemble supporting documents: capability statement, clinical impact analysis',
  ...
  function _generateTargetClasses(path, nodeLabel, dxLabel) {
    if (path === 'INVESTABLE') return 'Healthcare-focused hedge funds, biotech investors, hospital REITs, and institutional investors with direct ' + (nodeLabel || 'sector') + ' exposure.';
    if (path === 'PATENTABLE') return 'Health IT vendors, medical device companies, pharma firms, and hospital systems serving ' + (nodeLabel || 'the sector') + '.';
    return 'NIH, BARDA, CMS Innovation Center, HRSA, state health departments. Public institutions with ' + (nodeLabel || 'clinical') + ' mandates.';
  }
  ...
  function _inferType(label) {
    ...
    if (l.indexOf('clinical') !== -1 || l.indexOf('protocol') !== -1 || l.indexOf('pathway') !== -1) return 'clinical';
    if (l.indexOf('drug') !== -1 || l.indexOf('pharma') !== -1 || l.indexOf('formul') !== -1) return 'pharma';
    return 'DIAGNOSTIC';
  }
  ...
  function shape(translated, shapeCount) {
    ...
    console.log('[MedicineDirectiveTranslator] Shaped ' + shaped + '/' + shapeCount + ' directives into deal-grade outputs');
    return result;
  }

  window.LIMENMedicineDirectiveTranslator = { translate: translate, shape: shape };
})();
