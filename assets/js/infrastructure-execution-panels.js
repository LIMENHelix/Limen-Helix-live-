/**
 * infrastructure-execution-panels.js — GRANT and PATENT Execution Workspaces
 *
 * Infrastructure domain only. Expandable workflow panels for each opportunity.
 ...
 *
 * Self-gates: only runs when ?domain=infrastructure
 * Exposes: window.LIMENInfrastructureExecutionPanels
 */
(function () {
  'use strict';

  var params = new URLSearchParams(window.location.search);
  var isInfrastructureDomain = params.get('domain') === 'infrastructure';
  var isInfrastructureWorkspace = window.location.pathname.indexOf('infrastructure-workspace') !== -1;
  if (!isInfrastructureDomain && !isInfrastructureWorkspace) return;

  var STORE_KEY = 'limen_infrastructure_exec_panels';
  ...
  var GRANT_SECTIONS = [
    ...
    { id: 'why', title: 'Why This Opportunity Qualifies', type: 'textarea',
      guidance: 'Explain in plain English why this Infrastructure opportunity is a good fit for grant funding. Think about: Is there a public benefit? Does it address infrastructure, safety, or innovation? Is there a government priority that matches?',
    ...
    { id: 'problem', title: 'Need / Problem Statement', type: 'textarea',
      ...
      prompt: 'Describe the infrastructure problem this project addresses. What happens if nothing is done?', prefillFrom: 'diagnosis' },
    ...
    if (!ws._prefilled && opp) {
      ...
      for (var si = 0; si < sections.length; si++) {
        ...
        if (sec.prefillFrom && !ws.fields[sec.id]) {
          ...
          if (sec.prefillFrom === 'whyNow') ws.fields[sec.id] = (opp.title || '') + '. This Infrastructure condition creates demand because: ' + (opp.reason || opp.whyNow || 'active diagnosis pathway detected.');
          if (sec.prefillFrom === 'diagnosis') ws.fields[sec.id] = 'Infrastructure domain diagnosis: ' + (opp.diagnosisId || 'active').replace(/_/g, ' ') + '. ' + (opp.title || '');
        }
      }
  ...
  window.LIMENInfrastructureExecutionPanels = {
  ...
  console.log('[InfrastructureExecutionPanels] Loaded — GRANT + PATENT workspaces ready');
