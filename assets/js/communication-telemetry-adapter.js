/*
 * communication-telemetry-adapter.js  — wires runtime fields to the LIVE Communication telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/communication-pulse-engine.js  (window.LIMENCommunicationPulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Communication issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to communication.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into communication.json (a
...
(function (root) {
   ...
   * inputs: { communication, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    inp = inp || {};
    var communication = inp.communication || {};
    ...
    var issuesOverlay = {};
    (communication.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (communication.activations || []).forEach(function (a) {
  ...
  /* Merge communication.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(communication, overlay) {
    var base = communication.runtime || {};
    ...
    var acts = {}; (communication.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (communication.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the communication definition + brainState. */
  function fromLive(communication, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENCommunicationPulse) ? window.LIMENCommunicationPulse : null;
    if (!P) return null;
    return fromPulse({ communication: communication, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN communication definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/communication.json', 'communication'),
      _load('/assets/data/communication-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ communication: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.CommunicationTelemetryAdapter) root.CommunicationTelemetryAdapter = API;
