/*
 * defense-telemetry-adapter.js  — wires runtime fields to the LIVE Defense telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/defense-pulse-engine.js  (window.LIMENDefensePulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Defense issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to defense.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into defense.json (a
...
(function (root) {
   ...
   * inputs: { defense, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (defense.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (defense.activations || []).forEach(function (a) {
  ...
  /* Merge defense.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(defense, overlay) {
    var base = defense.runtime || {};
    ...
    var acts = {}; (defense.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (defense.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the defense definition + brainState. */
  function fromLive(defense, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENDefensePulse) ? window.LIMENDefensePulse : null;
    if (!P) return null;
    return fromPulse({ defense: defense, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN defense definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/defense.json', 'defense'),
      _load('/assets/data/defense-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ defense: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.DefenseTelemetryAdapter) root.DefenseTelemetryAdapter = API;
