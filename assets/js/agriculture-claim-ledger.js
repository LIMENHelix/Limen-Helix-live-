/**
 * agriculture-claim-ledger.js — Agriculture Domain Claim Ledger (Local Only)
 *
 * Stores claim records in localStorage. Tracks opportunity claims
 * from acceptance through outcome. Agriculture-scoped only.
 *
 * Namespace: window.LIMENAgriculture.economy.claims
 */
(function () {
  'use strict';
  ...
  var STORE_KEY = 'LIMEN_AGRICULTURE_CLAIMS';
  ...
  function createClaimRecord(opportunity) {
    ...
    try { window.dispatchEvent(new CustomEvent('limen:agriculture-claim-update')); } catch (e) {}
  ...
  window.LIMENAgriculture.economy.claims = {
  ...
  console.log('[AgricultureClaimLedger] Loaded — claim ledger ready');
})();
