/**
 * technology-pulse-engine.js — Live Feed Pulse Engine for Technology Domain
 * Self-gates: only runs when ?domain=technology
 * Exposes: window.LIMENTechnologyPulse
 */
(function () {
  'use strict';
  ...
  if (dom !== 'technology') return;

  var FRESHNESS_TTL = {
    'technology_indicator': 24 * 60 * 60 * 1000,
  ...
  var SOURCE_TYPES = {
    'USPTO Patents':             'patent_feed',
    'arXiv CS':                  'research_feed',
    'NIST NVD':                  'vulnerability_feed',
    'CISA Known Exploited Vulnerabilities': 'vulnerability_feed',
    'Krebs Security':            'news_feed',
    'Hacker News':               'news_feed',
    'GitHub Security Advisories':'vulnerability_feed'
  };

  var EVIDENCE_CONTRACTS = {
    'CYBER_ATTACK': {
      label: 'Cyber Attack Surge',
      requiredFamilies: ['vulnerability_event', 'news_event'],
      minimumEvidence: 1,
      freshnessThreshold: FRESHNESS_TTL.news_feed,
      primaryIndicators: ['cyber_breach', 'network_intrusion', 'ransomware', 'infrastructure_attack'],
      secondaryConfirmation: ['macro_shock'],
      catchAllBlocked: true
    },
    'AI_ALIGNMENT_FAILURE': {
      label: 'AI Alignment Failure',
      requiredFamilies: ['research_event', 'news_event'],
      minimumEvidence: 1,
      freshnessThreshold: FRESHNESS_TTL.research_feed,
      primaryIndicators: ['ai_misalignment', 'autonomy_overreach', 'bias_amplification', 'alignment_gap'],
      secondaryConfirmation: [],
      catchAllBlocked: true
    },
    'INFRASTRUCTURE_COLLAPSE': {
      label: 'Infrastructure Collapse',
      requiredFamilies: ['news_event', 'vulnerability_event'],
      minimumEvidence: 1,
      freshnessThreshold: FRESHNESS_TTL.news_feed,
      primaryIndicators: ['system_failure', 'outage_cascade', 'cloud_disruption', 'dependency_failure'],
      secondaryConfirmation: [],
      catchAllBlocked: false
    },
    'DATA_BREACH': {
      label: 'Data Breach Surge',
      requiredFamilies: ['news_event', 'vulnerability_event'],
      minimumEvidence: 1,
      freshnessThreshold: FRESHNESS_TTL.news_feed,
      primaryIndicators: ['data_exfiltration', 'privacy_exposure', 'credential_compromise', 'compliance_violation'],
      secondaryConfirmation: ['cyber_breach'],
      catchAllBlocked: true
    },
    'CHIP_SHORTAGE': {
      label: 'Chip / Compute Shortage',
      requiredFamilies: ['news_event', 'patent_event'],
      minimumEvidence: 1,
      freshnessThreshold: FRESHNESS_TTL.news_feed,
      primaryIndicators: ['supply_constraint', 'semiconductor_gap', 'manufacturing_bottleneck', 'component_scarcity'],
      secondaryConfirmation: ['structural_stress'],
      catchAllBlocked: true
    },
    'PLATFORM_MONOPOLY': {
      label: 'Platform Monopoly Pressure',
      requiredFamilies: ['news_event', 'research_event'],
      minimumEvidence: 1,
      freshnessThreshold: FRESHNESS_TTL.news_feed,
      primaryIndicators: ['market_concentration', 'platform_lock_in', 'innovation_suppression', 'competitive_distortion'],
      secondaryConfirmation: ['structural_stress'],
      catchAllBlocked: true
    }
  };
  ...
  function computePulse(brainState) {
    ...
    for (var ci = 0; ci < conditions.length; ci++) {
      ...
      if (cond === 'technology_high_stress' || cond === 'structural_stress') addFamily('structural_event');
  ...
  console.log('[TechnologyPulse] Loaded');
})();
