/**
 * environment-claim-flow.js — Population Domain Claim Acceptance Flow
 *
 * Adds CLAIM button to Population opportunity cards. On click, opens
 ...
 * Namespace: window.LIMENPopulation.economy.claimFlow
 */
(function () {
  'use strict';
  ...
  function openClaimModal(opportunity) {
    ...
    var comp = window.LIMENPopulation.economy.compensation;
    ...
    h += '<div class="scf-title">Claim Population Opportunity</div>';
    ...
    h += '<div>\u2022 Operator is responsible for population integrity, data protection, and authorship ethics</div>';
    h += '</div></div>';

    h += '<label class="scf-accept">';
    h += '<input type="checkbox" id="scfAcceptCheck">';
    h += '<span class="scf-accept-text">I understand this is a Science-domain execution claim, I am responsible for truthful submission, documented progress, and compliance with applicable population integrity, IRB, and data protection rules.</span>';
    h += '</label>';
  ...
  window.addEventListener('limen:population-claim-update', function () { setTimeout(injectIntoCards, 100); });

  window.LIMENPopulation.economy.claimFlow = {
