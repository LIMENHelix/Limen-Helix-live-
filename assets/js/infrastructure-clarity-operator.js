      ...
      execBtns[eb].addEventListener('click', function (e) {
        ...
        try {
          ...
          sessionStorage.setItem('limen_exec_context', JSON.stringify({
            ...
            returnTo: '/domain-console?domain=infrastructure&mode=operator'
          }));
        } catch (ex) {}
        var trackName = { 'GRANT-ELIGIBLE': 'grant', 'PATENTABLE': 'patent', 'BUSINESS': 'business' }[execPath] || 'grant';
        window.location.href = 'infrastructure-workspace.html?track=' + trackName + '&opp=' + encodeURIComponent(execKey) + '&returnTo=' + encodeURIComponent('/domain-console?domain=infrastructure&mode=operator');
      });
    }
