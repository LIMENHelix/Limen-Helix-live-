/*
 * culture-telemetry-adapter.js  — wires runtime fields to the LIVE Culture telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/culture-pulse-engine.js  (window.LIMENCulturePulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Culture issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to culture.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into culture.json (a
...
(function (root) {
   ...
   * inputs: { culture, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (culture.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (culture.activations || []).forEach(function (a) {
  ...
  /* Merge culture.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(culture, overlay) {
    var base = culture.runtime || {};
    ...
    var acts = {}; (culture.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (culture.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the culture definition + brainState. */
  function fromLive(culture, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENCulturePulse) ? window.LIMENCulturePulse : null;
    if (!P) return null;
    return fromPulse({ culture: culture, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN culture definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/culture.json', 'culture'),
      _load('/assets/data/culture-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ culture: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.CultureTelemetryAdapter) root.CultureTelemetryAdapter = API;
