/**
 * culture-agent-box.js — the Culture domain's operator AI box (client side).
 *
 * A floating "Talk to Culture" panel on the culture domain-console. It KNOWS its
 * domain (window.LIMENCultureBrain.getStateSummary()) and can make bounded
 ...
 *    tries /api/culture-agent first and falls back to LOCAL on any failure, so it
...
(function () {
  ...
  function parseLocal(prompt, state) {
    ...
    if (mF) {
      var topic = mF[1].replace(/\b(please|now|today|this week|domain|culture|closely|carefully)\b/g, '').trim();
  ...
  if (typeof window !== 'undefined') window.__cultureAgentLocal = { parse: parseLocal, query: answerQuery };

  ready(function () {
    if (window.__cultureAgentBox) return; window.__cultureAgentBox = true;
    var LS_KEY = 'limenCultureAgentKey';
    var esc = function (s) { return String(s == null ? '' : s).replace(/[&<>"]/g, function (c) { return ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[c]; }); };
    function brain() { return window.LIMENCultureBrain || null; }
    ...
    var toggle = document.createElement('div'); toggle.id = 'eab-toggle'; toggle.textContent = '⚡ Talk to Culture';
    var panel = document.createElement('div'); panel.id = 'eab';
    panel.innerHTML =
      '<div class="eab-h"><span><b>Culture</b> · ask or steer</span><span class="eab-x">✕</span></div>' +
    ...
    function renderResult(pending, result, mode, left) {
      ...
      if (applied) html += '<div class="eab-tool">✓ applied to culture domain:\n' + esc(applied) + '</div>';
    ...
    function send() {
      var b = brain();
      if (!b) { add('<span class="eab-ai">Culture brain not loaded on this page.</span>'); return; }
    ...
    add('<span class="eab-ai">Culture domain online (free local mode). Ask about it, or steer it: "focus on grid", "prefer research", "autonomy off", "how is culture doing", "show the forecast". I bias the domain, never force a finding, move capital, or touch code. Add a passcode above for conversational AI once billing is on.</span>');
  });
})();
