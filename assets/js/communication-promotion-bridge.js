/**
 * communication-promotion-bridge.js — Inject promoted Communication portal directives
 * Exposes: window.LIMENCommunicationPromotionBridge
 */
(function () {
  'use strict';
  ...
  function _computeSignature(state) {
    ...
    return ['communication', dxKeys.join(','), nodeKeys.join(','), stressBucket.toFixed(2), window.LIMEN_ENABLE_DIRECTIVE_EXTRACTION ? '1' : '0'].join('|');
  }
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENCommunicationDirectiveExtractor;
    var ranker = window.LIMENCommunicationDirectiveRanker;
    var translator = window.LIMENCommunicationDirectiveTranslator;
    if (!extractor || !ranker || !translator) {
      console.warn('[CommunicationPromotionBridge] Missing pipeline component');
      _running = false;
    ...
    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        ...
        var targeting = window.LIMENCommunicationTargetingEngine;
        if (targeting) {
        ...
        console.log('[CommunicationPromotionBridge] Promoted ' + dedupedPromoted.length);
        return dedupedPromoted;
      })
      .catch(function (err) {
        console.error('[CommunicationPromotionBridge] Pipeline failed:', err.message || err);
        _lastSignature = sig; _lastPromoted = []; _running = false; return [];
      });
  }
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      state.treatments.push({
        id: 'ptreat_' + (dir.portalDomainId || 'communication') + '_' + (dir.nodeId || '') + '_' + pi,
  ...
  window.LIMENCommunicationPromotionBridge = { promote: promote, getLastPromoted: getLastPromoted };
})();
