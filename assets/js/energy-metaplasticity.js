/*
 * energy-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Energy's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (energy-offline-maintenance.js)
 *   - refractory absolute window       (energy-refractory-limiter.js)
 *   - prediction-error threshold       (energy-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Energy has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Energy has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.EnergyMetaplasticity) root.EnergyMetaplasticity = API;
