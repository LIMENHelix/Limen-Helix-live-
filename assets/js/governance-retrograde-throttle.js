/*
 * governance-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes governance.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Governance has no runtime load field). { nodeId: loadFraction }
  ...
  /* governance: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(governance, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (governance.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. governance.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Governance has no runtime load field). throttleGain operator-set. Never writes governance.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.GovernanceRetrogradeThrottle) root.GovernanceRetrogradeThrottle = API;
