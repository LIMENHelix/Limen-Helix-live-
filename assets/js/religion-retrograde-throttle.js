/*
 * religion-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes religion.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Religion has no runtime load field). { nodeId: loadFraction }
  ...
  /* religion: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(religion, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (religion.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. religion.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Religion has no runtime load field). throttleGain operator-set. Never writes religion.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.ReligionRetrogradeThrottle) root.ReligionRetrogradeThrottle = API;
