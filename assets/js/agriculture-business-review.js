/**
 * agriculture-business-review.js — Agriculture Business Assignment Review Surface
 *
 * AGRICULTURE DOMAIN ONLY. Renders a review panel for proposed business mappings.
 * Requires agriculture-node-business-engine.js to be loaded first.
 *
 * Provides APPROVE / DENY / HOLD buttons per proposal.
 * Only APPROVED mappings become active in the Agriculture operator surface.
 *
...
(function () {
  ...
  var PANEL_ID = 'eos-business-review';
  ...
  var _expandedCards = {};  // Track expanded card keys across re-renders
  var _delegationWired = false;  // Prevent duplicate event delegation
  ...
  function renderCard(entry) {
    ...
    // ── ALWAYS-VISIBLE SUMMARY ROW (clickable to toggle) ──
    ...
    // Neurology / Business translation
    if (entry.neuroTranslation) {
      ...
      h += '<div style="font-size:0.32rem;color:#b0a898;line-height:1.5"><b style="color:rgba(201,169,78,0.7)">In business (Agriculture systems):</b> ' + esc(entry.neuroTranslation.inBusiness) + '</div>';
      h += '</div>';
    }
    ...
    if (entry.existingCompanies && entry.existingCompanies.length > 0) {
      ...
      h += '<div class="ebr-card-existing" style="color:#C9A94E">Missing from current Agriculture system</div>';
    }
    ...
    // Review action bar — shown for any card with an approval record or in reviewable state
    if (entry.approvalRequired) {
      // Current review state label
  ...
  function render() {
    var engine = window.LIMENAgricultureBusinessEngine;
    if (!engine) return;
    ...
    h += '</div>'; // close collapsible body

    return h;
  }

  // ══════════════════════════════════════════════════════════════════════
  // MOUNT — into operator view or standalone panel
  // ══════════════════════════════════════════════════════════════════════
  ...
  function mount(container) {
    ...
    if (!panel) {
      ...
      // Panel was recreated — delegation must be re-bound
    ...
    // Wire BUSINESS buttons — navigate to business build workspace
      ...
      bizBtns[bi].addEventListener('click', function (e) {
        ...
        try {
          sessionStorage.setItem('limen_exec_context', JSON.stringify({
            ...
            source: 'agriculture', returnTo: '/domain-console?domain=agriculture&mode=operator'
          }));
        } catch (ex) {}
        window.location.href = 'agriculture-workspace.html?track=business&opp=' + encodeURIComponent(oppKey) + '&returnTo=' + encodeURIComponent('/domain-console?domain=agriculture&mode=operator');
      });
    }
    ...
    // Wire per-card collapsible toggle via event delegation (once only)
    if (!_delegationWired) {
  ...
  window.LIMENAgricultureBusinessReview = {
    render: render,
    mount: mount
  };

  console.log('[AgricultureBusinessReview] Loaded — review surface ready');
