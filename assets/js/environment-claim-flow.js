/**
 * environment-claim-flow.js — Environment Domain Claim Acceptance Flow
 *
 * Adds CLAIM button to Environment opportunity cards. On click, opens
 ...
 * Namespace: window.LIMENEnvironment.economy.claimFlow
 */
(function () {
  'use strict';
  ...
  function openClaimModal(opportunity) {
    ...
    h += '<div class="scf-title">Claim Environment Opportunity</div>';
    ...
    h += '<div>\u2022 Operator is responsible for environment integrity, data protection, and authorship ethics</div>';
    h += '</div></div>';

    h += '<label class="scf-accept">';
    h += '<input type="checkbox" id="scfAcceptCheck">';
    h += '<span class="scf-accept-text">I understand this is a Science-domain execution claim, I am responsible for truthful submission, documented progress, and compliance with applicable environment integrity, IRB, and data protection rules.</span>';
    h += '</label>';
  ...
  window.addEventListener('limen:environment-claim-update', function () { setTimeout(injectIntoCards, 100); });

  window.LIMENEnvironment.economy.claimFlow = {
