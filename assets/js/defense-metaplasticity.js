/*
 * defense-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Defense's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (defense-offline-maintenance.js)
 *   - refractory absolute window       (defense-refractory-limiter.js)
 *   - prediction-error threshold       (defense-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Defense has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Defense has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.DefenseMetaplasticity) root.DefenseMetaplasticity = API;
