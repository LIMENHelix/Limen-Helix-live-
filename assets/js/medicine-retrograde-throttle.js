/*
 * medicine-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes medicine.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Medicine has no runtime load field). { nodeId: loadFraction }
  ...
  /* medicine: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(medicine, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (medicine.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. medicine.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Medicine has no runtime load field). throttleGain operator-set. Never writes medicine.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.MedicineRetrogradeThrottle) root.MedicineRetrogradeThrottle = API;
