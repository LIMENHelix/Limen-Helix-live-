/*
 * technology-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes technology.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Technology has no runtime load field). { nodeId: loadFraction }
  ...
  /* technology: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(technology, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (technology.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. technology.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Technology has no runtime load field). throttleGain operator-set. Never writes technology.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.TechnologyRetrogradeThrottle) root.TechnologyRetrogradeThrottle = API;
