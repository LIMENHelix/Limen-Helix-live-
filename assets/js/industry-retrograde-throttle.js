/*
 * industry-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes industry.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Industry has no runtime load field). { nodeId: loadFraction }
  ...
  /* industry: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(industry, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (industry.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. industry.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Industry has no runtime load field). throttleGain operator-set. Never writes industry.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.IndustryRetrogradeThrottle) root.IndustryRetrogradeThrottle = API;
