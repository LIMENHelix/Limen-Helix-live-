...
(function () {
  ...
  TechnologyBrain.prototype.init = function () {
    ...
    //                          them (comm-through-coherence). (B) phase-transition credit is treated as
    //                          SELF-CONSISTENCY CALIBRATION (interpretive) — NOT a ground-truth/dopaminergic
    //                          reward. It is routed through the central honest gate (window.LIMENK4); the
    //                          P3/P7 family only gates the phase-consistency TIER, and technology (not
    //                          externalRewardEligible) can never emit reward — externalOutcome stays null.
  ...
  TechnologyBrain.prototype.cycle = function () {
    ...
    return _origCycle.call(this).then(function () {
      ...
      try { self._computeTechnologyOverlays(); } catch (e) {}   // NEURO-SUBSTRATE OVERLAY WIRING (per-domain, ported from energy 2026-07-21): invokes technology's OWN overlay modules each cycle; shadow, proposals only
    });
  };

  // ════════════════════════════════════════════════════════════════════════════
  // NEURO-SUBSTRATE OVERLAY WIRING — per-domain copy of energy's _computeEnergyOverlays
  // (2026-07-21, full per-domain independence). Invokes technology's OWN modules
  // (window.Technology{Metaplasticity,Extinction,RetrogradeThrottle,PredictionErrorCompressor,
  // OfflineMaintenance,NeuroSubstrate,ConnectivityAudit}) against technology's OWN runtime def
  // (technology.json runtime.params + edges/issues/activations). SHADOW: writes state.technologyOverlays;
  ...
  // technology console), 'loading' until technology.json arrives.
  // ════════════════════════════════════════════════════════════════════════════
  TechnologyBrain.prototype._loadTechnologyDef = function () {
    if (this._technologyDef) return this._technologyDef;
    if (this._technologyDefLoading) return null;
    this._technologyDefLoading = true;
    var self = this;
    if (typeof fetch === 'function') {
      try {
        fetch('/assets/data/domains/technology.json').then(function (r) { return r.json(); })
          .then(function (def) { self._technologyDef = def; })
          .catch(function () { self._technologyDefLoading = false; });
      } catch (e) { this._technologyDefLoading = false; }
    }
    return null;
  };

  TechnologyBrain.prototype._computeTechnologyOverlays = function () {
    var s = this.state;
    ...
    var META = mod('TechnologyMetaplasticity', '../technology-metaplasticity.js');
    var EXT = mod('TechnologyExtinction', '../technology-extinction.js');
    var RETRO = mod('TechnologyRetrogradeThrottle', '../technology-retrograde-throttle.js');
    var PEC = mod('TechnologyPredictionErrorCompressor', '../technology-prediction-error-compressor.js');
    var OFF = mod('TechnologyOfflineMaintenance', '../technology-offline-maintenance.js');
    var NS = mod('TechnologyNeuroSubstrate', '../technology-neuro-substrate.js');
    var CONN = mod('TechnologyConnectivityAudit', '../technology-connectivity-audit.js');
    if (!(META && EXT && RETRO && PEC && OFF && NS && CONN)) {
      s.technologyOverlays = { version: 1, mode: 'off', note: 'overlay modules not loaded on this page (present only on the technology console)' };
      return null;
    }
    var def = this._loadTechnologyDef();
    if (!def) { s.technologyOverlays = { version: 1, mode: 'loading', note: 'technology.json runtime def loading; overlays compute next cycle' }; return null; }
    ...
    var intero = s.technologyInteroception || (s.cognition && s.cognition.interoception) || {};
    ...
    s.technologyOverlays = {
      version: 1, mode: 'shadow', armed: armed,
      volatility: Math.round(volatility * 1000) / 1000,
      metaplasticity: { changes: meta.changes, adapted: adapted, noop: meta.noop },
      extinction: { candidates: ext.candidates, count: ext.candidates.length, noop: ext.noop, actuation: 'PROPOSAL-ONLY (retiring a node edits technology.json — human-gated forever)' },
    ...
    s.technologyOverlays.mode = armed ? 'armed' : 'shadow';
    s.technologyOverlays.applied = applied;
    s.technologyOverlays.actuationScope = 'metaplasticity->refractory dead-time ONLY (bounded, fail-toward-quiet, reversible). throttle=no-live-consumer; PE=observe-only; extinction+offline-prune=PROPOSAL-ONLY (remove structure, human-gated forever).';
    s.technologyOverlays.note = 'PER-DOMAIN OVERLAY WIRING (ported from energy): technology runs its OWN 6 overlay modules + connectivity recurrenceAudit each cycle on technology.json. ARMED: metaplasticity raises the refractory dead-time with volatility (clamped, reversible). Everything else is proposal/observe-only.';

    if (s.cognition && typeof s.cognition === 'object') s.cognition.overlays = s.technologyOverlays;
    return s.technologyOverlays;
  };
  ...
  (function () {
    ...
    P._updateTechnologyModel = function () {
      ...
      // Technology is NOT externalRewardEligible: it has no external realized-outcome label, so
      // externalOutcome is ALWAYS null and its credit is SELF-CONSISTENCY CALIBRATION (interpretive),
      // NEVER reward. The gate's preemption order (external-reward > phase-consistency > call-consistency
      // > stress-consistency > none) picks which self-consistency signal teaches K4; here only tiers 3/2/1
      // are ever reachable. A realized thing2 phase transition is interpretive self-consistency, NOT a
      // ground-truth/dopaminergic reward. Reads the PRIOR cycle's phase dynamics + outcome model
      // (one-cycle lag, recurrent). Phase contribution reversible via _actuation.phase. If window.LIMENK4
      // is absent we fall back to the prior phase-only credit behavior (unchanged, never throws).
      var _lr = cm.plasticity.learningRate;
      var _pt = (this.state.technologyPhaseDynamics || {}).transition;
      var _om = this.state.technologyOutcomeModel;   // stress self-prediction (predicted-vs-realized)
      var _led = this.state.technologyOutcomeLedger; // resolved-call TRUTH BRAKE ledger (if this brain keeps one)
      var _phaseOn = !!(this._actuation && this._actuation.phase);
      var _k4source, _k4reward = false;
      if (typeof window !== 'undefined' && window.LIMENK4 && typeof window.LIMENK4.credit === 'function') {
        // Build the honest signal from what the brain already computes. externalOutcome MUST stay null
        // (not externalRewardEligible) — self-consistency calibration only.
        var _sig = {
          externalOutcome: null,
          phaseValidated: !!(_phaseOn && _pt && _pt.validated),
          phaseTransitionHit: (_phaseOn && _pt && _pt.hit != null) ? (_pt.hit ? 1 : 0) : null,
          callHitRate: (_led && typeof _led.callHitRate === 'number') ? _led.callHitRate : undefined,
          callSamples: (_led && typeof _led.resolvedSamples === 'number') ? _led.resolvedSamples : 0,
          stressSelfPred: (_om && typeof _om.hitRate === 'number') ? _om.hitRate : undefined,
          stressSamples: (_om && typeof _om.samples === 'number') ? _om.samples : 0
        };
        var _k4 = window.LIMENK4.credit(_sig);
        if (_k4 && _k4.credit != null) { _lr = _tmClamp(_lr * (1 + (1 - _k4.credit)), TM_SLOW_RATE, 0.6); }
        _k4source = _k4 ? _k4.creditSource : 'none';
        _k4reward = !!(_k4 && _k4.isReward);   // always false for technology (not externalRewardEligible)
      } else {
        // FALLBACK (central gate absent): preserve the prior phase-only self-consistency credit behavior.
        var _phaseReward = !!(_phaseOn && _pt && _pt.validated && _pt.hit !== null);
        if (_phaseReward) { var _hit = _pt.hit ? 1 : 0; _lr = _tmClamp(_lr * (1 + (1 - _hit)), TM_SLOW_RATE, 0.6); }
        _k4source = _phaseReward ? 'phase-consistency' : 'fixed-plasticity';
        _k4reward = false;
      }
      cm._creditSource = _k4source;
      cm._creditIsReward = _k4reward;   // self-consistency calibration is NEVER reward for this domain
      cm._externalRewardEligible = (typeof window !== 'undefined' && window.LIMENK4 &&
        typeof window.LIMENK4.externalRewardEligible === 'function') ? window.LIMENK4.externalRewardEligible('technology') : false;
    ...
    // PHASE-COHERENCE ROUTER + PHASE-TRANSITION SELF-CONSISTENCY (patent Section 3.4 Loop-1 M matrix; thing2
    // lineage). (A) couples to co-phased, stressed peer domains; (B) scores a realized phase transition as
    // SELF-CONSISTENCY CALIBRATION (interpretive) — the P3/P7 family only gates the phase-consistency TIER
    // of the central K4 gate; it is NEVER external/ground-truth reward. Deterministic; no AI; no writes to technology.json.
    ...
    // and the coherence router + phase-transition self-consistency use the existing naive/static PHASE_M phase.
    ...
    P._computeTechnologyPhaseDynamics = function () {
      ...
      // myPhase drives BOTH (A) the coherence router and (B) the phase-transition self-consistency below.
      ...
      //     Interpretive calibration, NOT external reward; consumed by the central K4 gate one cycle later.
      ...
      if (prev != null && myPhase != null && prev !== myPhase) {
        ...
        reward = { from: prev, to: myPhase, predictedUp: !!predictedUp, wentUp: wentUp, hit: hit,
          validated: validated, kind: validated ? 'self-consistency calibration (P3/P7 phase, interpretive)' : 'self-consistency calibration (interpretive)' };
      }
      ...
      var out = {
        ...
        transition: reward,
        note: 'phase source = ' + _phaseSource + ' (thing2 recursive kernel preferred, naive/static PHASE_M fallback); phase-coherence router (patent M matrix) + phase-transition self-consistency calibration (thing2 lineage; interpretive, NOT external reward — P3/P7 only gates the phase-consistency tier).'
      };
    ...
    // NOT an external reward — technology has no external ground-truth outcome label (ENERGY_NEURO_AUDIT
    // impossibility #4; not externalRewardEligible). This is the stress-consistency TIER fed to the central
    // K4 gate in _updateTechnologyModel; the P3/P7 phase transition is a higher self-consistency tier, also
    // interpretive — neither is a validated/dopaminergic reward. This never fabricates a reward.
    P._scoreTechnologyOutcomes = function () {
      ...
      var om = {
        ...
        note: 'ADVISORY (self-consistency calibration, interpretive): measures whether technology\'s own stress forecast comes true. This is the stress-consistency tier; the P3/P7 phase transition is a higher self-consistency tier — neither is external reward. Never fabricates a dopaminergic reward.',
  ...
  // ── TECHNOLOGY-LOCAL FIVE-ORGAN EXTENSION ─────────────────────────────
  // The base schedules these organs; Technology owns their implementation, state and
  // authority here. Thing 2 is not consumed and external action remains inhibited.
  TechnologyBrain.prototype._refreshTechnologyActionOutcome = function () {
    if (typeof fetch !== 'function') return null;
    var cycle = this._cycleCount || 0;
    if (this._lastTechnologyActionOutcomeCycle && cycle - this._lastTechnologyActionOutcomeCycle < 12) return this._technologyActionOutcome || null;
    this._lastTechnologyActionOutcomeCycle = cycle;
    var self = this;
    try {
      fetch('/api/product-domain-learning-state?domain=' + encodeURIComponent(this.domainId))
        ...
        .then(function (result) {
          ...
          self._technologyActionOutcome = result;
          self.state[self.domainId + 'ActionLearning'] = result;
          self.state.domainActionLearning = result;
        }).catch(function () {});
    } catch (e) {}
    return this._technologyActionOutcome || null;
  };

  TechnologyBrain.prototype._computeTechnologyPlasticity = function () {
    var s = this.state;
    ...
    if (!pl || !pl.P) {
      s.technologyPlasticity = s.domainPlasticity = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'plasticity primitive unavailable' };
      return null;
    }
    try { this._refreshDomainExternalOutcome(); } catch (e) {}
    try { this._refreshTechnologyActionOutcome(); } catch (e) {}
    ...
    var actionOutcome = this._technologyActionOutcome;
    ...
    var out = {
      ...
      note: 'Technology-local three-factor plasticity; outcome-gated and fail-toward-seed.'
    };
    s.technologyPlasticity = s.domainPlasticity = out;
  ...
  TechnologyBrain.prototype._computeTechnologyEmissionQueue = function () {
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, maxConcurrent: 3,
      ...
      packages: packages, lastQueueAt: Date.now(), note: 'Technology-local queue; research internal-only, capital staged.' };
    s.technologyEmissionQueue = s.domainEmissionQueue = out;
    return out;
  };

  TechnologyBrain.prototype._runTechnologyAutonomousEmission = function () {
    ...
    var emitted = [], staged = [], queue = (s.technologyEmissionQueue && s.technologyEmissionQueue.packages) || [];
    ...
    if (emitted.length) {
      var stream = s.technologyEmitted = s.technologyEmitted || [];
      emitted.forEach(function (p) { stream.push({ at: Date.now(), title: p.title, lane: p.lane, forecast: p.forecast }); });
      if (stream.length > 50) s.technologyEmitted = stream.slice(-50);
    }
    var out = { version: 2, domain: this.domainId, localOwner: true, autonomy: globalOn && localOn,
      ...
      externalEffects: 0, lastEmissionAt: Date.now(), note: hold ? 'held fail-safe' : 'Technology-local internal research emission; capital staged.' };
    s.technologyAutoEmission = s.domainAutoEmission = out;
    return out;
  };

  TechnologyBrain.prototype._computeTechnologyInteroception = function () {
    ...
    var channels = [{ name: 'domain-stress', alarm: r(clamp(s.stress || 0)), confidence: 0.9, weight: 1, provenance: 'Technology structured stress' }];
    var pe = model.predictionError && typeof model.predictionError === 'object' ? model.predictionError.total : model.predictionError;
    if (typeof pe === 'number') channels.push({ name: 'prediction', alarm: r(clamp(pe)), confidence: 0.7, weight: 0.85, provenance: 'Technology prediction error' });
    var ledger = n.outcomeLedger || {};
    if (typeof ledger.hitRate === 'number' && ledger.samples >= 3) channels.push({ name: 'metacognitive', alarm: r(clamp(1 - ledger.hitRate)), confidence: ledger.samples >= 5 ? 0.7 : 0.4, weight: 0.75, provenance: 'Technology outcomes' });
    var immune = cog.immune || {}, ia = { clear: 0, watch: 0.34, active: 0.4, alert: 1 };
    if (immune.immuneState) channels.push({ name: 'immune', alarm: ia[immune.immuneState] == null ? 0.34 : ia[immune.immuneState], confidence: 0.6, weight: 0.6, provenance: 'Technology immune state' });
    var forecast = n.forecast;
    if (forecast && typeof forecast.projectedStress === 'number') channels.push({ name: 'allostatic', alarm: r(clamp((forecast.projectedStress - (s.stress || 0)) * 3)), confidence: typeof forecast.confidence === 'number' ? clamp(forecast.confidence) : 0.4, weight: 0.6, provenance: 'Technology forecast' });
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, method: 'domain-local-weighted-divergence',
      ...
      attend: salience === 'blind-channel' ? loudest.name : null, note: 'Technology-local interoception; observe-only.' };
    s.technologyInteroception = s.domainInteroception = s.interoception = out;
    cog.interoception = out;
    return out;
  };

  TechnologyBrain.prototype._computeTechnologyActiveInference = function () {
    var s = this.state, cog = s.cognition;
    if (!cog) return null;
    ...
    if (!box || !box.A) {
      s.technologyActiveInference = s.domainActiveInference = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'active-inference primitive unavailable' };
      return null;
    }
    ...
    var queued = (s.technologyEmissionQueue && s.technologyEmissionQueue.packages || []).length;
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, mode: 'shadow',
      ...
      note: 'Technology-local belief update and expected-free-energy advisory; no live consumer.' };
    s.technologyActiveInference = s.domainActiveInference = out;
    cog.activeInference = out;
    return out;
  };

  TechnologyBrain.prototype._computeDomainPlasticity = TechnologyBrain.prototype._computeTechnologyPlasticity;
  TechnologyBrain.prototype._computeGenericEmissionQueue = TechnologyBrain.prototype._computeTechnologyEmissionQueue;
  TechnologyBrain.prototype._runGenericAutonomousEmission = TechnologyBrain.prototype._runTechnologyAutonomousEmission;
  TechnologyBrain.prototype._computeGenericInteroception = TechnologyBrain.prototype._computeTechnologyInteroception;
  TechnologyBrain.prototype._computeGenericActiveInference = TechnologyBrain.prototype._computeTechnologyActiveInference;
