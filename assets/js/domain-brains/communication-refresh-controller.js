/**
 * communication-refresh-controller.js — Communication Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENCommunicationFresh (shared state).
 * Dispatches 'limen:communication-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'communication';

  window.LIMENCommunicationFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENCommunicationFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:communication-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:communication-refresh', true, true, F);
  ...
  window.LIMENCommunicationRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
