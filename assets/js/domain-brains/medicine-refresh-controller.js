/**
 * medicine-refresh-controller.js — Medicine Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENHealthFresh (shared state).
 * Dispatches 'limen:medicine-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'health';

  window.LIMENHealthFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:medicine-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:medicine-refresh', true, true, F);
  ...
  window.LIMENHealthRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
