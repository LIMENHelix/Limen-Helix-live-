/**
 * science-refresh-controller.js — Science Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENResearchFresh (shared state).
 * Dispatches 'limen:science-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'research';

  window.LIMENResearchFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENResearchFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:science-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:science-refresh', true, true, F);
  ...
  window.LIMENResearchRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
