/**
 * communication-cortex-retrieval.js — PHASE K4 safe retrieval layer (read-only, inert library).
 * Communication mirror of energy-cortex-retrieval.js.
 *
 ...
 *
 * Node + browser: window.LIMENCommunicationCortexRetrieval and module.exports both expose { retrieve, classifyQuery }.
 */
(function () {
  'use strict';

  var DX_KEYWORDS = {
    NETWORK_OUTAGE: ['outage','downtime','network failure','blackout','disruption'],
    SPECTRUM_CRUNCH: ['spectrum','bandwidth','congestion','capacity','5g'],
    MISINFORMATION_SURGE: ['misinformation','disinformation','fake news','propaganda','viral false'],
    CENSORSHIP_EVENT: ['censorship','shutdown','block','throttl','speech'],
    FISCAL_COMMUNICATION: ['deficit','debt ceiling','budget','appropriation','cbo','gao'],
    CONNECTIVITY_GAP: ['digital divide','access','broadband','underserved','coverage']
  };
  ...
  function retrieve(query, options, index) {
    ...
    var idx = index || (typeof window !== 'undefined' && window.LIMENCommunicationCertifiedCortex) || null;
    var warnings = [];
    ...
    if (!dx) warnings.push('no diagnosis matched the query — narrow it (outage / spectrum / misinformation / censorship / media-trust / connectivity)');
  ...
  if (typeof window !== 'undefined') window.LIMENCommunicationCortexRetrieval = api;
