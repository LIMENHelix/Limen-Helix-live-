/**
 * religion-refresh-controller.js — Religion Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENReligionFresh (shared state).
 * Dispatches 'limen:religion-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'religion';

  window.LIMENReligionFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENReligionFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:religion-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:religion-refresh', true, true, F);
  ...
  window.LIMENReligionRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
