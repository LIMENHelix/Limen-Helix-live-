/**
 * intelligence-refresh-controller.js — Intelligence Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENIntelligenceFresh (shared state).
 * Dispatches 'limen:intelligence-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'intelligence';

  window.LIMENIntelligenceFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENIntelligenceFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:intelligence-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:intelligence-refresh', true, true, F);
  ...
  window.LIMENIntelligenceRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
