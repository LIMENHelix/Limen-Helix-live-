...
(function () {
  ...
  // ── ECONOMY-LOCAL FIVE-ORGAN EXTENSION ─────────────────────────────
  // The base schedules these organs; Economy owns their implementation, state and
  // authority here. Thing 2 is not consumed and external action remains inhibited.
  EconomyBrain.prototype._refreshEconomyActionOutcome = function () {
    if (typeof fetch !== 'function') return null;
    var cycle = this._cycleCount || 0;
    if (this._lastEconomyActionOutcomeCycle && cycle - this._lastEconomyActionOutcomeCycle < 12) return this._economyActionOutcome || null;
    this._lastEconomyActionOutcomeCycle = cycle;
    var self = this;
    try {
      fetch('/api/product-domain-learning-state?domain=' + encodeURIComponent(this.domainId))
        ...
        .then(function (result) {
          ...
          self._economyActionOutcome = result;
          self.state[self.domainId + 'ActionLearning'] = result;
          self.state.domainActionLearning = result;
        }).catch(function () {});
    } catch (e) {}
    return this._economyActionOutcome || null;
  };

  EconomyBrain.prototype._computeEconomyPlasticity = function () {
    var s = this.state;
    ...
    if (!pl || !pl.P) {
      s.economyPlasticity = s.domainPlasticity = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'plasticity primitive unavailable' };
      return null;
    }
    try { this._refreshDomainExternalOutcome(); } catch (e) {}
    try { this._refreshEconomyActionOutcome(); } catch (e) {}
    ...
    var actionOutcome = this._economyActionOutcome;
    ...
    var out = {
      ...
      note: 'Economy-local three-factor plasticity; outcome-gated and fail-toward-seed.'
    };
    s.economyPlasticity = s.domainPlasticity = out;
  ...
  EconomyBrain.prototype._computeEconomyEmissionQueue = function () {
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, maxConcurrent: 3,
      ...
      packages: packages, lastQueueAt: Date.now(), note: 'Economy-local queue; research internal-only, capital staged.' };
    s.economyEmissionQueue = s.domainEmissionQueue = out;
    return out;
  };

  EconomyBrain.prototype._runEconomyAutonomousEmission = function () {
    ...
    var emitted = [], staged = [], queue = (s.economyEmissionQueue && s.economyEmissionQueue.packages) || [];
    ...
    if (emitted.length) {
      var stream = s.economyEmitted = s.economyEmitted || [];
      emitted.forEach(function (p) { stream.push({ at: Date.now(), title: p.title, lane: p.lane, forecast: p.forecast }); });
      if (stream.length > 50) s.economyEmitted = stream.slice(-50);
    }
    var out = { version: 2, domain: this.domainId, localOwner: true, autonomy: globalOn && localOn,
      ...
      externalEffects: 0, lastEmissionAt: Date.now(), note: hold ? 'held fail-safe' : 'Economy-local internal research emission; capital staged.' };
    s.economyAutoEmission = s.domainAutoEmission = out;
    return out;
  };

  EconomyBrain.prototype._computeEconomyInteroception = function () {
    ...
    var channels = [{ name: 'domain-stress', alarm: r(clamp(s.stress || 0)), confidence: 0.9, weight: 1, provenance: 'Economy structured stress' }];
    var pe = model.predictionError && typeof model.predictionError === 'object' ? model.predictionError.total : model.predictionError;
    if (typeof pe === 'number') channels.push({ name: 'prediction', alarm: r(clamp(pe)), confidence: 0.7, weight: 0.85, provenance: 'Economy prediction error' });
    var ledger = n.outcomeLedger || {};
    if (typeof ledger.hitRate === 'number' && ledger.samples >= 3) channels.push({ name: 'metacognitive', alarm: r(clamp(1 - ledger.hitRate)), confidence: ledger.samples >= 5 ? 0.7 : 0.4, weight: 0.75, provenance: 'Economy outcomes' });
    var immune = cog.immune || {}, ia = { clear: 0, watch: 0.34, active: 0.4, alert: 1 };
    if (immune.immuneState) channels.push({ name: 'immune', alarm: ia[immune.immuneState] == null ? 0.34 : ia[immune.immuneState], confidence: 0.6, weight: 0.6, provenance: 'Economy immune state' });
    var forecast = n.forecast;
    if (forecast && typeof forecast.projectedStress === 'number') channels.push({ name: 'allostatic', alarm: r(clamp((forecast.projectedStress - (s.stress || 0)) * 3)), confidence: typeof forecast.confidence === 'number' ? clamp(forecast.confidence) : 0.4, weight: 0.6, provenance: 'Economy forecast' });
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, method: 'domain-local-weighted-divergence',
      ...
      attend: salience === 'blind-channel' ? loudest.name : null, note: 'Economy-local interoception; observe-only.' };
    s.economyInteroception = s.domainInteroception = s.interoception = out;
    cog.interoception = out;
    return out;
  };

  EconomyBrain.prototype._computeEconomyActiveInference = function () {
    var s = this.state, cog = s.cognition;
    if (!cog) return null;
    ...
    if (!box || !box.A) {
      s.economyActiveInference = s.domainActiveInference = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'active-inference primitive unavailable' };
      return null;
    }
    ...
    var queued = (s.economyEmissionQueue && s.economyEmissionQueue.packages || []).length;
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, mode: 'shadow',
      ...
      note: 'Economy-local belief update and expected-free-energy advisory; no live consumer.' };
    s.economyActiveInference = s.domainActiveInference = out;
    cog.activeInference = out;
    return out;
  };

  EconomyBrain.prototype._computeDomainPlasticity = EconomyBrain.prototype._computeEconomyPlasticity;
  EconomyBrain.prototype._computeGenericEmissionQueue = EconomyBrain.prototype._computeEconomyEmissionQueue;
  EconomyBrain.prototype._runGenericAutonomousEmission = EconomyBrain.prototype._runEconomyAutonomousEmission;
  EconomyBrain.prototype._computeGenericInteroception = EconomyBrain.prototype._computeEconomyInteroception;
  EconomyBrain.prototype._computeGenericActiveInference = EconomyBrain.prototype._computeEconomyActiveInference;
