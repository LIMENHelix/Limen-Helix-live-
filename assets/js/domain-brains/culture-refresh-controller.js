/**
 * culture-refresh-controller.js — Culture Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENCultureFresh (shared state).
 * Dispatches 'limen:culture-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'culture';

  window.LIMENCultureFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENCultureFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:culture-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:culture-refresh', true, true, F);
  ...
  window.LIMENCultureRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
