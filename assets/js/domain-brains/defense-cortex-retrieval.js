/**
 * defense-cortex-retrieval.js — PHASE K4 safe retrieval layer (read-only, inert library).
 * Defense mirror of energy-cortex-retrieval.js.
 *
 ...
 *
 * Node + browser: window.LIMENDefenseCortexRetrieval and module.exports both expose { retrieve, classifyQuery }.
 */
(function () {
  'use strict';

  var DX_KEYWORDS = {
    FORCE_READINESS: ['readiness','mobiliz','deploy','posture','sortie'],
    PROCUREMENT_CYCLE: ['procurement','contract','acquisition','program','budget'],
    CONFLICT_ESCALATION: ['conflict','escalation','hostilit','war','strike'],
    DETERRENCE_FAILURE: ['deterrence','provocation','incursion','gray zone'],
    INDUSTRIAL_BASE_STRAIN: ['industrial base','munitions','depletion','supply','shortfall'],
    ALLIANCE_STRESS: ['alliance','nato','basing','burden','coalition']
  };
  ...
  function retrieve(query, options, index) {
    ...
    var idx = index || (typeof window !== 'undefined' && window.LIMENDefenseCertifiedCortex) || null;
    var warnings = [];
    ...
    if (!dx) warnings.push('no diagnosis matched the query — narrow it (readiness / procurement / conflict / deterrence / industrial-base / alliance)');
  ...
  if (typeof window !== 'undefined') window.LIMENDefenseCortexRetrieval = api;
