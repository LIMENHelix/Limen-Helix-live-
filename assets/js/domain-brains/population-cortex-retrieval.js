/**
 * population-cortex-retrieval.js — PHASE K4 safe retrieval layer (read-only, inert library).
 * Population mirror of energy-cortex-retrieval.js.
 *
 ...
 *
 * Node + browser: window.LIMENPopulationCortexRetrieval and module.exports both expose { retrieve, classifyQuery }.
 */
(function () {
  'use strict';

  var DX_KEYWORDS = {
    DEMOGRAPHIC_COLLAPSE: ['depopulation','decline','shrinking','collapse','census'],
    MIGRATION_SURGE: ['migration','immigration','refugee','border','displacement'],
    AGING_CRISIS: ['aging','elderly','retire','dependency ratio','boomer'],
    FERTILITY_DECLINE: ['fertility','birth rate','replacement','baby bust'],
    FISCAL_POPULATION: ['deficit','debt ceiling','budget','appropriation','cbo','gao'],
    URBANIZATION_STRAIN: ['urban','housing','density','sprawl','settlement']
  };
  ...
  function retrieve(query, options, index) {
    ...
    var idx = index || (typeof window !== 'undefined' && window.LIMENPopulationCertifiedCortex) || null;
    var warnings = [];
    ...
    if (!dx) warnings.push('no diagnosis matched the query — narrow it (depopulation / migration / aging / fertility / labor / urbanization)');
  ...
  if (typeof window !== 'undefined') window.LIMENPopulationCortexRetrieval = api;
