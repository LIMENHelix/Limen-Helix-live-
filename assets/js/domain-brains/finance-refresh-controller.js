/**
 * finance-refresh-controller.js — Finance Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENFinanceFresh (shared state).
 * Dispatches 'limen:finance-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'finance';

  window.LIMENFinanceFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENFinanceFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:finance-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:finance-refresh', true, true, F);
  ...
  window.LIMENFinanceRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
