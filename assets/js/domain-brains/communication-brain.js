...
(function () {
  ...
  function CommunicationBrain() {
    ...
    this.resourceAuthority = { ownerDomain: 'communication', policyId: 'communication-resource/1', sandboxLane: 'social', lanes: ['publication', 'social', 'email'], budgets: { computeUnitsPerCycle: 512, queueCapacity: 64 }, switches: { internalCycle: true, internalEmission: true, externalAction: true, spend: false, capital: false } };
    // External eligibility is configured here, but it remains inert until the
    // short-lived production executor and independent AppView capability pair
    // is restored into this exact brain by the server-side overlay.
    this.motorAuthority = { ownerDomain: 'communication', contractId: 'communication-motor/1', externalValveId: 'communication:social', lane: 'social', decisionContract: 'public-message-decision/1', budgetId: 'communication-social-budget/1', receiptClass: 'platform-post-receipt', outcomeClass: 'engagement-or-conversion', rollbackClass: 'delete-or-correct', executorVerified: false, outcomeObserverVerified: false, switches: { prepare: true, simulate: true, external: true } };
  }
  ...
  // ── COMMUNICATION-LOCAL FIVE-ORGAN EXTENSION ─────────────────────────────
  // The base schedules these organs; Communication owns their implementation, state and
  // authority here. Thing 2 is not consumed and external action remains inhibited.
  CommunicationBrain.prototype._refreshCommunicationActionOutcome = function () {
    if (typeof fetch !== 'function') return null;
    var cycle = this._cycleCount || 0;
    if (this._lastCommunicationActionOutcomeCycle && cycle - this._lastCommunicationActionOutcomeCycle < 12) return this._communicationActionOutcome || null;
    this._lastCommunicationActionOutcomeCycle = cycle;
    var self = this;
    try {
      fetch('/api/product-domain-learning-state?domain=' + encodeURIComponent(this.domainId))
        ...
        .then(function (result) {
          ...
          self._communicationActionOutcome = result;
          self.state[self.domainId + 'ActionLearning'] = result;
          self.state.domainActionLearning = result;
        }).catch(function () {});
    } catch (e) {}
    return this._communicationActionOutcome || null;
  };

  CommunicationBrain.prototype._computeCommunicationPlasticity = function () {
    var s = this.state;
    ...
    if (!pl || !pl.P) {
      s.communicationPlasticity = s.domainPlasticity = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'plasticity primitive unavailable' };
      return null;
    }
    try { this._refreshDomainExternalOutcome(); } catch (e) {}
    try { this._refreshCommunicationActionOutcome(); } catch (e) {}
    ...
    var actionOutcome = this._communicationActionOutcome;
    ...
    var out = {
      ...
      note: 'Communication-local three-factor plasticity; outcome-gated and fail-toward-seed.'
    };
    s.communicationPlasticity = s.domainPlasticity = out;
  ...
  CommunicationBrain.prototype._computeCommunicationEmissionQueue = function () {
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, maxConcurrent: 3,
      ...
      packages: packages, lastQueueAt: Date.now(), note: 'Communication-local queue; research internal-only, capital staged.' };
    s.communicationEmissionQueue = s.domainEmissionQueue = out;
    return out;
  };

  CommunicationBrain.prototype._runCommunicationAutonomousEmission = function () {
    ...
    var emitted = [], staged = [], queue = (s.communicationEmissionQueue && s.communicationEmissionQueue.packages) || [];
    ...
    if (emitted.length) {
      var stream = s.communicationEmitted = s.communicationEmitted || [];
      emitted.forEach(function (p) { stream.push({ at: Date.now(), title: p.title, lane: p.lane, forecast: p.forecast }); });
      if (stream.length > 50) s.communicationEmitted = stream.slice(-50);
    }
    var out = { version: 2, domain: this.domainId, localOwner: true, autonomy: globalOn && localOn,
      ...
      externalEffects: 0, lastEmissionAt: Date.now(), note: hold ? 'held fail-safe' : 'Communication-local internal research emission; capital staged.' };
    s.communicationAutoEmission = s.domainAutoEmission = out;
    return out;
  };

  CommunicationBrain.prototype._computeCommunicationInteroception = function () {
    ...
    var channels = [{ name: 'domain-stress', alarm: r(clamp(s.stress || 0)), confidence: 0.9, weight: 1, provenance: 'Communication structured stress' }];
    var pe = model.predictionError && typeof model.predictionError === 'object' ? model.predictionError.total : model.predictionError;
    if (typeof pe === 'number') channels.push({ name: 'prediction', alarm: r(clamp(pe)), confidence: 0.7, weight: 0.85, provenance: 'Communication prediction error' });
    var ledger = n.outcomeLedger || {};
    if (typeof ledger.hitRate === 'number' && ledger.samples >= 3) channels.push({ name: 'metacognitive', alarm: r(clamp(1 - ledger.hitRate)), confidence: ledger.samples >= 5 ? 0.7 : 0.4, weight: 0.75, provenance: 'Communication outcomes' });
    var immune = cog.immune || {}, ia = { clear: 0, watch: 0.34, active: 0.4, alert: 1 };
    if (immune.immuneState) channels.push({ name: 'immune', alarm: ia[immune.immuneState] == null ? 0.34 : ia[immune.immuneState], confidence: 0.6, weight: 0.6, provenance: 'Communication immune state' });
    var forecast = n.forecast;
    if (forecast && typeof forecast.projectedStress === 'number') channels.push({ name: 'allostatic', alarm: r(clamp((forecast.projectedStress - (s.stress || 0)) * 3)), confidence: typeof forecast.confidence === 'number' ? clamp(forecast.confidence) : 0.4, weight: 0.6, provenance: 'Communication forecast' });
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, method: 'domain-local-weighted-divergence',
      ...
      attend: salience === 'blind-channel' ? loudest.name : null, note: 'Communication-local interoception; observe-only.' };
    s.communicationInteroception = s.domainInteroception = s.interoception = out;
    cog.interoception = out;
    return out;
  };

  CommunicationBrain.prototype._computeCommunicationActiveInference = function () {
    var s = this.state, cog = s.cognition;
    if (!cog) return null;
    ...
    if (!box || !box.A) {
      s.communicationActiveInference = s.domainActiveInference = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'active-inference primitive unavailable' };
      return null;
    }
    ...
    var queued = (s.communicationEmissionQueue && s.communicationEmissionQueue.packages || []).length;
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, mode: 'shadow',
      ...
      note: 'Communication-local belief update and expected-free-energy advisory; no live consumer.' };
    s.communicationActiveInference = s.domainActiveInference = out;
    cog.activeInference = out;
    return out;
  };

  CommunicationBrain.prototype._computeDomainPlasticity = CommunicationBrain.prototype._computeCommunicationPlasticity;
  CommunicationBrain.prototype._computeGenericEmissionQueue = CommunicationBrain.prototype._computeCommunicationEmissionQueue;
  CommunicationBrain.prototype._runGenericAutonomousEmission = CommunicationBrain.prototype._runCommunicationAutonomousEmission;
  CommunicationBrain.prototype._computeGenericInteroception = CommunicationBrain.prototype._computeCommunicationInteroception;
  CommunicationBrain.prototype._computeGenericActiveInference = CommunicationBrain.prototype._computeCommunicationActiveInference;
