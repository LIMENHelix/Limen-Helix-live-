/**
 * industry-refresh-controller.js — Industry Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENIndustryFresh (shared state).
 * Dispatches 'limen:industry-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'industry';

  window.LIMENIndustryFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENIndustryFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:industry-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:industry-refresh', true, true, F);
  ...
  window.LIMENIndustryRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
