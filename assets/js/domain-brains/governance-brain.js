...
(function () {
  ...
  GovernanceBrain.prototype.cycle = function () {
    ...
    return _origCycle.call(this).then(function () {
      ...
      try { self._computeGovernanceOverlays(); } catch (e) {}   // NEURO-SUBSTRATE OVERLAY WIRING (per-domain, ported from energy 2026-07-21): invokes governance's OWN overlay modules each cycle; shadow, proposals only
    });
  };

  // ════════════════════════════════════════════════════════════════════════════
  // NEURO-SUBSTRATE OVERLAY WIRING — per-domain copy of energy's _computeEnergyOverlays
  // (2026-07-21, full per-domain independence). Invokes governance's OWN modules
  // (window.Governance{Metaplasticity,Extinction,RetrogradeThrottle,PredictionErrorCompressor,
  // OfflineMaintenance,NeuroSubstrate,ConnectivityAudit}) against governance's OWN runtime def
  // (governance.json runtime.params + edges/issues/activations). SHADOW: writes state.governanceOverlays;
  ...
  // governance console), 'loading' until governance.json arrives.
  // ════════════════════════════════════════════════════════════════════════════
  GovernanceBrain.prototype._loadGovernanceDef = function () {
    if (this._governanceDef) return this._governanceDef;
    if (this._governanceDefLoading) return null;
    this._governanceDefLoading = true;
    var self = this;
    if (typeof fetch === 'function') {
      try {
        fetch('/assets/data/domains/governance.json').then(function (r) { return r.json(); })
          .then(function (def) { self._governanceDef = def; })
          .catch(function () { self._governanceDefLoading = false; });
      } catch (e) { this._governanceDefLoading = false; }
    }
    return null;
  };

  GovernanceBrain.prototype._computeGovernanceOverlays = function () {
    var s = this.state;
    ...
    var META = mod('GovernanceMetaplasticity', '../governance-metaplasticity.js');
    var EXT = mod('GovernanceExtinction', '../governance-extinction.js');
    var RETRO = mod('GovernanceRetrogradeThrottle', '../governance-retrograde-throttle.js');
    var PEC = mod('GovernancePredictionErrorCompressor', '../governance-prediction-error-compressor.js');
    var OFF = mod('GovernanceOfflineMaintenance', '../governance-offline-maintenance.js');
    var NS = mod('GovernanceNeuroSubstrate', '../governance-neuro-substrate.js');
    var CONN = mod('GovernanceConnectivityAudit', '../governance-connectivity-audit.js');
    if (!(META && EXT && RETRO && PEC && OFF && NS && CONN)) {
      s.governanceOverlays = { version: 1, mode: 'off', note: 'overlay modules not loaded on this page (present only on the governance console)' };
      return null;
    }
    var def = this._loadGovernanceDef();
    if (!def) { s.governanceOverlays = { version: 1, mode: 'loading', note: 'governance.json runtime def loading; overlays compute next cycle' }; return null; }
    ...
    var intero = s.governanceInteroception || (s.cognition && s.cognition.interoception) || {};
    ...
    s.governanceOverlays = {
      version: 1, mode: 'shadow', armed: armed,
      volatility: Math.round(volatility * 1000) / 1000,
      metaplasticity: { changes: meta.changes, adapted: adapted, noop: meta.noop },
      extinction: { candidates: ext.candidates, count: ext.candidates.length, noop: ext.noop, actuation: 'PROPOSAL-ONLY (retiring a node edits governance.json — human-gated forever)' },
    ...
    s.governanceOverlays.mode = armed ? 'armed' : 'shadow';
    s.governanceOverlays.applied = applied;
    s.governanceOverlays.actuationScope = 'metaplasticity->refractory dead-time ONLY (bounded, fail-toward-quiet, reversible). throttle=no-live-consumer; PE=observe-only; extinction+offline-prune=PROPOSAL-ONLY (remove structure, human-gated forever).';
    s.governanceOverlays.note = 'PER-DOMAIN OVERLAY WIRING (ported from energy): governance runs its OWN 6 overlay modules + connectivity recurrenceAudit each cycle on governance.json. ARMED: metaplasticity raises the refractory dead-time with volatility (clamped, reversible). Everything else is proposal/observe-only.';

    if (s.cognition && typeof s.cognition === 'object') s.cognition.overlays = s.governanceOverlays;
    return s.governanceOverlays;
  };
  ...
  // ── GOVERNANCE-LOCAL FIVE-ORGAN EXTENSION ─────────────────────────────
  // The base schedules these organs; Governance owns their implementation, state and
  // authority here. Thing 2 is not consumed and external action remains inhibited.
  GovernanceBrain.prototype._refreshGovernanceActionOutcome = function () {
    if (typeof fetch !== 'function') return null;
    var cycle = this._cycleCount || 0;
    if (this._lastGovernanceActionOutcomeCycle && cycle - this._lastGovernanceActionOutcomeCycle < 12) return this._governanceActionOutcome || null;
    this._lastGovernanceActionOutcomeCycle = cycle;
    var self = this;
    try {
      fetch('/api/product-domain-learning-state?domain=' + encodeURIComponent(this.domainId))
        ...
        .then(function (result) {
          ...
          self._governanceActionOutcome = result;
          self.state[self.domainId + 'ActionLearning'] = result;
          self.state.domainActionLearning = result;
        }).catch(function () {});
    } catch (e) {}
    return this._governanceActionOutcome || null;
  };

  GovernanceBrain.prototype._computeGovernancePlasticity = function () {
    var s = this.state;
    ...
    if (!pl || !pl.P) {
      s.governancePlasticity = s.domainPlasticity = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'plasticity primitive unavailable' };
      return null;
    }
    try { this._refreshDomainExternalOutcome(); } catch (e) {}
    try { this._refreshGovernanceActionOutcome(); } catch (e) {}
    ...
    var actionOutcome = this._governanceActionOutcome;
    ...
    var out = {
      ...
      note: 'Governance-local three-factor plasticity; outcome-gated and fail-toward-seed.'
    };
    s.governancePlasticity = s.domainPlasticity = out;
  ...
  GovernanceBrain.prototype._computeGovernanceEmissionQueue = function () {
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, maxConcurrent: 3,
      ...
      packages: packages, lastQueueAt: Date.now(), note: 'Governance-local queue; research internal-only, capital staged.' };
    s.governanceEmissionQueue = s.domainEmissionQueue = out;
    return out;
  };

  GovernanceBrain.prototype._runGovernanceAutonomousEmission = function () {
    ...
    var emitted = [], staged = [], queue = (s.governanceEmissionQueue && s.governanceEmissionQueue.packages) || [];
    ...
    if (emitted.length) {
      var stream = s.governanceEmitted = s.governanceEmitted || [];
      emitted.forEach(function (p) { stream.push({ at: Date.now(), title: p.title, lane: p.lane, forecast: p.forecast }); });
      if (stream.length > 50) s.governanceEmitted = stream.slice(-50);
    }
    var out = { version: 2, domain: this.domainId, localOwner: true, autonomy: globalOn && localOn,
      ...
      externalEffects: 0, lastEmissionAt: Date.now(), note: hold ? 'held fail-safe' : 'Governance-local internal research emission; capital staged.' };
    s.governanceAutoEmission = s.domainAutoEmission = out;
    return out;
  };

  GovernanceBrain.prototype._computeGovernanceInteroception = function () {
    ...
    var channels = [{ name: 'domain-stress', alarm: r(clamp(s.stress || 0)), confidence: 0.9, weight: 1, provenance: 'Governance structured stress' }];
    var pe = model.predictionError && typeof model.predictionError === 'object' ? model.predictionError.total : model.predictionError;
    if (typeof pe === 'number') channels.push({ name: 'prediction', alarm: r(clamp(pe)), confidence: 0.7, weight: 0.85, provenance: 'Governance prediction error' });
    var ledger = n.outcomeLedger || {};
    if (typeof ledger.hitRate === 'number' && ledger.samples >= 3) channels.push({ name: 'metacognitive', alarm: r(clamp(1 - ledger.hitRate)), confidence: ledger.samples >= 5 ? 0.7 : 0.4, weight: 0.75, provenance: 'Governance outcomes' });
    var immune = cog.immune || {}, ia = { clear: 0, watch: 0.34, active: 0.4, alert: 1 };
    if (immune.immuneState) channels.push({ name: 'immune', alarm: ia[immune.immuneState] == null ? 0.34 : ia[immune.immuneState], confidence: 0.6, weight: 0.6, provenance: 'Governance immune state' });
    var forecast = n.forecast;
    if (forecast && typeof forecast.projectedStress === 'number') channels.push({ name: 'allostatic', alarm: r(clamp((forecast.projectedStress - (s.stress || 0)) * 3)), confidence: typeof forecast.confidence === 'number' ? clamp(forecast.confidence) : 0.4, weight: 0.6, provenance: 'Governance forecast' });
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, method: 'domain-local-weighted-divergence',
      ...
      attend: salience === 'blind-channel' ? loudest.name : null, note: 'Governance-local interoception; observe-only.' };
    s.governanceInteroception = s.domainInteroception = s.interoception = out;
    cog.interoception = out;
    return out;
  };

  GovernanceBrain.prototype._computeGovernanceActiveInference = function () {
    var s = this.state, cog = s.cognition;
    if (!cog) return null;
    ...
    if (!box || !box.A) {
      s.governanceActiveInference = s.domainActiveInference = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'active-inference primitive unavailable' };
      return null;
    }
    ...
    var queued = (s.governanceEmissionQueue && s.governanceEmissionQueue.packages || []).length;
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, mode: 'shadow',
      ...
      note: 'Governance-local belief update and expected-free-energy advisory; no live consumer.' };
    s.governanceActiveInference = s.domainActiveInference = out;
    cog.activeInference = out;
    return out;
  };

  GovernanceBrain.prototype._computeDomainPlasticity = GovernanceBrain.prototype._computeGovernancePlasticity;
  GovernanceBrain.prototype._computeGenericEmissionQueue = GovernanceBrain.prototype._computeGovernanceEmissionQueue;
  GovernanceBrain.prototype._runGenericAutonomousEmission = GovernanceBrain.prototype._runGovernanceAutonomousEmission;
  GovernanceBrain.prototype._computeGenericInteroception = GovernanceBrain.prototype._computeGovernanceInteroception;
  GovernanceBrain.prototype._computeGenericActiveInference = GovernanceBrain.prototype._computeGovernanceActiveInference;
