...
(function () {
  ...
  function TradeBrain() {
    ...
    this._actuation = { overlays: true, refractory: true, servo: true, eiBrake: true, phase: true, selfAudit: true };
  ...
  TradeBrain.prototype.normalizeSignals = function () {
    ...
    //    SURVIVES on real feeds/events : 0 of these 2 tokens. Trade's other ~17 conditions are feed/event-derived and unaffected
    //    REMOVED, no other source in this file : 2: supply_high_stress, structural_stress
  ...
  // ── TRADE-LOCAL FIVE-ORGAN EXTENSION ─────────────────────────────
  // The base schedules these organs; Trade owns their implementation, state and
  // authority here. Thing 2 is not consumed and external action remains inhibited.
  TradeBrain.prototype._refreshTradeActionOutcome = function () {
    if (typeof fetch !== 'function') return null;
    var cycle = this._cycleCount || 0;
    if (this._lastTradeActionOutcomeCycle && cycle - this._lastTradeActionOutcomeCycle < 12) return this._tradeActionOutcome || null;
    this._lastTradeActionOutcomeCycle = cycle;
    var self = this;
    try {
      fetch('/api/product-domain-learning-state?domain=' + encodeURIComponent(this.domainId))
        ...
        .then(function (result) {
          ...
          self._tradeActionOutcome = result;
          self.state[self.domainId + 'ActionLearning'] = result;
          self.state.domainActionLearning = result;
        }).catch(function () {});
    } catch (e) {}
    return this._tradeActionOutcome || null;
  };

  TradeBrain.prototype._computeTradePlasticity = function () {
    var s = this.state;
    ...
    if (!pl || !pl.P) {
      s.supplyChainPlasticity = s.domainPlasticity = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'plasticity primitive unavailable' };
      return null;
    }
    try { this._refreshDomainExternalOutcome(); } catch (e) {}
    try { this._refreshTradeActionOutcome(); } catch (e) {}
    ...
    var actionOutcome = this._tradeActionOutcome;
    ...
    var out = {
      ...
      note: 'Trade-local three-factor plasticity; outcome-gated and fail-toward-seed.'
    };
    s.supplyChainPlasticity = s.domainPlasticity = out;
  ...
  TradeBrain.prototype._computeTradeEmissionQueue = function () {
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, maxConcurrent: 3,
      ...
      packages: packages, lastQueueAt: Date.now(), note: 'Trade-local queue; research internal-only, capital staged.' };
    s.supplyChainEmissionQueue = s.domainEmissionQueue = out;
    return out;
  };

  TradeBrain.prototype._runTradeAutonomousEmission = function () {
    ...
    var emitted = [], staged = [], queue = (s.supplyChainEmissionQueue && s.supplyChainEmissionQueue.packages) || [];
    ...
    if (emitted.length) {
      var stream = s.supplyChainEmitted = s.supplyChainEmitted || [];
      emitted.forEach(function (p) { stream.push({ at: Date.now(), title: p.title, lane: p.lane, forecast: p.forecast }); });
      if (stream.length > 50) s.supplyChainEmitted = stream.slice(-50);
    }
    var out = { version: 2, domain: this.domainId, localOwner: true, autonomy: globalOn && localOn,
      ...
      externalEffects: 0, lastEmissionAt: Date.now(), note: hold ? 'held fail-safe' : 'Trade-local internal research emission; capital staged.' };
    s.supplyChainAutoEmission = s.domainAutoEmission = out;
    return out;
  };

  TradeBrain.prototype._computeTradeInteroception = function () {
    ...
    var channels = [{ name: 'domain-stress', alarm: r(clamp(s.stress || 0)), confidence: 0.9, weight: 1, provenance: 'Trade structured stress' }];
    var pe = model.predictionError && typeof model.predictionError === 'object' ? model.predictionError.total : model.predictionError;
    if (typeof pe === 'number') channels.push({ name: 'prediction', alarm: r(clamp(pe)), confidence: 0.7, weight: 0.85, provenance: 'Trade prediction error' });
    var ledger = n.outcomeLedger || {};
    if (typeof ledger.hitRate === 'number' && ledger.samples >= 3) channels.push({ name: 'metacognitive', alarm: r(clamp(1 - ledger.hitRate)), confidence: ledger.samples >= 5 ? 0.7 : 0.4, weight: 0.75, provenance: 'Trade outcomes' });
    var immune = cog.immune || {}, ia = { clear: 0, watch: 0.34, active: 0.4, alert: 1 };
    if (immune.immuneState) channels.push({ name: 'immune', alarm: ia[immune.immuneState] == null ? 0.34 : ia[immune.immuneState], confidence: 0.6, weight: 0.6, provenance: 'Trade immune state' });
    var forecast = n.forecast;
    if (forecast && typeof forecast.projectedStress === 'number') channels.push({ name: 'allostatic', alarm: r(clamp((forecast.projectedStress - (s.stress || 0)) * 3)), confidence: typeof forecast.confidence === 'number' ? clamp(forecast.confidence) : 0.4, weight: 0.6, provenance: 'Trade forecast' });
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, method: 'domain-local-weighted-divergence',
      ...
      attend: salience === 'blind-channel' ? loudest.name : null, note: 'Trade-local interoception; observe-only.' };
    s.supplyChainInteroception = s.domainInteroception = s.interoception = out;
    cog.interoception = out;
    return out;
  };

  TradeBrain.prototype._computeTradeActiveInference = function () {
    var s = this.state, cog = s.cognition;
    if (!cog) return null;
    ...
    if (!box || !box.A) {
      s.supplyChainActiveInference = s.domainActiveInference = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'active-inference primitive unavailable' };
      return null;
    }
    ...
    var queued = (s.supplyChainEmissionQueue && s.supplyChainEmissionQueue.packages || []).length;
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, mode: 'shadow',
      ...
      note: 'Trade-local belief update and expected-free-energy advisory; no live consumer.' };
    s.supplyChainActiveInference = s.domainActiveInference = out;
    cog.activeInference = out;
    return out;
  };

  TradeBrain.prototype._computeDomainPlasticity = TradeBrain.prototype._computeTradePlasticity;
  TradeBrain.prototype._computeGenericEmissionQueue = TradeBrain.prototype._computeTradeEmissionQueue;
  TradeBrain.prototype._runGenericAutonomousEmission = TradeBrain.prototype._runTradeAutonomousEmission;
  TradeBrain.prototype._computeGenericInteroception = TradeBrain.prototype._computeTradeInteroception;
  TradeBrain.prototype._computeGenericActiveInference = TradeBrain.prototype._computeTradeActiveInference;
