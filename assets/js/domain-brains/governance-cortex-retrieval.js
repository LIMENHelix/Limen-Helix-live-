/**
 * governance-cortex-retrieval.js — PHASE K4 safe retrieval layer (read-only, inert library).
 * Governance mirror of energy-cortex-retrieval.js.
 *
 ...
 *
 * Node + browser: window.LIMENGovernanceCortexRetrieval and module.exports both expose { retrieve, classifyQuery }.
 */
(function () {
  'use strict';

  var DX_KEYWORDS = {
    INSTITUTIONAL_EROSION: ['institution','erosion','norm','checks and balances','democratic backslid'],
    POLICY_GRIDLOCK: ['gridlock','shutdown','stalemate','impasse','dysfunction'],
    LEGITIMACY_CRISIS: ['legitimacy','trust','confidence','protest','unrest'],
    REGULATORY_CAPTURE: ['capture','lobby','revolving door','influence','conflict of interest'],
    FISCAL_GOVERNANCE: ['deficit','debt ceiling','budget','appropriation','cbo','gao'],
    CORRUPTION_SURGE: ['corruption','bribery','scandal','graft','transparency']
  };
  ...
  function retrieve(query, options, index) {
    ...
    var idx = index || (typeof window !== 'undefined' && window.LIMENGovernanceCertifiedCortex) || null;
    var warnings = [];
    ...
    if (!dx) warnings.push('no diagnosis matched the query — narrow it (institution / gridlock / legitimacy / capture / fiscal / corruption)');
  ...
  if (typeof window !== 'undefined') window.LIMENGovernanceCortexRetrieval = api;
