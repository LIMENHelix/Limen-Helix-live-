...
(function () {
  ...
  ScienceBrain.prototype.surfaceOpportunities = function () {
    ...
    for (var di = 0; di < activeDx.length; di++) {
      ...
      add({ title: dxLabel + ' — research integrity and replication platform', rank: stress * dx.relevance, path: 'PATENTABLE', urgency: stress > 0.70 ? 'high' : 'medium', source: 'diagnosis', diagnosisId: dx.id, tier: 1, stress: stress });
      if (stress >= 0.50) add({ title: dxLabel + ' — research funding and grant optimization', rank: stress * dx.relevance * 0.9, path: 'GRANT-ELIGIBLE', urgency: 'medium', source: 'diagnosis', diagnosisId: dx.id, tier: 1, stress: stress });
      ...
      add({ title: dxLabel + ' — scientific talent retention and mobility', rank: stress * dx.relevance * 0.75, path: 'GRANT-ELIGIBLE', urgency: 'medium', source: 'diagnosis', diagnosisId: dx.id, tier: 1, stress: stress });
    }
    ...
    if (this.state.convergence && this.state.convergence.primary_signal) add({ title: this.state.convergence.primary_signal.replace(/_/g, ' ').toLowerCase() + ' — science convergence response', rank: 0.98, path: 'GRANT-ELIGIBLE', urgency: 'high', source: 'convergence', tier: 1, stress: stress });
    ...
    if (stress >= 0.50) { add({ title: 'Research methodology modernization', rank: stress * 0.65, path: 'GRANT-ELIGIBLE', urgency: 'medium', source: 'lagging', tier: 3, diagnosisId: 'methodology_mod', stress: stress }); add({ title: 'Open science infrastructure — data sharing and transparency', rank: stress * 0.70, path: 'PATENTABLE', urgency: 'medium', source: 'lagging', tier: 3, diagnosisId: 'open_science', stress: stress }); }
    if (stress >= 0.60) { add({ title: 'Research-to-market translation acceleration', rank: stress * 0.75, path: 'INVESTABLE', urgency: 'medium', source: 'lagging', tier: 3, diagnosisId: 'research_translation', stress: stress }); add({ title: 'Scientific workforce development and retention', rank: stress * 0.68, path: 'GRANT-ELIGIBLE', urgency: 'medium', source: 'lagging', tier: 3, diagnosisId: 'workforce_dev', stress: stress }); }
