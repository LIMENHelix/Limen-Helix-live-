/**
 * trade-refresh-controller.js — Trade Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENSupplyChainFresh (shared state).
 * Dispatches 'limen:trade-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'supplyChain';

  window.LIMENSupplyChainFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENSupplyChainFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:trade-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:trade-refresh', true, true, F);
  ...
  window.LIMENSupplyChainRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
