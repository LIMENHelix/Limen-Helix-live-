/**
 * economy-refresh-controller.js — Economy Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENEconomyFresh (shared state).
 * Dispatches 'limen:economy-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'economy';

  window.LIMENEconomyFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENEconomyFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:economy-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:economy-refresh', true, true, F);
  ...
  window.LIMENEconomyRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
