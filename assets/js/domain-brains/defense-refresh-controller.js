/**
 * defense-refresh-controller.js — Defense Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENDefenseFresh (shared state).
 * Dispatches 'limen:defense-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'defense';

  window.LIMENDefenseFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENDefenseFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:defense-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:defense-refresh', true, true, F);
  ...
  window.LIMENDefenseRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
