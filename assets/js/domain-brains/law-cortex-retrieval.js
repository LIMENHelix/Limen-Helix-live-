/**
 * law-cortex-retrieval.js — PHASE K4 safe retrieval layer (read-only, inert library).
 * Law mirror of energy-cortex-retrieval.js.
 *
 ...
 *
 * Node + browser: window.LIMENLawCortexRetrieval and module.exports both expose { retrieve, classifyQuery }.
 */
(function () {
  'use strict';

  var DX_KEYWORDS = {
    LITIGATION_SURGE: ['litigation','lawsuit','class action','docket','caseload'],
    REGULATORY_CRACKDOWN: ['enforcement','crackdown','penalty','sanction','doj','sec','ftc'],
    RULE_OF_LAW_EROSION: ['rule of law','independence','impunity','corruption','authoritarian'],
    COURT_BACKLOG: ['backlog','delay','pending','vacancy','judge shortage'],
    FISCAL_LAW: ['deficit','debt ceiling','budget','appropriation','cbo','gao'],
    CONSTITUTIONAL_CHALLENGE: ['constitutional','scotus','precedent','rights','ruling']
  };
  ...
  function retrieve(query, options, index) {
    ...
    var idx = index || (typeof window !== 'undefined' && window.LIMENLawCertifiedCortex) || null;
    var warnings = [];
    ...
    if (!dx) warnings.push('no diagnosis matched the query — narrow it (delaw / migration / aging / fertility / labor / urbanization)');
  ...
  if (typeof window !== 'undefined') window.LIMENLawCortexRetrieval = api;
