...
(function () {
  ...
  // ── INTELLIGENCE-LOCAL FIVE-ORGAN EXTENSION ─────────────────────────────
  // The base schedules these organs; Intelligence owns their implementation, state and
  // authority here. Thing 2 is not consumed and external action remains inhibited.
  IntelligenceBrain.prototype._refreshIntelligenceActionOutcome = function () {
    if (typeof fetch !== 'function') return null;
    var cycle = this._cycleCount || 0;
    if (this._lastIntelligenceActionOutcomeCycle && cycle - this._lastIntelligenceActionOutcomeCycle < 12) return this._intelligenceActionOutcome || null;
    this._lastIntelligenceActionOutcomeCycle = cycle;
    var self = this;
    try {
      fetch('/api/product-domain-learning-state?domain=' + encodeURIComponent(this.domainId))
        ...
        .then(function (result) {
          ...
          self._intelligenceActionOutcome = result;
          self.state[self.domainId + 'ActionLearning'] = result;
          self.state.domainActionLearning = result;
        }).catch(function () {});
    } catch (e) {}
    return this._intelligenceActionOutcome || null;
  };

  IntelligenceBrain.prototype._computeIntelligencePlasticity = function () {
    var s = this.state;
    ...
    if (!pl || !pl.P) {
      s.intelligencePlasticity = s.domainPlasticity = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'plasticity primitive unavailable' };
      return null;
    }
    try { this._refreshDomainExternalOutcome(); } catch (e) {}
    try { this._refreshIntelligenceActionOutcome(); } catch (e) {}
    ...
    var actionOutcome = this._intelligenceActionOutcome;
    ...
    var out = {
      ...
      note: 'Intelligence-local three-factor plasticity; outcome-gated and fail-toward-seed.'
    };
    s.intelligencePlasticity = s.domainPlasticity = out;
  ...
  IntelligenceBrain.prototype._computeIntelligenceEmissionQueue = function () {
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, maxConcurrent: 3,
      ...
      packages: packages, lastQueueAt: Date.now(), note: 'Intelligence-local queue; research internal-only, capital staged.' };
    s.intelligenceEmissionQueue = s.domainEmissionQueue = out;
    return out;
  };

  IntelligenceBrain.prototype._runIntelligenceAutonomousEmission = function () {
    ...
    var emitted = [], staged = [], queue = (s.intelligenceEmissionQueue && s.intelligenceEmissionQueue.packages) || [];
    ...
    if (emitted.length) {
      var stream = s.intelligenceEmitted = s.intelligenceEmitted || [];
      emitted.forEach(function (p) { stream.push({ at: Date.now(), title: p.title, lane: p.lane, forecast: p.forecast }); });
      if (stream.length > 50) s.intelligenceEmitted = stream.slice(-50);
    }
    var out = { version: 2, domain: this.domainId, localOwner: true, autonomy: globalOn && localOn,
      ...
      externalEffects: 0, lastEmissionAt: Date.now(), note: hold ? 'held fail-safe' : 'Intelligence-local internal research emission; capital staged.' };
    s.intelligenceAutoEmission = s.domainAutoEmission = out;
    return out;
  };

  IntelligenceBrain.prototype._computeIntelligenceInteroception = function () {
    ...
    var channels = [{ name: 'domain-stress', alarm: r(clamp(s.stress || 0)), confidence: 0.9, weight: 1, provenance: 'Intelligence structured stress' }];
    var pe = model.predictionError && typeof model.predictionError === 'object' ? model.predictionError.total : model.predictionError;
    if (typeof pe === 'number') channels.push({ name: 'prediction', alarm: r(clamp(pe)), confidence: 0.7, weight: 0.85, provenance: 'Intelligence prediction error' });
    var ledger = n.outcomeLedger || {};
    if (typeof ledger.hitRate === 'number' && ledger.samples >= 3) channels.push({ name: 'metacognitive', alarm: r(clamp(1 - ledger.hitRate)), confidence: ledger.samples >= 5 ? 0.7 : 0.4, weight: 0.75, provenance: 'Intelligence outcomes' });
    var immune = cog.immune || {}, ia = { clear: 0, watch: 0.34, active: 0.4, alert: 1 };
    if (immune.immuneState) channels.push({ name: 'immune', alarm: ia[immune.immuneState] == null ? 0.34 : ia[immune.immuneState], confidence: 0.6, weight: 0.6, provenance: 'Intelligence immune state' });
    var forecast = n.forecast;
    if (forecast && typeof forecast.projectedStress === 'number') channels.push({ name: 'allostatic', alarm: r(clamp((forecast.projectedStress - (s.stress || 0)) * 3)), confidence: typeof forecast.confidence === 'number' ? clamp(forecast.confidence) : 0.4, weight: 0.6, provenance: 'Intelligence forecast' });
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, method: 'domain-local-weighted-divergence',
      ...
      attend: salience === 'blind-channel' ? loudest.name : null, note: 'Intelligence-local interoception; observe-only.' };
    s.intelligenceInteroception = s.domainInteroception = s.interoception = out;
    cog.interoception = out;
    return out;
  };

  IntelligenceBrain.prototype._computeIntelligenceActiveInference = function () {
    var s = this.state, cog = s.cognition;
    if (!cog) return null;
    ...
    if (!box || !box.A) {
      s.intelligenceActiveInference = s.domainActiveInference = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'active-inference primitive unavailable' };
      return null;
    }
    ...
    var queued = (s.intelligenceEmissionQueue && s.intelligenceEmissionQueue.packages || []).length;
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, mode: 'shadow',
      ...
      note: 'Intelligence-local belief update and expected-free-energy advisory; no live consumer.' };
    s.intelligenceActiveInference = s.domainActiveInference = out;
    cog.activeInference = out;
    return out;
  };

  IntelligenceBrain.prototype._computeDomainPlasticity = IntelligenceBrain.prototype._computeIntelligencePlasticity;
  IntelligenceBrain.prototype._computeGenericEmissionQueue = IntelligenceBrain.prototype._computeIntelligenceEmissionQueue;
  IntelligenceBrain.prototype._runGenericAutonomousEmission = IntelligenceBrain.prototype._runIntelligenceAutonomousEmission;
  IntelligenceBrain.prototype._computeGenericInteroception = IntelligenceBrain.prototype._computeIntelligenceInteroception;
  IntelligenceBrain.prototype._computeGenericActiveInference = IntelligenceBrain.prototype._computeIntelligenceActiveInference;
