/**
 * religion-cortex-retrieval.js — PHASE K4 safe retrieval layer (read-only, inert library).
 * Religion mirror of energy-cortex-retrieval.js.
 *
 ...
 *
 * Node + browser: window.LIMENReligionCortexRetrieval and module.exports both expose { retrieve, classifyQuery }.
 */
(function () {
  'use strict';

  var DX_KEYWORDS = {
    DISAFFILIATION_SURGE: ['disaffiliat','nones','unaffiliated','secular','leaving'],
    RELIGIOUS_PERSECUTION: ['persecution','uscirf','freedom','repression','blasphemy'],
    RULE_OF_RELIGION_EROSION: ['rule of religion','independence','impunity','corruption','authoritarian'],
    SECULARIZATION: ['secularization','decline','attendance','belief','practice'],
    FISCAL_RELIGION: ['deficit','debt ceiling','budget','appropriation','cbo','gao'],
    INTERFAITH_TENSION: ['interfaith','pluralism','tolerance','coexistence','tension']
  };
  ...
  function retrieve(query, options, index) {
    ...
    var idx = index || (typeof window !== 'undefined' && window.LIMENReligionCertifiedCortex) || null;
    var warnings = [];
    ...
    if (!dx) warnings.push('no diagnosis matched the query — narrow it (dereligion / migration / aging / fertility / labor / urbanization)');
  ...
  if (typeof window !== 'undefined') window.LIMENReligionCortexRetrieval = api;
