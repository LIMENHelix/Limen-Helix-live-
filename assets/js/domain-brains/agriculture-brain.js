...
(function () {
  ...
  // ── AGRICULTURE-LOCAL FIVE-ORGAN EXTENSION ─────────────────────────────
  // The base schedules these organs; Agriculture owns their implementation, state and
  // authority here. Thing 2 is not consumed and external action remains inhibited.
  AgricultureBrain.prototype._refreshAgricultureActionOutcome = function () {
    if (typeof fetch !== 'function') return null;
    var cycle = this._cycleCount || 0;
    if (this._lastAgricultureActionOutcomeCycle && cycle - this._lastAgricultureActionOutcomeCycle < 12) return this._agricultureActionOutcome || null;
    this._lastAgricultureActionOutcomeCycle = cycle;
    var self = this;
    try {
      fetch('/api/product-domain-learning-state?domain=' + encodeURIComponent(this.domainId))
        ...
        .then(function (result) {
          ...
          self._agricultureActionOutcome = result;
          self.state[self.domainId + 'ActionLearning'] = result;
          self.state.domainActionLearning = result;
        }).catch(function () {});
    } catch (e) {}
    return this._agricultureActionOutcome || null;
  };

  AgricultureBrain.prototype._computeAgriculturePlasticity = function () {
    var s = this.state;
    ...
    if (!pl || !pl.P) {
      s.agriculturePlasticity = s.domainPlasticity = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'plasticity primitive unavailable' };
      return null;
    }
    try { this._refreshDomainExternalOutcome(); } catch (e) {}
    try { this._refreshAgricultureActionOutcome(); } catch (e) {}
    ...
    var actionOutcome = this._agricultureActionOutcome;
    ...
    var out = {
      ...
      note: 'Agriculture-local three-factor plasticity; outcome-gated and fail-toward-seed.'
    };
    s.agriculturePlasticity = s.domainPlasticity = out;
  ...
  AgricultureBrain.prototype._computeAgricultureEmissionQueue = function () {
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, maxConcurrent: 3,
      ...
      packages: packages, lastQueueAt: Date.now(), note: 'Agriculture-local queue; research internal-only, capital staged.' };
    s.agricultureEmissionQueue = s.domainEmissionQueue = out;
    return out;
  };

  AgricultureBrain.prototype._runAgricultureAutonomousEmission = function () {
    ...
    var emitted = [], staged = [], queue = (s.agricultureEmissionQueue && s.agricultureEmissionQueue.packages) || [];
    ...
    if (emitted.length) {
      var stream = s.agricultureEmitted = s.agricultureEmitted || [];
      emitted.forEach(function (p) { stream.push({ at: Date.now(), title: p.title, lane: p.lane, forecast: p.forecast }); });
      if (stream.length > 50) s.agricultureEmitted = stream.slice(-50);
    }
    var out = { version: 2, domain: this.domainId, localOwner: true, autonomy: globalOn && localOn,
      ...
      externalEffects: 0, lastEmissionAt: Date.now(), note: hold ? 'held fail-safe' : 'Agriculture-local internal research emission; capital staged.' };
    s.agricultureAutoEmission = s.domainAutoEmission = out;
    return out;
  };

  AgricultureBrain.prototype._computeAgricultureInteroception = function () {
    ...
    var channels = [{ name: 'domain-stress', alarm: r(clamp(s.stress || 0)), confidence: 0.9, weight: 1, provenance: 'Agriculture structured stress' }];
    var pe = model.predictionError && typeof model.predictionError === 'object' ? model.predictionError.total : model.predictionError;
    if (typeof pe === 'number') channels.push({ name: 'prediction', alarm: r(clamp(pe)), confidence: 0.7, weight: 0.85, provenance: 'Agriculture prediction error' });
    var ledger = n.outcomeLedger || {};
    if (typeof ledger.hitRate === 'number' && ledger.samples >= 3) channels.push({ name: 'metacognitive', alarm: r(clamp(1 - ledger.hitRate)), confidence: ledger.samples >= 5 ? 0.7 : 0.4, weight: 0.75, provenance: 'Agriculture outcomes' });
    var immune = cog.immune || {}, ia = { clear: 0, watch: 0.34, active: 0.4, alert: 1 };
    if (immune.immuneState) channels.push({ name: 'immune', alarm: ia[immune.immuneState] == null ? 0.34 : ia[immune.immuneState], confidence: 0.6, weight: 0.6, provenance: 'Agriculture immune state' });
    var forecast = n.forecast;
    if (forecast && typeof forecast.projectedStress === 'number') channels.push({ name: 'allostatic', alarm: r(clamp((forecast.projectedStress - (s.stress || 0)) * 3)), confidence: typeof forecast.confidence === 'number' ? clamp(forecast.confidence) : 0.4, weight: 0.6, provenance: 'Agriculture forecast' });
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, method: 'domain-local-weighted-divergence',
      ...
      attend: salience === 'blind-channel' ? loudest.name : null, note: 'Agriculture-local interoception; observe-only.' };
    s.agricultureInteroception = s.domainInteroception = s.interoception = out;
    cog.interoception = out;
    return out;
  };

  AgricultureBrain.prototype._computeAgricultureActiveInference = function () {
    var s = this.state, cog = s.cognition;
    if (!cog) return null;
    ...
    if (!box || !box.A) {
      s.agricultureActiveInference = s.domainActiveInference = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'active-inference primitive unavailable' };
      return null;
    }
    ...
    var queued = (s.agricultureEmissionQueue && s.agricultureEmissionQueue.packages || []).length;
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, mode: 'shadow',
      ...
      note: 'Agriculture-local belief update and expected-free-energy advisory; no live consumer.' };
    s.agricultureActiveInference = s.domainActiveInference = out;
    cog.activeInference = out;
    return out;
  };

  AgricultureBrain.prototype._computeDomainPlasticity = AgricultureBrain.prototype._computeAgriculturePlasticity;
  AgricultureBrain.prototype._computeGenericEmissionQueue = AgricultureBrain.prototype._computeAgricultureEmissionQueue;
  AgricultureBrain.prototype._runGenericAutonomousEmission = AgricultureBrain.prototype._runAgricultureAutonomousEmission;
  AgricultureBrain.prototype._computeGenericInteroception = AgricultureBrain.prototype._computeAgricultureInteroception;
  AgricultureBrain.prototype._computeGenericActiveInference = AgricultureBrain.prototype._computeAgricultureActiveInference;
