/**
 * trade-cortex-retrieval.js — PHASE K4 safe retrieval layer (read-only, inert library).
 * Trade mirror of energy-cortex-retrieval.js.
 *
 ...
 *
 * Node + browser: window.LIMENTradeCortexRetrieval and module.exports both expose { retrieve, classifyQuery }.
 */
(function () {
  'use strict';

  var DX_KEYWORDS = {
    TARIFF_SHOCK: ['tariff','trade war','duty','protectionis','retaliation'],
    PORT_CONGESTION: ['port','congestion','backlog','dwell','berth'],
    FREIGHT_SPIKE: ['freight','shipping rate','container','baltic','spot rate'],
    CHOKEPOINT_CLOSURE: ['chokepoint','strait','canal','suez','panama','blockade'],
    SANCTIONS_IMPACT: ['sanction','embargo','export ban','restriction','decoupl'],
    CUSTOMS_DISRUPTION: ['customs','clearance','border','inspection','compliance']
  };
  ...
  function retrieve(query, options, index) {
    ...
    var idx = index || (typeof window !== 'undefined' && window.LIMENTradeCertifiedCortex) || null;
    var warnings = [];
    ...
    if (!dx) warnings.push('no diagnosis matched the query — narrow it (tariff / port / freight / chokepoint / sanctions / customs)');
  ...
  if (typeof window !== 'undefined') window.LIMENTradeCortexRetrieval = api;
