/**
 * population-refresh-controller.js — Population Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENPopulationFresh (shared state).
 * Dispatches 'limen:population-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'population';

  window.LIMENPopulationFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENPopulationFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:population-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:population-refresh', true, true, F);
  ...
  window.LIMENPopulationRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
