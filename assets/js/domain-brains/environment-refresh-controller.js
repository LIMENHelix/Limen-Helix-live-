/**
 * environment-refresh-controller.js — Environment Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENEnvironmentFresh (shared state).
 * Dispatches 'limen:environment-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'environment';

  window.LIMENEnvironmentFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENEnvironmentFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:environment-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:environment-refresh', true, true, F);
  ...
  window.LIMENEnvironmentRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
