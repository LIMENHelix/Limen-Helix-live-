/**
 * energy-refresh-controller.js — Energy Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENEnergyFresh (shared state).
 * Dispatches 'limen:energy-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'energy';

  window.LIMENEnergyFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:energy-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:energy-refresh', true, true, F);
  ...
  window.LIMENEnergyRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
