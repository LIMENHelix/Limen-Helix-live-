/**
 * finance-cortex-retrieval.js — PHASE K4 safe retrieval layer (read-only, inert library).
 * Financial mirror of energy-cortex-retrieval.js. ADDITIVE — does NOT touch the validated P3 kernel.
 *
 ...
 *
 * Node + browser: window.LIMENFinanceCortexRetrieval and module.exports both expose { retrieve, classifyQuery }.
 */
(function () {
  'use strict';

  var DX_KEYWORDS = {
    BANKING_CRISIS: ['bank', 'deposit', 'bank run', 'fdic', 'failure', 'receivership', 'svb'],
    CREDIT_FREEZE: ['credit', 'lending', 'frozen', 'spread', 'covenant', 'downgrade'],
    MARKET_CRASH: ['crash', 'selloff', 'volatility', 'vix', 'plunge', 'correction', 'bear'],
    LIQUIDITY_CRUNCH: ['liquidity', 'funding', 'repo', 'margin call', 'redemption', 'fire sale'],
    SOLVENCY_STRESS: ['solvency', 'capital', 'insolvent', 'leverage', 'default', 'bankrupt'],
    SYSTEMIC_RISK: ['systemic', 'contagion', 'sifi', 'interconnect', 'too big to fail', 'cascade']
  };
  ...
  function retrieve(query, options, index) {
    ...
    var idx = index || (typeof window !== 'undefined' && window.LIMENFinanceCertifiedCortex) || null;
    var warnings = [];
    ...
    if (!dx) warnings.push('no diagnosis matched the query — narrow it (banking / credit / market / liquidity / solvency / systemic)');
  ...
  if (typeof window !== 'undefined') window.LIMENFinanceCortexRetrieval = api;
