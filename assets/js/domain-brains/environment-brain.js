...
(function () {
  ...
  // ── ENVIRONMENT-LOCAL FIVE-ORGAN EXTENSION ─────────────────────────────
  // The base schedules these organs; Environment owns their implementation, state and
  // authority here. Thing 2 is not consumed and external action remains inhibited.
  EnvironmentBrain.prototype._refreshEnvironmentActionOutcome = function () {
    if (typeof fetch !== 'function') return null;
    var cycle = this._cycleCount || 0;
    if (this._lastEnvironmentActionOutcomeCycle && cycle - this._lastEnvironmentActionOutcomeCycle < 12) return this._environmentActionOutcome || null;
    this._lastEnvironmentActionOutcomeCycle = cycle;
    var self = this;
    try {
      fetch('/api/product-domain-learning-state?domain=' + encodeURIComponent(this.domainId))
        ...
        .then(function (result) {
          ...
          self._environmentActionOutcome = result;
          self.state[self.domainId + 'ActionLearning'] = result;
          self.state.domainActionLearning = result;
        }).catch(function () {});
    } catch (e) {}
    return this._environmentActionOutcome || null;
  };

  EnvironmentBrain.prototype._computeEnvironmentPlasticity = function () {
    var s = this.state;
    ...
    if (!pl || !pl.P) {
      s.environmentPlasticity = s.domainPlasticity = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'plasticity primitive unavailable' };
      return null;
    }
    try { this._refreshDomainExternalOutcome(); } catch (e) {}
    try { this._refreshEnvironmentActionOutcome(); } catch (e) {}
    ...
    var actionOutcome = this._environmentActionOutcome;
    ...
    var out = {
      ...
      note: 'Environment-local three-factor plasticity; outcome-gated and fail-toward-seed.'
    };
    s.environmentPlasticity = s.domainPlasticity = out;
  ...
  EnvironmentBrain.prototype._computeEnvironmentEmissionQueue = function () {
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, maxConcurrent: 3,
      ...
      packages: packages, lastQueueAt: Date.now(), note: 'Environment-local queue; research internal-only, capital staged.' };
    s.environmentEmissionQueue = s.domainEmissionQueue = out;
    return out;
  };

  EnvironmentBrain.prototype._runEnvironmentAutonomousEmission = function () {
    ...
    var emitted = [], staged = [], queue = (s.environmentEmissionQueue && s.environmentEmissionQueue.packages) || [];
    ...
    if (emitted.length) {
      var stream = s.environmentEmitted = s.environmentEmitted || [];
      emitted.forEach(function (p) { stream.push({ at: Date.now(), title: p.title, lane: p.lane, forecast: p.forecast }); });
      if (stream.length > 50) s.environmentEmitted = stream.slice(-50);
    }
    var out = { version: 2, domain: this.domainId, localOwner: true, autonomy: globalOn && localOn,
      ...
      externalEffects: 0, lastEmissionAt: Date.now(), note: hold ? 'held fail-safe' : 'Environment-local internal research emission; capital staged.' };
    s.environmentAutoEmission = s.domainAutoEmission = out;
    return out;
  };

  EnvironmentBrain.prototype._computeEnvironmentInteroception = function () {
    ...
    var channels = [{ name: 'domain-stress', alarm: r(clamp(s.stress || 0)), confidence: 0.9, weight: 1, provenance: 'Environment structured stress' }];
    var pe = model.predictionError && typeof model.predictionError === 'object' ? model.predictionError.total : model.predictionError;
    if (typeof pe === 'number') channels.push({ name: 'prediction', alarm: r(clamp(pe)), confidence: 0.7, weight: 0.85, provenance: 'Environment prediction error' });
    var ledger = n.outcomeLedger || {};
    if (typeof ledger.hitRate === 'number' && ledger.samples >= 3) channels.push({ name: 'metacognitive', alarm: r(clamp(1 - ledger.hitRate)), confidence: ledger.samples >= 5 ? 0.7 : 0.4, weight: 0.75, provenance: 'Environment outcomes' });
    var immune = cog.immune || {}, ia = { clear: 0, watch: 0.34, active: 0.4, alert: 1 };
    if (immune.immuneState) channels.push({ name: 'immune', alarm: ia[immune.immuneState] == null ? 0.34 : ia[immune.immuneState], confidence: 0.6, weight: 0.6, provenance: 'Environment immune state' });
    var forecast = n.forecast;
    if (forecast && typeof forecast.projectedStress === 'number') channels.push({ name: 'allostatic', alarm: r(clamp((forecast.projectedStress - (s.stress || 0)) * 3)), confidence: typeof forecast.confidence === 'number' ? clamp(forecast.confidence) : 0.4, weight: 0.6, provenance: 'Environment forecast' });
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, method: 'domain-local-weighted-divergence',
      ...
      attend: salience === 'blind-channel' ? loudest.name : null, note: 'Environment-local interoception; observe-only.' };
    s.environmentInteroception = s.domainInteroception = s.interoception = out;
    cog.interoception = out;
    return out;
  };

  EnvironmentBrain.prototype._computeEnvironmentActiveInference = function () {
    var s = this.state, cog = s.cognition;
    if (!cog) return null;
    ...
    if (!box || !box.A) {
      s.environmentActiveInference = s.domainActiveInference = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'active-inference primitive unavailable' };
      return null;
    }
    ...
    var queued = (s.environmentEmissionQueue && s.environmentEmissionQueue.packages || []).length;
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, mode: 'shadow',
      ...
      note: 'Environment-local belief update and expected-free-energy advisory; no live consumer.' };
    s.environmentActiveInference = s.domainActiveInference = out;
    cog.activeInference = out;
    return out;
  };

  EnvironmentBrain.prototype._computeDomainPlasticity = EnvironmentBrain.prototype._computeEnvironmentPlasticity;
  EnvironmentBrain.prototype._computeGenericEmissionQueue = EnvironmentBrain.prototype._computeEnvironmentEmissionQueue;
  EnvironmentBrain.prototype._runGenericAutonomousEmission = EnvironmentBrain.prototype._runEnvironmentAutonomousEmission;
  EnvironmentBrain.prototype._computeGenericInteroception = EnvironmentBrain.prototype._computeEnvironmentInteroception;
  EnvironmentBrain.prototype._computeGenericActiveInference = EnvironmentBrain.prototype._computeEnvironmentActiveInference;
