...
(function () {
  ...
  function CultureBrain() {
    ...
    this.motorAuthority = { ownerDomain: 'culture', contractId: 'culture-motor/1', externalValveId: 'culture:hero-image', lane: 'hero-image', decisionContract: 'media-artifact-decision/1', budgetId: 'culture-media-budget/1', receiptClass: 'asset-receipt', outcomeClass: 'usage-engagement-or-conversion', rollbackClass: 'replace-or-remove', executorVerified: false, outcomeObserverVerified: false, switches: { prepare: true, simulate: true, external: false } };
  }
  ...
  // ── CULTURE-LOCAL FIVE-ORGAN EXTENSION ─────────────────────────────
  // The base schedules these organs; Culture owns their implementation, state and
  // authority here. Thing 2 is not consumed and external action remains inhibited.
  CultureBrain.prototype._refreshCultureActionOutcome = function () {
    if (typeof fetch !== 'function') return null;
    var cycle = this._cycleCount || 0;
    if (this._lastCultureActionOutcomeCycle && cycle - this._lastCultureActionOutcomeCycle < 12) return this._cultureActionOutcome || null;
    this._lastCultureActionOutcomeCycle = cycle;
    var self = this;
    try {
      fetch('/api/product-domain-learning-state?domain=' + encodeURIComponent(this.domainId))
        ...
        .then(function (result) {
          ...
          self._cultureActionOutcome = result;
          self.state[self.domainId + 'ActionLearning'] = result;
          self.state.domainActionLearning = result;
        }).catch(function () {});
    } catch (e) {}
    return this._cultureActionOutcome || null;
  };

  CultureBrain.prototype._computeCulturePlasticity = function () {
    var s = this.state;
    ...
    if (!pl || !pl.P) {
      s.culturePlasticity = s.domainPlasticity = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'plasticity primitive unavailable' };
      return null;
    }
    try { this._refreshDomainExternalOutcome(); } catch (e) {}
    try { this._refreshCultureActionOutcome(); } catch (e) {}
    ...
    var actionOutcome = this._cultureActionOutcome;
    ...
    var out = {
      ...
      note: 'Culture-local three-factor plasticity; outcome-gated and fail-toward-seed.'
    };
    s.culturePlasticity = s.domainPlasticity = out;
  ...
  CultureBrain.prototype._computeCultureEmissionQueue = function () {
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, maxConcurrent: 3,
      ...
      packages: packages, lastQueueAt: Date.now(), note: 'Culture-local queue; research internal-only, capital staged.' };
    s.cultureEmissionQueue = s.domainEmissionQueue = out;
    return out;
  };

  CultureBrain.prototype._runCultureAutonomousEmission = function () {
    ...
    var emitted = [], staged = [], queue = (s.cultureEmissionQueue && s.cultureEmissionQueue.packages) || [];
    ...
    if (emitted.length) {
      var stream = s.cultureEmitted = s.cultureEmitted || [];
      emitted.forEach(function (p) { stream.push({ at: Date.now(), title: p.title, lane: p.lane, forecast: p.forecast }); });
      if (stream.length > 50) s.cultureEmitted = stream.slice(-50);
    }
    var out = { version: 2, domain: this.domainId, localOwner: true, autonomy: globalOn && localOn,
      ...
      externalEffects: 0, lastEmissionAt: Date.now(), note: hold ? 'held fail-safe' : 'Culture-local internal research emission; capital staged.' };
    s.cultureAutoEmission = s.domainAutoEmission = out;
    return out;
  };

  CultureBrain.prototype._computeCultureInteroception = function () {
    ...
    var channels = [{ name: 'domain-stress', alarm: r(clamp(s.stress || 0)), confidence: 0.9, weight: 1, provenance: 'Culture structured stress' }];
    var pe = model.predictionError && typeof model.predictionError === 'object' ? model.predictionError.total : model.predictionError;
    if (typeof pe === 'number') channels.push({ name: 'prediction', alarm: r(clamp(pe)), confidence: 0.7, weight: 0.85, provenance: 'Culture prediction error' });
    var ledger = n.outcomeLedger || {};
    if (typeof ledger.hitRate === 'number' && ledger.samples >= 3) channels.push({ name: 'metacognitive', alarm: r(clamp(1 - ledger.hitRate)), confidence: ledger.samples >= 5 ? 0.7 : 0.4, weight: 0.75, provenance: 'Culture outcomes' });
    var immune = cog.immune || {}, ia = { clear: 0, watch: 0.34, active: 0.4, alert: 1 };
    if (immune.immuneState) channels.push({ name: 'immune', alarm: ia[immune.immuneState] == null ? 0.34 : ia[immune.immuneState], confidence: 0.6, weight: 0.6, provenance: 'Culture immune state' });
    var forecast = n.forecast;
    if (forecast && typeof forecast.projectedStress === 'number') channels.push({ name: 'allostatic', alarm: r(clamp((forecast.projectedStress - (s.stress || 0)) * 3)), confidence: typeof forecast.confidence === 'number' ? clamp(forecast.confidence) : 0.4, weight: 0.6, provenance: 'Culture forecast' });
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, method: 'domain-local-weighted-divergence',
      ...
      attend: salience === 'blind-channel' ? loudest.name : null, note: 'Culture-local interoception; observe-only.' };
    s.cultureInteroception = s.domainInteroception = s.interoception = out;
    cog.interoception = out;
    return out;
  };

  CultureBrain.prototype._computeCultureActiveInference = function () {
    var s = this.state, cog = s.cognition;
    if (!cog) return null;
    ...
    if (!box || !box.A) {
      s.cultureActiveInference = s.domainActiveInference = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'active-inference primitive unavailable' };
      return null;
    }
    ...
    var queued = (s.cultureEmissionQueue && s.cultureEmissionQueue.packages || []).length;
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, mode: 'shadow',
      ...
      note: 'Culture-local belief update and expected-free-energy advisory; no live consumer.' };
    s.cultureActiveInference = s.domainActiveInference = out;
    cog.activeInference = out;
    return out;
  };

  CultureBrain.prototype._computeDomainPlasticity = CultureBrain.prototype._computeCulturePlasticity;
  CultureBrain.prototype._computeGenericEmissionQueue = CultureBrain.prototype._computeCultureEmissionQueue;
  CultureBrain.prototype._runGenericAutonomousEmission = CultureBrain.prototype._runCultureAutonomousEmission;
  CultureBrain.prototype._computeGenericInteroception = CultureBrain.prototype._computeCultureInteroception;
  CultureBrain.prototype._computeGenericActiveInference = CultureBrain.prototype._computeCultureActiveInference;
