...
(function () {
    ...
    return this._getPortalContent().then(function (portal) {
      ...
      try {
        if (window.DomainTelemetryAdapter && typeof window.DomainTelemetryAdapter.fromLiveCached === "function") {
          window.DomainTelemetryAdapter.fromLiveCached("education", self.state, self._runtimeOverlay || null)
  ...
  EducationBrain.prototype.cycle = function () {
    ...
    return _origCycle.call(this).then(function () {
      ...
      try { self._computeEducationOverlays(); } catch (e) {}   // NEURO-SUBSTRATE OVERLAY WIRING (per-domain, ported from energy 2026-07-21): invokes education's OWN overlay modules each cycle; shadow, proposals only
    });
  };

  // ════════════════════════════════════════════════════════════════════════════
  // NEURO-SUBSTRATE OVERLAY WIRING — per-domain copy of energy's _computeEnergyOverlays
  // (2026-07-21, full per-domain independence). Invokes education's OWN modules
  // (window.Education{Metaplasticity,Extinction,RetrogradeThrottle,PredictionErrorCompressor,
  // OfflineMaintenance,NeuroSubstrate,ConnectivityAudit}) against education's OWN runtime def
  // (education.json runtime.params + edges/issues/activations). SHADOW: writes state.educationOverlays;
  ...
  // education console), 'loading' until education.json arrives.
  // ════════════════════════════════════════════════════════════════════════════
  EducationBrain.prototype._loadEducationDef = function () {
    if (this._educationDef) return this._educationDef;
    if (this._educationDefLoading) return null;
    this._educationDefLoading = true;
    var self = this;
    if (typeof fetch === 'function') {
      try {
        fetch('/assets/data/domains/education.json').then(function (r) { return r.json(); })
          .then(function (def) { self._educationDef = def; })
          .catch(function () { self._educationDefLoading = false; });
      } catch (e) { this._educationDefLoading = false; }
    }
    return null;
  };

  EducationBrain.prototype._computeEducationOverlays = function () {
    var s = this.state;
    ...
    var META = mod('EducationMetaplasticity', '../education-metaplasticity.js');
    var EXT = mod('EducationExtinction', '../education-extinction.js');
    var RETRO = mod('EducationRetrogradeThrottle', '../education-retrograde-throttle.js');
    var PEC = mod('EducationPredictionErrorCompressor', '../education-prediction-error-compressor.js');
    var OFF = mod('EducationOfflineMaintenance', '../education-offline-maintenance.js');
    var NS = mod('EducationNeuroSubstrate', '../education-neuro-substrate.js');
    var CONN = mod('EducationConnectivityAudit', '../education-connectivity-audit.js');
    if (!(META && EXT && RETRO && PEC && OFF && NS && CONN)) {
      s.educationOverlays = { version: 1, mode: 'off', note: 'overlay modules not loaded on this page (present only on the education console)' };
      return null;
    }
    var def = this._loadEducationDef();
    if (!def) { s.educationOverlays = { version: 1, mode: 'loading', note: 'education.json runtime def loading; overlays compute next cycle' }; return null; }
    ...
    var intero = s.educationInteroception || (s.cognition && s.cognition.interoception) || {};
    ...
    s.educationOverlays = {
      ...
      extinction: { candidates: ext.candidates, count: ext.candidates.length, noop: ext.noop, actuation: 'PROPOSAL-ONLY (retiring a node edits education.json — human-gated forever)' },
    ...
    s.educationOverlays.mode = armed ? 'armed' : 'shadow';
    s.educationOverlays.applied = applied;
    s.educationOverlays.actuationScope = 'metaplasticity->refractory dead-time ONLY (bounded, fail-toward-quiet, reversible). throttle=no-live-consumer; PE=observe-only; extinction+offline-prune=PROPOSAL-ONLY (remove structure, human-gated forever).';
    s.educationOverlays.note = 'PER-DOMAIN OVERLAY WIRING (ported from energy): education runs its OWN 6 overlay modules + connectivity recurrenceAudit each cycle on education.json. ARMED: metaplasticity raises the refractory dead-time with volatility (clamped, reversible). Everything else is proposal/observe-only.';

    if (s.cognition && typeof s.cognition === 'object') s.cognition.overlays = s.educationOverlays;
    return s.educationOverlays;
  };
  ...
  // ── EDUCATION-LOCAL FIVE-ORGAN EXTENSION ─────────────────────────────
  // The base schedules these organs; Education owns their implementation, state and
  // authority here. Thing 2 is not consumed and external action remains inhibited.
  EducationBrain.prototype._refreshEducationActionOutcome = function () {
    if (typeof fetch !== 'function') return null;
    var cycle = this._cycleCount || 0;
    if (this._lastEducationActionOutcomeCycle && cycle - this._lastEducationActionOutcomeCycle < 12) return this._educationActionOutcome || null;
    this._lastEducationActionOutcomeCycle = cycle;
    var self = this;
    try {
      fetch('/api/product-domain-learning-state?domain=' + encodeURIComponent(this.domainId))
        ...
        .then(function (result) {
          ...
          self._educationActionOutcome = result;
          self.state[self.domainId + 'ActionLearning'] = result;
          self.state.domainActionLearning = result;
        }).catch(function () {});
    } catch (e) {}
    return this._educationActionOutcome || null;
  };

  EducationBrain.prototype._computeEducationPlasticity = function () {
    var s = this.state;
    ...
    if (!pl || !pl.P) {
      s.educationPlasticity = s.domainPlasticity = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'plasticity primitive unavailable' };
      return null;
    }
    try { this._refreshDomainExternalOutcome(); } catch (e) {}
    try { this._refreshEducationActionOutcome(); } catch (e) {}
    ...
    var actionOutcome = this._educationActionOutcome;
    ...
    var out = {
      ...
      note: 'Education-local three-factor plasticity; outcome-gated and fail-toward-seed.'
    };
    s.educationPlasticity = s.domainPlasticity = out;
  ...
  EducationBrain.prototype._computeEducationEmissionQueue = function () {
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, maxConcurrent: 3,
      ...
      packages: packages, lastQueueAt: Date.now(), note: 'Education-local queue; research internal-only, capital staged.' };
    s.educationEmissionQueue = s.domainEmissionQueue = out;
    return out;
  };

  EducationBrain.prototype._runEducationAutonomousEmission = function () {
    ...
    var emitted = [], staged = [], queue = (s.educationEmissionQueue && s.educationEmissionQueue.packages) || [];
    ...
    if (emitted.length) {
      var stream = s.educationEmitted = s.educationEmitted || [];
      emitted.forEach(function (p) { stream.push({ at: Date.now(), title: p.title, lane: p.lane, forecast: p.forecast }); });
      if (stream.length > 50) s.educationEmitted = stream.slice(-50);
    }
    var out = { version: 2, domain: this.domainId, localOwner: true, autonomy: globalOn && localOn,
      ...
      externalEffects: 0, lastEmissionAt: Date.now(), note: hold ? 'held fail-safe' : 'Education-local internal research emission; capital staged.' };
    s.educationAutoEmission = s.domainAutoEmission = out;
    return out;
  };

  EducationBrain.prototype._computeEducationInteroception = function () {
    ...
    var channels = [{ name: 'domain-stress', alarm: r(clamp(s.stress || 0)), confidence: 0.9, weight: 1, provenance: 'Education structured stress' }];
    var pe = model.predictionError && typeof model.predictionError === 'object' ? model.predictionError.total : model.predictionError;
    if (typeof pe === 'number') channels.push({ name: 'prediction', alarm: r(clamp(pe)), confidence: 0.7, weight: 0.85, provenance: 'Education prediction error' });
    var ledger = n.outcomeLedger || {};
    if (typeof ledger.hitRate === 'number' && ledger.samples >= 3) channels.push({ name: 'metacognitive', alarm: r(clamp(1 - ledger.hitRate)), confidence: ledger.samples >= 5 ? 0.7 : 0.4, weight: 0.75, provenance: 'Education outcomes' });
    var immune = cog.immune || {}, ia = { clear: 0, watch: 0.34, active: 0.4, alert: 1 };
    if (immune.immuneState) channels.push({ name: 'immune', alarm: ia[immune.immuneState] == null ? 0.34 : ia[immune.immuneState], confidence: 0.6, weight: 0.6, provenance: 'Education immune state' });
    var forecast = n.forecast;
    if (forecast && typeof forecast.projectedStress === 'number') channels.push({ name: 'allostatic', alarm: r(clamp((forecast.projectedStress - (s.stress || 0)) * 3)), confidence: typeof forecast.confidence === 'number' ? clamp(forecast.confidence) : 0.4, weight: 0.6, provenance: 'Education forecast' });
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, method: 'domain-local-weighted-divergence',
      ...
      attend: salience === 'blind-channel' ? loudest.name : null, note: 'Education-local interoception; observe-only.' };
    s.educationInteroception = s.domainInteroception = s.interoception = out;
    cog.interoception = out;
    return out;
  };

  EducationBrain.prototype._computeEducationActiveInference = function () {
    var s = this.state, cog = s.cognition;
    if (!cog) return null;
    ...
    if (!box || !box.A) {
      s.educationActiveInference = s.domainActiveInference = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'active-inference primitive unavailable' };
      return null;
    }
    ...
    var queued = (s.educationEmissionQueue && s.educationEmissionQueue.packages || []).length;
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, mode: 'shadow',
      ...
      note: 'Education-local belief update and expected-free-energy advisory; no live consumer.' };
    s.educationActiveInference = s.domainActiveInference = out;
    cog.activeInference = out;
    return out;
  };

  EducationBrain.prototype._computeDomainPlasticity = EducationBrain.prototype._computeEducationPlasticity;
  EducationBrain.prototype._computeGenericEmissionQueue = EducationBrain.prototype._computeEducationEmissionQueue;
  EducationBrain.prototype._runGenericAutonomousEmission = EducationBrain.prototype._runEducationAutonomousEmission;
  EducationBrain.prototype._computeGenericInteroception = EducationBrain.prototype._computeEducationInteroception;
  EducationBrain.prototype._computeGenericActiveInference = EducationBrain.prototype._computeEducationActiveInference;
