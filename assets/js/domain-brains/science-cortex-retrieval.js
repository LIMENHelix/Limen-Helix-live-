/**
 * science-cortex-retrieval.js — PHASE K4 safe retrieval layer (read-only, inert library).
 * Science mirror of energy-cortex-retrieval.js.
 *
 ...
 *
 * Node + browser: window.LIMENScienceCortexRetrieval and module.exports both expose { retrieve, classifyQuery }.
 */
(function () {
  'use strict';

  var DX_KEYWORDS = {
    REPLICATION_CRISIS: ['replication','reproducib','p-hack','retraction','crisis'],
    FUNDING_CUT: ['funding','grant','nsf','nih','appropriation','budget'],
    RESEARCH_FRAUD: ['fraud','misconduct','falsif','fabricat','integrity','ori'],
    BRAIN_DRAIN: ['brain drain','talent','exodus','postdoc','researcher'],
    FISCAL_SCIENCE: ['deficit','debt ceiling','budget','appropriation','cbo','gao'],
    LAB_CAPACITY: ['lab','instrument','facility','equipment','capacity']
  };
  ...
  function retrieve(query, options, index) {
    ...
    var idx = index || (typeof window !== 'undefined' && window.LIMENScienceCertifiedCortex) || null;
    var warnings = [];
    ...
    if (!dx) warnings.push('no diagnosis matched the query — narrow it (descience / migration / aging / fertility / labor / urbanization)');
  ...
  if (typeof window !== 'undefined') window.LIMENScienceCortexRetrieval = api;
