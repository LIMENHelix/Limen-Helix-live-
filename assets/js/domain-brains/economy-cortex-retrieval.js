/**
 * economy-cortex-retrieval.js — PHASE K4 safe retrieval layer (read-only, inert library).
 * Macroeconomic mirror of energy-cortex-retrieval.js.
 *
 ...
 *
 * Node + browser: window.LIMENEconomyCortexRetrieval and module.exports both expose { retrieve, classifyQuery }.
 */
(function () {
  'use strict';

  var DX_KEYWORDS = {
    RECESSION: ['recession', 'contraction', 'downturn', 'gdp decline', 'nber', 'negative growth'],
    INFLATION_SPIKE: ['inflation', 'cpi', 'pce', 'price level', 'overheating', 'cost of living'],
    LABOR_SHOCK: ['unemployment', 'jobs', 'payrolls', 'layoff', 'labor market', 'participation'],
    STAGFLATION: ['stagflation', 'stagnant', 'high inflation low growth', 'misery index'],
    DEMAND_COLLAPSE: ['demand', 'consumer spending', 'retail sales', 'sentiment', 'confidence'],
    MONETARY_TIGHTENING: ['fed', 'rate hike', 'fedfunds', 'tightening', 'yield curve', 'monetary policy']
  };
  ...
  function retrieve(query, options, index) {
    ...
    var idx = index || (typeof window !== 'undefined' && window.LIMENEconomyCertifiedCortex) || null;
    var warnings = [];
    ...
    if (!dx) warnings.push('no diagnosis matched the query — narrow it (recession / inflation / labor / stagflation / demand / monetary)');
    ...
    if (options.includeContext !== false) {
      ...
      if (ctx.length) warnings.push('context: indicators relevance-unverified; treatments templated — context only, NOT evidence');
    }
  ...
  if (typeof window !== 'undefined') window.LIMENEconomyCortexRetrieval = api;
