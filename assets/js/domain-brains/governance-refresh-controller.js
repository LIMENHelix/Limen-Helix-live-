/**
 * governance-refresh-controller.js — Governance Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENGovernanceFresh (shared state).
 * Dispatches 'limen:governance-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'governance';

  window.LIMENGovernanceFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENGovernanceFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:governance-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:governance-refresh', true, true, F);
  ...
  window.LIMENGovernanceRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
