...
(function () {
  ...
  function LawBrain() {
    ...
    this.motorAuthority = { ownerDomain: 'law', contractId: 'law-motor/1', externalValveId: 'law:automail', lane: 'automail', decisionContract: 'physical-message-decision/1', budgetId: 'law-automail-budget/1', receiptClass: 'mail-provider-receipt', outcomeClass: 'delivery-or-response', rollbackClass: 'cancel-before-tender-or-suppress', executorVerified: false, outcomeObserverVerified: false, switches: { prepare: true, simulate: true, external: false } };
  }
  ...
  // ── LAW-LOCAL FIVE-ORGAN EXTENSION ─────────────────────────────
  // The base schedules these organs; Law owns their implementation, state and
  // authority here. Thing 2 is not consumed and external action remains inhibited.
  LawBrain.prototype._refreshLawActionOutcome = function () {
    if (typeof fetch !== 'function') return null;
    var cycle = this._cycleCount || 0;
    if (this._lastLawActionOutcomeCycle && cycle - this._lastLawActionOutcomeCycle < 12) return this._lawActionOutcome || null;
    this._lastLawActionOutcomeCycle = cycle;
    var self = this;
    try {
      fetch('/api/product-domain-learning-state?domain=' + encodeURIComponent(this.domainId))
        ...
        .then(function (result) {
          ...
          self._lawActionOutcome = result;
          self.state[self.domainId + 'ActionLearning'] = result;
          self.state.domainActionLearning = result;
        }).catch(function () {});
    } catch (e) {}
    return this._lawActionOutcome || null;
  };

  LawBrain.prototype._computeLawPlasticity = function () {
    var s = this.state;
    ...
    if (!pl || !pl.P) {
      s.lawPlasticity = s.domainPlasticity = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'plasticity primitive unavailable' };
      return null;
    }
    try { this._refreshDomainExternalOutcome(); } catch (e) {}
    try { this._refreshLawActionOutcome(); } catch (e) {}
    ...
    var actionOutcome = this._lawActionOutcome;
    ...
    var out = {
      ...
      note: 'Law-local three-factor plasticity; outcome-gated and fail-toward-seed.'
    };
    s.lawPlasticity = s.domainPlasticity = out;
  ...
  LawBrain.prototype._computeLawEmissionQueue = function () {
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, maxConcurrent: 3,
      ...
      packages: packages, lastQueueAt: Date.now(), note: 'Law-local queue; research internal-only, capital staged.' };
    s.lawEmissionQueue = s.domainEmissionQueue = out;
    return out;
  };

  LawBrain.prototype._runLawAutonomousEmission = function () {
    ...
    var emitted = [], staged = [], queue = (s.lawEmissionQueue && s.lawEmissionQueue.packages) || [];
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, autonomy: globalOn && localOn,
      ...
      externalEffects: 0, lastEmissionAt: Date.now(), note: hold ? 'held fail-safe' : 'Law-local internal research emission; capital staged.' };
    s.lawAutoEmission = s.domainAutoEmission = out;
    return out;
  };

  LawBrain.prototype._computeLawInteroception = function () {
    ...
    var channels = [{ name: 'domain-stress', alarm: r(clamp(s.stress || 0)), confidence: 0.9, weight: 1, provenance: 'Law structured stress' }];
    var pe = model.predictionError && typeof model.predictionError === 'object' ? model.predictionError.total : model.predictionError;
    if (typeof pe === 'number') channels.push({ name: 'prediction', alarm: r(clamp(pe)), confidence: 0.7, weight: 0.85, provenance: 'Law prediction error' });
    var ledger = n.outcomeLedger || {};
    if (typeof ledger.hitRate === 'number' && ledger.samples >= 3) channels.push({ name: 'metacognitive', alarm: r(clamp(1 - ledger.hitRate)), confidence: ledger.samples >= 5 ? 0.7 : 0.4, weight: 0.75, provenance: 'Law outcomes' });
    var immune = cog.immune || {}, ia = { clear: 0, watch: 0.34, active: 0.4, alert: 1 };
    if (immune.immuneState) channels.push({ name: 'immune', alarm: ia[immune.immuneState] == null ? 0.34 : ia[immune.immuneState], confidence: 0.6, weight: 0.6, provenance: 'Law immune state' });
    var forecast = n.forecast;
    if (forecast && typeof forecast.projectedStress === 'number') channels.push({ name: 'allostatic', alarm: r(clamp((forecast.projectedStress - (s.stress || 0)) * 3)), confidence: typeof forecast.confidence === 'number' ? clamp(forecast.confidence) : 0.4, weight: 0.6, provenance: 'Law forecast' });
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, method: 'domain-local-weighted-divergence',
      ...
      attend: salience === 'blind-channel' ? loudest.name : null, note: 'Law-local interoception; observe-only.' };
    s.lawInteroception = s.domainInteroception = s.interoception = out;
    cog.interoception = out;
    return out;
  };

  LawBrain.prototype._computeLawActiveInference = function () {
    var s = this.state, cog = s.cognition;
    if (!cog) return null;
    ...
    if (!box || !box.A) {
      s.lawActiveInference = s.domainActiveInference = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'active-inference primitive unavailable' };
      return null;
    }
    ...
    var queued = (s.lawEmissionQueue && s.lawEmissionQueue.packages || []).length;
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, mode: 'shadow',
      ...
      note: 'Law-local belief update and expected-free-energy advisory; no live consumer.' };
    s.lawActiveInference = s.domainActiveInference = out;
    cog.activeInference = out;
    return out;
  };

  LawBrain.prototype._computeDomainPlasticity = LawBrain.prototype._computeLawPlasticity;
  LawBrain.prototype._computeGenericEmissionQueue = LawBrain.prototype._computeLawEmissionQueue;
  LawBrain.prototype._runGenericAutonomousEmission = LawBrain.prototype._runLawAutonomousEmission;
  LawBrain.prototype._computeGenericInteroception = LawBrain.prototype._computeLawInteroception;
  LawBrain.prototype._computeGenericActiveInference = LawBrain.prototype._computeLawActiveInference;
