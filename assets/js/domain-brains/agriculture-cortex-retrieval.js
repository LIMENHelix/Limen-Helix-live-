/**
 * agriculture-cortex-retrieval.js — PHASE K4 safe retrieval layer (read-only, inert library).
 * Agriculture mirror of energy-cortex-retrieval.js.
 *
 ...
 *
 * Node + browser: window.LIMENAgricultureCortexRetrieval and module.exports both expose { retrieve, classifyQuery }.
 */
(function () {
  'use strict';

  var DX_KEYWORDS = {
    CROP_FAILURE: ['crop','failure','yield','blight','disease','pest'],
    DROUGHT_STRESS: ['drought','water','irrigation','dry','aquifer'],
    FERTILIZER_SPIKE: ['fertilizer','nitrogen','potash','phosphate','input cost'],
    FOOD_PRICE_SURGE: ['food price','grain','commodity','corn','soy','wheat','wasde'],
    FISCAL_AGRICULTURE: ['deficit','debt ceiling','budget','appropriation','cbo','gao'],
    HARVEST_SHORTFALL: ['harvest','shortfall','planting','acreage','export ban']
  };
  ...
  function retrieve(query, options, index) {
    ...
    var idx = index || (typeof window !== 'undefined' && window.LIMENAgricultureCertifiedCortex) || null;
    var warnings = [];
    ...
    if (!dx) warnings.push('no diagnosis matched the query — narrow it (crop / drought / fertilizer / food-price / livestock / harvest)');
  ...
  if (typeof window !== 'undefined') window.LIMENAgricultureCortexRetrieval = api;
