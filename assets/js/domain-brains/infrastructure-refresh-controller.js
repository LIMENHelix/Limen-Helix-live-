/**
 * infrastructure-refresh-controller.js — Infra Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENInfrastructureFresh (shared state).
 * Dispatches 'limen:infrastructure-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'infrastructure';

  window.LIMENInfrastructureFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENInfrastructureFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:infrastructure-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:infrastructure-refresh', true, true, F);
  ...
  window.LIMENInfrastructureRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
