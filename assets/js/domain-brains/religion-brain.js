...
(function () {
  ...
  function ReligionBrain() {
    ...
    this.motorAuthority = { ownerDomain: 'religion', contractId: 'religion-motor/1', externalValveId: 'religion:subscriber-email', lane: 'subscriber-email', decisionContract: 'direct-message-decision/1', budgetId: 'religion-email-budget/1', receiptClass: 'delivery-provider-receipt', outcomeClass: 'delivery-click-reply-or-unsubscribe', rollbackClass: 'suppress-or-correct', executorVerified: false, outcomeObserverVerified: false, switches: { prepare: true, simulate: true, external: false } };
  }
  ...
  // ── RELIGION-LOCAL FIVE-ORGAN EXTENSION ─────────────────────────────
  // The base schedules these organs; Religion owns their implementation, state and
  // authority here. Thing 2 is not consumed and external action remains inhibited.
  ReligionBrain.prototype._refreshReligionActionOutcome = function () {
    if (typeof fetch !== 'function') return null;
    var cycle = this._cycleCount || 0;
    if (this._lastReligionActionOutcomeCycle && cycle - this._lastReligionActionOutcomeCycle < 12) return this._religionActionOutcome || null;
    this._lastReligionActionOutcomeCycle = cycle;
    var self = this;
    try {
      fetch('/api/product-domain-learning-state?domain=' + encodeURIComponent(this.domainId))
        ...
        .then(function (result) {
          ...
          self._religionActionOutcome = result;
          self.state[self.domainId + 'ActionLearning'] = result;
          self.state.domainActionLearning = result;
        }).catch(function () {});
    } catch (e) {}
    return this._religionActionOutcome || null;
  };

  ReligionBrain.prototype._computeReligionPlasticity = function () {
    var s = this.state;
    ...
    if (!pl || !pl.P) {
      s.religionPlasticity = s.domainPlasticity = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'plasticity primitive unavailable' };
      return null;
    }
    try { this._refreshDomainExternalOutcome(); } catch (e) {}
    try { this._refreshReligionActionOutcome(); } catch (e) {}
    ...
    var actionOutcome = this._religionActionOutcome;
    ...
    var out = {
      ...
      note: 'Religion-local three-factor plasticity; outcome-gated and fail-toward-seed.'
    };
    s.religionPlasticity = s.domainPlasticity = out;
  ...
  ReligionBrain.prototype._computeReligionEmissionQueue = function () {
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, maxConcurrent: 3,
      ...
      packages: packages, lastQueueAt: Date.now(), note: 'Religion-local queue; research internal-only, capital staged.' };
    s.religionEmissionQueue = s.domainEmissionQueue = out;
    return out;
  };

  ReligionBrain.prototype._runReligionAutonomousEmission = function () {
    ...
    var emitted = [], staged = [], queue = (s.religionEmissionQueue && s.religionEmissionQueue.packages) || [];
    ...
    if (emitted.length) {
      var stream = s.religionEmitted = s.religionEmitted || [];
      emitted.forEach(function (p) { stream.push({ at: Date.now(), title: p.title, lane: p.lane, forecast: p.forecast }); });
      if (stream.length > 50) s.religionEmitted = stream.slice(-50);
    }
    var out = { version: 2, domain: this.domainId, localOwner: true, autonomy: globalOn && localOn,
      ...
      externalEffects: 0, lastEmissionAt: Date.now(), note: hold ? 'held fail-safe' : 'Religion-local internal research emission; capital staged.' };
    s.religionAutoEmission = s.domainAutoEmission = out;
    return out;
  };

  ReligionBrain.prototype._computeReligionInteroception = function () {
    ...
    var channels = [{ name: 'domain-stress', alarm: r(clamp(s.stress || 0)), confidence: 0.9, weight: 1, provenance: 'Religion structured stress' }];
    var pe = model.predictionError && typeof model.predictionError === 'object' ? model.predictionError.total : model.predictionError;
    if (typeof pe === 'number') channels.push({ name: 'prediction', alarm: r(clamp(pe)), confidence: 0.7, weight: 0.85, provenance: 'Religion prediction error' });
    var ledger = n.outcomeLedger || {};
    if (typeof ledger.hitRate === 'number' && ledger.samples >= 3) channels.push({ name: 'metacognitive', alarm: r(clamp(1 - ledger.hitRate)), confidence: ledger.samples >= 5 ? 0.7 : 0.4, weight: 0.75, provenance: 'Religion outcomes' });
    var immune = cog.immune || {}, ia = { clear: 0, watch: 0.34, active: 0.4, alert: 1 };
    if (immune.immuneState) channels.push({ name: 'immune', alarm: ia[immune.immuneState] == null ? 0.34 : ia[immune.immuneState], confidence: 0.6, weight: 0.6, provenance: 'Religion immune state' });
    var forecast = n.forecast;
    if (forecast && typeof forecast.projectedStress === 'number') channels.push({ name: 'allostatic', alarm: r(clamp((forecast.projectedStress - (s.stress || 0)) * 3)), confidence: typeof forecast.confidence === 'number' ? clamp(forecast.confidence) : 0.4, weight: 0.6, provenance: 'Religion forecast' });
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, method: 'domain-local-weighted-divergence',
      ...
      attend: salience === 'blind-channel' ? loudest.name : null, note: 'Religion-local interoception; observe-only.' };
    s.religionInteroception = s.domainInteroception = s.interoception = out;
    cog.interoception = out;
    return out;
  };

  ReligionBrain.prototype._computeReligionActiveInference = function () {
    var s = this.state, cog = s.cognition;
    if (!cog) return null;
    ...
    if (!box || !box.A) {
      s.religionActiveInference = s.domainActiveInference = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'active-inference primitive unavailable' };
      return null;
    }
    ...
    var queued = (s.religionEmissionQueue && s.religionEmissionQueue.packages || []).length;
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, mode: 'shadow',
      ...
      note: 'Religion-local belief update and expected-free-energy advisory; no live consumer.' };
    s.religionActiveInference = s.domainActiveInference = out;
    cog.activeInference = out;
    return out;
  };

  ReligionBrain.prototype._computeDomainPlasticity = ReligionBrain.prototype._computeReligionPlasticity;
  ReligionBrain.prototype._computeGenericEmissionQueue = ReligionBrain.prototype._computeReligionEmissionQueue;
  ReligionBrain.prototype._runGenericAutonomousEmission = ReligionBrain.prototype._runReligionAutonomousEmission;
  ReligionBrain.prototype._computeGenericInteroception = ReligionBrain.prototype._computeReligionInteroception;
  ReligionBrain.prototype._computeGenericActiveInference = ReligionBrain.prototype._computeReligionActiveInference;
