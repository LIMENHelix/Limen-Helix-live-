/**
 * infrastructure-cortex-retrieval.js — PHASE K4 safe retrieval layer (read-only, inert library).
 * Civil mirror of energy-cortex-retrieval.js.
 *
 * Lets Infrastructure retrieve deep-portal CONTEXT safely, gated by the K3 certified cortex index.
 ...
 * NOT wired into the brain cycle or finalizer — an available library, changes no runtime behavior.
 *
 * Node + browser: window.LIMENInfrastructureCortexRetrieval and module.exports both expose { retrieve, classifyQuery }.
 */
(function () {
  'use strict';

  var DX_KEYWORDS = {
    CYBER_PHYSICAL: ['cyber', 'scada', 'ics', 'cisa', 'kev', 'cve', 'ransomware', 'nvd'],
    GRID_STRESS: ['grid', 'transmission', 'distribution', 'substation', 'reliability', 'nerc', 'ferc', 'outage'],
    STRUCTURAL_FAILURE: ['bridge', 'structural', 'deficien', 'collapse', 'fhwa', 'asce', 'inspection'],
    WATER_MAIN_STRESS: ['water', 'sewer', 'main', 'treatment', 'epa', 'pipe', 'lead service'],
    DAM_LEVEE_RISK: ['dam', 'levee', 'spillway', 'flood', 'usace', 'reservoir'],
    CAPITAL_FUNDING_PRESSURE: ['funding', 'capital', 'bond', 'appropriat', 'deferred maintenance', 'iija', 'reauthoriz'],
    TRANSPORT_CONGESTION: ['transit', 'transport', 'highway', 'congestion', 'road', 'rail', 'dot']
  };
  ...
  function retrieve(query, options, index) {
    ...
    var idx = index || (typeof window !== 'undefined' && window.LIMENInfrastructureCertifiedCortex) || null;
    var warnings = [];
    ...
    if (!dx) warnings.push('no diagnosis matched the query — narrow it (cyber / grid / bridge / water / dam / funding / transit)');
  ...
  if (typeof window !== 'undefined') window.LIMENInfrastructureCortexRetrieval = api;
