/**
 * environment-cortex-retrieval.js — PHASE K4 safe retrieval layer (read-only, inert library).
 * Environment mirror of energy-cortex-retrieval.js.
 *
 ...
 *
 * Node + browser: window.LIMENEnvironmentCortexRetrieval and module.exports both expose { retrieve, classifyQuery }.
 */
(function () {
  'use strict';

  var DX_KEYWORDS = {
    EMISSIONS_SPIKE: ['emission','carbon','co2','greenhouse','methane','ghg'],
    POLLUTION_EVENT: ['pollution','spill','contaminat','toxic','air quality'],
    BIODIVERSITY_LOSS: ['biodiversity','ecosystem','species','habitat','extinction'],
    CLIMATE_DISASTER: ['climate','wildfire','flood','drought','hurricane','heat'],
    WATER_SCARCITY: ['water','scarcity','aquifer','watershed','shortage'],
    REMEDIATION_BACKLOG: ['remediation','cleanup','superfund','waste','epa']
  };
  ...
  function retrieve(query, options, index) {
    ...
    var idx = index || (typeof window !== 'undefined' && window.LIMENEnvironmentCertifiedCortex) || null;
    var warnings = [];
    ...
    if (!dx) warnings.push('no diagnosis matched the query — narrow it (emissions / pollution / biodiversity / climate / water / remediation)');
  ...
  if (typeof window !== 'undefined') window.LIMENEnvironmentCortexRetrieval = api;
