/**
 * technology-refresh-controller.js — Technology Domain State (shared snapshot consumer)
 *
 * Consumes shared snapshot from LIMENSharedSnapshot instead of fetching independently.
 * Writes to window.LIMENTechnologyFresh (shared state).
 * Dispatches 'limen:technology-refresh' event.
 */
(function () {
  'use strict';

  var DOMAIN_KEY = 'technology';

  window.LIMENTechnologyFresh = {
    domain: DOMAIN_KEY,
    stress: 0,
    confidence: 0,
    activity: 0,
  ...
  var F = window.LIMENTechnologyFresh;
  ...
  function onSnapshot(snap) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('limen:technology-refresh', { detail: F }));
    } catch (e) {
      var evt = document.createEvent('CustomEvent');
      evt.initCustomEvent('limen:technology-refresh', true, true, F);
  ...
  window.LIMENTechnologyRefresh = { refresh: function () { if (window.LIMENSharedSnapshot) window.LIMENSharedSnapshot.requestFresh(); }, state: F };
