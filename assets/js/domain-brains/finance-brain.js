...
(function () {
  ...
  FinanceBrain.prototype.init = function () {
    ...
    //    source for this brain's coherence router + phase-transition consistency. The kernel needs a
    ...
    //                      phase-transition signal is self-consistency calibration (interpretive) — the
    //                      P3/P7 family gates the phase-consistency TIER of the central K4 gate, but it is
    //                      NOT external reward. External reward is reserved for a real Thing1 realized
    //                      outcome (K4 hook), never the phase transition itself.
  ...
  FinanceBrain.prototype.cycle = function () {
    ...
    return _origCycle.call(this).then(function () {
      ...
      try { self._computeFinanceOverlays(); } catch (e) {}   // NEURO-SUBSTRATE OVERLAY WIRING (per-domain, ported from energy 2026-07-21): invokes finance's OWN overlay modules each cycle; shadow, proposals only
    });
  };

  // ════════════════════════════════════════════════════════════════════════════
  // NEURO-SUBSTRATE OVERLAY WIRING — per-domain copy of energy's _computeEnergyOverlays
  // (2026-07-21, full per-domain independence). Invokes finance's OWN modules
  // (window.Finance{Metaplasticity,Extinction,RetrogradeThrottle,PredictionErrorCompressor,
  // OfflineMaintenance,NeuroSubstrate,ConnectivityAudit}) against finance's OWN runtime def
  // (finance.json runtime.params + edges/issues/activations). SHADOW: writes state.financeOverlays;
  ...
  // finance console), 'loading' until finance.json arrives.
  // ════════════════════════════════════════════════════════════════════════════
  FinanceBrain.prototype._loadFinanceDef = function () {
    if (this._financeDef) return this._financeDef;
    if (this._financeDefLoading) return null;
    this._financeDefLoading = true;
    var self = this;
    if (typeof fetch === 'function') {
      try {
        fetch('/assets/data/domains/finance.json').then(function (r) { return r.json(); })
          .then(function (def) { self._financeDef = def; })
          .catch(function () { self._financeDefLoading = false; });
      } catch (e) { this._financeDefLoading = false; }
    }
    return null;
  };

  FinanceBrain.prototype._computeFinanceOverlays = function () {
    var s = this.state;
    ...
    var META = mod('FinanceMetaplasticity', '../finance-metaplasticity.js');
    var EXT = mod('FinanceExtinction', '../finance-extinction.js');
    var RETRO = mod('FinanceRetrogradeThrottle', '../finance-retrograde-throttle.js');
    var PEC = mod('FinancePredictionErrorCompressor', '../finance-prediction-error-compressor.js');
    var OFF = mod('FinanceOfflineMaintenance', '../finance-offline-maintenance.js');
    var NS = mod('FinanceNeuroSubstrate', '../finance-neuro-substrate.js');
    var CONN = mod('FinanceConnectivityAudit', '../finance-connectivity-audit.js');
    if (!(META && EXT && RETRO && PEC && OFF && NS && CONN)) {
      s.financeOverlays = { version: 1, mode: 'off', note: 'overlay modules not loaded on this page (present only on the finance console)' };
      return null;
    }
    var def = this._loadFinanceDef();
    if (!def) { s.financeOverlays = { version: 1, mode: 'loading', note: 'finance.json runtime def loading; overlays compute next cycle' }; return null; }
    ...
    var intero = s.financeInteroception || (s.cognition && s.cognition.interoception) || {};
    ...
    s.financeOverlays = {
      ...
      extinction: { candidates: ext.candidates, count: ext.candidates.length, noop: ext.noop, actuation: 'PROPOSAL-ONLY (retiring a node edits finance.json — human-gated forever)' },
    ...
    s.financeOverlays.mode = armed ? 'armed' : 'shadow';
    s.financeOverlays.applied = applied;
    s.financeOverlays.actuationScope = 'metaplasticity->refractory dead-time ONLY (bounded, fail-toward-quiet, reversible). throttle=no-live-consumer; PE=observe-only; extinction+offline-prune=PROPOSAL-ONLY (remove structure, human-gated forever).';
    s.financeOverlays.note = 'PER-DOMAIN OVERLAY WIRING (ported from energy): finance runs its OWN 6 overlay modules + connectivity recurrenceAudit each cycle on finance.json. ARMED: metaplasticity raises the refractory dead-time with volatility (clamped, reversible). Everything else is proposal/observe-only.';

    if (s.cognition && typeof s.cognition === 'object') s.cognition.overlays = s.financeOverlays;
    return s.financeOverlays;
  };
  ...
  // PHASE-COHERENCE ROUTER + PHASE-TRANSITION CONSISTENCY (patent §3.4 Loop 1 coupling matrix M; thing2 lineage).
  ...
  //  Transition: a realized phase TRANSITION over time is SELF-CONSISTENCY CALIBRATION (interpretive) — the
  //          brain's own through-time self-prediction, NOT external reward. The P3/P7 family (Thing1's
  //          validated envelope; Finance live phase P7) gates the phase-consistency tier of the central K4
  //          gate; it is NEVER a fabricated validated/ground-truth reward. External reward for Finance comes
  //          ONLY via a real Thing1 realized outcome (handled in the K4 hook), not the transition itself.
  //          Deterministic; no AI; no writes to finance.json.
  FinanceBrain.prototype._computeFinancePhaseDynamics = function () {
    ...
    // Both the coherence router AND the phase-transition consistency below read this single myPhase, so
    ...
    //     This is self-consistency calibration (interpretive), NOT external reward.
    ...
    if (prev != null && myPhase != null && prev !== myPhase) {
      ...
      transition = { from: prev, to: myPhase, predictedUp: !!predictedUp, wentUp: wentUp, hit: hit,
        validated: validated, kind: validated ? 'self-consistency calibration (P3/P7 phase, interpretive)' : 'self-consistency calibration (phase, interpretive)' };
    }
    ...
    var out = {
      ...
      note: 'phase-coherence router (patent M matrix) + phase-transition consistency (thing2 lineage; self-consistency calibration, interpretive — NOT external reward; P3/P7 only gates the phase-consistency tier). Phase source = ' + phaseSource + ' (thing2-kernel = real recursive phase kernel; fallback = naive s.phase).'
    };
  ...
  FinanceBrain.prototype._updateFinanceModel = function () {
    ...
    //    router (myPhase) + phase-transition consistency in _computeFinancePhaseDynamics below;
    ...
    // ── ACTUATION: PHASE-COHERENCE ROUTER + PHASE-TRANSITION CONSISTENCY (reads this cycle's
    //    generic forecast in state.domainNeuro). Self-consistency calibration (interpretive), NOT
    //    external reward; P3/P7 only gates the phase-consistency tier of the central K4 gate. ──
    ...
    // ── K4 CREDIT-SOURCE HOOK — routed through the CENTRAL honest reward gate
    //    (assets/js/limen-k4-selfconsistency.js -> window.LIMENK4.credit). THE RULE (enforced in the
    //    gate, so it cannot be re-overclaimed here): isReward is TRUE only for a REAL external realized
    //    outcome (externalOutcome). A thing2 phase-transition hit, the TRUTH BRAKE call hit-rate, and
    //    the stress self-prediction are ALL self-consistency calibration (interpretive), NEVER reward.
    //    Gate preemption: external-reward(4) > phase-consistency(3) > call-consistency(2) >
    //    stress-consistency(1) > none. A low credit raises the effective learning rate. Bounded,
    //    reversible; if window.LIMENK4 is absent the prior self-consistency behavior is preserved. ──
    var _lr = fm.plasticity.learningRate;
    var _pt = (this.state.financePhaseDynamics || {}).transition;                       // thing2 phase transition (self-consistency)
    var _led = (this.state.domainNeuro || {}).outcomeLedger || null;                    // TRUTH BRAKE: forecast-vs-realized (self-consistency)
    var _om = this.state.financeOutcomeModel;                                           // K4 stress self-prediction (prior cycle)
    // Feed the phase-transition signal only when the phase actuation is on (reversible flag).
    var _phaseOn = !!(this._actuation && this._actuation.phase && _pt && _pt.hit !== null);

    // externalOutcome — Finance IS externalRewardEligible, BUT a GENUINE external realized outcome
    // (a surfaced validated-distress call that actually resolved/deteriorated per the real Thing1
    // signal) is NOT available client-side: _pubSignals carries current distress bands, not
    // resolutions (and is {} until per-company scoring validates), and every ledger below is
    // self-consistency. So pass null rather than fabricate reward. When a real resolved-outcome feed
    // exists client-side, set _externalOutcome = { hit: <0..1> } from THAT (not stress self-pred).
    var _externalOutcome = null;
    ...
    var _sig = {
      ...
      phaseTransitionHit: _phaseOn ? (_pt.hit ? 1 : 0) : null,
      callHitRate: (_led && typeof _led.hitRate === 'number') ? _led.hitRate : null,
    ...
    if (_k4) {
      _hit = _k4.credit;                 // 0..1 | null
      _creditSource = _k4.creditSource;  // external-reward | phase-consistency | call-consistency | stress-consistency | none
      _isReward = !!_k4.isReward;        // TRUE only for a real external realized outcome (never a phase/hit-rate)
    } else {
      // FALLBACK (gate absent) — prior self-consistency preemption preserved: phase > call-ledger > stress.
      var _phaseC = _phaseOn && !!(_pt && _pt.validated);
      var _fromLedger = !_phaseC && !!(_led && typeof _led.hitRate === 'number' && _led.samples >= 3);
      _hit = _phaseC ? (_pt.hit ? 1 : 0)
        : _fromLedger ? _led.hitRate
        : (_om && typeof _om.hitRate === 'number' && _om.samples >= 5) ? _om.hitRate : null;
      _creditSource = _phaseC ? 'phase-consistency' : (_fromLedger ? 'call-consistency' : (_hit !== null ? 'stress-consistency' : 'none'));
      _isReward = false;                 // no external realized outcome here either — self-consistency only
    }
    if (_hit !== null) _lr = _fmClamp(_lr * (1 + (1 - _hit)), FM_SLOW_RATE, 0.6);
    fm._effectiveLearningRate = _lr;
    fm._creditSource = _creditSource;
    fm._creditIsReward = _isReward;      // honest: interpretive self-consistency unless a real external outcome was supplied
  ...
  // ── FINANCE-LOCAL FIVE-ORGAN EXTENSION ─────────────────────────────
  // The base schedules these organs; Finance owns their implementation, state and
  // authority here. Thing 2 is not consumed and external action remains inhibited.
  FinanceBrain.prototype._refreshFinanceActionOutcome = function () {
    if (typeof fetch !== 'function') return null;
    var cycle = this._cycleCount || 0;
    if (this._lastFinanceActionOutcomeCycle && cycle - this._lastFinanceActionOutcomeCycle < 12) return this._financeActionOutcome || null;
    this._lastFinanceActionOutcomeCycle = cycle;
    var self = this;
    try {
      fetch('/api/product-domain-learning-state?domain=' + encodeURIComponent(this.domainId))
        ...
        .then(function (result) {
          ...
          self._financeActionOutcome = result;
          self.state[self.domainId + 'ActionLearning'] = result;
          self.state.domainActionLearning = result;
        }).catch(function () {});
    } catch (e) {}
    return this._financeActionOutcome || null;
  };

  FinanceBrain.prototype._computeFinancePlasticity = function () {
    var s = this.state;
    ...
    if (!pl || !pl.P) {
      s.financePlasticity = s.domainPlasticity = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'plasticity primitive unavailable' };
      return null;
    }
    try { this._refreshDomainExternalOutcome(); } catch (e) {}
    try { this._refreshFinanceActionOutcome(); } catch (e) {}
    ...
    var actionOutcome = this._financeActionOutcome;
    ...
    var out = {
      ...
      note: 'Finance-local three-factor plasticity; outcome-gated and fail-toward-seed.'
    };
    s.financePlasticity = s.domainPlasticity = out;
  ...
  FinanceBrain.prototype._computeFinanceEmissionQueue = function () {
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, maxConcurrent: 3,
      ...
      packages: packages, lastQueueAt: Date.now(), note: 'Finance-local queue; research internal-only, capital staged.' };
    s.financeEmissionQueue = s.domainEmissionQueue = out;
    return out;
  };

  FinanceBrain.prototype._runFinanceAutonomousEmission = function () {
    ...
    var emitted = [], staged = [], queue = (s.financeEmissionQueue && s.financeEmissionQueue.packages) || [];
    ...
    if (emitted.length) {
      var stream = s.financeEmitted = s.financeEmitted || [];
      emitted.forEach(function (p) { stream.push({ at: Date.now(), title: p.title, lane: p.lane, forecast: p.forecast }); });
      if (stream.length > 50) s.financeEmitted = stream.slice(-50);
    }
    var out = { version: 2, domain: this.domainId, localOwner: true, autonomy: globalOn && localOn,
      ...
      externalEffects: 0, lastEmissionAt: Date.now(), note: hold ? 'held fail-safe' : 'Finance-local internal research emission; capital staged.' };
    s.financeAutoEmission = s.domainAutoEmission = out;
    return out;
  };

  FinanceBrain.prototype._computeFinanceInteroception = function () {
    ...
    var channels = [{ name: 'domain-stress', alarm: r(clamp(s.stress || 0)), confidence: 0.9, weight: 1, provenance: 'Finance structured stress' }];
    var pe = model.predictionError && typeof model.predictionError === 'object' ? model.predictionError.total : model.predictionError;
    if (typeof pe === 'number') channels.push({ name: 'prediction', alarm: r(clamp(pe)), confidence: 0.7, weight: 0.85, provenance: 'Finance prediction error' });
    var ledger = n.outcomeLedger || {};
    if (typeof ledger.hitRate === 'number' && ledger.samples >= 3) channels.push({ name: 'metacognitive', alarm: r(clamp(1 - ledger.hitRate)), confidence: ledger.samples >= 5 ? 0.7 : 0.4, weight: 0.75, provenance: 'Finance outcomes' });
    var immune = cog.immune || {}, ia = { clear: 0, watch: 0.34, active: 0.4, alert: 1 };
    if (immune.immuneState) channels.push({ name: 'immune', alarm: ia[immune.immuneState] == null ? 0.34 : ia[immune.immuneState], confidence: 0.6, weight: 0.6, provenance: 'Finance immune state' });
    var forecast = n.forecast;
    if (forecast && typeof forecast.projectedStress === 'number') channels.push({ name: 'allostatic', alarm: r(clamp((forecast.projectedStress - (s.stress || 0)) * 3)), confidence: typeof forecast.confidence === 'number' ? clamp(forecast.confidence) : 0.4, weight: 0.6, provenance: 'Finance forecast' });
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, method: 'domain-local-weighted-divergence',
      ...
      attend: salience === 'blind-channel' ? loudest.name : null, note: 'Finance-local interoception; observe-only.' };
    s.financeInteroception = s.domainInteroception = s.interoception = out;
    cog.interoception = out;
    return out;
  };

  FinanceBrain.prototype._computeFinanceActiveInference = function () {
    var s = this.state, cog = s.cognition;
    if (!cog) return null;
    ...
    if (!box || !box.A) {
      s.financeActiveInference = s.domainActiveInference = { version: 2, domain: this.domainId, localOwner: true, mode: 'off', note: 'active-inference primitive unavailable' };
      return null;
    }
    ...
    var queued = (s.financeEmissionQueue && s.financeEmissionQueue.packages || []).length;
    ...
    var out = { version: 2, domain: this.domainId, localOwner: true, mode: 'shadow',
      ...
      note: 'Finance-local belief update and expected-free-energy advisory; no live consumer.' };
    s.financeActiveInference = s.domainActiveInference = out;
    cog.activeInference = out;
    return out;
  };

  FinanceBrain.prototype._computeDomainPlasticity = FinanceBrain.prototype._computeFinancePlasticity;
  FinanceBrain.prototype._computeGenericEmissionQueue = FinanceBrain.prototype._computeFinanceEmissionQueue;
  FinanceBrain.prototype._runGenericAutonomousEmission = FinanceBrain.prototype._runFinanceAutonomousEmission;
  FinanceBrain.prototype._computeGenericInteroception = FinanceBrain.prototype._computeFinanceInteroception;
  FinanceBrain.prototype._computeGenericActiveInference = FinanceBrain.prototype._computeFinanceActiveInference;
