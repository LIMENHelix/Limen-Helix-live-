/*
 * finance-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes finance.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Finance has no runtime load field). { nodeId: loadFraction }
  ...
  /* finance: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(finance, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (finance.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. finance.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Finance has no runtime load field). throttleGain operator-set. Never writes finance.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.FinanceRetrogradeThrottle) root.FinanceRetrogradeThrottle = API;
