/*
 * education-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Education's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (education-offline-maintenance.js)
 *   - refractory absolute window       (education-refractory-limiter.js)
 *   - prediction-error threshold       (education-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Education has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Education has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.EducationMetaplasticity) root.EducationMetaplasticity = API;
