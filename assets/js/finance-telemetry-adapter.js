/*
 * finance-telemetry-adapter.js  — wires runtime fields to the LIVE Finance telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/finance-pulse-engine.js  (window.LIMENFinancePulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Finance issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to finance.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into finance.json (a
...
(function (root) {
   ...
   * inputs: { finance, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (finance.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (finance.activations || []).forEach(function (a) {
  ...
  /* Merge finance.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(finance, overlay) {
    var base = finance.runtime || {};
    ...
    var acts = {}; (finance.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (finance.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the finance definition + brainState. */
  function fromLive(finance, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENFinancePulse) ? window.LIMENFinancePulse : null;
    if (!P) return null;
    return fromPulse({ finance: finance, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN finance definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/finance.json', 'finance'),
      _load('/assets/data/finance-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ finance: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.FinanceTelemetryAdapter) root.FinanceTelemetryAdapter = API;
