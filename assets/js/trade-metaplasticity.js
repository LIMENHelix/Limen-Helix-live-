/*
 * trade-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Trade's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (trade-offline-maintenance.js)
 *   - refractory absolute window       (trade-refractory-limiter.js)
 *   - prediction-error threshold       (trade-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Trade has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Trade has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.TradeMetaplasticity) root.TradeMetaplasticity = API;
