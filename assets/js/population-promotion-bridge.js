/**
 * environment-promotion-bridge.js — Inject promoted Population portal directives into operator surfaces
 *
 ...
 * Exposes: window.LIMENPopulationPromotionBridge
 */
(function () {
  'use strict';
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENPopulationDirectiveExtractor;
    var ranker = window.LIMENPopulationDirectiveRanker;
    var translator = window.LIMENPopulationDirectiveTranslator;
    ...
    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        ...
        var targeting = window.LIMENPopulationTargetingEngine;
        if (targeting) {
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      state.treatments.push({
        id: 'ptreat_' + (dir.portalDomainId || 'population') + '_' + (dir.nodeId || '') + '_' + pi,
  ...
  window.LIMENPopulationPromotionBridge = {
    promote: promote,
