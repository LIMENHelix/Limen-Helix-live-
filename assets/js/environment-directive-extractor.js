/**
 * environment-directive-extractor.js — Exhaustive Fractal Directive Extraction (Science)
 *
 * Traverses the Environment portal fractal tree starting from active diagnosis circuits,
 * following ALL childPortal references recursively until terminal leaf nodes
 * are reached. No depth caps.
 *
 * Architecture:
 *   - Prebuilt-first: loads /assets/data/deep/environment-deep-directives.json if available
 *   - Live fallback: stage-1 diagnosis-mapped portals first, stage-2 L0 root traversal
 *
 * ADDITIVE ONLY. Feature-flagged: window.LIMEN_ENABLE_DIRECTIVE_EXTRACTION
 * Exposes: window.LIMENEnvironmentDirectiveExtractor
 */
(function () {
  'use strict';
  ...
  function _loadPrebuiltPayload() {
    ...
    return fetch('/assets/data/deep/environment-deep-directives.json')
      .then(function (r) { if (!r.ok) throw new Error(r.status); return r.json(); })
      .then(function (data) {
        if (data && data.directives && data.directives.length > 0) {
          _prebuiltPayload = data;
          console.log('[ScienceExtractor] Prebuilt: ' + data.directives.length + ' directives, maxDepth=' + (data.traversal && data.traversal.maxDepth));
        }
  ...
  function extract(state, portal, opts) {
    ...
    return _loadPrebuiltPayload().then(function (prebuilt) {
      if (prebuilt && prebuilt.directives) {
        ...
        console.log('[ScienceExtractor] Prebuilt: ' + filtered.length + ' candidates, ' + activeDx.length + ' active dx');
        return filtered;
      }
  ...
  function _liveExtract(activeNodeIds, activeDx, stress, portal) {
      ...
      (function (pid) {
        chain = chain.then(function () {
          return _fetchPortalJSON(pid).then(function (data) {
            if (!data) return;
            return _extractFromPortal(pid, data, activeNodeIds, stress, 1, ['environment'], seen, results);
          });
        });
    ...
    return chain
      .then(function () {
        return _extractFromPortal('environment', portal, activeNodeIds, stress, 0, [], seen, results);
      })
      .then(function () {
        console.log('[ScienceExtractor] Extracted ' + results.length + ' candidates from ' +
  ...
  window.LIMENEnvironmentDirectiveExtractor = {
    extract: extract,
