/**
 * environment-claim-ledger.js — Population Domain Claim Ledger (Local Only)
 *
 * Stores claim records in localStorage. Tracks opportunity claims
 * from acceptance through outcome. Science-scoped only.
 *
 * Namespace: window.LIMENPopulation.economy.claims
 */
(function () {
  'use strict';
  ...
  var STORE_KEY = 'LIMEN_POPULATION_CLAIMS';
  ...
  function createClaimRecord(opportunity) {
    ...
    try { window.dispatchEvent(new CustomEvent('limen:population-claim-update')); } catch (e) {}
  ...
  window.LIMENPopulation.economy.claims = {
