/*
 * defense-offline-maintenance.js  — ADDITIVE reference implementation
 * ----------------------------------------------------------------------------
 * Concrete form of the OFFLINE_CYCLE proposal (docs/defense-offline-cycle-proposal.md).
 * Source (only): LIMEN Helix Comprehensive Neurology Reference — XIII.9, XIII.6, IV.6, XIV.
 *
 * CONTRACT:
 *   - Operates on a DEEP COPY of the passed Defense object. Never mutates the input.
 *   - NEVER writes to disk / never touches assets/data/domains/defense.json.
 ...
 *   - Operation 2 (signal clearance) is SKIPPED with a flag: Defense has no signal-age field.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';
  ...
  function runOfflineMaintenance(defense, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var out = deepCopy(defense); // never touch the input
    ...
    report.operations.clearance = {
      ...
      status: 'SKIPPED',
      reason: 'Defense activations carry no age/timestamp/persistence field; nothing to key "stale" on. Not faked. Needs a per-signal last-active field first.'
    };
    ...
    report.operations.eiCheck = {
      ...
      caveat: 'Defense inhib edges target canonical nodes outside the 21 activations; do NOT flag seizure risk on activation in-degree alone.',
  ...
  if (root && !root.DefenseOfflineMaintenance) root.DefenseOfflineMaintenance = API;
