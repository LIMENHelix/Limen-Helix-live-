/*
 * economy-telemetry-adapter.js  — wires runtime fields to the LIVE Economy telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/economy-pulse-engine.js  (window.LIMENEconomyPulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Economy issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to economy.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into economy.json (a
...
(function (root) {
   ...
   * inputs: { economy, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (economy.issues || []).forEach(function (is) {
  ...
  /* Merge economy.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(economy, overlay) {
    var base = economy.runtime || {};
    ...
    var acts = {}; (economy.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (economy.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the economy definition + brainState. */
  function fromLive(economy, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENEconomyPulse) ? window.LIMENEconomyPulse : null;
    if (!P) return null;
    return fromPulse({ economy: economy, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN economy definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/economy.json', 'economy'),
      _load('/assets/data/economy-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ economy: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.EconomyTelemetryAdapter) root.EconomyTelemetryAdapter = API;
