/**
 * defense-directive-extractor.js — Exhaustive Fractal Directive Extraction (Defense)
 *
 * Prebuilt-first, live fallback. Self-gates: ?domain=defense
 * Exposes: window.LIMENDefenseDirectiveExtractor
 */
(function () {
  'use strict';
  ...
  function _loadPrebuiltPayload() {
    ...
    return fetch('/assets/data/deep/defense-deep-directives.json')
      ...
      if (prebuilt && prebuilt.directives) {
        ...
        for (var pi = 0; pi < prebuilt.directives.length; pi++) {
          ...
          if (dxCtx) {
            ...
            d.stress = stress; d.domain = 'defense';
            filtered.push(d);
      ...
      (function (pid) {
        chain = chain.then(function () {
          return _fetchPortalJSON(pid).then(function (data) {
            if (!data) return;
            return _extractFromPortal(pid, data, activeNodeIds, stress, 1, ['defense'], seen, results);
          });
        });
      })(diagnosisMappedPortals[pi]);
    }
    return chain
      .then(function () { return _extractFromPortal('defense', portal, activeNodeIds, stress, 0, [], seen, results); })
      .then(function () { console.log('[DefenseExtractor] Extracted ' + results.length); return results; });
  }

  window.LIMENDefenseDirectiveExtractor = { extract: extract, _fetchPortalJSON: _fetchPortalJSON, _portalFileToId: _portalFileToId };
})();
