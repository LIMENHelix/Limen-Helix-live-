/*
 * population-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/population-neuro-substrate.json
 * Prompts:       assets/prompts/population-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/population.json
 * or any existing population-*.js. It reads existing Population data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Population brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Population file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/population-neuro-substrate.json';
  var POPULATION_URL = '/assets/data/domains/population.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Population issue id. */
     ...
     * Reads an Population issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Population issue. */
    auditAllIssues: function () {
      return loadJSON(POPULATION_URL).then(function (population) {
        return (population.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.PopulationNeuroSubstrate) root.PopulationNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Population brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import PopulationNeuroSubstrate from '/assets/js/population-neuro-substrate.js';
   *      // then, after issues render:
   *      PopulationNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
