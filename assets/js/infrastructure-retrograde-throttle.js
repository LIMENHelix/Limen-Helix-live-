/*
 * infrastructure-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes infrastructure.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Infrastructure has no runtime load field). { nodeId: loadFraction }
  ...
  /* infrastructure: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(infrastructure, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (infrastructure.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. infrastructure.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Infrastructure has no runtime load field). throttleGain operator-set. Never writes infrastructure.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.InfrastructureRetrogradeThrottle) root.InfrastructureRetrogradeThrottle = API;
