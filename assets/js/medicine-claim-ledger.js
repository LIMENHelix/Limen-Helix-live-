/**
 * medicine-claim-ledger.js — Medicine Domain Claim Ledger (Local Only)
 *
 * Stores claim records in localStorage. Tracks opportunity claims
 * from acceptance through outcome. Medicine-scoped only.
 *
 * Namespace: window.LIMENMedicine.economy.claims
 */
(function () {
  'use strict';
  ...
  var STORE_KEY = 'LIMEN_MEDICINE_CLAIMS';
  ...
  function createClaimRecord(opportunity) {
    ...
    try { window.dispatchEvent(new CustomEvent('limen:medicine-claim-update')); } catch (e) {}
  ...
  window.LIMENMedicine.economy.claims = {
  ...
  console.log('[MedicineClaimLedger] Loaded — claim ledger ready');
})();
