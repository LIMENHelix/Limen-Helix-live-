/*
 * communication-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Communication's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (communication-offline-maintenance.js)
 *   - refractory absolute window       (communication-refractory-limiter.js)
 *   - prediction-error threshold       (communication-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Communication has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Communication has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.CommunicationMetaplasticity) root.CommunicationMetaplasticity = API;
