/*
 * agriculture-extinction.js  — ADDITIVE mechanism (completes EXTINCTION_LEARNING)
 * ----------------------------------------------------------------------------
 ...
 * (complements the GLOBAL down-scaling in agriculture-offline-maintenance.js).
 *
 * CONTRACT: read-only / copy-only. NEVER writes agriculture.json. Proposes which acquired responses to
...
(function (root) {
  ...
  /* Extinction over Agriculture activations: an activation whose diagnosticTriggers are ALL absent
   * from the caller-supplied activeTriggers is a candidate to retire (its trigger is gone).
   * activeTriggers: array of currently-firing trigger strings (runtime state; Agriculture has none baked in). */
  function proposeExtinction(agriculture, activeTriggers, opts) {
    ...
    return {
      ...
      note: 'Read-only PROPOSAL. Applies nothing; retiring a node would edit agriculture.json. activeTriggers is caller-supplied runtime state.'
    };
  }

  var API = { proposeExtinction: proposeExtinction };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.AgricultureExtinction) root.AgricultureExtinction = API;
