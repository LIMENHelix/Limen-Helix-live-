/**
 * education-directive-extractor.js — Exhaustive Fractal Directive Extraction (Education)
 *
 * Prebuilt-first, live fallback. Self-gates: ?domain=education
 * Exposes: window.LIMENEducationDirectiveExtractor
 */
(function () {
  'use strict';
  ...
  function _loadPrebuiltPayload() {
    ...
    return fetch('/assets/data/deep/education-deep-directives.json')
      .then(function (r) { if (!r.ok) throw new Error(r.status); return r.json(); })
      .then(function (data) {
        if (data && data.directives && data.directives.length > 0) {
          _prebuiltPayload = data;
          console.log('[EducationExtractor] Prebuilt: ' + data.directives.length + ' directives, maxDepth=' + (data.traversal && data.traversal.maxDepth));
        }
      ...
      if (prebuilt && prebuilt.directives) {
        ...
        for (var pi = 0; pi < prebuilt.directives.length; pi++) {
          ...
          if (dxCtx) {
            ...
            d.stress = stress; d.domain = 'education';
            filtered.push(d);
        ...
        console.log('[EducationExtractor] Prebuilt: ' + filtered.length + ' candidates, ' + activeDx.length + ' active dx');
        return filtered;
      }
      ...
      (function (pid) {
        chain = chain.then(function () {
          return _fetchPortalJSON(pid).then(function (data) {
            if (!data) return;
            return _extractFromPortal(pid, data, activeNodeIds, stress, 1, ['education'], seen, results);
          });
        });
      })(diagnosisMappedPortals[pi]);
    }
    return chain
      .then(function () { return _extractFromPortal('education', portal, activeNodeIds, stress, 0, [], seen, results); })
      .then(function () { console.log('[EducationExtractor] Extracted ' + results.length); return results; });
  }

  window.LIMENEducationDirectiveExtractor = { extract: extract, _fetchPortalJSON: _fetchPortalJSON, _portalFileToId: _portalFileToId };
})();
