/**
 * energy-claim-flow.js — Energy Domain Claim Acceptance Flow
 *
 * Adds CLAIM button to Energy opportunity cards. On click, opens
 * modal requiring acceptance of framework terms before creating
 * a claim record. Energy-scoped only. No backend.
 *
 * Depends on: energy-compensation.js, energy-claim-ledger.js
 * Namespace: window.LIMENEnergy.economy.claimFlow
 */
(function () {
  'use strict';
  ...
  function openClaimModal(opportunity) {
    ...
    h += '<div class="ecf-title">Claim Energy Opportunity</div>';
    ...
    h += '<span class="ecf-accept-text">I understand this is an Energy-domain execution claim, I am responsible for truthful submission, documented progress, and compliance with the applicable workflow.</span>';
    h += '</label>';
  ...
  window.addEventListener('limen:energy-claim-update', function () { setTimeout(injectIntoCards, 100); });

  // ── PUBLIC API ──────────────────────────────────────────────────────

  window.LIMENEnergy.economy.claimFlow = {
    openClaimModal: openClaimModal,
    injectIntoCards: injectIntoCards
  };

  console.log('[EnergyClaimFlow] Loaded — claim acceptance flow ready');
})();
