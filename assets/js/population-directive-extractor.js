 ...
 *
 * Traverses the Population portal fractal tree starting from active diagnosis circuits,
 * following ALL childPortal references recursively until terminal leaf nodes
 * are reached. No depth caps.
 *
 * Architecture:
 *   - Prebuilt-first: loads /assets/data/deep/population-deep-directives.json if available
 ...
 * Exposes: window.LIMENPopulationDirectiveExtractor
 */
(function () {
  'use strict';
  ...
  function _loadPrebuiltPayload() {
    ...
    return fetch('/assets/data/deep/population-deep-directives.json')
  ...
  function _liveExtract(activeNodeIds, activeDx, stress, portal) {
      ...
      (function (pid) {
        chain = chain.then(function () {
          return _fetchPortalJSON(pid).then(function (data) {
            if (!data) return;
            return _extractFromPortal(pid, data, activeNodeIds, stress, 1, ['population'], seen, results);
          });
        });
    ...
    return chain
      .then(function () {
        return _extractFromPortal('population', portal, activeNodeIds, stress, 0, [], seen, results);
      })
      .then(function () {
  ...
  window.LIMENPopulationDirectiveExtractor = {
    extract: extract,
