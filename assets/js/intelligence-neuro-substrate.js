/*
 * intelligence-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/intelligence-neuro-substrate.json
 * Prompts:       assets/prompts/intelligence-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/intelligence.json
 * or any existing intelligence-*.js. It reads existing Intelligence data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Intelligence brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Intelligence file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/intelligence-neuro-substrate.json';
  var INTELLIGENCE_URL = '/assets/data/domains/intelligence.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Intelligence issue id. */
     ...
     * Reads an Intelligence issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Intelligence issue. */
    auditAllIssues: function () {
      return loadJSON(INTELLIGENCE_URL).then(function (intelligence) {
        return (intelligence.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.IntelligenceNeuroSubstrate) root.IntelligenceNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Intelligence brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import IntelligenceNeuroSubstrate from '/assets/js/intelligence-neuro-substrate.js';
   *      // then, after issues render:
   *      IntelligenceNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
