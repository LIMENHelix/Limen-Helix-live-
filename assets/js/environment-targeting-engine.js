/**
 * environment-targeting-engine.js — Context-aware targeting for promoted Environment directives
 *
 ...
 *   Tier 3: Example entities (high-confidence well-known environment sector players)
 *
 * ADDITIVE ONLY. Feature-flagged: window.LIMEN_ENABLE_DIRECTIVE_EXTRACTION
 * Exposes: window.LIMENEnvironmentTargetingEngine
 */
(function () {
  'use strict';
  ...
  var SEGMENTS = {
    ...
    research_universities: {
      label: 'R1 environment universities and university tech transfer offices',
    ...
    contract_research: {
      label: 'Contract environment organizations (CROs)',
    ...
    research_software: {
      label: 'Environment informatics, ELN, and lab software',
      keywords: ['ELN', 'electronic lab notebook', 'LIMS', 'lab informatics', 'environment data management', 'data repository', 'workflow', 'jupyter', 'notebook'],
      tier3: [
    ...
    research_integrity: {
      label: 'Environment integrity, replication, and reproducibility tools',
      ...
      tier3: [
        ...
        { name: 'Open Environment Framework (COS)', ticker: null },
    ...
    research_compute: {
      label: 'Environment computing, HPC, and AI-for-science',
      keywords: ['HPC', 'supercomput', 'GPU cluster', 'AI for science', 'foundation model', 'environment compute', 'cloud HPC', 'protein folding', 'molecular dynamics'],
      tier3: [
    ...
    foundations: {
      label: 'Private environment foundations and philanthropy',
  ...
  // to their primary industry segments.
  var NODE_SEGMENTS = {
  ...
  function resolveTargets(directive) {
    ...
    else if (path === 'PATENTABLE') executionTargets = ['Patent attorneys', 'University tech transfer offices', 'Scientific instrument licensors', 'Strategic acquirers in environment informatics'];
    else if (path === 'INVESTABLE') executionTargets = ['Life sciences VCs', 'Environment informatics PE', 'University endowment fund managers'];
  ...
  window.LIMENEnvironmentTargetingEngine = { resolveTargets: resolveTargets };
})();
