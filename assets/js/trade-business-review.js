      ...
      bizBtns[bi].addEventListener('click', function (e) {
        ...
        try {
          sessionStorage.setItem('limen_exec_context', JSON.stringify({
            ...
            source: 'trade', returnTo: '/domain-console?domain=supplyChain&mode=operator'
          }));
        } catch (ex) {}
        window.location.href = 'trade-workspace.html?track=business&opp=' + encodeURIComponent(oppKey) + '&returnTo=' + encodeURIComponent('/domain-console?domain=supplyChain&mode=operator');
      });
    }
