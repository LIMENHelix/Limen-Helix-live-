/**
 * religion-agent-box.js — the Religion domain's operator AI box (client side).
 *
 * A floating "Talk to Religion" panel on the religion domain-console. It KNOWS its
 * domain (window.LIMENReligionBrain.getStateSummary()) and can make bounded
 ...
 *    tries /api/religion-agent first and falls back to LOCAL on any failure, so it
...
(function () {
  ...
  function parseLocal(prompt, state) {
    ...
    if (mF) {
      var topic = mF[1].replace(/\b(please|now|today|this week|domain|religion|closely|carefully)\b/g, '').trim();
  ...
  if (typeof window !== 'undefined') window.__religionAgentLocal = { parse: parseLocal, query: answerQuery };

  ready(function () {
    if (window.__religionAgentBox) return; window.__religionAgentBox = true;
    var LS_KEY = 'limenReligionAgentKey';
    var esc = function (s) { return String(s == null ? '' : s).replace(/[&<>"]/g, function (c) { return ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[c]; }); };
    function brain() { return window.LIMENReligionBrain || null; }
    ...
    var toggle = document.createElement('div'); toggle.id = 'eab-toggle'; toggle.textContent = '⚡ Talk to Religion';
    var panel = document.createElement('div'); panel.id = 'eab';
    panel.innerHTML =
      '<div class="eab-h"><span><b>Religion</b> · ask or steer</span><span class="eab-x">✕</span></div>' +
    ...
    function renderResult(pending, result, mode, left) {
      ...
      if (applied) html += '<div class="eab-tool">✓ applied to religion domain:\n' + esc(applied) + '</div>';
    ...
    function send() {
      var b = brain();
      if (!b) { add('<span class="eab-ai">Religion brain not loaded on this page.</span>'); return; }
    ...
    add('<span class="eab-ai">Religion domain online (free local mode). Ask about it, or steer it: "focus on grid", "prefer research", "autonomy off", "how is religion doing", "show the forecast". I bias the domain, never force a finding, move capital, or touch code. Add a passcode above for conversational AI once billing is on.</span>');
  });
})();
