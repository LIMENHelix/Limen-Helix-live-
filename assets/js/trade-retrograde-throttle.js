/*
 * trade-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes trade.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Trade has no runtime load field). { nodeId: loadFraction }
  ...
  /* trade: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(trade, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (trade.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. trade.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Trade has no runtime load field). throttleGain operator-set. Never writes trade.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.TradeRetrogradeThrottle) root.TradeRetrogradeThrottle = API;
