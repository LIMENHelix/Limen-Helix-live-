/**
 * environment-promotion-bridge.js — Inject promoted Industry portal directives into operator surfaces
 *
 ...
 * Exposes: window.LIMENIndustryPromotionBridge
 */
(function () {
  'use strict';
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENIndustryDirectiveExtractor;
    var ranker = window.LIMENIndustryDirectiveRanker;
    var translator = window.LIMENIndustryDirectiveTranslator;
    ...
    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        ...
        var targeting = window.LIMENIndustryTargetingEngine;
        if (targeting) {
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      state.treatments.push({
        id: 'ptreat_' + (dir.portalDomainId || 'industry') + '_' + (dir.nodeId || '') + '_' + pi,
  ...
  window.LIMENIndustryPromotionBridge = {
    promote: promote,
