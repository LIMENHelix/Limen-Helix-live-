/**
 * medicine-promotion-bridge.js — Inject promoted portal directives into operator surfaces
 *
 ...
 *
 * Exposes: window.LIMENMedicinePromotionBridge
 */
(function () {
  'use strict';
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENMedicineDirectiveExtractor;
    var ranker = window.LIMENMedicineDirectiveRanker;
    var translator = window.LIMENMedicineDirectiveTranslator;

    if (!extractor || !ranker || !translator) {
      console.warn('[MedicinePromotionBridge] Missing module(s) \u2014 extractor:', !!extractor, 'ranker:', !!ranker, 'translator:', !!translator);
      _running = false;
      return Promise.resolve([]);
    }

    console.log('[MedicinePromotionBridge] Input signature changed \u2014 running extraction pipeline');
    console.log('[MedicinePromotionBridge] Signature: ' + sig);
    var t0 = Date.now();

    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        if (!candidates || candidates.length === 0) {
          console.log('[MedicinePromotionBridge] No candidates extracted');
          _lastSignature = sig;
          _lastPromoted = [];
          _running = false;
          return [];
        ...
        console.log('[MedicinePromotionBridge] Extracted ' + candidates.length + ' candidates in ' + (Date.now() - t0) + 'ms');
        ...
        if (bestProofIdx > limit - 1 && bestProofIdx !== -1) {
          ...
          console.log('[MedicinePromotionBridge] Elevated proof directive from position ' + bestProofIdx + ': ' + proofItem.treatmentLabel.substring(0, 50));
        }
        ...
        if (bestDeepIdx !== -1 && bestDeepIdx >= limit) {
          ...
          console.log('[MedicinePromotionBridge] Deep proof slot: L' + deepItem.depth + ' ' + deepItem.treatmentLabel.substring(0, 50) + ' (deepScore=' + bestDeepScore.toFixed(3) + ')');
        } else if (bestDeepIdx !== -1 && bestDeepIdx < limit) {
          ranked[bestDeepIdx]._isDeepProofSlot = true;
          console.log('[MedicinePromotionBridge] Deep proof already in top set: L' + ranked[bestDeepIdx].depth);
        }

        console.log('[MedicinePromotionBridge] Top 5 after depth-aware selection:');
        ...
        var targeting = window.LIMENMedicineTargetingEngine;
        if (targeting) {
        ...
        console.log('[MedicinePromotionBridge] Promoted ' + dedupedPromoted.length + ' directives (' + (Date.now() - t0) + 'ms total)');
        return dedupedPromoted;
      })
      .catch(function (err) {
        console.error('[MedicinePromotionBridge] Pipeline failed \u2014 existing opportunities untouched:', err.message || err);
        _lastSignature = sig;
        _lastPromoted = [];
        _running = false;
        return [];
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      var overlay = {
        id: 'ptreat_' + (dir.portalDomainId || 'medicine') + '_' + (dir.nodeId || '') + '_' + pi,
  ...
  window.LIMENMedicinePromotionBridge = {
    promote: promote,
