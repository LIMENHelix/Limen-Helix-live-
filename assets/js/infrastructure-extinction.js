/*
 * infrastructure-extinction.js  — ADDITIVE mechanism (completes EXTINCTION_LEARNING)
 * ----------------------------------------------------------------------------
 ...
 * (complements the GLOBAL down-scaling in infrastructure-offline-maintenance.js).
 *
 * CONTRACT: read-only / copy-only. NEVER writes infrastructure.json. Proposes which acquired responses to
...
(function (root) {
  ...
  /* Extinction over Infrastructure activations: an activation whose diagnosticTriggers are ALL absent
   * from the caller-supplied activeTriggers is a candidate to retire (its trigger is gone).
   * activeTriggers: array of currently-firing trigger strings (runtime state; Infrastructure has none baked in). */
  function proposeExtinction(infrastructure, activeTriggers, opts) {
    ...
    return {
      ...
      note: 'Read-only PROPOSAL. Applies nothing; retiring a node would edit infrastructure.json. activeTriggers is caller-supplied runtime state.'
    };
  }

  var API = { proposeExtinction: proposeExtinction };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.InfrastructureExtinction) root.InfrastructureExtinction = API;
