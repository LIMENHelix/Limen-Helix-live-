/*
 * population-retrograde-throttle.js  — ADDITIVE mechanism
 * ----------------------------------------------------------------------------
 ...
 *
 * CONTRACT: read-only / copy-only. NEVER writes population.json. Computes, for an OVERLOADED
...
(function (root) {
  ...
  var DEFAULTS = {
    // Overload is caller-supplied (Population has no runtime load field). { nodeId: loadFraction }
  ...
  /* population: the domain object (read only). Returns { proposedEdges, actions } on a copy. */
  function computeThrottle(population, params) {
    var p = Object.assign({}, DEFAULTS, params || {});
    var edges = (population.edges || []).map(function (e) { return Object.assign({}, e); }); // copy
    var actions = [];
    ...
    return {
      proposedEdges: edges,   // a COPY; caller decides whether to apply. population.json untouched.
      actions: actions,
      ...
      note: 'Overload is caller-supplied (Population has no runtime load field). throttleGain operator-set. Never writes population.json.'
    };
  }

  var API = { computeThrottle: computeThrottle, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.PopulationRetrogradeThrottle) root.PopulationRetrogradeThrottle = API;
