/**
 * environment-claim-ledger.js — Environment Domain Claim Ledger (Local Only)
 *
 * Stores claim records in localStorage. Tracks opportunity claims
 * from acceptance through outcome. Science-scoped only.
 *
 * Namespace: window.LIMENEnvironment.economy.claims
 */
(function () {
  'use strict';
  ...
  var STORE_KEY = 'LIMEN_ENVIRONMENT_CLAIMS';
  ...
  function createClaimRecord(opportunity) {
    ...
    try { window.dispatchEvent(new CustomEvent('limen:environment-claim-update')); } catch (e) {}
  ...
  window.LIMENEnvironment.economy.claims = {
