/*
 * environment-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Environment's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (environment-offline-maintenance.js)
 *   - refractory absolute window       (environment-refractory-limiter.js)
 *   - prediction-error threshold       (environment-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Environment has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Environment has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.EnvironmentMetaplasticity) root.EnvironmentMetaplasticity = API;
