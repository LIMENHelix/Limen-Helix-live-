/*
 * law-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/law-neuro-substrate.json
 * Prompts:       assets/prompts/law-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/law.json
 * or any existing law-*.js. It reads existing Law data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Law brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Law file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/law-neuro-substrate.json';
  var LAW_URL = '/assets/data/domains/law.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Law issue id. */
     ...
     * Reads an Law issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Law issue. */
    auditAllIssues: function () {
      return loadJSON(LAW_URL).then(function (law) {
        return (law.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.LawNeuroSubstrate) root.LawNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Law brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import LawNeuroSubstrate from '/assets/js/law-neuro-substrate.js';
   *      // then, after issues render:
   *      LawNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
