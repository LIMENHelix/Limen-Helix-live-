/*
 * defense-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/defense-neuro-substrate.json
 * Prompts:       assets/prompts/defense-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/defense.json
 * or any existing defense-*.js. It reads existing Defense data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Defense brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Defense file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/defense-neuro-substrate.json';
  var DEFENSE_URL = '/assets/data/domains/defense.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Defense issue id. */
     ...
     * Reads an Defense issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Defense issue. */
    auditAllIssues: function () {
      return loadJSON(DEFENSE_URL).then(function (defense) {
        return (defense.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.DefenseNeuroSubstrate) root.DefenseNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Defense brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import DefenseNeuroSubstrate from '/assets/js/defense-neuro-substrate.js';
   *      // then, after issues render:
   *      DefenseNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
