 ...
 * Functions are byte-identical to the original finance-promotion-bridge.js; data comes from the
 * factory __DATA param. Registers window.LIMENTaxonomy.makePromotionBridge. */
/**
 ...
 */
(function () { window.LIMENTaxonomy = window.LIMENTaxonomy || {}; window.LIMENTaxonomy.makePromotionBridge = function (__DATA, ctx) {
  'use strict';
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window['LIMEN' + ctx.globalName + 'DirectiveExtractor'];
    var ranker = window['LIMEN' + ctx.globalName + 'DirectiveRanker'];
    var translator = window['LIMEN' + ctx.globalName + 'DirectiveTranslator'];
    ...
    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        ...
        var targeting = window['LIMEN' + ctx.globalName + 'TargetingEngine'];
        if (targeting) {
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      var overlay = {
        id: 'ptreat_' + (dir.portalDomainId || ctx.domain) + '_' + (dir.nodeId || '') + '_' + pi,
