/**
 * economy-agent-box.js — the Economy domain's operator AI box (client side).
 *
 * A floating "Talk to Economy" panel on the economy domain-console. It KNOWS its
 * domain (window.LIMENEconomyBrain.getStateSummary()) and can make bounded
 ...
 *    tries /api/economy-agent first and falls back to LOCAL on any failure, so it
...
(function () {
  ...
  function parseLocal(prompt, state) {
    ...
    if (mF) {
      var topic = mF[1].replace(/\b(please|now|today|this week|domain|economy|closely|carefully)\b/g, '').trim();
  ...
  if (typeof window !== 'undefined') window.__economyAgentLocal = { parse: parseLocal, query: answerQuery };

  ready(function () {
    if (window.__economyAgentBox) return; window.__economyAgentBox = true;
    var LS_KEY = 'limenEconomyAgentKey';
    var esc = function (s) { return String(s == null ? '' : s).replace(/[&<>"]/g, function (c) { return ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[c]; }); };
    function brain() { return window.LIMENEconomyBrain || null; }
    ...
    var toggle = document.createElement('div'); toggle.id = 'eab-toggle'; toggle.textContent = '⚡ Talk to Economy';
    var panel = document.createElement('div'); panel.id = 'eab';
    panel.innerHTML =
      '<div class="eab-h"><span><b>Economy</b> · ask or steer</span><span class="eab-x">✕</span></div>' +
    ...
    function renderResult(pending, result, mode, left) {
      ...
      if (applied) html += '<div class="eab-tool">✓ applied to economy domain:\n' + esc(applied) + '</div>';
    ...
    function send() {
      var b = brain();
      if (!b) { add('<span class="eab-ai">Economy brain not loaded on this page.</span>'); return; }
    ...
    add('<span class="eab-ai">Economy domain online (free local mode). Ask about it, or steer it: "focus on grid", "prefer research", "autonomy off", "how is economy doing", "show the forecast". I bias the domain, never force a finding, move capital, or touch code. Add a passcode above for conversational AI once billing is on.</span>');
  });
})();
