/*
 * science-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/science-neuro-substrate.json
 * Prompts:       assets/prompts/science-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/science.json
 * or any existing science-*.js. It reads existing Science data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Science brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Science file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/science-neuro-substrate.json';
  var SCIENCE_URL = '/assets/data/domains/science.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Science issue id. */
     ...
     * Reads an Science issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Science issue. */
    auditAllIssues: function () {
      return loadJSON(SCIENCE_URL).then(function (science) {
        return (science.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.ScienceNeuroSubstrate) root.ScienceNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Science brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import ScienceNeuroSubstrate from '/assets/js/science-neuro-substrate.js';
   *      // then, after issues render:
   *      ScienceNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
