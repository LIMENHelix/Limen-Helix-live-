/**
 * defense-business-review.js — Defense Business Assignment Review Surface
 *
 * DEFENSE DOMAIN ONLY. Renders a review panel for proposed business mappings
 * across the full 103-node hierarchy.
 * Requires defense-node-business-engine.js to be loaded first.
 *
 * Self-gates: only runs when ?domain=defense
 * Exposes: window.LIMENDefenseBusinessReview
 */
(function () {
  'use strict';
  ...
  function renderCard(entry) {
    ...
    if (entry.neuroTranslation) {
      ...
      h += '<div style="font-size:0.26rem;letter-spacing:2px;color:rgba(74,143,212,0.5);margin-bottom:4px">IN NEUROLOGY / IN DEFENSE TRANSLATION</div>';
      ...
      h += '<div style="font-size:0.32rem;color:#b0a898;line-height:1.5"><b style="color:rgba(201,169,78,0.7)">In defense (learning systems):</b> ' + esc(entry.neuroTranslation.inBusiness) + '</div>';
      h += '</div>';
    }
    ...
    if (entry.existingCompanies && entry.existingCompanies.length > 0) {
      ...
      h += '<div class="dbr-card-existing" style="color:#C9A94E">Missing from current Defense system</div>';
    }
    ...
    if (entry.approvalRequired) {
      ...
      h += '<button class="dbr-btn" style="color:#C9A94E;border-color:rgba(201,169,78,0.3);background:rgba(201,169,78,0.04);margin-left:4px" data-business-node="' + esc(entry.nodeId) + '" data-business-type="' + esc(entry.businessType) + '" data-business-label="' + esc(entry.nodeLabel) + '" data-business-reason="' + esc(entry.reason) + '">BUSINESS \u2192</button>';
      h += '</div>';
    }
    h += '</div></div>';
    return h;
  ...
  function render() {
    var engine = window.LIMENDefenseBusinessEngine;
    if (!engine) return;
    injectStyles();
      ...
      bizBtns[bi].addEventListener('click', function (e) {
        ...
        try {
          sessionStorage.setItem('limen_exec_context', JSON.stringify({
            ...
            source: 'defense', returnTo: '/domain-console?domain=defense&mode=operator'
          }));
        } catch (ex) {}
        window.location.href = 'defense-workspace.html?track=business&opp=' + encodeURIComponent(oppKey) + '&returnTo=' + encodeURIComponent('/domain-console?domain=defense&mode=operator');
      });
    }
  ...
  window.LIMENDefenseBusinessReview = { render: render, mount: mount };
