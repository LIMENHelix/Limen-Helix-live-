/*
 * trade-telemetry-adapter.js  — wires runtime fields to the LIVE Trade telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/trade-pulse-engine.js  (window.LIMENTradePulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Trade issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to trade.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into trade.json (a
...
(function (root) {
   ...
   * inputs: { trade, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (trade.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (trade.activations || []).forEach(function (a) {
  ...
  /* Merge trade.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(trade, overlay) {
    var base = trade.runtime || {};
    ...
    var acts = {}; (trade.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (trade.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the trade definition + brainState. */
  function fromLive(trade, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENTradePulse) ? window.LIMENTradePulse : null;
    if (!P) return null;
    return fromPulse({ trade: trade, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN trade definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/trade.json', 'trade'),
      _load('/assets/data/trade-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ trade: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.TradeTelemetryAdapter) root.TradeTelemetryAdapter = API;
