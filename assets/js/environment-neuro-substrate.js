/*
 * environment-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/environment-neuro-substrate.json
 * Prompts:       assets/prompts/environment-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/environment.json
 * or any existing environment-*.js. It reads existing Environment data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Environment brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Environment file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/environment-neuro-substrate.json';
  var ENVIRONMENT_URL = '/assets/data/domains/environment.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Environment issue id. */
     ...
     * Reads an Environment issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Environment issue. */
    auditAllIssues: function () {
      return loadJSON(ENVIRONMENT_URL).then(function (environment) {
        return (environment.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.EnvironmentNeuroSubstrate) root.EnvironmentNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Environment brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import EnvironmentNeuroSubstrate from '/assets/js/environment-neuro-substrate.js';
   *      // then, after issues render:
   *      EnvironmentNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
