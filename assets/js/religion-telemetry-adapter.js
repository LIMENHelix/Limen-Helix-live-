/*
 * religion-telemetry-adapter.js  — wires runtime fields to the LIVE Religion telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/religion-pulse-engine.js  (window.LIMENReligionPulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Religion issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to religion.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into religion.json (a
...
(function (root) {
   ...
   * inputs: { religion, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (religion.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (religion.activations || []).forEach(function (a) {
  ...
  /* Merge religion.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(religion, overlay) {
    var base = religion.runtime || {};
    ...
    var acts = {}; (religion.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (religion.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the religion definition + brainState. */
  function fromLive(religion, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENReligionPulse) ? window.LIMENReligionPulse : null;
    if (!P) return null;
    return fromPulse({ religion: religion, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN religion definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/religion.json', 'religion'),
      _load('/assets/data/religion-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ religion: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.ReligionTelemetryAdapter) root.ReligionTelemetryAdapter = API;
