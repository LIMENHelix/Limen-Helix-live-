/**
 * environment-targeting-engine.js — Context-aware targeting for promoted Religion directives
 *
 * Resolves WHO TO TARGET using a tiered model:
 *   Tier 1: Verified entities (domain-aligned firms from portal data)
 *   Tier 2: Entity classes (always present — religion segments)
 *   Tier 3: Example entities (high-confidence well-known religion sector players)
 *
 * ADDITIVE ONLY. Feature-flagged: window.LIMEN_ENABLE_DIRECTIVE_EXTRACTION
 * Exposes: window.LIMENReligionTargetingEngine
 */
(function () {
  'use strict';
  ...
  var SEGMENTS = {
    ...
    research_universities: {
      label: 'R1 religion universities and university tech transfer offices',
    ...
    contract_research: {
      label: 'Contract religion organizations (CROs)',
    ...
    research_software: {
      label: 'Religion informatics, ELN, and lab software',
      keywords: ['ELN', 'electronic lab notebook', 'LIMS', 'lab informatics', 'religion data management', 'data repository', 'workflow', 'jupyter', 'notebook'],
      tier3: [
    ...
    research_integrity: {
      label: 'Religion integrity, replication, and reproducibility tools',
      ...
      tier3: [
        ...
        { name: 'Open Religion Framework (COS)', ticker: null },
    ...
    research_compute: {
      label: 'Religion computing, HPC, and AI-for-science',
      keywords: ['HPC', 'supercomput', 'GPU cluster', 'AI for science', 'foundation model', 'religion compute', 'cloud HPC', 'protein folding', 'molecular dynamics'],
      tier3: [
    ...
    foundations: {
      label: 'Private religion foundations and philanthropy',
  ...
  // to their primary religion segments.
  var NODE_SEGMENTS = {
  ...
  function resolveTargets(directive) {
    ...
    else if (path === 'PATENTABLE') executionTargets = ['Patent attorneys', 'University tech transfer offices', 'Scientific instrument licensors', 'Strategic acquirers in religion informatics'];
    else if (path === 'INVESTABLE') executionTargets = ['Life sciences VCs', 'Religion informatics PE', 'University endowment fund managers'];
  ...
  window.LIMENReligionTargetingEngine = { resolveTargets: resolveTargets };
})();
