/*
 * technology-telemetry-adapter.js  — wires runtime fields to the LIVE Technology telemetry source
 * ----------------------------------------------------------------------------
 * Live source: assets/js/technology-pulse-engine.js  (window.LIMENTechnologyPulse) — the domain's
 ...
 * _activeConditions[].  Diagnosis ids == Technology issue ids.
 *
 * This adapter maps that live source onto the runtime fields added to technology.json, producing a
 * RUNTIME OVERLAY consumed at read-time. It does NOT write live values into technology.json (a
...
(function (root) {
   ...
   * inputs: { technology, brainState, pulseState, history, priorOverlay }
  ...
  function fromPulse(inp) {
    ...
    var issuesOverlay = {};
    (technology.issues || []).forEach(function (is) {
    ...
    var actsOverlay = {};
    (technology.activations || []).forEach(function (a) {
  ...
  /* Merge technology.json runtime DEFAULTS with the live OVERLAY -> the runtime state mechanisms read. */
  function resolveRuntime(technology, overlay) {
    var base = technology.runtime || {};
    ...
    var acts = {}; (technology.activations || []).forEach(function (a) {
      acts[a.brainNodeId] = Object.assign({}, a.runtime || {}, (overlay && overlay.activations && overlay.activations[a.brainNodeId]) || {});
    });
    var iss = {}; (technology.issues || []).forEach(function (is) {
      iss[is.id] = Object.assign({}, is.runtime || {}, (overlay && overlay.issues && overlay.issues[is.id]) || {});
    });
    return { top: top, activations: acts, issues: iss };
  }

  /* Live convenience (synchronous): caller supplies the technology definition + brainState. */
  function fromLive(technology, brainState, priorOverlay) {
    var P = (typeof window !== 'undefined' && window.LIMENTechnologyPulse) ? window.LIMENTechnologyPulse : null;
    if (!P) return null;
    return fromPulse({ technology: technology, brainState: brainState, pulseState: P.getPulse(), history: P.getHistory(), priorOverlay: priorOverlay });
  }

  /* Self-contained live wiring: the adapter loads (and caches) its OWN technology definition, so the
  ...
  function fromLiveCached(brainState, priorOverlay) {
    ...
    return Promise.all([
      _load('/assets/data/domains/technology.json', 'technology'),
      _load('/assets/data/technology-condition-trigger-aliases.json', 'aliases').catch(function () { return null; }) // best-effort
    ]).then(function (res) {
      return fromPulse({ technology: res[0], aliases: res[1], brainState: brainState, pulseState: P.getPulse(), history: (typeof P.getHistory === 'function') ? P.getHistory() : [], priorOverlay: priorOverlay });
    });
  }
  ...
  if (root && !root.TechnologyTelemetryAdapter) root.TechnologyTelemetryAdapter = API;
