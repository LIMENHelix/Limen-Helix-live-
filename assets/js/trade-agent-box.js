/**
 * trade-agent-box.js — the Trade domain's operator AI box (client side).
 *
 * A floating "Talk to Trade" panel on the trade domain-console. It KNOWS its
 * domain (window.LIMENTradeBrain.getStateSummary()) and can make bounded
 ...
 *    tries /api/trade-agent first and falls back to LOCAL on any failure, so it
...
(function () {
  ...
  function parseLocal(prompt, state) {
    ...
    if (mF) {
      var topic = mF[1].replace(/\b(please|now|today|this week|domain|trade|closely|carefully)\b/g, '').trim();
  ...
  if (typeof window !== 'undefined') window.__tradeAgentLocal = { parse: parseLocal, query: answerQuery };

  ready(function () {
    if (window.__tradeAgentBox) return; window.__tradeAgentBox = true;
    var LS_KEY = 'limenTradeAgentKey';
    var esc = function (s) { return String(s == null ? '' : s).replace(/[&<>"]/g, function (c) { return ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[c]; }); };
    function brain() { return window.LIMENTradeBrain || null; }
    ...
    var toggle = document.createElement('div'); toggle.id = 'eab-toggle'; toggle.textContent = '⚡ Talk to Trade';
    var panel = document.createElement('div'); panel.id = 'eab';
    panel.innerHTML =
      '<div class="eab-h"><span><b>Trade</b> · ask or steer</span><span class="eab-x">✕</span></div>' +
    ...
    function renderResult(pending, result, mode, left) {
      ...
      if (applied) html += '<div class="eab-tool">✓ applied to trade domain:\n' + esc(applied) + '</div>';
    ...
    function send() {
      var b = brain();
      if (!b) { add('<span class="eab-ai">Trade brain not loaded on this page.</span>'); return; }
    ...
    add('<span class="eab-ai">Trade domain online (free local mode). Ask about it, or steer it: "focus on grid", "prefer research", "autonomy off", "how is trade doing", "show the forecast". I bias the domain, never force a finding, move capital, or touch code. Add a passcode above for conversational AI once billing is on.</span>');
  });
})();
