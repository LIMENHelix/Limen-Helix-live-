/**
 * defense-promotion-bridge.js — Inject promoted Defense portal directives
 * Exposes: window.LIMENDefensePromotionBridge
 */
(function () {
  'use strict';
  ...
  function _computeSignature(state) {
    ...
    return ['defense', dxKeys.join(','), nodeKeys.join(','), stressBucket.toFixed(2), window.LIMEN_ENABLE_DIRECTIVE_EXTRACTION ? '1' : '0'].join('|');
  }
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENDefenseDirectiveExtractor;
    var ranker = window.LIMENDefenseDirectiveRanker;
    var translator = window.LIMENDefenseDirectiveTranslator;
    ...
    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        ...
        var targeting = window.LIMENDefenseTargetingEngine;
        if (targeting) {
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      state.treatments.push({
        id: 'ptreat_' + (dir.portalDomainId || 'defense') + '_' + (dir.nodeId || '') + '_' + pi,
  ...
  window.LIMENDefensePromotionBridge = { promote: promote, getLastPromoted: getLastPromoted };
})();
