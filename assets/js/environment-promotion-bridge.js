/**
 * environment-promotion-bridge.js — Inject promoted Environment portal directives into operator surfaces
 *
 ...
 * Exposes: window.LIMENEnvironmentPromotionBridge
 */
(function () {
  'use strict';
  ...
  function promote(state, portal, opts) {
    ...
    var extractor = window.LIMENEnvironmentDirectiveExtractor;
    var ranker = window.LIMENEnvironmentDirectiveRanker;
    var translator = window.LIMENEnvironmentDirectiveTranslator;

    if (!extractor || !ranker || !translator) {
      console.warn('[SciencePromotionBridge] Missing pipeline component');
      _running = false;
      return Promise.resolve([]);
    }

    console.log('[SciencePromotionBridge] Input signature changed \u2014 running extraction pipeline');
    ...
    return extractor.extract(state, portal, opts)
      .then(function (candidates) {
        ...
        // Pass 1: Elevate richest directive
        var bestProofIdx = -1;
        var bestProof = 0;
        ...
        var targeting = window.LIMENEnvironmentTargetingEngine;
        if (targeting) {
        ...
        console.log('[SciencePromotionBridge] Promoted ' + dedupedPromoted.length + ' directives');
        return dedupedPromoted;
      })
      .catch(function (err) {
        console.error('[SciencePromotionBridge] Pipeline failed:', err.message || err);
        _lastSignature = sig;
        _lastPromoted = [];
        _running = false;
        return [];
  ...
  function _injectTreatmentOverlays(state, promoted) {
    ...
    for (var pi = 0; pi < promoted.length; pi++) {
      ...
      state.treatments.push({
        id: 'ptreat_' + (dir.portalDomainId || 'environment') + '_' + (dir.nodeId || '') + '_' + pi,
  ...
  window.LIMENEnvironmentPromotionBridge = {
    promote: promote,
