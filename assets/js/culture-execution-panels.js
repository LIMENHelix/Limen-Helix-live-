 ...
 *
 * Culture domain only. Expandable workflow panels for each opportunity.
 ...
 * Exposes: window.LIMENCultureExecutionPanels
 */
(function () {
  'use strict';
  ...
  var isScienceDomain = _dom === 'culture' || _dom === 'culture';
  var isEnvironmentWorkspace = window.location.pathname.indexOf('culture-workspace') !== -1;
  if (!isScienceDomain && !isEnvironmentWorkspace) return;

  var STORE_KEY = 'limen_culture_exec_panels';
  ...
  var GRANT_SECTIONS = [
    { id: 'what', title: 'What This Grant Is', type: 'info', guidance: 'A culture grant is non-repayable funding from a federal agency or private foundation. Major culture funders: NSF, NIH, DARPA, ARPA-E, ARPA-H, DOE Office of Science, NASA, HHMI, Gates, Wellcome, Simons, Sloan, Moore, Chan Zuckerberg.', prompt: 'No input needed.' },
    { id: 'why', title: 'Why This Opportunity Qualifies', type: 'textarea', guidance: 'Explain why this Culture / Culture opportunity is a good fit for grant funding. Address: scientific significance, innovation, broader impacts, alignment with the agency mission.', prompt: 'Write 3-5 sentences explaining significance, innovation, and alignment.', prefillFrom: 'whyNow' },
    { id: 'eligibility', title: 'Eligibility', type: 'checklist', guidance: 'Most federal culture grants have eligibility rules. Check each that applies.', items: ['Organization is a US accredited institution', 'Organization has a UEI (formerly DUNS) number', 'Organization is registered on SAM.gov / Research.gov', 'PI has terminal degree (PhD / MD / equivalent)', 'No debarment or exclusion status', 'Has financial management compliant with Uniform Guidance (2 CFR 200)', 'Has approved IRB / IACUC / IBC if human or animal subjects'] },
    ...
    { id: 'documents', title: 'Required Documents', type: 'checklist', guidance: 'Most culture grants require many supporting documents.', items: ['Specific aims page (1 page)', 'Culture strategy / approach (12 pages)', 'Bibliography', 'PI biosketch (NIH/NSF format)', 'Co-investigator biosketches', 'Facilities & resources statement', 'Equipment statement', 'Letters of support / collaboration', 'Data management & sharing plan', 'IRB / IACUC / IBC approval (if applicable)', 'Vertebrate animals section (if applicable)', 'Human subjects section (if applicable)'] },
  ...
  var PATENT_SECTIONS = [
    ...
    { id: 'novelty', title: 'Novelty', type: 'textarea', guidance: 'What makes this different from existing scientific tools, methods, or instruments?', prompt: 'What is new compared to existing culture / instrumentation?' },
    ...
    if (!ws._prefilled && opp) {
      ...
      for (var si = 0; si < sections.length; si++) { var sec = sections[si];
        if (sec.prefillFrom && !ws.fields[sec.id]) {
          ...
          if (sec.prefillFrom === 'whyNow') ws.fields[sec.id] = (opp.title || '') + '. This Culture condition creates demand because: ' + (opp.reason || opp.whyNow || 'active diagnosis pathway detected.');
          if (sec.prefillFrom === 'diagnosis') ws.fields[sec.id] = 'Culture domain diagnosis: ' + (opp.diagnosisId || 'active').replace(/_/g, ' ') + '. ' + (opp.title || '');
        }
      }
  ...
  window.LIMENCultureExecutionPanels = { renderForOpportunity: renderForOpportunity, wireForOpportunity: wireForOpportunity, GRANT_SECTIONS: GRANT_SECTIONS, PATENT_SECTIONS: PATENT_SECTIONS };
