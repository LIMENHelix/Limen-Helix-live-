/**
 * energy-clarity-operator.js — Money-Driven Action Surface for Energy Domain
 *
...
(function () {
  ...
  var STATUS_KEY = 'limen_energy_operator_status';
  var COLLAPSE_KEY = 'limen_energy_collapse_state';
  ...
  var DX_CONTEXT = {
    'OIL_SHOCK': {
      what: 'Oil prices are spiking',
      money: 'Energy producers with spare capacity capture premium pricing. Midstream operators see margin expansion. Alternative energy companies gain competitive ground.',
      step: 'Check crude price on finance.yahoo.com. If above $90, position in XLE or individual producers (XOM, CVX, COP).',
      outcome: '10-30% sector premium during sustained price shock'
    },
    'GRID_COLLAPSE': {
      what: 'Grid infrastructure is under stress — data center load growth and transmission congestion are primary drivers',
      money: 'Utilities with hyperscale data center exposure see accelerated T&D capex. Substation upgrades, interconnection queue resolution, and grid modernization create procurement and patent windows.',
      step: 'Check Dominion (D), Duke (DUK), Constellation (CEG), Vistra (VST) for data center load exposure. Search sam.gov for DOE grid modernization and transmission contracts.',
      outcome: '$250K-$5M government contracts, 10-30% utility premium on data center load, or patent licensing in grid capacity planning'
    },
    'PIPELINE_DISRUPTION': {
      what: 'Pipeline and distribution routes are disrupted',
      money: 'Alternative routing and midstream capacity commands premium. Pipeline operators with redundancy outperform.',
      step: 'Check midstream ETFs (AMLP). Position in operators with alternative routes (ET, KMI, WMB).',
      outcome: '15-25% midstream premium during disruption'
    },
    'RENEWABLE_INTERMITTENCY': {
      what: 'Renewable energy output is unreliable',
      money: 'Battery storage and grid balancing demand surges. Storage technology patents have wide-open filing windows.',
      step: 'Check IEA storage deployment data. Position in battery/storage (ENPH, SEDG) or file provisional patent.',
      outcome: '20-40% storage sector growth or $500K-$5M patent value'
    },
    'NUCLEAR_INCIDENT': {
      what: 'Nuclear safety concern is active',
      money: 'Nuclear engineering and safety monitoring firms see procurement. Uranium prices shift. Safety compliance demand rises.',
      step: 'Monitor IAEA alerts. Position in uranium (URA ETF) if supply concern, or safety compliance firms.',
      outcome: '10-30% nuclear sector repricing'
    },
    'SYSTEMIC_ENERGY_STRESS': {
      what: 'Broad energy system is under sustained pressure',
      money: 'Cross-sector energy stress creates wide opportunity windows across infrastructure, technology, and policy. Multiple paths open simultaneously.',
      step: 'Check VDE (Vanguard Energy ETF) for broad positioning. Search grants.gov for energy resilience funding.',
      outcome: 'Multiple 10-25% returns across diversified energy positions'
    }
  };
  ...
  // Diagnosis → playbook ID for Energy-domain invest opportunities
  var DX_TO_PLAYBOOK = {
    'OIL_SHOCK':                'infra_demand',
    'GRID_COLLAPSE':            'infra_demand',
    'PIPELINE_DISRUPTION':      'infra_demand',
    'RENEWABLE_INTERMITTENCY':  'climate_energy',
    'NUCLEAR_INCIDENT':         'infra_demand',
    'SYSTEMIC_ENERGY_STRESS':   'infra_demand'
  };

  // Source-type → playbook ID for non-diagnosis opportunities
  var SOURCE_TO_PLAYBOOK = {
    'company_terminal':  'infra_demand',
    'company_stressed':  'infra_demand',
    'convergence':       'climate_energy',
    'cross_domain':      'econ_supply'
  };

  // Registered playbook definitions for sessionStorage handoff
  var PLAYBOOK_DEFS = {
    'infra_demand': { title: 'Energy Infrastructure Demand', domains: ['energy', 'infrastructure'], type: 'invest' },
    'climate_energy': { title: 'Climate Energy Transition', domains: ['energy', 'environment'], type: 'invest' },
    'econ_supply': { title: 'Economic Supply Pressure', domains: ['economy', 'energy'], type: 'invest' },
    'gov_econ': { title: 'Policy-Market Feedback', domains: ['governance', 'economy'], type: 'invest' },
    'fin_econ': { title: 'Financial-Economic Coupling', domains: ['finance', 'economy'], type: 'invest' }
  };

  // Suggested targets per playbook — companies validated via Helix command board
  // validation: HELIX_VALIDATED (in command-board-data, phase-scored), NODE_MAPPED (in energy.json portal),
  //             DOMAIN_MAPPED (in energy command board), ETF_PROXY (sector proxy, no company-level validation)
  var INVEST_TARGETS = {
    'infra_demand': [
      { ticker: 'XLE',  name: 'Energy Select Sector SPDR', cik: null,      validation: 'ETF_PROXY',       reason: 'Broad energy sector ETF — thematic proxy for energy infrastructure demand' },
      { ticker: 'NEE',  name: 'NextEra Energy',            cik: '753308',  validation: 'HELIX_VALIDATED', reason: 'Largest US renewable + utility operator; grid expansion and generation capacity' },
      { ticker: 'DUK',  name: 'Duke Energy',               cik: '1004980', validation: 'HELIX_VALIDATED', reason: 'Major regulated utility; distribution and transmission infrastructure operator' },
      { ticker: 'PWR',  name: 'Quanta Services',           cik: '1040971', validation: 'NODE_MAPPED',     reason: 'Grid construction and transmission line builder; mapped to THAL (Grid Distribution)' },
      { ticker: 'EPD',  name: 'Enterprise Products',       cik: '797468',  validation: 'HELIX_VALIDATED', reason: 'Midstream pipeline operator; infrastructure backbone for natural gas and NGL' },
      { ticker: 'GEV',  name: 'GE Vernova',                cik: '1957894', validation: 'NODE_MAPPED',     reason: 'Power generation equipment; mapped to M1 (Power Generation) node' },
      { ticker: 'PAVE', name: 'Global X US Infrastructure', cik: null,     validation: 'ETF_PROXY',       reason: 'Infrastructure ETF — thematic proxy for grid and utility buildout' }
    ],
    'climate_energy': [
      { ticker: 'ICLN', name: 'iShares Global Clean Energy', cik: null,     validation: 'ETF_PROXY',       reason: 'Clean energy ETF — thematic proxy for renewable transition' },
      { ticker: 'TAN',  name: 'Invesco Solar ETF',           cik: null,     validation: 'ETF_PROXY',       reason: 'Solar-focused ETF — tracks solar industry performance' },
      { ticker: 'FSLR', name: 'First Solar',                 cik: '1274494', validation: 'HELIX_VALIDATED', reason: 'US solar panel manufacturer; benefits from domestic production incentives' },
      { ticker: 'ENPH', name: 'Enphase Energy',              cik: '1657312', validation: 'HELIX_VALIDATED', reason: 'Microinverter and storage systems; mapped to dACC (Demand Response) node' },
      { ticker: 'CEG',  name: 'Constellation Energy',        cik: '1868275', validation: 'HELIX_VALIDATED', reason: 'Largest US nuclear fleet operator; zero-carbon baseload generation' },
      { ticker: 'CCJ',  name: 'Cameco',                      cik: '1211348', validation: 'HELIX_VALIDATED', reason: 'Uranium mining and fuel supply for nuclear generation' }
    ],
    'econ_supply': [
      { ticker: 'XOM',  name: 'Exxon Mobil',    cik: '34088',   validation: 'HELIX_VALIDATED', reason: 'Integrated energy major; upstream production capacity absorbs supply pressure' },
      { ticker: 'CVX',  name: 'Chevron',         cik: '93410',   validation: 'HELIX_VALIDATED', reason: 'Integrated energy major; refining and midstream resilience during supply stress' },
      { ticker: 'COP',  name: 'ConocoPhillips',  cik: '1163165', validation: 'HELIX_VALIDATED', reason: 'Pure-play E&P; production capacity benefits from supply-constrained pricing' },
      { ticker: 'SLB',  name: 'Schlumberger',    cik: '87347',   validation: 'HELIX_VALIDATED', reason: 'Oilfield services leader; activity increases during supply pressure' },
      { ticker: 'HYG',  name: 'iShares High Yield Bond', cik: null, validation: 'ETF_PROXY',   reason: 'Credit stress proxy — monitors economic transmission of energy supply pressure' }
    ]
  };
  ...
  function renderTargets(pbId) {
    ...
    h += '<div class="eos-invest-meaning">INVEST means: take a position in companies or ETFs expected to benefit from this Energy condition. These are not buy recommendations \u2014 candidates for your own due diligence.</div>';
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if (t.validation === 'NODE_MAPPED') h += '<div style="margin-bottom:4px;color:#4a8fd4">Mapped to an Energy portal node. Company-level Helix validation pending.</div>';
      // Action links
      h += '<div style="margin-top:4px">';
      if (t.cik) h += '<a class="eos-target-link" href="helix-report.html?cik=' + esc(t.cik) + '&company=' + esc(t.ticker.toLowerCase()) + '&source_surface=domain_clarity_operator&domain=energy&requested_report_type=partial_phase_snapshot" target="_blank">HELIX REPORT</a>';
  ...
  var MECH_EXPLAIN = {
    'OIL_SHOCK': {
      'supply_disruption':    { why: 'Oil supply is cut. Producers can\'t deliver. Buyers scramble for alternatives. Whoever maps the gap first controls the information.', move: 'Build a supply gap scorecard. Sell it to traders, procurement teams, and portfolio managers who need to know which contracts are exposed.' },
      'price_dislocation':    { why: 'Oil prices are spiking or crashing. Historical pricing models break. Anyone holding contracts or inventory is repricing in real time.', move: 'Build a stress-repricing model. Sell it to trading desks, commodity funds, and energy buyers who need to re-mark their books.' },
      'contract_mispricing':  { why: 'Existing contracts were priced for normal supply. The shock makes them worth more or less than face value. Counterparties need to renegotiate.', move: 'Build a contract exposure audit. Sell it to utilities, refiners, and trading firms holding mispriced long-term supply agreements.' },
      'logistics_constraint': { why: 'Pipelines, tankers, or terminals are blocked or rerouted. Physical delivery is disrupted. Operators need to find alternative paths or reduce throughput.', move: 'Build a logistics rerouting model. Sell it to pipeline operators, shipping companies, and midstream firms managing physical flow.' },
      'refining_shift':       { why: 'Refiners are receiving different crude grades than planned. Yields change. Margins shift. Process settings need to be recalibrated.', move: 'Build a crude-switch optimization model. Sell it to refiners who need to maximize yield from unfamiliar feedstock under time pressure.' },
      'inventory_imbalance':  { why: 'Strategic reserves are being drawn down or commercial inventories are spiking. Storage economics flip. Someone is caught long or short.', move: 'Build an inventory position tracker. Sell it to traders and storage operators who need to time drawdowns or injections.' },
      'production_impact':    { why: 'Wells, mines, or plants are shutting down or ramping emergency output. Production capacity is shifting. The market needs to know who is still running.', move: 'Build a production status dashboard. Sell it to commodity desks and infrastructure funds tracking which assets are still operational.' }
    },
    'GRID_COLLAPSE': {
      'infrastructure_stress':{ why: 'Grid hardware is failing under load. Substations, relays, and transmission lines are at risk. Utilities need to harden or reroute.', move: 'Build a grid vulnerability scorecard. Sell it to utilities, EPC contractors, and grid operators prioritizing capital deployment.' },
      'supply_disruption':    { why: 'Generation capacity dropped. The grid can\'t serve demand. Emergency dispatch and load shedding are in play.', move: 'Build a generation gap analysis. Sell it to grid operators and emergency planners who need to allocate scarce capacity.' },
      'production_impact':    { why: 'Power plants are tripping offline. Baseload is disappearing. Peakers and reserves are being called.', move: 'Build a plant availability tracker. Sell it to dispatch operators and energy traders pricing the shortfall.' },
      'market_structure':     { why: 'Energy market prices are in crisis mode. Scarcity pricing, emergency auctions, and capacity market rules are in play.', move: 'Build a crisis pricing model. Sell it to trading desks and regulators managing the emergency market.' },
      'policy_response':      { why: 'Regulators are issuing emergency orders. New rules are being written in real time. Utilities need to comply immediately.', move: 'Build a regulatory response tracker. Sell it to utilities and compliance teams managing new mandates.' }
    },
    'PIPELINE_DISRUPTION': {
      'logistics_constraint': { why: 'A major pipeline is down. Fuel can\'t get from source to buyer. Alternative routes are congested or nonexistent.', move: 'Build a pipeline outage impact model. Sell it to midstream operators, shippers, and downstream buyers managing supply gaps.' },
      'supply_disruption':    { why: 'Physical fuel supply is interrupted at the source. Downstream facilities are running out of feedstock.', move: 'Build a supply continuity scorecard. Sell it to refiners and utilities dependent on the affected pipeline.' },
      'contract_mispricing':  { why: 'Delivery contracts assumed pipeline availability. Force majeure clauses are being invoked. Contract values are shifting.', move: 'Build a contract renegotiation package. Sell it to counterparties on both sides of affected pipeline contracts.' },
      'infrastructure_stress':{ why: 'The pipeline network is under physical stress. Adjacent infrastructure may be at risk of cascading failure.', move: 'Build an infrastructure risk assessment. Sell it to operators managing connected pipeline and terminal assets.' },
      'inventory_imbalance':  { why: 'Tank farms and storage facilities are filling up or running dry depending on which side of the disruption they sit.', move: 'Build a storage position optimizer. Sell it to terminal operators and commodity traders managing physical inventory.' }
    },
    'NUCLEAR_INCIDENT': {
      'production_impact':    { why: 'Nuclear capacity is offline. Baseload generation gap is immediate. Fossil and renewable alternatives must fill the hole.', move: 'Build a generation replacement model. Sell it to grid operators and utilities scrambling to backfill lost nuclear capacity.' },
      'supply_disruption':    { why: 'Electricity supply dropped sharply. Rolling blackouts or emergency imports are possible.', move: 'Build an emergency supply plan. Sell it to grid operators and regional transmission organizations.' },
      'infrastructure_stress':{ why: 'The incident stresses adjacent grid infrastructure as power flows reroute around the lost plant.', move: 'Build a grid stress model. Sell it to transmission operators managing the new flow patterns.' },
      'policy_response':      { why: 'Regulators are ordering safety reviews, potential shutdowns of similar reactors, and new compliance rules.', move: 'Build a regulatory impact tracker. Sell it to nuclear operators and utilities facing new mandates.' }
    },
    'RENEWABLE_INTERMITTENCY': {
      'supply_disruption':    { why: 'Solar and wind output collapsed. The grid is short on generation. Dispatchable backup is being called.', move: 'Build a renewable shortfall model. Sell it to grid operators and backup generation owners pricing their response.' },
      'production_impact':    { why: 'Variable generation is near zero. Capacity factors are cratering. Curtailment economics flip to scarcity.', move: 'Build a generation recovery forecast. Sell it to renewable operators and offtake counterparties managing delivery risk.' },
      'inventory_imbalance':  { why: 'Battery storage is being depleted faster than it can recharge. Reserve margins are disappearing.', move: 'Build a storage depletion model. Sell it to battery operators and grid planners managing reserve adequacy.' },
      'infrastructure_stress':{ why: 'The grid is struggling to balance without its largest variable source. Frequency and voltage are at risk.', move: 'Build a grid stability model. Sell it to system operators managing real-time balance.' },
      'market_structure':     { why: 'Energy prices are spiking on scarcity. Demand response is being activated. Market rules are being stretched.', move: 'Build a scarcity pricing model. Sell it to traders and aggregators operating in the emergency price environment.' }
    }
  };
  ...
  var MECH_FALLBACK = {
    ...
    'refining_shift':       { why: 'Input mix has changed. Processing must adapt.', move: 'Build an optimization model and sell it to processors.' },
    'inventory_imbalance':  { why: 'Storage positions are wrong for current conditions.', move: 'Build a position tracker and sell it to storage operators.' },
  ...
  function _loadBranchIndex() {
    ...
    return fetch('/assets/data/deep/energy-branch-index.json')
  ...
  function buildMoneySummary(state) {
    ...
    // Get real price from feeds
    var crudePrice = null;
    var feeds = state.feeds || [];
    for (var fpi = 0; fpi < feeds.length; fpi++) {
      if (feeds[fpi].value && feeds[fpi].live) { crudePrice = feeds[fpi].value; break; }
    }
    ...
    // Build summary from REAL feed data, not static templates
    var text = '';
    if (crudePrice && crudePrice > 100) {
      text = '<b>Crude oil at $' + crudePrice.toFixed(0) + '/bbl \u2014 above $100.</b> This is a crisis-level price. Energy producers are capturing premium pricing. Infrastructure and logistics companies face immediate cost transmission. ';
      if (activeDx.length > 0) {
        text += activeDx.length + ' active diagnosis pathway' + (activeDx.length > 1 ? 's' : '') + ' confirmed by live evidence.';
      }
    } else if (crudePrice && crudePrice > 90) {
      text = '<b>Crude oil at $' + crudePrice.toFixed(0) + '/bbl \u2014 elevated above $90.</b> Supply pressure is real. Energy companies with capacity or alternatives are positioned to benefit. ';
      if (activeDx.length > 0) text += activeDx.length + ' active diagnosis pathway' + (activeDx.length > 1 ? 's' : '') + '.';
    } else if (crudePrice && crudePrice > 75) {
      text = '<b>Crude oil at $' + crudePrice.toFixed(0) + '/bbl.</b> Above structural baseline but within normal volatility. ';
      if (activeDx.length > 0) {
      ...
      text = '<b>Crude oil at $' + crudePrice.toFixed(0) + '/bbl.</b> Within normal range. No acute pressure. ';
      if (activeDx.length === 0) text += 'No immediate money on the table from price alone.';
    } else {
      // No live price data
      text = '<b>Energy feeds loading.</b> Waiting for live crude oil price data. ';
  ...
  function buildTopPlays(state) {
    ...
    for (var i = 0; i < top.length; i++) {
      ...
      if ((o.path === 'GRANT-ELIGIBLE' || o.path === 'PATENTABLE') && window.LIMENEnergyExecutionPanels) {
        h += window.LIMENEnergyExecutionPanels.renderForOpportunity(oppKey(o), o.path, o);
      }
      h += '</div>';
    }
    h += '</div>';
  ...
  function buildActionQueue(state) {
    ...
    if (ledger) {
      var claims = ledger.getClaimsByDomain('energy');
      var oppIds = {};
    ...
    for (var i = 0; i < opps.length; i++) {
      ...
      var _claimExisting = window.LIMENClaimLedger ? window.LIMENClaimLedger.isOpportunityClaimed(o.id || key, 'energy') : null;
      if (_claimExisting) {
  ...
  function renderOperator() {
    ...
    var bridge = window.LIMENEnergyPromotionBridge;
    console.log('[EnergyOperator] renderOperator: bridge=' + !!bridge + ' flag=' + !!window.LIMEN_ENABLE_DIRECTIVE_EXTRACTION + ' bridgeInit=' + _bridgeInitialized);
    if (bridge && window.LIMEN_ENABLE_DIRECTIVE_EXTRACTION) {
      ...
      var activeDx = (state.diagnoses || []).filter(function (d) { return d.active; });
      console.log('[EnergyOperator] brain=' + !!brain + ' portalCache=' + !!portalCache + ' activeDx=' + activeDx.length + ' stress=' + state.stress);
      ...
      if (portalCache) {
        ...
        if (!_bridgeInitialized) {
          ...
          console.log('[EnergyOperator] First bridge run — extracting...');
          bridge.promote(state, portalCache, bridgeOpts).then(function (promoted) {
            console.log('[EnergyOperator] Bridge returned ' + (promoted ? promoted.length : 0) + ' promoted directives');
            if (promoted && promoted.length > 0) {
              var freshState = getState();
              if (freshState) {
                console.log('[EnergyOperator] Re-rendering with ' + promoted.length + ' directives');
                _renderOperatorDOM(freshState);
              }
            }
          }).catch(function (err) {
            console.error('[EnergyOperator] Bridge error:', err);
          });
        } else {
          bridge.promote(state, portalCache, bridgeOpts);
        }
      } else if (!_bridgeInitialized) {
        console.log('[EnergyOperator] Portal cache not ready — scheduling 5s retry');
        setTimeout(function () {
          var retryBrain = brains ? brains.get('energy') : null;
          var retryCache = retryBrain ? retryBrain._portalCache : null;
          console.log('[EnergyOperator] Retry: brain=' + !!retryBrain + ' cache=' + !!retryCache);
          if (retryCache && bridge) {
            ...
            console.log('[EnergyOperator] Retry: state=' + !!retryState + ' activeDx=' + retryDx);
            if (retryState) {
              bridge.promote(retryState, retryCache, { limit: 5 }).then(function (promoted) {
                console.log('[EnergyOperator] Retry bridge returned ' + (promoted ? promoted.length : 0));
                if (promoted && promoted.length > 0) {
                  var fs = getState();
                  if (fs) _renderOperatorDOM(fs);
                }
              }).catch(function (err) {
                console.error('[EnergyOperator] Retry bridge error:', err);
              });
            }
          }
        }, 5000);
  ...
  function _renderOperatorDOM(state) {
    ...
    for (var ib = 0; ib < investBtns.length; ib++) {
      investBtns[ib].addEventListener('click', function (e) {
        ...
        var def = PLAYBOOK_DEFS[pbId] || { title: oppTitle, domains: ['energy'], type: 'invest' };
        ...
        if (!outcome) outcome = 'Linked energy companies capture sector premium during sustained stress. Monitor for confirmation and position sizing.';
        ...
        window.location.href = 'investment-console.html?opp=' + encodeURIComponent(pbId) + '&source=energy&returnTo=' + encodeURIComponent('/domain-console?domain=energy');
      });
    }
      ...
      execBtns[eb].addEventListener('click', function (e) {
        ...
        try {
          ...
          sessionStorage.setItem('limen_exec_context', JSON.stringify({
            ...
            source: 'energy',
            returnTo: '/domain-console?domain=energy&mode=operator'
          }));
        } catch (ex) {}
        var trackName = trackMap[execPath] || 'grant';
        window.location.href = 'energy-workspace.html?track=' + trackName + '&opp=' + encodeURIComponent(execKey) + '&returnTo=' + encodeURIComponent('/domain-console?domain=energy&mode=operator');
      });
    }
    ...
    for (var cb = 0; cb < claimBtns.length; cb++) {
      claimBtns[cb].addEventListener('click', function (e) {
        ...
        // Open claim modal
        window.LIMENClaimFlow.openClaimModal(opp, 'energy', function (confirmedOpp, estimate) {
          // Create claim in ledger
          if (window.LIMENClaimLedger) {
            window.LIMENClaimLedger.createClaim(confirmedOpp, 'energy', estimate);
          }
    ...
    if (window.LIMENOperatorPanel) {
      window.LIMENOperatorPanel.mount(_operatorView, 'energy');
    }
    ...
    if (window.LIMENEnergyBusinessReview) {
      window.LIMENEnergyBusinessReview.mount(_operatorView);
    }
  }
  ...
  function boot() {
    ...
    console.log('[EnergyOperator] Booted — operator view created, toggle wired');
  }
