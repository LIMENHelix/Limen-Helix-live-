/**
 * communication-claim-ledger.js — Communication Domain Claim Ledger (Local Only)
 * Namespace: window.LIMENCommunication.economy.claims
 */
(function () {
  'use strict';
  ...
  var STORE_KEY = 'LIMEN_COMMUNICATION_CLAIMS';
  ...
  function createClaimRecord(opportunity) {
    ...
    all.push(record);
    saveAll(all);
    try { window.dispatchEvent(new CustomEvent('limen:communication-claim-update')); } catch (e) {}
    return record;
  }
  ...
  window.LIMENCommunication.economy.claims = {
  ...
  console.log('[CommunicationClaimLedger] Loaded');
})();
