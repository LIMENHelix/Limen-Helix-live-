/**
 * finance-execution-panels.js — GRANT and PATENT Execution Workspaces
 *
 * Finance domain only. Expandable workflow panels for each opportunity.
 ...
 *
 * Self-gates: only runs when ?domain=finance
 * Exposes: window.LIMENFinanceExecutionPanels
 */
(function () {
  'use strict';

  var params = new URLSearchParams(window.location.search);
  var isFinanceDomain = params.get('domain') === 'finance';
  var isFinanceWorkspace = window.location.pathname.indexOf('finance-workspace') !== -1;
  if (!isFinanceDomain && !isFinanceWorkspace) return;

  var STORE_KEY = 'limen_finance_exec_panels';
  ...
  // ══════════════════════════════════════════════════════════════════════
  // SECTION DEFINITIONS — GRANT (12 sections, finance-specific)
  // ══════════════════════════════════════════════════════════════════════

  var GRANT_SECTIONS = [
    { id: 'what', title: 'What This Grant Is', type: 'info',
      guidance: 'A grant is non-dilutive capital from a government agency, foundation, or institutional funder to advance a specific financial infrastructure project. You do not repay it. In exchange, you deliver the project outcomes you committed to. This section provides context for operators new to grant funding in the financial services sector.',
      prompt: 'No input needed — this is context for the operator.' },
    { id: 'why', title: 'Why This Opportunity Qualifies', type: 'textarea',
      guidance: 'Explain in plain language why this Finance opportunity is a strong candidate for grant funding. Consider: Does it address financial inclusion, regulatory modernization, consumer protection, or systemic risk reduction? Is there a government or institutional priority that aligns?',
      prompt: 'Write 2-3 sentences explaining why this opportunity deserves grant funding in the financial sector.', prefillFrom: 'whyNow' },
    { id: 'eligibility', title: 'Eligibility', type: 'checklist',
      guidance: 'Financial services grants have specific eligibility requirements. Check each item that applies to your organization or project.',
      items: ['Organization is a registered US entity', 'Organization has a DUNS/UEI number', 'Organization is registered on SAM.gov', 'No debarment or exclusion status', 'Meets CDFI certification requirements (if applicable)', 'Compliant with Bank Secrecy Act and AML regulations', 'Has audited financial statements available', 'Meets SBA size standards (if applicable)', 'Can provide cost-share or match if required', 'Has financial management and internal controls in place'] },
    { id: 'applicant', title: 'Applicant Profile', type: 'fields',
      guidance: 'Basic information about who is applying. The grant reviewer needs to verify you are a qualified financial institution or services organization.',
      fields: [
        { id: 'org_name', label: 'Organization name', type: 'text', placeholder: 'LIMEN Helix Financial Services LLC' },
        { id: 'org_type', label: 'Organization type', type: 'select', options: ['For-profit financial institution', 'Non-profit / CDFI', 'Credit union', 'Fintech company', 'Insurance company', 'Registered investment advisor', 'Government agency', 'University / research institute', 'Other'] },
        { id: 'uei', label: 'UEI / DUNS number', type: 'text', placeholder: 'e.g., ABC123456789' },
        { id: 'regulatory_id', label: 'Primary regulatory registration (SEC/FINRA/State)', type: 'text', placeholder: 'e.g., CRD#, SEC File#, State License#' },
    ...
    { id: 'problem', title: 'Need / Problem Statement', type: 'textarea',
      guidance: 'What financial system problem does this project address? Be specific. Use real data if you have it. Think about: underbanked populations, systemic risk gaps, compliance cost burdens, fraud vulnerabilities, or capital access barriers.',
      prompt: 'Describe the financial services problem this project addresses. What happens if nothing is done?', prefillFrom: 'diagnosis' },
    { id: 'solution', title: 'Proposed Project / Solution', type: 'textarea',
      guidance: 'What will you actually build or deploy? Be concrete. "We will build a compliance automation platform that reduces AML review time by 60% for community banks." Avoid vague language — say what you will physically create or implement.',
      prompt: 'Describe what you will build, deploy, or implement. Be specific about deliverables and financial services impact.' },
    { id: 'impact', title: 'Impact / Expected Outcomes', type: 'textarea',
      guidance: 'What changes because of this project? Quantify if possible. "Reduces compliance cost by X% for Y institutions" or "Enables Z underbanked households to access credit" or "Prevents $W in fraud losses annually."',
      prompt: 'What measurable impact will this project have on the financial system? Include numbers.' },
    { id: 'budget', title: 'Budget / Use of Funds', type: 'fields',
      guidance: 'How much money do you need and what will you spend it on? Grant reviewers in financial services expect rigorous budgeting. Every dollar must have a purpose and tie to a deliverable.',
      fields: [
        { id: 'total_amount', label: 'Total funding requested ($)', type: 'text', placeholder: 'e.g., 250000' },
        { id: 'personnel', label: 'Personnel costs', type: 'text', placeholder: 'Compliance officers, analysts, developers' },
        { id: 'technology', label: 'Technology / infrastructure', type: 'text', placeholder: 'Software licenses, cloud, security, integrations' },
        { id: 'professional', label: 'Professional services', type: 'text', placeholder: 'Legal counsel, audit, actuarial, consulting' },
        { id: 'travel', label: 'Travel / fieldwork', type: 'text', placeholder: 'Regulatory meetings, client site visits' },
    ...
    { id: 'timeline', title: 'Timeline / Milestones', type: 'textarea',
      guidance: 'When will each phase of the project happen? Financial services grants often require quarterly reporting. Align milestones to quarters.',
      prompt: 'List major milestones with target dates. Example: Q1: Design and compliance review. Q2: Build and test. Q3: Pilot with partner institutions. Q4: Full deployment and impact measurement.' },
    { id: 'documents', title: 'Required Documents', type: 'checklist',
      guidance: 'Most financial services grants require supporting documents demonstrating regulatory standing and organizational capacity.',
      items: ['Organizational capability statement', 'Budget narrative / justification', 'Letters of support from partner institutions', 'Resumes of key personnel', 'SAM.gov registration confirmation', 'Audited financial statements (2 years)', 'Regulatory compliance certifications', 'Data security and privacy policy', 'Anti-money laundering (AML) program documentation', 'Environmental review (if applicable)'] },
    { id: 'review', title: 'Final Review', type: 'review',
      guidance: 'Review everything before submitting. The auditor will check completeness, coherence, regulatory alignment, and feasibility within the financial services sector.' },
    { id: 'auditor', title: 'Auditor Decision', type: 'auditor',
      guidance: 'This section is for the auditor reviewing this grant application for financial services compliance and viability.' }
  ];

  // ══════════════════════════════════════════════════════════════════════
  // SECTION DEFINITIONS — PATENT (14 sections, finance-specific)
  // ══════════════════════════════════════════════════════════════════════

  var PATENT_SECTIONS = [
    { id: 'what', title: 'What This Patent Is', type: 'info',
      guidance: 'A patent gives you the legal right to prevent others from making, using, or selling your invention for 20 years. In financial services, patents commonly cover: algorithmic trading methods, risk scoring systems, payment processing architectures, compliance automation tools, and blockchain-based financial instruments. A provisional patent costs ~$320 and gives you 12 months to file the full version.',
      prompt: 'No input needed — this is context for the operator.' },
    { id: 'why', title: 'Why This May Be Patentable', type: 'textarea',
      guidance: 'For something to be patentable, it must be: (1) new — nobody has done it before, (2) useful — it solves a real financial problem, (3) non-obvious — a person skilled in financial technology would not have thought of it easily. Note: pure business methods may face Alice Corp. challenges — focus on technical implementation.',
      prompt: 'Why is this financial invention new, useful, and non-obvious? How does it go beyond an abstract business method?', prefillFrom: 'whyNow' },
    { id: 'title', title: 'Invention Title', type: 'text',
      guidance: 'A clear, descriptive title. Not a marketing name — a functional description. Example: "System and Method for Real-Time Anti-Money Laundering Detection Using Graph Neural Networks on Transaction Streams"',
      prompt: 'Enter a descriptive title for the financial technology invention.', prefillFrom: 'title' },
    { id: 'summary', title: 'Invention Summary', type: 'textarea',
      guidance: 'Describe the invention in 2-4 sentences as if explaining to a smart person who understands finance but not the specific technology. What does it do? How does it work at a high level?',
      prompt: 'Summarize the financial technology invention in plain English.' },
    { id: 'problem_solved', title: 'Financial Problem Being Solved', type: 'textarea',
      guidance: 'What specific financial services problem does this invention solve? The patent examiner needs to understand why existing financial systems or processes are inadequate. Think about: speed, accuracy, cost, risk, compliance burden, or access barriers.',
      prompt: 'What financial services problem exists today that this invention fixes?', prefillFrom: 'diagnosis' },
    { id: 'novelty', title: 'Novelty / What Is Different', type: 'textarea',
      guidance: 'What makes this different from everything that already exists in financial technology? Be specific. "Unlike existing credit scoring systems that use X, this invention uses Y because Z." Pay special attention to the Alice Corp. standard — emphasize technical novelty over pure business process.',
      prompt: 'What is new about this invention compared to what exists in financial services?' },
    { id: 'prior_art', title: 'Prior Art / Existing Alternatives', type: 'textarea',
      guidance: 'List anything similar that already exists — patents (search patents.google.com), fintech products, academic papers, existing banking systems. The patent examiner will search for these, so address them proactively. Include existing patents from major financial institutions.',
    ...
    { id: 'technical', title: 'Technical Description', type: 'textarea',
      guidance: 'Describe how the invention works in enough detail that a financial technologist could build it. Include: system architecture, data flows, algorithm details, API specifications, and security measures. This is critical for surviving Alice Corp. scrutiny — the more technically specific, the stronger the patent.',
      prompt: 'Describe the technical implementation. Components, algorithms, data flows, security architecture.' },
    { id: 'embodiments', title: 'Embodiments / Alternative Versions', type: 'textarea',
      guidance: 'Describe at least 2-3 different ways the invention could be implemented in financial services. This broadens your patent coverage. Example: "In one embodiment, the system operates within a banking core processor. In another, it functions as a standalone SaaS platform accessible via API."',
      prompt: 'List alternative implementations or variations across different financial services contexts.' },
    { id: 'claims', title: 'Claims Development', type: 'textarea',
      guidance: 'Claims define exactly what you own. Start with one broad claim and add narrower ones. Format: "A computer-implemented method for [financial function] comprising: (a) [step 1], (b) [step 2], (c) [step 3]." Include system claims and method claims. Emphasize technical steps to strengthen against Alice Corp. challenges.',
    ...
    { id: 'ownership', title: 'Inventorship / Ownership', type: 'fields',
      guidance: 'Who invented this? Patent law requires listing the actual inventors. In financial services, ensure any employment agreements or prior IP assignments from financial institutions are addressed.',
      fields: [
        ...
        { id: 'assignee', label: 'Assignee (organization that owns the patent)', type: 'text', placeholder: 'LIMEN Helix Financial Services LLC' },
    ...
    { id: 'documents', title: 'Documents / Diagrams', type: 'checklist',
      guidance: 'Financial technology patents benefit greatly from detailed system diagrams, flowcharts, and data architecture visualizations.',
      items: ['System architecture diagram', 'Data flow diagram', 'Algorithm flowchart / pseudocode', 'API specification document', 'Prior art comparison table', 'Performance benchmarks vs existing systems', 'Inventor declaration signed', 'Assignment agreement (if assigning to company)', 'Prior art search results printout', 'Alice Corp. analysis memo'] },
    { id: 'review', title: 'Filing Readiness Review', type: 'review',
      guidance: 'Review everything before filing. Pay particular attention to the Alice Corp. analysis — financial method patents are closely scrutinized. Ensure technical specificity is sufficient.' },
    { id: 'auditor', title: 'Auditor Decision', type: 'auditor',
      guidance: 'This section is for the auditor reviewing this patent filing for financial technology viability and patentability.' }
  ];
    ...
    if (!ws._prefilled && opp) {
      ...
      for (var si = 0; si < sections.length; si++) {
        ...
        if (sec.prefillFrom && !ws.fields[sec.id]) {
          ...
          if (sec.prefillFrom === 'whyNow') ws.fields[sec.id] = (opp.title || '') + '. This Finance condition creates demand because: ' + (opp.reason || opp.whyNow || 'active diagnosis pathway detected.');
          if (sec.prefillFrom === 'diagnosis') ws.fields[sec.id] = 'Finance domain diagnosis: ' + (opp.diagnosisId || 'active').replace(/_/g, ' ') + '. ' + (opp.title || '');
        }
      }
  ...
  window.LIMENFinanceExecutionPanels = {
  ...
  console.log('[FinanceExecutionPanels] Loaded — GRANT + PATENT workspaces ready');
