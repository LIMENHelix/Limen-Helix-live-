/*
 * trade-extinction.js  — ADDITIVE mechanism (completes EXTINCTION_LEARNING)
 * ----------------------------------------------------------------------------
 ...
 * (complements the GLOBAL down-scaling in trade-offline-maintenance.js).
 *
 * CONTRACT: read-only / copy-only. NEVER writes trade.json. Proposes which acquired responses to
...
(function (root) {
  ...
  /* Extinction over Trade activations: an activation whose diagnosticTriggers are ALL absent
   * from the caller-supplied activeTriggers is a candidate to retire (its trigger is gone).
   * activeTriggers: array of currently-firing trigger strings (runtime state; Trade has none baked in). */
  function proposeExtinction(trade, activeTriggers, opts) {
    ...
    return {
      ...
      note: 'Read-only PROPOSAL. Applies nothing; retiring a node would edit trade.json. activeTriggers is caller-supplied runtime state.'
    };
  }

  var API = { proposeExtinction: proposeExtinction };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.TradeExtinction) root.TradeExtinction = API;
