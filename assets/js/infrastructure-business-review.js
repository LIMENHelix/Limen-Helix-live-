/**
 * infrastructure-business-review.js — Infrastructure Business Assignment Review Surface
 *
 * INFRASTRUCTURE DOMAIN ONLY. Renders a review panel for proposed business mappings.
 * Requires infrastructure-node-business-engine.js to be loaded first.
 *
 * Provides APPROVE / DENY / HOLD buttons per proposal.
 * Only APPROVED mappings become active in the Infrastructure operator surface.
 *
...
(function () {
  ...
  function renderCard(entry) {
    ...
    if (entry.neuroTranslation) {
      ...
      h += '<div style="font-size:0.32rem;color:#b0a898;line-height:1.5"><b style="color:rgba(201,169,78,0.7)">In business (Infrastructure systems):</b> ' + esc(entry.neuroTranslation.inBusiness) + '</div>';
      h += '</div>';
    }
    ...
    if (entry.existingCompanies && entry.existingCompanies.length > 0) {
      ...
      h += '<div class="ebr-card-existing" style="color:#C9A94E">Missing from current Infrastructure system</div>';
    }
  ...
  function render() {
    var engine = window.LIMENInfrastructureBusinessEngine;
    if (!engine) return;
      ...
      bizBtns[bi].addEventListener('click', function (e) {
        ...
        try {
          sessionStorage.setItem('limen_exec_context', JSON.stringify({
            ...
            source: 'infrastructure', returnTo: '/domain-console?domain=infrastructure&mode=operator'
          }));
        } catch (ex) {}
        window.location.href = 'infrastructure-workspace.html?track=business&opp=' + encodeURIComponent(oppKey) + '&returnTo=' + encodeURIComponent('/domain-console?domain=infrastructure&mode=operator');
      });
    }
  ...
  window.LIMENInfrastructureBusinessReview = {
    render: render,
    mount: mount
  };

  console.log('[InfrastructureBusinessReview] Loaded — review surface ready');
