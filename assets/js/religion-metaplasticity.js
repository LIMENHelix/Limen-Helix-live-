/*
 * religion-metaplasticity.js  — ADDITIVE meta-controller  (THE NEW STRATEGY)
 * ----------------------------------------------------------------------------
 * METAPLASTICITY build-target — the capstone that makes Religion's neuro-substrate SELF-TUNING.
 ...
 *   - offline down-scaling factor      (religion-offline-maintenance.js)
 *   - refractory absolute window       (religion-refractory-limiter.js)
 *   - prediction-error threshold       (religion-prediction-error-compressor.js)
 ...
 * no adaptation (returns inputs unchanged). Volatility is caller-supplied (Religion has no time
...
(function (root) {
  ...
  function adaptParams(current, params) {
    ...
    return {
      ...
      note: 'Pure. Applies nothing. gain operator-set; volatility caller-supplied (Religion has no time series).'
    };
  }

  var API = { adaptParams: adaptParams, DEFAULTS: DEFAULTS };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root && !root.ReligionMetaplasticity) root.ReligionMetaplasticity = API;
