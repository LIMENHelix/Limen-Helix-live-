/*
 * technology-neuro-substrate.js  — ADDITIVE WIRING OVERLAY
 * ----------------------------------------------------------------------------
 * Source (only): LIMEN Helix Comprehensive Neurology Reference.
 * Data:          assets/data/deep/technology-neuro-substrate.json
 * Prompts:       assets/prompts/technology-neuro-substrate-authoring.md
 *
 * CONTRACT: purely additive. This file does NOT modify assets/data/domains/technology.json
 * or any existing technology-*.js. It reads existing Technology data by value and exposes the
 * neuro-substrate layer + the document's Part XIV "incomplete-circuit" diagnostic in code.
 *
 * It changes nothing on load. To actually wire it into the running Technology brain, add the
 * ONE line marked [INTEGRATION HOOK] below to your existing loader when ready. It is left
 * un-applied here on purpose, because applying it would edit an existing Technology file.
 * ----------------------------------------------------------------------------
 */
(function (root) {
  'use strict';

  var SUBSTRATE_URL = '/assets/data/deep/technology-neuro-substrate.json';
  var TECHNOLOGY_URL = '/assets/data/domains/technology.json';
  ...
  var API = {
    ...
    /* GROUNDED lookup: the Part XIV failure-mode class for an Technology issue id. */
     ...
     * Reads an Technology issue's authored circuit and reports whether a regulatory/feedback
    ...
    /* Run the diagnostic across every Technology issue. */
    auditAllIssues: function () {
      return loadJSON(TECHNOLOGY_URL).then(function (technology) {
        return (technology.issues || []).map(API.validateIncompleteCircuit);
      });
    },
  ...
  if (root && !root.TechnologyNeuroSubstrate) root.TechnologyNeuroSubstrate = API;

  /* -------------------------------------------------------------------------
   * [INTEGRATION HOOK]  (NOT applied — add this one line to the existing
   *  Technology brain/loader when you choose to wire the overlay in; leaving it
   *  here means no existing file is modified now):
   *
   *      import TechnologyNeuroSubstrate from '/assets/js/technology-neuro-substrate.js';
   *      // then, after issues render:
   *      TechnologyNeuroSubstrate.auditAllIssues().then(console.table);
   * ------------------------------------------------------------------------- */
