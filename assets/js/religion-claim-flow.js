/**
 * environment-claim-flow.js — Religion Domain Claim Acceptance Flow
 *
 * Adds CLAIM button to Religion opportunity cards. On click, opens
 ...
 * Namespace: window.LIMENReligion.economy.claimFlow
 */
(function () {
  'use strict';
  ...
  function openClaimModal(opportunity) {
    ...
    var comp = window.LIMENReligion.economy.compensation;
    ...
    h += '<div class="scf-title">Claim Religion Opportunity</div>';
    ...
    h += '<div>\u2022 Operator is responsible for religion integrity, data protection, and authorship ethics</div>';
    h += '</div></div>';

    h += '<label class="scf-accept">';
    h += '<input type="checkbox" id="scfAcceptCheck">';
    h += '<span class="scf-accept-text">I understand this is a Science-domain execution claim, I am responsible for truthful submission, documented progress, and compliance with applicable religion integrity, IRB, and data protection rules.</span>';
    h += '</label>';
  ...
  window.addEventListener('limen:religion-claim-update', function () { setTimeout(injectIntoCards, 100); });

  window.LIMENReligion.economy.claimFlow = {
