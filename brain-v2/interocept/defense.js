/**
 * brain-v2/interocept/defense.js — DEFENSE's own interoception. INDIVIDUAL.
 *
 * Three structural measures this brain takes OF ITSELF, from its own declarations in
 * bind/defense.js only. No new feeds, no labels, no thresholds, no scoring, no activation.
 *
 ...
 *
 * ═══════════════════════════════════════════════════════════════════════════════════
 * WHAT DEFENSE MEASURES ABOUT ITSELF (measured 2026-08-15).
 *
 * FOUR findings over FIFTEEN channels, only THREE channels load-bearing — and none of the
 * three is about defense. They are `nwsAlerts` (severe weather), `earthquakes` (seismic)
 * and `cisaKev` (software vulnerabilities). Eight of the fifteen channels are `r7` article
 * counts and feed nothing.
 *
 * So this brain's actual detection capacity is: weather, ground motion, and published
 * vulnerabilities. Those are real measured quantities, which is why findings were built on
 * them, but a reader who sees "defense: 4 findings" will assume something else. Stating the
 * mismatch here is the whole reason this file exists.
 *
 * All three load-bearing channels are SHARED with other domains: nwsAlerts and earthquakes
 * with environment, infrastructure and trade; cisaKev with law, intelligence and technology.
 * Every one of this domain's detectors is therefore a correlated lesion — no outage that
 * blinds defense blinds ONLY defense.
 *
 * Latency floor 1h to 1d. Worst-case knockout depth 3 of 15.
 * ═══════════════════════════════════════════════════════════════════════════════════
 */

'use strict';

var BINDER = require('../bind/defense.js');
...
function latency() {
  ...
  return {
    ...
    note: 'Floor set by cadence alone. A conjunctive finding is bound by the SLOWER of its channels, never the faster.'
  };
}
...
function diaschisis() {
  ...
  return {
    domain: 'defense', channels: per,
    loadBearingCount: loadBearing.length,
    silentCount: per.length - loadBearing.length,
    totalChannels: per.length,
    note: 'Deficit is measured at the FINDINGS a channel feeds. Every load-bearing channel here is shared with another domain, so a lesion is never local.'
  };
}
...
function reserve() {
  ...
  return {
    domain: 'defense', worstCaseKnockout: order.length, order: order,
