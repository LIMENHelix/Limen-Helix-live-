/**
 * brain-v2/interocept/agriculture.js — AGRICULTURE's own interoception. INDIVIDUAL.
 *
 * Three structural measures this brain takes OF ITSELF, from its own declarations in
 * bind/agriculture.js only. No new feeds, no labels, no thresholds, no scoring, no
 * activation.
 *
 ...
 *
 * ═══════════════════════════════════════════════════════════════════════════════════
 * WHAT AGRICULTURE MEASURES ABOUT ITSELF (measured 2026-08-15).
 *
 * SIX findings over THIRTEEN channels, five of them load-bearing. This domain has the
 * WIDEST latency spread in the system: 1h at the fastest, 1y at the slowest.
 *
 * That spread is the point. AG_ALERT_DEPARTURE and DROUGHT_AND_ALERTS_CO_DEPARTING can move
 * within the hour, while CORN_YIELD_DEPARTURE, WHEAT_OUTPUT_DEPARTURE and
 * FOOD_PRODUCTION_DEPARTURE are bound to annual series. Reporting a single "agriculture
 * reading" merges detectors whose floors differ by four orders of magnitude, and a reader
 * cannot tell which one moved. Latency per finding is the only honest presentation.
 *
 * DROUGHT_AND_ALERTS_CO_DEPARTING is the one conjunctive finding, requiring droughtArea AND
 * agAlerts. `droughtArea` therefore carries two findings and is this domain's first lesion
 * target: losing it costs both the single-channel drought detector and the co-departure.
 *
 * Worst-case knockout depth 5 of 13.
 * ═══════════════════════════════════════════════════════════════════════════════════
 */

'use strict';

var BINDER = require('../bind/agriculture.js');
...
function latency() {
  ...
  return {
    domain: 'agriculture', findings: per,
    ...
    note: 'This domain spans 1h to 1y. A single domain-level reading merges detectors whose floors differ by orders of magnitude; report per finding.'
  };
}
...
function diaschisis() {
  ...
  return {
    domain: 'agriculture', channels: per,
...
function reserve() {
  ...
  return {
    domain: 'agriculture', worstCaseKnockout: order.length, order: order,
