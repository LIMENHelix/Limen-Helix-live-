/**
 * brain-v2/interocept/population.js — POPULATION's own interoception. INDIVIDUAL.
 *
 * Three structural measures this brain takes OF ITSELF, from its own declarations in
 * bind/population.js only. No new feeds, no labels, no thresholds, no scoring, no
 * activation.
 *
 ...
 *
 * ═══════════════════════════════════════════════════════════════════════════════════
 * WHAT POPULATION MEASURES ABOUT ITSELF (measured 2026-08-15).
 *
 * TWO findings over FIFTEEN channels, both with a ONE YEAR latency floor, and TEN of the
 * fifteen channels are `r7` article-recency counts feeding nothing.
 *
 * POPULATION_TOTAL_DEPARTURE and FERTILITY_RATE_DEPARTURE rest on `populationTotal` and
 * `fertilityRate`, annual UN-family series. Here the annual floor is arguably CORRECT
 * rather than a limitation: population totals and fertility rates genuinely move on annual
 * timescales, so a faster detector would be detecting revision noise, not demography.
 *
 * That distinction matters. Governance, education and communication have annual floors on
 * quantities that could move faster; population has an annual floor on a quantity that does
 * not. The measure is the same, the verdict is different, and only a per-domain file can
 * say so — a system-wide table would flag all four identically as "slow".
 *
 * Findings are independent, so worst-case knockout depth is 2. Load-bearing 2 of 15.
 * ═══════════════════════════════════════════════════════════════════════════════════
 */

'use strict';

var BINDER = require('../bind/population.js');
...
function latency() {
  ...
  return {
    domain: 'population', findings: per,
    ...
    note: 'Annual floor, and here it MATCHES the quantity: demography moves annually. A faster detector would read revision noise.'
  };
}
...
function diaschisis() {
  ...
  return {
    domain: 'population', channels: per,
...
function reserve() {
  ...
  return {
    domain: 'population', worstCaseKnockout: order.length, order: order,
    unreachableFindings: surviving.map(function (f) { return f.id; }),
    totalFindings: FINDINGS.length,
    note: 'Findings here are independent, so depth equals finding count.'
  };
}
